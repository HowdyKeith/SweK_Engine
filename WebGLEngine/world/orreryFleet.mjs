// FILE: world/orreryFleet.mjs -- v4329
//
...
//
// The obvious asteroid belt is "one rock per commit that touched this body". Counted, at v4329, over 858
// commits of history:
//
 ...
 */
// v4333 -- AND THE MERGE PUT IT BACK TO 2, WHICH IS NOT A REGRESSION OF EITHER SIDE. This record is re-taken
// from git by the gate that reads it, and at the merged head `git log -- WebGLEngine/vendor/three` returns TWO
// commits: e08b1b6 (v4048, the rigged Krbn glb export) and 66db97c (the v3845 mirror publish). main's line had
// only ever seen one of them, so v4330's correction was right for main's head and wrong for this one -- the
// number is a property of the HISTORY YOU ARE STANDING ON, and joining two histories changes it. Re-measured
// here across all fifteen bodies: every other count is unchanged, so `three` is the only edit, and `at` stays
// v4329 because this is still the v4329 belt -- correcting a number does not rename the round that took it.
// AND IT MAKES THE SENTENCE BELOW TRUE AGAIN. `why` says "twelve of fifteen ... touched by exactly one commit",
// which was written when three was 2. v4330 moved three to 1 -- making it thirteen -- and left the prose saying
