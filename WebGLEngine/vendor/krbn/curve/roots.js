 ...
 * case.
 */
export function solveCubicReal(a, b, c, d) {
    if (Math.abs(a) <= EPS_ABS) {
    ...
    const cubeRoot = (v) => (v < 0 ? -Math.pow(-v, 1 / 3) : Math.pow(v, 1 / 3));
    if (Math.abs(p) <= EPS_ABS && Math.abs(q) <= EPS_ABS) {
        ...
        Math.max(-1, Math.min(1, (3 * q) / (p * m)))) / 3;
        for (let k = 0; k < 3; k++) {
 ...
 */
export function solveQuarticReal(a, b, c, d, e) {
    if (Math.abs(a) <= EPS_ABS)
        return solveCubicReal(b, c, d, e);
    ...
    if (Math.abs(q) <= 1e-12) {
        ...
        let w = 0;
        for (const cr of solveCubicReal(1, 2 * p, p * p - 4 * r, -q * q))
            if (cr > w)
                w = cr;
        if (w > 0) {
    ...
    const f = (x) => (((a * x + b) * x + c) * x + d) * x + e;
    const fp = (x) => ((4 * a * x + 3 * b) * x + 2 * c) * x + d;
    for (let i = 0; i < roots.length; i++) {
        let x = roots[i];
        for (let k = 0; k < 3; k++) {
            const yp = fp(x);
            if (Math.abs(yp) <= EPS_ABS)
                break;
            const nx = x - f(x) / yp;
