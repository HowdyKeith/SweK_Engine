...
function spatialOffset(p, seed, ampPx, freq) {
 ...
 */
export function applyWobble(path, arclength, seed, amount) {
...
function wobblePath(path, arclength, seed, amount, amplitudePx, frequency) {
    if (amount <= 0 || path.length < 2)
 ...
 */
export function densify(path, points3, stepPx) {
    const outP = [[path[0][0], path[0][1]]];
    const outQ = [[points3[0][0], points3[0][1], points3[0][2]]];
...
export function arclengthOf(points) {
    const s = [0];
 ...
 */
export function createWobble(params = {}) {
...
export const defaultWobble = createWobble();
