...
export const DEFAULT_CONSOLIDATE = {
    angleToleranceDeg: 2,
 ...
 */
export function consolidateLines(strokes, cam, opts = DEFAULT_CONSOLIDATE) {
    const singles = [];
    const items = [];
    ...
    for (let i = 0; i < items.length; i++) {
        ...
        const A2 = [p0[0] + smin * dir[0], p0[1] + smin * dir[1]];
        const B2 = [p0[0] + smax * dir[0], p0[1] + smax * dir[1]];
        ...
        merged.push({
            ...
            a: A3,
            b: B3,
            memberIds: cluster.map((it) => it.stroke.feature.id).filter((id) => id !== undefined).sort(),
        });
    }
    return { singles, merged };
}
function collinear(a, b, sinTol, distTol) {
    if (Math.abs(cross2(a.dir, b.dir)) > sinTol)
...
function overlapsAlong(cluster, other, gapTol) {
...
function backProject(cam, pt, a, b) {
