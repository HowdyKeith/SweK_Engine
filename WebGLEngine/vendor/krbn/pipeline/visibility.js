...
export function sceneScale(sources) {
    let d = 0;
 ...
 */
export function isOccluded(p, cam, sources, scale, owner, ownerTol) {
    ...
    const origin = [eyeRay.origin[0] + dir[0] * advance, eyeRay.origin[1] + dir[1] * advance, eyeRay.origin[2] + dir[2] * advance];
    ...
    for (const s of sources) {
        ...
        for (const h of s.raycast(ray)) {
            if (h.t > baseTol && h.t < dFront)
                return true; // a surface strictly in front of `p`
        }
    }
    return false;
}
...
function screenLength(model, cam) {
...
export function classifyFeature(feature, cam, sources, scale = sceneScale(sources), owner) {
    ...
    const occ = (t) => isOccluded(model.point3(t), cam, sources, scale, owner, ownerTol);
    ...
    for (const s of sources) {
        for (const sil of s.projectedSilhouettes(cam)) {
            for (const pt of crossScreenCurves(model.screen, sil)) {
                ...
                if (t === null)
                    continue;
                if (t > model.t0 + epsT && t < model.t1 - epsT)
                    events.push(t);
            }
        }
    }
    ...
    for (let i = 1; i <= scanN; i++) {
        ...
        if (curOcc !== prevOcc) {
            ...
            if (!events.some((e) => Math.abs(e - b) <= scanTol))
                events.push(b);
        }
        prevT = t;
        prevOcc = curOcc;
    ...
    events.sort((a, b) => a - b);
    // dedup
    const bounds = [model.t0];
    for (const e of events)
        if (e - bounds[bounds.length - 1] > epsT)
            bounds.push(e);
    ...
    for (const iv of raw) {
        ...
        if (last && last.visible === iv.visible)
            last.t1 = iv.t1;
        else
            intervals.push({ ...iv });
    }
...
function bisectFlip(occ, ta, occA, tb) {
    let lo = ta;
    let hi = tb;
 ...
 */
export function classifyScene(sources, cam, reconcile) {
    ...
    const strokes = [];
    sources.forEach((s, i) => {
        for (const feature of perSource[i])
