...
function sampledScreen(P, point3, t0, t1) {
    const pts = [];
    for (let i = 0; i <= SAMPLE_FALLBACK; i++)
        pts.push(projectPoint(P, point3(t0 + ((t1 - t0) * i) / SAMPLE_FALLBACK)).point);
    return { kind: "polyline", pts };
}
export function buildFeatureModel(curve, cam) {
    const P = projectionMatrix(cam);
    switch (curve.kind) {
        case "line": {
            const { a, b } = curve;
            const point3 = (t) => [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t, a[2] + (b[2] - a[2]) * t];
            return {
                t0: 0,
                t1: 1,
                closed: false,
        ...
        case "arc": {
            ...
            return {
                ...
                paramOf: (pt) => {
                    ...
                    // bring into [a0, a1]
                    while (theta < a0 - EPS_ANGLE)
                        theta += 2 * Math.PI;
                    while (theta > a1 + EPS_ANGLE)
                        theta -= 2 * Math.PI;
                    return theta;
                },
            };
...
function buildPolylineModel(pts, cam, P) {
    ...
    return {
        ...
        paramOf: (pt) => {
            ...
            for (let i = 0; i < n; i++) {
                ...
                if (!hit || hit.u < -EPS_ONCURVE || hit.u > 1 + EPS_ONCURVE)
                    continue;
                if (!best || hit.residual < best.residual)
                    best = { t: i + Math.max(0, Math.min(1, hit.u)), residual: hit.residual };
            }
            return best ? best.t : null;
        },
    };
}
//# sourceMappingURL=feature-curve.js.map