...
export function hatchAngles(mode, angleDeg) {
    switch (mode) {
        case "single":
            return [angleDeg];
        case "cross":
...
function clipToLoops(point, dir, loops) {
    const ts = [];
...
function clipToConic(point, dir, k) {
    const res = intersectLineConic({ point, dir }, k);
    if (res.kind === "two")
        return [[res.hits[0].point, res.hits[1].point]];
    return [];
}
...
export function generateHatchLines(region, opts = {}) {
    ...
    const loops = region.outline.kind === "polyline" || region.holes?.length
    ...
    for (let ai = 0; ai < angles.length; ai++) {
        ...
        for (let n = n0; aOff + n * spacing <= hi; n++) {
            ...
            const point = [normal[0] * off, normal[1] * off];
            const clipped = region.outline.kind === "conic" && !region.holes?.length
                ? clipToConic(point, dir, region.outline.params)
                : clipToLoops(point, dir, loops);
            for (const s of clipped)
                lines.push({ seg: s, key: `${ai}:${n}` });
        }
    }
    return lines;
}
