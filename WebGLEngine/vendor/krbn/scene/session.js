...
const polylineOf = (f) => (f.curve.kind === "polyline" ? f.curve.pts : null);
    ...
    reconcile(features) {
        ...
        for (const f of leftoverFeatures) {
            ...
            for (const t of leftoverTracks) {
                if (t.owner !== f.owner || t.type !== f.type)
                    continue;
                const d = dist(centroid, t.centroid);
                if (d <= GATE_EXTENT_FACTOR * Math.max(extent, t.extent))
                    candidates.push({ f, t, d });
            }
        }
        candidates.sort((a, b) => a.d - b.d);
        for (const c of candidates) {
            if (matched.has(c.f) || claimed.has(c.t.pid))
                continue;
            matched.set(c.f, c.t);
            claimed.add(c.t.pid);
        }
