...
export function cameraFrame(cam) {
...
export function projectionMatrix(cam) {
...
export function projectPoint(P, p) {
    const u = P[0] * p[0] + P[1] * p[1] + P[2] * p[2] + P[3];
    const v = P[4] * p[0] + P[5] * p[1] + P[6] * p[2] + P[7];
    const w = P[8] * p[0] + P[9] * p[1] + P[10] * p[2] + P[11];
 ...
 */
export function unproject(cam, pixel) {
    const { right, up, forward } = cameraFrame(cam);
    const W = cam.viewport.width;
    const H = cam.viewport.height;
