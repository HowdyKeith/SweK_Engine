...
export function rayPlaneT(ray, planeOrigin, planeNormal) {
...
export function rayPlanePoint(ray, planeOrigin, planeNormal) {
 ...
 */
export function rayLineClosestU(ray, a, b) {
    const e = sub(b, a);
    const d = ray.dir;
    ...
    const ed = dot(e, d);
    const G = ee * dd - ed * ed;
    if (Math.abs(G) <= EPS_ABS)
        return null; // ray parallel to the line
    const ew0 = dot(e, w0);
    const dw0 = dot(d, w0);
    const u = (ew0 * dd - ed * dw0) / G;
    const s = (ee * dw0 - ed * ew0) / G;
    const lp = [a[0] + u * e[0], a[1] + u * e[1], a[2] + u * e[2]];
    const rp = [ray.origin[0] + s * d[0], ray.origin[1] + s * d[1], ray.origin[2] + s * d[2]];
    const residual = Math.hypot(lp[0] - rp[0], lp[1] - rp[1], lp[2] - rp[2]);
    return { u, residual };
}
//# sourceMappingURL=intersect3d.js.map