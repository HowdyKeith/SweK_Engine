...
export function planeToScreenHomography(P, xhat, yhat, origin) {
    const cols = [
...
export function screenConicFromPlaneConic(H, planeConic) {
    const d = det(H);
    if (Math.abs(d) <= EPS_ABS)
        return null; // plane projects to a line (edge-on)
...
export function projectCircle(P, center, radius, xhat, yhat) {
    const H = planeToScreenHomography(P, xhat, yhat, center);
    // u² + v² − r² = 0
    const circle = [1, 0, 0, 0, 1, 0, 0, 0, -radius * radius];
