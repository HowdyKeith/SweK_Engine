...
export class Torus {
    center;
    axis; // unit
    majorRadius; // R: centre → tube centre
    minorRadius; // r: tube radius
    kind = "torus";
    id;
    autoNamed;
    frame;
    constructor(center, axisVec, majorRadius, minorRadius, id) {
        this.center = center;
    ...
    bounds() {
        ...
        const a = this.axis;
        const half = (i) => (R + r) * Math.sqrt(Math.max(0, 1 - a[i] * a[i])) + r * Math.abs(a[i]);
        const c = this.center;
        return {
    ...
    hatchRegions(cam, _light) {
        ...
        if (outer.length < 3)
            return [];
        const holes = projected.slice(1).filter((h) => h.length >= 3).map((pts) => ({ kind: "polyline", pts }));
    ...
    hatchField(cam, opts) {
        ...
        const radial = (u) => addScaled(addScaled([0, 0, 0], e1, Math.cos(u)), e2, Math.sin(u));
        const families = [];
        ...
        if (opts.maxFamilies >= 3) {
            ...
            for (const s of dyadicLadder((Math.PI * majDiam) / Math.SQRT2 / spacing, { periodic: true, min: 6, max: 64 })) {
                const u0 = TWO_PI * s.t;
                diagonal.push(tagCurve(paramCurve((t) => {
                    const u = u0 + TWO_PI * t;
                    const v = TWO_PI * t;
                    const rad = radial(u);
    ...
    raycast(ray) {
        ...
        const o = [dot(oc, e1), dot(oc, e2), dot(oc, e3)];
        const d = [dot(ray.dir, e1), dot(ray.dir, e2), dot(ray.dir, e3)];
        const R = this.majorRadius;
        const r = this.minorRadius;
        const K = R * R - r * r;
        const alpha = dot(d, d);
        ...
        for (const t of solveQuarticReal(c4, c3, c2, c1, c0)) {
            const point = [ray.origin[0] + t * ray.dir[0], ray.origin[1] + t * ray.dir[1], ray.origin[2] + t * ray.dir[2]];
            const lp = [o[0] + t * d[0], o[1] + t * d[1], o[2] + t * d[2]];
            ...
            const nLocal = [lp[0] * k, lp[1] * k, lp[2]];
            const n = normalize([
