...
export class Cylinder {
    base; // center of the base cap
    axis; // unit axis, base → top
    height;
    radius;
    kind = "cylinder";
    id;
    autoNamed;
    constructor(base, axisVec, radius, id) {
        this.base = base;
    ...
    bounds() {
        const c0 = this.base;
        const c1 = this.top;
        const half = (i) => this.radius * Math.sqrt(Math.max(0, 1 - this.axis[i] * this.axis[i]));
        const min = [0, 0, 0].map((_, i) => Math.min(c0[i], c1[i]) - half(i));
        const max = [0, 0, 0].map((_, i) => Math.max(c0[i], c1[i]) + half(i));
        return { min, max };
    }
    ...
    hatchField(cam, opts) {
        ...
        for (const [ci, [c, sign]] of [[this.base, -1], [top, 1]].entries()) {
            const nrm = [sign * this.axis[0], sign * this.axis[1], sign * this.axis[2]];
        ...
        if (opts.maxFamilies >= 3) {
            ...
            for (const s of dyadicLadder((Math.PI * diam) / Math.SQRT2 / spacing, { periodic: true, min: 4, max: 64 })) {
                ...
                helices.push(tagCurve(paramCurve((t) => {
    ...
    raycast(ray) {
        ...
        if (roots.kind === "two")
            ...
            ts.push(roots.x);
        else if (roots.kind === "single")
            ts.push(roots.x);
        for (const t of ts) {
            const s = ocDotA + t * dDotA; // axial coordinate
            if (s < -EPS_ABS || s > this.height + EPS_ABS)
                continue;
    ...
    silhouetteBasePoints(cam) {
        const plane = basisFromNormal(this.base, this.axis);
        if (cam.projection === "orthographic") {
            const v = cameraFrame(cam).forward;
            const axb = cross(this.axis, v);
            if (length(axb) <= EPS_REL)
                return null; // view ∥ axis
        ...
        if (dist <= this.radius)
            return null; // eye inside the cylinder radius
        const phi0 = Math.atan2(ey, ex);
        const alpha = Math.acos(this.radius / dist);
        const at = (phi) => add(this.base, add(addScaled([0, 0, 0], plane.x, this.radius * Math.cos(phi)), addScaled([0, 0, 0], plane.y, this.radius * Math.sin(phi))));
