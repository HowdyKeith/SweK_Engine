...
export function screenDist(cam, a, b) {
...
export function curveCount(screenSpan, spacingPx, min, max) {
 ...
 */
export function dyadicLadder(desired, opts = {}) {
    const periodic = opts.periodic ?? false;
    const toLevel = (n) => (periodic ? Math.log2(Math.max(1, n)) : Math.log2(Math.max(1, n) + 1) - 1);
    ...
    for (let L = 0; L <= k; L++) {
        ...
        for (let num = 1; num < den; num += 2)
            stops.push({ t: num / den, key: `${num}/${den}` });
    }
    return stops;
}
/** Tag a field curve with its ladder identity (in place, for chaining). */
export function tagCurve(curve, key) {
    curve.key = key;
    return curve;
}
/**
 ...
 */
export function circleCurve(center, ex, ey, radius, normalAt, segments) {
    const samples = [];
 ...
 */
export function paramCurve(at, segments) {
    const samples = [];
    for (let i = 0; i <= segments; i++) {
 ...
 */
export function segmentCurve(a, b, normal, segments) {
...
//# sourceMappingURL=hatch-field.js.map