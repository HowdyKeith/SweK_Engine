...
import { adjugate, det, mulVec } from "../math/mat3.js";
...
export class IntersectionCurve {
    ...
    extractFeatures(_cam) {
        return this.sections.map((s) => ({ type: "intersection", owner: this.id, curve: s.curve, attrs: {} }));
    }
...
function conicSection(plane, params) {
    ...
    if (worldPts.length < 3)
        return null; // plane misses the quadric
...
export function intersectQuadricPlane(Q, planePoint, planeNormal) {
...
function radicalPlane(Q1, Q2) {
    ...
    for (let i = 0; i < 16; i++)
        D[i] = Q1[i] * s1 - Q2[i] * s2;
    const quadraticLeftover = Math.abs(D[0]) + Math.abs(D[1]) + Math.abs(D[2]) + Math.abs(D[5]) + Math.abs(D[6]) + Math.abs(D[10]);
    if (quadraticLeftover > EPS_REL)
        return null; // genuinely different quadratics
    const n = [2 * D[3], 2 * D[7], 2 * D[11]];
    const nn = n[0] * n[0] + n[1] * n[1] + n[2] * n[2];
    if (nn <= EPS_ABS)
        return null; // concentric
    const t = -D[15] / nn;
 ...
 * section curves.
 */
export function intersectQuadrics(Q1, Q2, b1, b2) {
...
function aabbRangeAlong(box, dir) {
    let lo = Infinity;
    let hi = -Infinity;
    for (let i = 0; i < 8; i++) {
 ...
 */
export function intersectQuadricQuadric(Q1, Q2, b1, b2) {
    const c1 = quadricCenter(Q1);
    const c2 = quadricCenter(Q2);
    let dir = [0, 0, 1];
    if (c1 && c2) {
        const dd = sub(c2, c1);
        if (length(dd) > EPS_REL)
            dir = normalize(dd);
    }
    ...
    for (let i = 1; i < N; i++) {
        ...
        const origin = [dir[0] * s, dir[1] * s, dir[2] * s];
 ...
 */
function projectToCurve(Q1, Q2, p0) {
    let x = p0;
    for (let k = 0; k < 5; k++) {
        const n1 = quadricNormal(Q1, x);
        const n2 = quadricNormal(Q2, x);
        const G1 = [2 * n1[0], 2 * n1[1], 2 * n1[2]];
        const G2 = [2 * n2[0], 2 * n2[1], 2 * n2[2]];
        const f1 = quadricValue(Q1, x);
        const f2 = quadricValue(Q2, x);
        if (Math.abs(f1) + Math.abs(f2) < 1e-13)
            break;
        const a = dot(G1, G1);
        const b = dot(G1, G2);
        const c = dot(G2, G2);
        const detJ = a * c - b * b;
        if (Math.abs(detJ) <= EPS_ABS)
            break; // gradients parallel (tangency)
        // w = (J Jᵀ)⁻¹ r,   δ = −Jᵀ w
...
function refineChain(chain, Q1, Q2) {
    if (chain.length < 2)
        return chain.map((p) => [p[0], p[1], p[2]]);
    const bb = aabbFromPoints(chain);
    const maxSeg = Math.max(dist3(bb.min, bb.max) * 0.02, 1e-4);
    const out = [[chain[0][0], chain[0][1], chain[0][2]]];
...
export function intersectPlanes(pA, nA, pB, nB, extent) {
    ...
    const M = [nA[0], nA[1], nA[2], nB[0], nB[1], nB[2], dir[0], dir[1], dir[2]];
    const d = det(M);
    if (Math.abs(d) <= EPS_ABS)
        return null;
    const rhs = [dot(nA, pA), dot(nB, pB), 0];
    // p0 = M⁻¹ rhs = (adj(M)/det) rhs
    const p0 = mulVec(adjugate(M), rhs);
    const p0s = [p0[0] / d, p0[1] / d, p0[2] / d];
