...
export class Quadric {
    ...
    raycast(ray) {
        ...
        return ts
            .sort((p, q) => p - q)
            .map((t) => {
            const point = [o[0] + t * d[0], o[1] + t * d[1], o[2] + t * d[2]];
    ...
    silhouetteObjectSpace(cam) {
        const pi = this.polarPlane(cam);
        const n = [pi[0], pi[1], pi[2]];
        const nn = n[0] * n[0] + n[1] * n[1] + n[2] * n[2];
        if (nn <= EPS_ABS)
            return null; // no silhouette (e.g. eye at center)
        // A point on the plane π·(x,1)=0 nearest the origin: x = -π_w/|n|² · n.
        const s = -pi[3] / nn;
        const origin = [n[0] * s, n[1] * s, n[2] * s];
...
export function quadricPlaneConic(Q, plane) {
    ...
    return matrixToConic(Cin);
}
/** Apparent outline in screen (pixel) coordinates: dual C* = P · adj(Q) · Pᵀ. */
export function outlineScreenConic(Q, P) {
    ...
    for (let r = 0; r < 3; r++)
        for (let c = 0; c < 4; c++) {
            let sum = 0;
            for (let k = 0; k < 4; k++)
                sum += P[r * 4 + k] * Qadj[k * 4 + c];
            T[r * 4 + c] = sum;
        }
    // Cdual = T (3×4) · Pᵀ (4×3) → 3×3, Cdual[i][j] = Σ_k T[i][k]·P[j][k]
    const Cdual = new Array(9).fill(0);
    for (let i = 0; i < 3; i++)
        for (let j = 0; j < 3; j++) {
            let sum = 0;
            for (let k = 0; k < 4; k++)
    ...
    const C = adjugate3(Cdual);
    const frob = Math.hypot(...C);
    if (frob <= EPS_ABS)
        return null;
...
export class Sphere extends Quadric {
    ...
    axis;
    constructor(center, radius, id, axis = [0, 0, 1]) {
        const [cx, cy, cz] = center;
        const Q = symmetric4(1, 0, 0, -cx, 1, 0, -cy, 1, -cz, cx * cx + cy * cy + cz * cz - radius * radius);
    ...
    hatchField(cam, opts) {
        ...
        const families = [{ curves: parallels(this.axis, "p") }];
...
export function sphere(center, radius, id) {
...
export class Ellipsoid extends Quadric {
    center;
    radii;
    constructor(center, radii, id) {
        const [cx, cy, cz] = center;
        const [ax, ay, az] = radii;
        const A = 1 / (ax * ax);
        const B = 1 / (ay * ay);
        const C = 1 / (az * az);
        const Q = symmetric4(A, 0, 0, -A * cx, B, 0, -B * cy, C, -C * cz, A * cx * cx + B * cy * cy + C * cz * cz - 1);
    ...
    hatchField(cam, opts) {
        ...
        const at = (th, ph) => {
            const p = [cx + ax * Math.cos(ph) * Math.cos(th), cy + ay * Math.cos(ph) * Math.sin(th), cz + az * Math.sin(ph)];
        ...
        const families = [{ curves: parallels }];
...
export function ellipsoid(center, radii, id) {
    return new Ellipsoid(center, radii, id);
}
//# sourceMappingURL=quadric.js.map