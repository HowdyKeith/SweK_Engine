...
export class Cone {
    ...
    tanA;
    slant;
    constructor(apex, axisVec, baseRadius, id) {
        this.apex = apex;
    ...
    bounds() {
        ...
        const half = (i) => this.baseRadius * Math.sqrt(Math.max(0, 1 - this.axis[i] * this.axis[i]));
        const min = [0, 1, 2].map((i) => Math.min(P[i], Cb[i] - half(i)));
        const max = [0, 1, 2].map((i) => Math.max(P[i], Cb[i] + half(i)));
        return { min, max };
    }
    hatchRegions(cam, _light) {
        ...
        const pts = [projectPoint(P, this.apex).point];
        for (let i = 0; i < 32; i++) {
    ...
    hatchField(cam, opts) {
        ...
        if (opts.maxFamilies >= 3) {
            ...
            for (const s of dyadicLadder((Math.PI * diam) / Math.SQRT2 / spacing, { periodic: true, min: 4, max: 64 })) {
                ...
                spirals.push(tagCurve(paramCurve((f) => {
    ...
    raycast(ray) {
        ...
        for (const t of ts) {
            ...
            if (s < -EPS_ABS || s > this.height + EPS_ABS)
                continue; // wrong nappe or past base
    ...
    silhouetteGenerators(cam) {
        const d = cam.projection === "perspective" ? sub(cam.eye, this.apex) : cameraFrame(cam).forward;
        ...
        if (D <= EPS_ABS)
            return []; // view along axis → silhouette is the base rim
        const ratio = k / D;
        if (Math.abs(ratio) > 1)
            return []; // no real generator (looking into the cone)
        const phi = Math.atan2(d2, d1);
        const delta = Math.acos(Math.max(-1, Math.min(1, ratio)));
        const endpointFor = (psi) => {
            const m = add(addScaled([0, 0, 0], plane.x, Math.cos(psi)), addScaled([0, 0, 0], plane.y, Math.sin(psi)));
