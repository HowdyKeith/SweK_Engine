...
function rotCoordSys(u, v, newNorm) {
    const oldNorm = cross(u, v);
    const ndot = dot(oldNorm, newNorm);
    if (ndot <= -1)
        return [scale(u, -1), scale(v, -1)];
...
function projCurv(ou, ov, ku, kuv, kv, nu, nv) {
...
function diagonalizeCurv(ou, ov, ku, kuv, kv, newNorm) {
...
function projDcurv(ou, ov, d, nu, nv) {
    const [ru, rv] = rotCoordSys(nu, nv, cross(ou, ov));
    const u1 = dot(ru, ou);
    const v1 = dot(ru, ov);
    const u2 = dot(rv, ou);
...
function pointAreas(mesh) {
    const point = new Array(mesh.vertexCount).fill(0);
    const corner = [];
    ...
    for (let f = 0; f < mesh.faceCount; f++) {
        ...
        const l2 = [lengthSq(e0), lengthSq(e1), lengthSq(e2)];
        const ew = [
            l2[0] * (l2[1] + l2[2] - l2[0]),
            l2[1] * (l2[2] + l2[0] - l2[1]),
            l2[2] * (l2[0] + l2[1] - l2[2]),
...
export function computeCurvature(mesh) {
    ...
    for (let f = 0; f < mesh.faceCount; f++) {
        const t = mesh.triangles[f];
        const e = [sub(P[t[2]], P[t[1]]), sub(P[t[0]], P[t[2]]), sub(P[t[1]], P[t[0]])];
