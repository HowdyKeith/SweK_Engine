...
function weld(positions, triangles, eps) {
    const map = new Map();
    const remap = [];
    const out = [];
...
export class HalfEdgeMesh {
    ...
    static build(input, opts = {}) {
        ...
        // Face geometry.
        const faceNormals = new Array(F);
        const faceAreas = new Array(F);
        for (let f = 0; f < F; f++) {
            const t = triangles[f];
        ...
        const vertexNormals = positions.map(() => [0, 0, 0]);
        for (let f = 0; f < F; f++) {
            const t = triangles[f];
            ...
            for (let k = 0; k < 3; k++) {
                ...
                let angle = 0;
                if (l1 > 0 && l2 > 0)
                    angle = Math.acos(Math.max(-1, Math.min(1, dot(e1, e2) / (l1 * l2))));
        ...
        for (const e of edges) {
            if (e.boundary || e.crease || e.halfEdges.length < 2)
                continue;
        ...
        const gkey = (v, g) => v * F + g; // F ≥ any group id ⇒ unique
        for (let f = 0; f < F; f++) {
            const t = triangles[f];
            const g = find(f);
            const n = faceNormals[f];
        ...
        const cornerNormals = new Array(H);
        for (let f = 0; f < F; f++) {
            const t = triangles[f];
            const g = find(f);
            for (let k = 0; k < 3; k++) {
        ...
        for (let f = 0; f < F; f++) {
            ...
            if (!ref)
                groupRef.set(g, n);
            else if (groupFlat.get(g) !== false && dot(ref, n) < FLAT_COS)
                groupFlat.set(g, false);
            if (!groupFlat.has(g))
                groupFlat.set(g, true);
        }
        let flatFacetFraction = 0;
        if (totalArea > 0)
            for (const [g, area] of groupArea)
                if (groupFlat.get(g))
                    flatFacetFraction = Math.max(flatFacetFraction, area / totalArea);
    ...
    get meanEdgeLength() {
        if (this._meanEdge < 0) {
            ...
            for (const e of this.edges)
                sum += length(sub(this.positions[e.v0], this.positions[e.v1]));
