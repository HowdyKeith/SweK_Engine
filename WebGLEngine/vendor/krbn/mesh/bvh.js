...
let bvhMode = "on";
export function setBvhMode(m) {
    bvhMode = m;
}
export function getBvhMode() {
    return bvhMode;
}
...
export function buildTriangleBVH(he, bounds) {
    ...
    const boundPrims = (n, start, count) => {
        ...
        for (let i = start; i < start + count; i++) {
            ...
            if (triBounds[6 * f] < x0)
                x0 = triBounds[6 * f];
            if (triBounds[6 * f + 1] < y0)
                y0 = triBounds[6 * f + 1];
            if (triBounds[6 * f + 2] < z0)
                z0 = triBounds[6 * f + 2];
            if (triBounds[6 * f + 3] > x1)
                x1 = triBounds[6 * f + 3];
            if (triBounds[6 * f + 4] > y1)
                y1 = triBounds[6 * f + 4];
            if (triBounds[6 * f + 5] > z1)
                z1 = triBounds[6 * f + 5];
        }
        nodeBounds[6 * n] = x0;
        nodeBounds[6 * n + 1] = y0;
    ...
    const surfaceArea = (x0, y0, z0, x1, y1, z1) => {
        const dx = x1 - x0;
        const dy = y1 - y0;
        const dz = z1 - z0;
        if (dx < 0 || dy < 0 || dz < 0)
    ...
    const build = (start, count, depth) => {
        ...
        for (let axis = 0; axis < 3; axis++) {
            const ext = cext[axis];
            if (!(ext > 0))
                continue; // flat on this axis ⇒ every centroid in one bin
            ...
            for (let i = start; i < start + count; i++) {
                ...
                binCount[b]++;
                for (let k = 0; k < 3; k++) {
                    if (triBounds[6 * f + k] < binBox[6 * b + k])
                        binBox[6 * b + k] = triBounds[6 * f + k];
                    if (triBounds[6 * f + 3 + k] > binBox[6 * b + 3 + k])
                        binBox[6 * b + 3 + k] = triBounds[6 * f + 3 + k];
                }
            }
            ...
            for (let p = 1; p < BVH_BINS; p++) {
                ...
                if (nL === 0 || nR === 0)
                    continue;
                const cost = BVH_TRAVERSAL_COST + BVH_TRI_COST * (nL * leftArea[p - 1] + nR * rightArea[p]);
                if (cost < bestCost) {
                    bestCost = cost;
                    bestAxis = axis;
                    bestPlane = p;
        ...
        const parentArea = surfaceArea(nodeBounds[6 * n], nodeBounds[6 * n + 1], nodeBounds[6 * n + 2], nodeBounds[6 * n + 3], nodeBounds[6 * n + 4], nodeBounds[6 * n + 5]);
        const leafCost = BVH_TRI_COST * count * parentArea;
        const wantLeaf = bestAxis < 0 || bestCost >= leafCost;
        if (wantLeaf && count <= BVH_MAX_LEAF)
            return makeLeaf();
        let mid;
        if (bestAxis < 0) {
            ...
            for (let i = start; i < start + count; i++) {
                const f = primIndex[i];
                if (binOf(centroids[3 * f + bestAxis], cmin[bestAxis], kInv) < bestPlane)
                    scratch[lo++] = f;
            }
            let hi = lo;
            for (let i = start; i < start + count; i++) {
 ...
 */
function traverse(bvh, ray, stack, out) {
    if (bvh.nodeCount === 0 || out.length === 0)
