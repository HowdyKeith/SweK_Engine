...
export function parseSTL(data) {
    if (typeof data === "string")
        return assemble(parseAsciiSTL(data));
...
export function parseOBJ(data) {
    ...
    for (const face of faces) {
        for (let i = 1; i + 1 < face.length; i++) {
            ...
            if (!pa || !pb || !pc)
                continue; // out-of-range index — skip, don't crash
            if (isDegenerate(cross(sub(pb, pa), sub(pc, pa)), pa, pb, pc))
                continue;
...
const num = (s) => (s === undefined ? 0 : Number(s));
...
function assemble(facets) {
    ...
    for (const { normal, loop } of facets) {
        ...
        for (let i = 1; i + 1 < loop.length; i++) {
            ...
            if (isDegenerate(gn, a, b, c))
                continue; // zero-area facet — no orientation
...
function isDegenerate(gn, a, b, c) {
