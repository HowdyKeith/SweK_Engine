...
export const raw = (svg) => ({ toSvg: () => svg });
/** Ship a single 3-D scene: bind it to a camera. */
export const view = (scene, cam) => ({
...
export function textAt(cam, world, text, anchor = "middle") {
...
export function withLabels(svg, labels) {
...
export const label = (x, y, s) => `<text x="${x}" y="${y}" font-family="sans-serif" font-size="14" fill="#999">${s}</text>`;
/** A centered caption `<text>` (for column headers). */
export const labelC = (x, y, s) => `<text x="${x}" y="${y}" text-anchor="middle" font-family="sans-serif" font-size="14" fill="#999">${s}</text>`;
// ---------------------------------------------------------------------------
...
function gridStitch(W, H, rows, rowLabels, colLabels) {
    const gapX = 22;
    const gapY = 18;
    ...
    rows.forEach((row, r) => {
        ...
        if (r < rows.length - 1)
            parts.push(`<line x1="10" y1="${y + H + gapY / 2}" x2="${totalW - 10}" y2="${y + H + gapY / 2}" stroke="#ddd" stroke-width="1" />`);
    });
    ...
    if (colLabels)
        colLabels.forEach((s, c) => parts.push(labelC(c * (W + gapX) + W / 2, 20, s)));
    parts.splice(1, 0, `<defs>${defs.join("")}</defs>`);
    parts.push(`</svg>`);
    return parts.join("\n");
}
/** Stack two same-size panels vertically into one SVG with a divider + labels. */
function stackRows(top, bottom, W, H, labelTop, labelBottom) {
    const gap = 18;
    return [
...
export function grid(vp, rows, opts = {}) {
    return raw(gridStitch(vp.width, vp.height, rows, opts.rowLabels ?? [""], opts.colLabels));
}
/** Ship a composed figure: two panels stacked vertically with captions. */
export function stack(top, bottom, vp, labels) {
 ...
 */
export function film(count, frame, opts = {}) {
    const frames = [];
