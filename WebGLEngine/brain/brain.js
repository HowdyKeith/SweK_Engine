...
const BRAIN_BUILD = "v4253";   // v4253 -- *** THE TREE COULD RETARGET A POSE AND MAKE A SKELETON GO LIMP, AND COULD NOT PUT A HAND ON A DOORKNOB. *** Grepped for FABRIK, ccdIK, solveIK and inverseKinematic across the whole tree: no hits anywhere. New anim/ik.mjs is FABRIK plus CCD plus the analytic two-bone case, and it is gradeable because IK has closed forms: the solved joint angle matches acos((l1^2+l2^2-D^2)/(2*l1*l2)) to 2.84e-6 rad, and BEYOND THE CHAIN'S REACH THE ERROR IS EXACTLY D - L to twelve decimals, which a bone-stretching cheat would UNDERSTATE. The control that makes those mean anything is bone length, and it produced the round's real finding: FABRIK preserves every length to 3.3e-16 while CCD drifts 9.08e-7 -- and that is not a bug in CCD. Rotating a vector by the IDENTITY quaternion through the tree's own qMul changes 0.7810249675906655 to 0.7810249924659729, exactly Math.fround of it, because qMul returns through a Float32Array. Confirmed a FLOOR not accumulation: 1 iteration costs 1.78e-7 and 9 cost 3.26e-7. Two more corrections the gate forced: I nearly shipped 'the two solvers agree' as a check, which holds on two bones (1.24e-6) and FAILS on four by 2.11 m because a four-bone chain in 3D is REDUNDANT and both answers are right; and FABRIK's convergence COLLAPSES near full extension -- 6 iterations at 87.5% of reach, no convergence in 200 at 99.75% -- with a bent-start control showing it is the geometry and not the fixture. A sabotage removing twoBoneAngles' clamp left the gate ALL GREEN, so a check was ADDED and the hole recorded. Full changelog on docs/CHANGELOG.md.
