...
const BRAIN_BUILD = "v4389";   // v4389 -- *** THE MECHANICAL MUTATION SCANNER COULD NAME MUTATIONS FROM THE DAY IT WAS WRITTEN AND HAD NEVER APPLIED ONE. *** tools/mutate/scan.mjs exists because a hand-picked set "measures the AUTHOR'S IMAGINATION, not the gate" -- so it walks the source and perturbs every numeric literal mechanically. It enumerates and prints. v4387 filed that as #151; tools/mutate/mechanical.mjs is the runner. *** THE GAP FIRST, BECAUSE IT IS THE MOTIVATION: scan.mjs finds 105 numeric constants in the eight files the hand-picked ten touch. Ten of a hundred and five -- THE SUITE THAT SCORES 10/10 EXAMINES 9.5% OF THE NUMBERS IN ITS OWN TARGET FILES, and that ratio appears nowhere in the 10/10. *** WHAT MAKES IT AFFORDABLE IS THAT A MUTATION TO ONE FILE CANNOT BE SEEN BY A GATE THAT DOES NOT REACH IT. *** tools/ship/affected.mjs has known the import graph since v3041 and nothing in the mutation directory had asked it: blobPolicyStore.js is reached by ONE gate of 1425, box3dLockstep.js by four. Not an approximation -- the same answer without the 1421 gates that could not have noticed. *** AND THE ORDER WITHIN THAT SET IS WHAT MAKES IT USABLE, WHICH THE FIRST DRAFT LEARNT THE EXPENSIVE WAY. *** Sorted by name, a 3-constant file burned 27 MINUTES without producing one verdict, because its 98-gate set holds several multi-minute gates. A mutation is CAUGHT at the FIRST red, so the set is now ordered cheapest-first from tools/ship/sweep-timings.json -- a 164x spread between 123 ms and 20165 ms -- and poly6's 315 was caught after ONE gate of 107. A gate with no recorded time sorts LAST, because unmeasured is not cheap. The control moved too: running all 98 gates up front to check they are green costs the full set (794 s) before a single mutation, so it is paid LAZILY -- only a gate that actually goes red is asked whether it was red already, and two (statedRuntime, proseAudit) were dropped by name mid-sweep. *** THE FIRST SWEEP: 19 constants across 5 files -- 6 CAUGHT, 7 SURVIVED, 6 UNMEASURED. *** Beside 10/10 that looks damning, and read carelessly it would be: IT IS MOSTLY A FINDING ABOUT THE OPERATOR. A single relative 3% perturbation is a PHYSICS mutation, meaningless for an integer tick count or a JSON indent, and five of the seven survivors are exactly that. *** THE redundancy LINE SETTLES IT, BECAUSE BOTH SUITES MUTATE IT: *** the hand-picked entry sets it to 0 and is CAUGHT; the mechanical operator sets it to 4.12 and it SURVIVES. Same line, same file, opposite verdicts -- so the difference is not gate coverage, it is whether the mutation is a plausible defect for that KIND of number. *** BUT TWO SURVIVORS ARE REAL, AND WERE PUT ON TRIAL. *** Each was re-applied and graded by the FULL verify -- 934 gates, not the four that reach the file -- and both came back ALL GREEN: a three-percent change to physics/box3dLockstepNet.js's LOCKSTEP TIMESTEP, and to box3dLockstep.js's shipHalf default. *** AND THE REASON THE TIMESTEP SURVIVES IS NOT THE OBVIOUS ONE. *** The first theory was "the default is never exercised"; reading the callers killed it -- all four createLockstepNet calls in the two gates that reach the file leave dt at its default. They build TWO PEERS and compare them to each other, so a change to a constant they SHARE moves both sides of the equality and cancels. A differential cannot see a shared constant, which is the same family as a twin built from the same shell; by contrast physics/lockstepDt-selfcheck.mjs compares the default against an explicit 1/30 -- an absolute -- which is exactly why box3dLockstep.js's own dt default WAS caught. Running the scanner also exposed a defect in it: it stripped "//" and not STRING LITERALS, so it perturbed the 90 in the prose "...amplify that ~90,000x" and filed a number in a sentence as a constant nothing checks. Fixed with a length-preserving stripper (105 constants, was 106 -- exactly the one prose number). Four sabotages, 2/2/3/1 red by name, all three files md5-identical; C is the instructive one -- folding UNMEASURED into SURVIVED does not merely overstate the count, it silently un-covers the trial meant to back it. UNCHECKED: the sweep is not re-run by the gate -- forty minutes against milliseconds -- and three of the eight files are unswept because their gate sets are 100-plus wide and hold gates past the 120 s cap. THE STANDING LIMIT IS THAT THE SWEEP IS AFFORDABLE EXACTLY WHERE THE DEPENDENCY GRAPH IS NARROW, and narrow means few gates care. Nineteen constants is not a mutation score for SweK; it is a measurement of five files and of the operator that measured them. The tree stands at 1426 gates. Full changelog on docs/CHANGELOG.md.
