...
const BRAIN_BUILD = "v4387";   // v4387 -- *** THE MUTATION SUITE'S SCORE WAS PROSE, AND IT HAD BEEN FALSE FOR 223 VERSIONS. *** tools/mutate/scan.mjs opened by stating a perfect score for the ten mutations as a bare literal in a comment. v4162 rewrote physics/sph/sph.js's shadow amplitude from Math.pow(o.h, 3) to (h * h * h) -- same arithmetic, different text, and that commit explains why (pow and multiplication need not agree bit-for-bit). But a mutation's find-string is a COPY OF SOURCE TEXT rather than a reference to it, so from that day the mutation applied NOTHING: nine experiments and one abstention, reported as ten results. *** AND THE GATE BUILT FOR EXACTLY THIS ROT CAUGHT IT, BY NAME, AND THEN STOOD RED FROM v4279 TO v4385. *** tools/ship/mutationTable-selfcheck.mjs (v2884) exists to check that every find-string is still present, and its own header already records a PREVIOUS occurrence of the same rot -- Math.pow(h, 6) becoming _p6(h) in kernels.js. So the harness was honest (mutate.mjs has a STALE branch that excludes a dead mutation from the score), the gate was right, and the only thing that failed for 106 versions was that nothing read it. *** THEN THE SUITE WAS ACTUALLY RUN, WHICH NOBODY HAD DONE. *** Eleven full verifies, about fourteen minutes: 10/10 CAUGHT, and the restored mutation was caught on its FIRST REAL RUN. So no hole in the net was ever hidden -- the fluid-shadow behaviour was guarded the whole time. What was broken was the MEASUREMENT of it, which is a smaller finding than a surviving mutation and a more insidious one, because a suite reporting a perfect score is the one nobody re-examines. *** THE FIX IS A RECORD THAT CAN DETECT ITS OWN ROT. *** tools/mutate/mutationScore.mjs carries the states, a tableFingerprint over every mutation's name/file/find/replace/breaks, and the COMMIT the run was taken at. Two rots, two detectors: edit any mutation and the fingerprint moves and the gate refuses the record by name; change any file the score is ABOUT and mutationScore-selfcheck asks git and says which. THAT SECOND CHECK IS THE ONE THAT WOULD HAVE FIRED AT v4162. What it deliberately does NOT do is fail on age -- a version count would force every round to re-run fourteen minutes or edit a number, and this tree knows what an editable threshold is worth. No count is stored beside the states either: tally() derives caught/survived/stale/total, because a total typed next to the rows it totals is the same defect one size smaller. *** AND THE ROUND'S FIRST HYPOTHESIS WAS WRONG TWICE OVER, WHICH IS THE MORE USEFUL RESULT. *** The guess was that the red register held gates somebody had since fixed, padding every green verify. All 28 entries of RED_AT_V4279 were re-run serially, 597 s of wall clock: 28 of 28 genuinely red. Then the second correction, and it is the one worth having: THE TREE HAD ALREADY SOLVED THIS. tools/ship/register-audit.mjs freezes an observed run of every register entry, tools/ship/freezeRegisterAudit.mjs produces it, and tools/ship/registerDrift-selfcheck.mjs compares the two ON EVERY SHIP -- so a register carrying a repaired gate could never have gone unnoticed, and the audit had been re-frozen as recently as v4380. The hand re-run was a corroboration of a mechanism already in place, not a discovery, and saying so is the point: the guess was that a record had rotted, and the answer was that this particular record has a keeper. One entry leaves it: mutationTable-selfcheck is pruned to FIXED_SINCE_V4279 with its cause, using the term the census added at v4318 precisely so that pruning is how the arithmetic balances. TWO STALE PROSE CLAIMS WERE ALSO CORRECTED: the gate's failure line said a dead mutation "WOULD REPORT A PHANTOM SURVIVOR", which mutate.mjs's STALE branch already prevents -- a gate about stale records carrying a stale claim about staleness; and scan.mjs's sentence now points at the record. *** THE CORRECTION TRIPPED THE SELF-COUNTING TRAP: *** quoting the false claim verbatim put the literal back in the file and the gate that greps for it went red on the very edit that removed it. Reworded, not excluded, because an exclusion would have left the next real occurrence unfindable too. Four sabotages, 3/3/1/1 red by name, md5-identical after; B restores the actual v4162 defect and both the find-string check and the fingerprint catch it. UNCHECKED: the score is not RE-MEASURED by the gate -- it costs milliseconds and the run costs fourteen minutes, so what is checked is that the record is CURRENT, never that it is right. And now named: tools/mutate/scan.mjs, the MECHANICAL scanner built because a hand-picked set measures the author's imagination rather than the gate, is imported by nothing, invoked by no rig job, and records nothing -- the better measurement sitting unrun beside the weaker one, which is the same shape as the defect this round fixed. The tree stands at 1425 gates. Full changelog on docs/CHANGELOG.md.
