...
const BRAIN_BUILD = "v4267";   // v4267 -- *** orrery.html HAD EXISTED SINCE v3195 AND NOTHING IN THE TREE LINKED TO IT. *** Backlog asked for three big buttons on server.html: SweK Orrery, GitHub Terrain, GitHub Universe. Two shipped. The orrery button opens a page that was built, gated, and unreachable -- the fifth time server.html's own comments record a built thing nobody could find. GitHub Terrain had the opposite problem: window.repoTerrain has been wired in main.js since v4157 and is CONSOLE-ONLY, so the button needed a door built for it -- a ?terrain= deep link mirroring the existing ?go= idiom, read on load. *** THE THIRD BUTTON IS DELIBERATELY ABSENT AND THE GATE ASSERTS ITS ABSENCE. *** There is no GitHub Universe page; es-universe.json is the Escape Velocity dataset ev.html reads, not a universe of repositories. A button opening a 404 is worse than no button, and backlog #139 is the round that would build it. tools/ship/serverButtons-selfcheck.mjs checks both halves of each link -- the button AND the thing that consumes it -- plus a GENERAL rule: every shipped page target must resolve on disk. That rule found a pre-existing hit in render-qa output, which on inspection is a generated artefact and is now excluded BY NAME rather than silently. CORRECTED BEFORE SHIP: the gate asserted es-universe.json's govts was an array; it is an object -- a shape I had not looked at. And the first sabotage log recorded 3 RED three times from what the checks LOOK like they do; re-run, they measure 2, 1 and 2. Full changelog on docs/CHANGELOG.md.
