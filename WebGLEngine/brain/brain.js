...
const BRAIN_BUILD = "v4132";   // v4132 -- the changelog had been recording eighteen rounds under no version at all: changelog.mjs checked ENTRY_HEAD only to find where to insert, never to check what was arriving, so headless prose went in above the newest heading and each round landed below the last -- an oldest-first run inside a newest-first file, invisible to namesVersion(). Repaired from git rather than memory (each headline confirmed unique before insertion, the run reversed, a multiset diff proving nothing was lost), and the write now refuses a headless entry outright. Also: two camera pages tore down on pagehide without a persisted check (fixed by routing a freeze through the page's own Stop, not by skipping the release -- a live getUserMedia keeps the light on while frozen); one gate's runtime measured and recorded; and boundaryLint's baseline re-frozen after reading all five changes, with the reasoning written into the baseline file. 1230 gates. Full changelog on docs/CHANGELOG.md.
