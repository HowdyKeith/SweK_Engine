...
const BRAIN_BUILD = "v4297";   // v4297 -- *** v4296 SAID "0 REGRESSED" AND COULD NOT HAVE KNOWN. THE FULL SWEEP SAYS SIX. *** The re-check ran 37 gates that were already red, so not one of them was eligible to regress, and the zero it reported covered the 1,329 it never ran; the prose in the same commit called the question UNKNOWN and the field beside it said 0, and the field is the half a reader greps. So this round ran everything: all 1,366 gate files, phase 1 under xargs -P 8 at 180 s, then EVERY candidate re-run alone at 300 s, cheapest first, resumable. 107 candidates (51 exit-1, 56 timeouts). Serially: 48 CONFIRMED RED, 38 FALSE REDS, 21 UNMEASURED, 1297 green. *** THIRTY-EIGHT OF ONE HUNDRED AND SEVEN -- 36% OF WHAT PHASE 1 CALLED RED WAS STARVATION. *** That figure is why phase 2 is now code rather than a paragraph. I argued three of them were "fast enough that starvation is a weak explanation" -- esFleetSize at 16 s, carrySpawn at 18 s, cinematicShot at 164 s exit-1 -- and all three went green alone (11.2 s, 17.6 s, 78.5 s). Zero for three; a parallel red is a hypothesis and I demonstrated it on myself. THE 48 SPLIT THREE WAYS AND THE SPLIT RECONCILES: 37 still red -- every entry in RED_AT_V4279, none repaired, eighteen rounds now; 5 from the v4279 slow bucket, never run serially then and red now that they have 300 s (detectionMap, sensitivity, doorKinds, graveyard, orphanDisposition -- that bucket is 5 for 5 on producing real reds once given time); and SIX REGRESSIONS, gates GREEN at v4279 and red at HEAD: backendParity (544 ms), copiedOutsideVendor (302 ms), gateQuality (604 ms -- one of the two v4279 introduced and fixed, red again), postChain (44 s), staleness (712 ms) and windowsImport (733 ms). 37 + 5 + 6 = 48. Five of the six cost under a second; they were cheap to run and nobody ran them. gateSweep.mjs is the method as data: classify() cannot return RED without a serial result, checked exhaustively over 110 input combinations; finalize() THROWS when a candidate was never re-run serially, control first; and it distinguishes NOT RUN (phase 2 skipped it -- refuse) from UNMEASURED (ran alone for 300 s and did not finish -- report, never fold into red or green). The first draft conflated those two and would have refused the whole result over twoFBind and toolFrontDoor, which is a delete-the-entry incentive, so they were split. coversRegressions() is the rule v4296 broke: a method may report on regressions only if it ran gates that were not already red; RECHECK now carries regressedAmongChecked: 0 and regressedOverall: "unmeasurable by this method", and the bare `regressed: 0` is asserted ABSENT. SWEEP_V4297 is the frozen record and gateSweep-selfcheck section 7 re-derives it: green + red + unmeasured = swept; red + false + unmeasured = candidates; the three-way split; no regression already red or already unmeasured at v4279; every named path exists; the module's own regressionsAgainst() recovers the same six. Six sabotages, all red by name; two earlier ones went 0 red and were fixed -- a 124 row at 180031 ms already satisfied the ms clause so the exit-code clause was untested, and no gate exists under node_modules/.claude/vendor so the skip-list assertion was vacuous. An input that never reaches the guarded branch makes a check unfalsifiable. Unmeasured and named, not hidden: 21 gates, 16 of them under tools/roundhouse, 3 under physics/sph, plus orphanTriage, shaderRefs and toolFrontDoor -- twoF itself went green at 180.4 s. Also corrected: the orrery I described as canvas-2D with no shader stage has had one since v4273. Next: the six regressions, cheapest first. The tree stands at 1367 gates. Full changelog on docs/CHANGELOG.md.
