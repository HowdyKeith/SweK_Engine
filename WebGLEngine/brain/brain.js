...
const BRAIN_BUILD = "v4298";   // v4298 -- *** THE GITHUB TERRAIN BUTTON HAS BEEN DEAD SINCE THE ROUND THAT ADDED IT, AND THE SONG BUTTON IS WHAT FOUND IT. *** Keith asked for "audio file to terrain, with a big button on the right side of server.html". The terrain half shipped at v4280 -- world/songHeightfield.mjs turns any clip into a heightfield, rows time, columns frequency, height loudness, silence water -- and was console-only, the exact state GitHub Terrain was in before v4267. So this round is the other half: bSongTerrain on server.html beside bRepoTerrain, wearing .go-link because it is an engine deep-link and not a page; ?song=1 in main.js, the same shape as ?terrain=1; and load({file}) so a File from a picker takes the same decoder as a URL, one decodeAudioData and not two. One thing the button cannot do is open a file dialog by itself. A picker opens only from a user gesture, a page cannot click <input type=file> for itself on boot, and the browser is right to refuse -- so the deep link flies a rising sweep at once, because something must happen when a button is pressed with nothing filled in, and puts a chooser on screen whose click IS the gesture; ?song=<url> walks that file instead of the sweep, and the chooser takes a dropped file too. songButton-selfcheck drives it the way a person would: boots index.html?song=1 in a real browser, waits for the sweep and the chooser, writes a 3 s 2000 Hz PCM WAV and picks it through the chooser with setInputFiles. The ridge lands at 1906 Hz where a column is 172 Hz wide at the decoder's 43.1 Hz per bin, the field is 2.98 s long, and the control that matters is that the tone field DIFFERS from the sweep field -- without it a ridge check against an unchanged field measures the sweep and calls it the song. *** THE FINDING: ?terrain=1 NEVER FIRED. *** The first browser run showed songTerrain present and lastField null forever, and the reason is that v4267 hung the link on window.addEventListener("load") -- but by the time that part of main.js runs the document's readyState is already "complete"; the load event fired while the engine was still importing, and the listener waited for an event that had already happened. I copied that shape for ?song= and inherited the bug, which is how it was found; the same run against ?terrain=1 showed repoTerrain.lastField null too. The GitHub Terrain button booted a plain engine from v4267 to v4297, thirty-one rounds, and nothing noticed because realTerrainFlyIn-selfcheck calls .load() itself rather than arriving by the link, and repoTerrain-selfcheck reads source. The box3d demo at main.js:20422 had the right shape all along -- check readyState first, listen only if it is not complete -- and both links now go through one afterBoot() that does that. The gate proves it in the browser with a setter trap installed before any page script: window.repoTerrain is wrapped the instant main.js assigns it, and ?terrain=1 now reaches repoTerrain.load({}) -- the button is alive for the first time since v4267. Four sabotages, all red by name: the onclick removed goes 1 red; load() ignoring o.file goes SIX red, the picked file failing to decode because fetch(undefined) fetched the page, the field staying the sweep's, the control catching it; afterBoot reverted to the bare load listener -- the exact v4267 shape -- goes red in source and shows BOTH links dead in the browser, which is the state the tree shipped in for thirty-one rounds and this file is the first gate that would have gone red over it; the chooser never called goes 2 red. Two mistakes caught while writing, not by sabotage: page.waitForFunction takes its options THIRD, and passed second the timeout silently stayed at 30 s; and the first draft served swek.local from the repo root rather than WebGLEngine/, so index.html was a 404 and the gate reported a timeout instead of the cause. Unchecked and said plainly: the drop path is not driven (the gate uses the input, not a DataTransfer), and any format beyond PCM WAV is decodeAudioData's coverage, not this file's. The six v4297 regressions are still next, cheapest first. The tree stands at 1368 gates. Full changelog on docs/CHANGELOG.md.
