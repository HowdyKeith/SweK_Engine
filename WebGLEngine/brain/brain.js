...
const BRAIN_BUILD = "v4285";   // v4285 -- *** v4284 SAID THE COMPOSITE WOULD DECIDE WHETHER THE POST CHAIN CAN BE ONE DISPATCH. IT DECIDES NOTHING; GOD RAYS DECIDES, AND IT DECIDES NO. *** render/passFootprint.mjs settles it by MEASUREMENT rather than by reading shaders: a pass can join a tile-local dispatch only if its read footprint is bounded by a constant apron independent of image size, and counting taps in source gives a CLAIM about that while rendering the pass twice -- identical but for ONE texel -- and collecting every output pixel that moved gives the footprint itself. Measured on a real WebGL2 driver, perturbing the centre texel: BRIGHT_FS 0 and 0, BLUR_FS 4 and 4, GODRAYS_FS 15 at N=32 and 31 at N=64. *** THE LAST ROW IS THE FINDING: *** a footprint that grows with resolution cannot be covered by any fixed apron, so god rays cannot join a tile-local dispatch at any size, and the numbers are not merely large -- 15 = 31-16 and 31 = 63-32, exactly the distance to the far corner, so the true footprint is THE REST OF THE IMAGE ALONG THE RAY FROM THE SUN. At 1920 wide the apron would be about 1700 texels, which is not an apron, it is the frame. The blur's 4 is the same at both resolutions AND equals the kernel reach parsed from the shipping shader, so measurement and source have to agree. THE ANSWER IS TWO DISPATCHES, STRUCTURALLY: bright + blur + SSAO + composite can share a tile; the composite is fusable IN ITSELF (its reads capped by literals -- a 3x3 depth Sobel and a heat displacement of at most 0.0035 UV) and still cannot be fused because it CONSUMES the pass that cannot -- fusable and reachable are different properties and only the second one ships. SSAO is a third category, bounded by a UNIFORM rather than a literal. Four sabotages: the radius accumulator starting at 0 instead of -1 reddens the control that separates "no dependency at all" from "purely local"; the row flip dropped gives 16 and 32 instead of 15 and 31, *** STILL DOUBLING SO THE TREND CHECK STILL PASSES, *** and only the check predicting the EXACT number catches it -- a trend is cheap to satisfy and a number is not. *** AND ONE WENT 0 RED BECAUSE THE GATE WAS OVERSTATING: *** hardcoding DENSITY changed no prediction, and the check claimed "the geometry predicts both numbers from DENSITY alone" when at the centre the radius is set by the corner and density never enters. Chasing it found a regime where it does -- one texel from the sun, 0.9 predicts 3 and 1.0 predicts 30 -- AND showed the model is WRONG there: the device says 12 against a modelled 3, because near the sun every pixel's ray converges. The model is scoped to the centre, the disagreement recorded as MODEL_LIMIT rather than tuned away, and the redone sabotage goes 2 red. Not measured and marked as such: the composite and SSAO need textures this harness cannot bind separately, so their entries are read off SOURCE -- the weaker evidence this round exists to replace. Full changelog on docs/CHANGELOG.md.
