...
const BRAIN_BUILD = "v4318";   // v4318 -- *** THE CONSTRUCTIONS THE v4315 PREVIEW NAMED, BUILT -- EVERY ONE COUNTED ON THE DEVICE. *** (1) GIT TIME ON THE GPU. The cull's uniforms grow a fourth vec4, clock = (t, enabled, 0, 0), forty floats; a record whose extra.z (its opening day: the day git says the body was vendored) is after t is dropped IN THE CULL, on WebGPU by the compute pass and on WebGL2 by the twin. orrery-gpu.html hands every body record its market's opening day and the sim day, so the sky on any day is the sky git had that day, decided by no CPU line. gpuGitTime-selfcheck: on days 0, 5, 11 and 41 the device's counts are exactly 1 + the markets open that day (10, 11, 15, 15 of 15) on BOTH backends, the last body to arrive (draco, day 11) is not under the pointer before its day and is on it, and the twin agrees with the compute pass day for day. (2) THE MASK ON THE DEVICE. gfx/device.js frame({ target }) draws a frame into a texture on both backends -- a render attachment in the canvas format on WebGPU, a framebuffer on WebGL2 blitted UPSIDE DOWN into the target so its row 0 is the top as a WebGPU target's is and ONE shader samples both -- and render/fleetMask.mjs makeMaskRig keeps the colour picture, the identity picture (scene.pickTo) and the mask (a pick-to-mask pass decoding the fleet from the blue byte) in three targets, badTv through them, nothing read back. fleetMask-selfcheck holds the device path to v4317's CPU path TO THE BYTE on both backends: the colour target is the offscreen readback (36,864 of 36,864), the device's mask is the CPU's mask pixel for pixel (435 inside), badTv through it is identical (worst 0) and the outside untouched; the device path took 62 ms against the CPU path's 498 on SwiftShader, reported not graded. The page's masked look draws through the rig now. (3) TWO UNIVERSES DRAWN AS ONE PICTURE. The same rig composites universe A where a race's silhouette is and universe B everywhere else (out = mix(B, A, mask)): inside the two races' silhouettes every pixel is A's, outside every pixel is B's, to the byte, both backends; ?twin=1 in the page runs a second economy (seed 8, the same crew) beside the first and the masked race is the window onto it. In one tab -- the wire carries one universe, and the entry says so. (4) A RACE THAT LIVES IN THE DEPTH PYRAMID. Every record's extra.w is a STEALTH BIAS the Hi-Z cull reads: a positive bias hides the record when it is within that margin of being occluded -- level with or just in front of whatever covers its footprint -- and never against open sky (a tile the clear reached has far = 1 and hides nothing). stealthRace-selfcheck, WebGPU (the only backend with the pyramid): a stealth record 0.006 in front of a wall vanishes on frame 2, the same place with bias 0 stays, a stealth record over the sky stays, and the twin given the device's own pyramid hides exactly the record the device hid; WebGL2 draws all four on both frames and is said to. No race wears it yet. (5) TEN THOUSAND LEARNERS IN ONE DISPATCH. world/populationPolicy.mjs generates a compute pass for a POPULATION of trader policies (FlightPolicy tanh MLPs, sizes baked in as constants) and scores learner p's candidate c per invocation on gfx/device.js: 10,000 learners x 8 candidates, every score within 1.8e-7 of FlightPolicy.act's and every learner's choice the CPU's (10,000 of 10,000); refused on WebGL2 by name. The scoring half; the economies are not stepped in lockstep (step() asks its policy synchronously), so the trainer still runs one economy at a time -- said in the gate and the preview. (6) THE BLACKBODY, WITH WIEN'S ROOT FOUND BY THE SHADER. render/blackbodyWgsl.mjs: Planck's shape x^n / (e^x - 1) and Wien's root x = n(1 - e^-x) by a Newton iteration IN THE WGSL and the GLSL; the compute probe reads x_lambda = 4.965114 and x_nu = 2.821439 off WebGPU within 2e-6 of the f64 module's (residual 0), the shape at the root is the module's peak to 1e-4, the grid of 2,048 is the twin's to 1.8e-6 relative, and on BOTH backends the full-screen key's brightest column on the n = 5 row is x_lambda to within a column. The third physics module with an exact key: the parity census moves to 142 GLSL, 52 WGSL, 10 both, seven shader modules in both languages. (7) THE LIVE COMMITS SOURCE. world/commitsFeed.mjs liveFeed asks the ai-bridge's GET /github/commits (ai-bridge/server.js -> githubBridge.listCommits) once per vendored repository and groups the answers by calendar day into the records applyCommitsFeed already books through the "commits" intervention -- journaled, replayed, lockstepped; a repository the bridge cannot answer for is a failure line, not a throw. universeJournal-selfcheck 3b with a mocked fetch: three commits on two days become two day-records with the right counts, a 404 and a thrown fetch are named and the feed goes on, the records book one intervention each at the right market, asked twice nothing is booked twice. The page's "live commits" button (?owner=). Four new gates (gpuGitTime, stealthRace, populationPolicy, and fleetMask's two device sections), physicsShaders and universeJournal grown. Sabotaged and measured, every gate: the git-time comparison flipped, the twin's clock removed and the heading read for the day (4 red each, the backends parting); the mask shader reading the red byte, the WebGL2 blit not turned over and the composite's mix swapped (3, 4 and 1 red, each on the backend whose language was touched); the stealth sky guard removed and the bias ignored (2 red each); the population's tanh dropped and every neuron reading neuron 0's bias (5 and 3 red -- the bias one was GREEN until the biases were made non-zero, and the gate says so); the blackbody's Newton started at x = 1 (6 red: the trivial root) and e^x for e^x - 1 (2 red), with two sabotages the gate cannot see recorded as such (a wrong derivative sign and 2 Newton steps both still reach the root). Still unchecked and said: a race that wears the stealth bias; the population's economies in lockstep (the trainer's half); the live source against the real bridge (the fetch is mocked); the device path's time on a real GPU; and the rig, the wire between machines and the second engine's hash, one page load each on Galaxina (RIG-PENDING in three gates). The tree stands at 1395 gates.
