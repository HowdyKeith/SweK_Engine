...
const BRAIN_BUILD = "v4372";   // v4372 -- *** THE SCULPTOR AND THE JUDGE IN ONE ROOM -- AND THE PREDICTION THIS ROUND WROTE DOWN FIRST WAS WRONG. *** The correction is the finding. Written before the run: a visual hull's excess sits where no view could see, so it is INTERIOR, so the silhouette gates would pass it everywhere; the tube carved from sixteen azimuths would score near 1 and the over-carved cube would be caught. What the run says: THE SAME HULL, WITH THE SAME 54.9% VOLUME ERROR, IS REFUSED FROM ONE CAMERA AND PASSES FROM ANOTHER. Obliquely it reads IoU 0.9298 with a scale delta of 0.1023 and is refused by name; from the side it reads IoU 0.9484 and 0.0548 and both hard gates pass, with 429 pixels differing that the gates read none of. The bore filled in is hidden behind the tube's own wall from the side and is sky the true tube shows through its rim from an angle -- so an interior error becomes an outline event by moving the camera, and the verdict is a property of the CAMERA rather than of the error. The camera that passes it is the one nearest the views it was carved from. THE ROUND: v4371 built mesh/carve.mjs and scored its hulls by REPROJECTION -- a hull against the silhouettes it was carved from, 1.000000 by construction and never at risk. v4366 ported img2threejs's judge (render/divineEye.mjs: their two hard gates, silhouette IoU >= 0.85 on a 224 grid and an area scale delta <= 0.08) and measured it on a model against a modified model. Neither asked what img2threejs's pipeline actually asks: RENDER the reconstruction and the reference from one camera and score the pictures. mesh/carve.mjs surfaceMesh turns an occupancy grid into one quad per exposed voxel face in the shape render/gpuDriven.mjs packMeshes eats, so a carved hull draws through the fleets' own lit pipeline on gfx/device.js, and voxel space maps to (-1..1) for the hull and the solid alike -- framed identically by construction rather than by a normalisation that could move one and not the other, which is v4366's own mistake wired in as a check. FOUR CASES, EACH WITH A KNOWN VOLUME ERROR AND A CONTAINMENT FACT CHECKED HERE RATHER THAN TAKEN FROM THE OTHER GATE: the tube from sixteen azimuths, 54.9% over; the tube from two azimuths plus a view from above, 0.0%; the cube from two views at 0 and 90, 0.0%; the cube from three at 0, 60 and 120, 31.0% over. Both EXACT hulls render to their solids on every pixel from both cameras -- 0 differing of 65,536 -- so the pixels the over-carved hulls move are real and the gates are declining to read them rather than not being given them. And volume error does not order the verdicts: the cube at 31.0%, little more than half the tube's error, is refused from BOTH cameras. *** WHAT THIS SETTLES BETWEEN TWO ROUNDS THAT LOOKED LIKE THEY DISAGREED. *** v4371 reported reprojection IoU 1.000000 across hulls spanning 0% to 90% volume error; v4366 measured the same judge catching a 3% LINEAR enlargement. Both hold. A scale change moves the outline and is read; a visual hull's error is exactly the material no silhouette could see, and whether that lands inside the outline depends on where you stand. The gates are not weak about volume -- they are silhouette statistics being asked a volume question. And the pictures here are RENDERS from cameras no view carved from, so the exact scores are not by construction. TWO SABOTAGES AND ONE OF THEM WENT 0 RED AND IS RIGHT: removing the mesher's interior-face cull changes no pixel, because a face between two solid voxels is behind a closed opaque surface, so the cull is a SIZE optimisation and the gate now counts what it buys -- 96 faces against 384 on a 4x4x4 cube, 75.0% dropped -- instead of a check pretending to guard a picture. That is the fourth 0-red sabotage of this session and the first that was a true statement rather than an unreachable branch. The other, framing the hull 0.02 differently from the solid it is compared with, costs 1 red by making the exact hulls inexact. ALSO THIS ROUND, AND IT IS BOOKKEEPING RATHER THAN WORK: the other line's v4370 and v4371 are merged in. Both branches had added a NINTH sweep closing in the same window; theirs is renamed since11 rather than either side renumbering the other, because a closing's `at` says which round it belongs to and that does not move. v4371's own note says the tree stood at 1415 gates, which was true on the line that wrote it; the merged tree stands at 1417 and a historical entry is not edited to match a later merge. UNCHECKED: where between the two cameras the tube's verdict flips, which is a sweep this round did not run; a REAL PHOTOGRAPH, which neither the carve nor the judge has met here; the five soft signals and the VLM layer the port leaves out, so no verdict here is their verdict; and every number the rig has not signed. The tree stands at 1417 gates. Full changelog on docs/CHANGELOG.md.
