...
const BRAIN_BUILD = "v4385";   // v4385 -- *** THE FIRST KERNEL, ON A DEVICE, FOR THE FIRST TIME -- AND ITS f32 FLOOR HAD BEEN MEASURED ONE ROUNDING SHORT OF THE KERNEL. *** tools/roundhouse/magmapKernel.mjs wrote the rule at v2903 to stop its f32 emulator ever being reported as a card: "The card is graded by magmapGpu.js in a browser, against this same reference, and only that counts as the kernel having been run." Right rule, one consequence nobody chased -- magmapGpu() and magmapRun() take a `device`, and the only callers that ever passed one were two HTML pages. tools/roundhouse/magmapDevice-selfcheck.mjs is the first gate to hand the tree's FIRST kernel a device, through node-webgpu. *** AND A DEVICE SETTLED SOMETHING AN EMULATOR COULD NOT. *** F32_FLOOR was 4.385e-6, which is exactly what magmapKernel's cellMag() returns -- and cellMag RETURNS A DOUBLE. The shipped kernel writes `out : array<f32>`, so a real run rounds once more on the store, and 4.385e-6 was measured without it. magmapEmulated(), which stores into a Float32Array and therefore models the kernel's own output buffer, gives 4.4196e-6 at n = 9, 21 and 33 alike. The device measures 4.420e-6: IT SIDES WITH THE STORE. The recorded floor was below what a correct kernel actually produces. *** IT WENT UNNOTICED FOR TWO REASONS AND THE SECOND IS THE ONE WORTH FIXING. *** magmap-selfcheck re-measured the floor with cellMag as well, so the constant and its guard shared the same one-rounding-short model and agreed with each other; and it asserted `worst <= F32_FLOOR * 2` while magmapGpu.mjs's own header promised "the selfcheck fails if the measured floor ever moves above it". Two models of one thing, both short by a rounding, with an unexplained 2x on top. Both fixed: F32_FLOOR is 4.5e-6, visible headroom over a MEASURED_F32_WORST of 4.4196e-6 taken THROUGH the store, which is the shape hmcGpu.mjs already uses for F32_FLOOR_HMC; the check is strict; and it re-derives the constant to the digit, because a constant nobody re-derives is a constant that drifts. *** AND THE ARCHITECTURE THE FLOOR WAS BUILT TO JUSTIFY IS CONFIRMED ON HARDWARE FOR THE FIRST TIME. *** v2903 chose "the GPU may propose the MAP; the PEAK is recomputed on the CPU, always" on an emulated claim: that the worst f32 cell of the whole grid is the CENTRE cell, because the caustic is where the magnification diverges and the quadrature works hardest. Measured on a device over 441 cells: the worst cell is 220 and the centre is 220. gradedPeak(), which recomputes rather than reading the proposal, lands 1.352e-7 from the closed form sqrt(1 + 4/rho^2) against the device's own 4.420e-6 at that same cell -- a factor of 33 that accepting a GPU-sourced peak would have cost while looking like a speedup. The header estimated that degradation as 1.35e-7 -> ~4e-6; the device says 1.35e-7 -> 4.42e-6. *** ONE NUMBER MOVED THE OTHER WAY AND ITS CONCLUSION SURVIVED, BETTER SUPPORTED. *** The header said the fma-contracted variant differs from the uncontracted one "by at most 6.3e-8, so vendor contraction is NOT the dominant term". The difference is 2.578e-7, four times that. But the device sits 7.634e-7 from the uncontracted emulator and 7.634e-7 from the contracted one -- THE SAME DISTANCE FROM BOTH -- so whatever separates a device from the model here, it is not fma. An arithmetic estimate has been replaced by a measurement with a card in the loop, and it says the same thing. Also confirmed on the device: the adjudicator accepts on its own 34-cell sample with 0 disagreeing and a worst of 2.789e-6 against tol 1e-5; and the shared-trig cap is real -- at nT = 128 the kernel falls back to the pristine base and SAYS SO in its own label ("magmap.wgsl/base-wg64(nT>cap)"), which is what catches it, because above SHARED_CAP the cooperative load would write out of bounds in workgroup memory with no error, no crash and wrong numbers from a kernel that looks like it ran. Three sabotages, 2/3/1, and the first is the state the tree actually shipped in: F32_FLOOR back at 4.385e-6 now costs a red in magmapDevice AND in magmap-selfcheck, where before this round it cost none in either. The second is magmapGpu's own prediction arriving on a device -- the analytic derivative replaced by a central difference in f32 gives 4.266e+0 and, more tellingly, MOVES THE WORST CELL OFF THE CENTRE to cell 0, breaking the property the architecture rests on rather than merely the magnitude. AND A CORRECTION TO THIS ROUND'S OWN FIRST FINDING, which was wrong and was caught by checking it: the first draft said the constant "does not reproduce" and proposed to correct it as drift. It reproduces exactly -- at cellMag's configuration. The difference is the store, not drift, and the round nearly shipped a true number attached to a false cause. UNCHECKED: SPEED, which is the reason this kernel exists -- the map is the slowest computation in the lab (441 cells x 4.2 ms) and SwiftShader is a software rasteriser, so nothing here times anything and magmap-bench.html remains the only path to that number. A real card's f32 against SwiftShader's, since the 7.634e-7 residual is this device's and the whole point of a floor is to bracket cards nobody here owns. rho and span beyond the one fixture, which the floor's own note says were never swept. And whether the residual is SwiftShader's sqrt(), the obvious candidate now that fma is ruled out, which this round did not isolate. The tree stands at 1426 gates.
