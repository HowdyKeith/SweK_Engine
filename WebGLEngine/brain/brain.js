...
const BRAIN_BUILD = "v4046";   // v4046 -- new krbn-avatar.html (the rigged GLB drawn by Krbn as a pencil sketch) joins ui/avatarSwitch.js's rotation as a ninth surface, second-to-last so gauges3000 stays the explicit last choice v4033 asked for. The glTF->MeshInput conversion moved to the shared tools/krbn/glbMesh.js rather than being copied into a second page -- skinning, Y-up->Z-up and the degenerate drop are each invisible when wrong. It redraws on a timer (~2.6s) and declares its cost, because a pencil frame is ~0.5s of main-thread CPU. Krbn's abstraction.minFeaturePx was shipped INERT once: cutoffFor(importance, base) = base*(1-importance), so importance 1 disables it silently; fixed to 0.45 and measured 3349 -> 588 strokes. Full changelog on main.js's ENGINE_VERSION line.
