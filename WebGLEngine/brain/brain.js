...
const BRAIN_BUILD = "v4125";   // v4125 -- a second install button for somebody else's work, zavierferodova/Mac-NTFS-Mounter, with no licence file at all (all-rights-reserved by default, checked directly) and a real disk on the line. Runs the script unmodified rather than reimplement its commands, given no permission is on file at all. Never handles a password -- every privileged call uses `sudo -n`, failing closed rather than prompting. Keith chose "confirm before each mount" over full automation: listing volumes needs no privilege, mounting one needs a separate explicit call naming the exact volume, verified against the script's own echoed selection before success is reported. Filed in System Tools and also surfaced in the Mac System panel as a view. Gate: ntfsMounter-selfcheck, 24 checks, all pass; the privileged mac-only half cannot run in this sandbox and says so. Full changelog on docs/CHANGELOG.md.
