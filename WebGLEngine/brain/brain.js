...
const BRAIN_BUILD = "v4092";   // v4092 -- windowsImport-selfcheck went red: core/ecs/World-selfcheck.mjs (my own v4079 file) had four `await import(path.join(...))` calls with a raw filesystem path instead of a file:// URL -- the exact crash shape this gate exists to catch, works on POSIX and throws on Windows. Wrapped all four with pathToFileURL(...).href, matching every sibling gate's existing pattern. Both gates verified green afterward. Full changelog on docs/CHANGELOG.md.
