...
const BRAIN_BUILD = "v4138";   // v4138 -- install button for nakagami/grdpwasm (GPL-3.0), an RDP client that runs in the browser: cloned to ~/.voxelbridge pinned to a reviewed commit on the MASTER branch, built with the local Go, run as a subprocess, nothing vendored. The shape of the button was decided by reading their proxy: it defaults to all interfaces, accepts any origin, and dials whatever host:port a caller names -- individually fine for make serve, jointly an open TCP relay on a LAN. It is started on 127.0.0.1 using THEIR OWN -listen flag, their source untouched, and the gate PROVES that on a real socket (reachable on loopback, refused from this box's LAN address, with 0.0.0.0 bound as the control). Filing the page also fixed a pre-existing red: System Tools was 16 against a cap of 15, so the five opt-in third-party front doors moved into a sub-drawer of their own. Full changelog on docs/CHANGELOG.md.
