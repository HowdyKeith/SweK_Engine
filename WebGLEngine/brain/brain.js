...
const BRAIN_BUILD = "v4276";   // v4276 -- *** v4275 SAID "THIS SESSION HAS NO NETWORK" IN FOUR PLACES AND NOBODY TESTED IT. *** A bare curl to github.com returns HTTP 400 through this environment's proxy, which reads like a wall and is not one: the proxy gates GitHub PER REPOSITORY and anonymous git reads of public repositories work. Six repositories were filed UNCHECKED as unreachable while being clonable the whole time -- the same shape as v4270's WebGPU error one round earlier, an environment's refusal read as a fact about the environment instead of a message to read. world/licenceSweep.mjs opens TWENTY-SIX and records for each the licence file, its sha256 prefix and its line count, so a later round can tell a reading from a recollection: 20 papered (18 MIT, 1 Apache-2.0, 1 unresolved), 6 not. *** FOUR FINDINGS A "REPO: MIT" LEDGER WOULD HAVE MISSED. *** (1) WHO GRANTS: five repositories carry a licence naming somebody who is not the account holder -- Brendan Duncan, Jamie Owen, Alin, Brandon Jones with Colin MacKenzie IV, Daniel Esteban Nombela -- so owner/name plus spdx would have credited a mirror for gl-matrix itself. Backlog #82's question in its ordinary direction: the grant is good, the attribution would have been wrong. And it is NOT AUTOMATABLE -- but0n/Ashes is licensed to "Jeff Ma" and its package.json author is "but0n", the same person under a legal name and a handle, which no comparison of the URL's owner segment against the copyright line could get right. (2) DECLARED-ONLY: two repositories name a licence in package.json (one MIT, one ISC) and ship no licence text anywhere. Neither papered nor unpapered -- called papered, the tree would believe it holds a grant it has never seen. (3) CONTRADICTION: but0n/recastCLI.js ships the MIT text in LICENSE and says ISC in package.json. Both permissive, and not the same grant; the ledger records licenceExists true with spdx null and refuses to pick, because writing down whichever was read first is writing down a coin toss. (4) BOTH DIRECTIONS OF THE SCAN ERROR IN ONE HOUR: SDFGen has no licence file and the full MIT text in its README, so a root scan is wrong about it; grepping READMEs for "MIT" hit two unpapered repositories on the words "emitter" and "transmitted"; and two more carry the MIT body with no "MIT License" title at all. Only reading the matched LINES catches any of it. boytchev/tsl-textures, relied on since v4243 on the strength of the tree's own file header, is opened and confirmed first-hand: MIT, (c) 2024 Pavel Boytchev, 21 lines. The six that stay UNCHECKED stay so for a duller reason than v4275 gave: not one records an OWNER, and a bare name is not an address. Ten sabotages, all measured: null evidence 3 red; spdx without licence 1; hash collapsed 1; settled repo re-shelved 2; first-hand note reverted 1 in another gate; mirror re-credited 1; declared-only collapsed into papered 1; every mirror re-credited 2; contradiction resolved by picking one 2; legal name replaced by the handle 1. Sabotage C's first run CRASHED the gate instead of reddening it -- a detail string assuming the condition it graded -- exit 1 with no FAIL line, which is why sabotages are run and not reasoned about. Five counts here were typed before being counted; the hash-vs-line-count check is DERIVED now and survived two batches of new repositories without an edit. Full changelog on docs/CHANGELOG.md.
