...
const BRAIN_BUILD = "v4317";   // v4317 -- *** LEVEL 17: THE RACES ACT, YOU FLY AMONG THEM, A HEADING RIDES IN THE RECORD, A BODY IS A PLACE TO LAND, AND A RACE CAN BE MASKED. *** The rung the v4301 preview named, built. (1) A HEADING IN THE RECORD. The compacted record grows a third vec4, extra = (yaw, pitch, param, 0), copied by the cull from an extras buffer the scene owns -- the caller's headings when it has them, else the golden angle times the id computed ONCE on the CPU -- so every fleet shader (and every pick shader) reads a heading and never invents one; the Level 15 spin is retired from the shaders. The economy's headings(): a trader faces its next market, a docked one keeps the heading it arrived with. Instance attribute `extra` at location 5, twelve floats a record, and every consumer -- terrain, orbits, haul, the universe, the pick, Hi-Z -- unchanged but for the three gate lines that counted bindings. (2) RACES THAT ACT. world/racePolicies.mjs gives each race an economy of its own through the policy hook and TRAITS the dock honours: nearest-first (Wedge, Glyph), the undercutter at 85% (Pixel), the hoarder that holds until 1.6x (Loft; Chaos at 1.3x), the scholar carrying docs only (Voxel), the raider that goes where the stock is and takes a tenth of the port's best good on arrival (Krbn). racesAct-selfcheck runs a hundred days with every race on -- 80 t raided, discounts and holds counted, tons and credits conserved -- and then each race switched OFF to the pure greedy control: every acting race changes the ledger by name, and the three that only look different (Union, Holo, Cells) are named as costumes rather than failed. The control had to be PURE greedy: the built-in route's 5% preference noise made even a costume's absence change something, and the gate said so first. (3) YOU, IN YOUR OWN SHIP. world/playerShip.mjs: a manual crew member the sim never routes, the smallest flight model that is one (thrust, turn, drag), landing, buying, selling and launching as INTERVENTIONS (land, trade, launch) -- journaled, replayed, lockstepped; a trade that cannot happen is refused as an event, never a crash, because a peer's bad trade must not kill everyone's step. playerShip-selfcheck: the scripted flight's buy moves exactly n x price from the person to the treasury and n tons the other way, the sale exactly the port's price, the books close, the flight replays, and two peers with one of them flying agree on every credit. orrery-gpu.html: arrows fly, L lands or launches, B and S trade, the cockpit line shows the port's prices. (4) LANDING ON A PLANET. render/bodyTerrain.mjs: a body's files as hills at places hashed from their paths, as tall as the log of their bytes, summed into the heightfield render/gpuTerrain.mjs already draws, every texel remembering whose hill stands tallest; landing-selfcheck: at the five tallest peaks the field names the file on the CPU, and on BOTH backends a pick at a peak names a chunk whose file is the hill's -- pointing at a ridge names the file. G lands on the pointed body in the page. (5) THE IDENTITY PICTURE AS A MASK. render/fleetMask.mjs turns the pick picture into a strength field (1 where one race is) and draws the colour picture through Level 11's badTv FIELD pipeline: fleetMask-selfcheck, both backends -- inside the Chaos race's mask badTv changed pixels, outside it the picture is UNCHANGED TO THE BYTE (0 of the outside pixels, worst difference 0), a mask of nothing changes nothing. The mask goes through the CPU this round (two readbacks, two uploads) and says so. (6) THE SCRUBBER, one of the constructions the preview named: a slider replays the journal to any tick on a scratch economy and the sky shows that day, the live sim paused meanwhile. Four new gates. Sabotaged twelve ways across the four gates, every one red as measured (3, 1 and 2 lines; 2, 1 and one crash; 3, 4 and 4; 7, 4 and 3), in the gates' logs. Still unchecked and said: the flying as a feel; the mask on the device (a render attachment); the zoom down to a body (G, not a threshold); and the rig, the wire and the second engine, which are one page load each on Galaxina. The tree stands at 1392 gates.
