...
// v4394 -- *** v4392 SAID THE ERASED-DEFAULT FINDING "GENERALISES FURTHER THAN IT IS CHECKED ... PROVED FOR ONE PAIR AND NOT CENSUSED". THIS IS THE CENSUS, AND THE PAIR IS ONE OF EIGHTEEN. *** The pattern is a default written into an object literal that is passed straight to a function imported from another module, which defaults the same key again: two numbers, two authors, one argument, and neither author can see the other from where they are standing. Grouping by option NAME alone finds 25,657 candidate pairs in this tree and nearly all are coincidence; requiring the callee to be a binding imported from the module holding the other default takes it to 18, and 18 rows can be read by hand, which is the only reason any verdict below can be trusted. *** AND THE ROUND WAS WRONG TWICE, BOTH TIMES CAUGHT BY SOMETHING OTHER THAN READING. *** First: the draft said a caller passing an explicit 0 gets the CALLEE's number. Running it said 32, not 256. A `||` FORWARDER never emits a zero -- it swallows the caller's zero at the near end and the callee is never consulted at all -- so there are two questions here and each is answered by a different operator in a different file. Q1, what a zero MUTATION of the outer literal does, is decided by the CALLEE: ERASED when it repeats the same number (the world is byte-identical), REDIRECTED when it uses a different one (the world moves, but the mutation that ran is V to W, not V to 0, so the sweep's LABEL is wrong), HONOURED under `??`. Q2, where a caller's RUNTIME zero dies, is decided by the FORWARDER, and the answer is the forwarder at 16 of the 18. Second: the draft wrote that an ERASED zero mutant "cannot be caught by any gate, present or future", and tools/mutate/mechanicalSweep.mjs -- a record already in this tree -- shows box3dLockstep.js:21's zero mutant CAUGHT, on the exact row that sentence was about. No BEHAVIOURAL gate can catch it, which stands; v4392 wrote a SOURCE check that greps both files and asserts the literals agree, which catches it without the world moving. So the useful reading of an ERASED row is not "stop counting it" but "this edge needs an agreement check", and the census says which. That is the fix shipped: v4392's one hand-written pair generalised to a frozen list of all five ERASED edges. Four of the five had nothing before -- 1,430 gates in the tree and not one of them names voxelizePage, so ui/physicsMontage.js's two `cell` sites were undefended by anything. Four sabotages, 1/3/2/1 RED by name, five subject files md5-identical: the one that matters is setting physicsMontage's `|| 6` to `|| 0` and watching the tree go red on a mutation it could not previously see. The blind spots are counted rather than waved at -- 212 defaults that resolve to a named constant, 2 rows AMBIGUOUS because a module defaults one key twice, 1 UNRESOLVED because `?? 6 * 60 * 1000` is not the number 6 -- and all three UNDERCOUNT, which is the safe direction for a claim that a site cannot be falsified. The tree stands at 1430 gates. Full changelog on docs/CHANGELOG.md.
const BRAIN_BUILD = "v4395";   // v4395 -- *** A GATE'S VERDICT IS A WORD; ITS ARGUMENT IS A TABLE, AND 1429 GATES LATER NOT ONE TABLE HAD EVER LEFT THE TERMINAL. *** Keith raised 3Blue1Brown's manim beside ValkyrienSkies/Valkyrien-Skies-2 and asked what the pair generates. The thread is that both get a simulation to EXPLAIN ITSELF, and manim's discipline is not animation: it is that THE EXPLANATION IS GENERATED FROM THE SAME OBJECTS AS THE ARGUMENT, so it cannot drift from it. A caption can be wrong about a number; a number rendered from the number cannot be. THE CENSUS RAN BEFORE ANYTHING WAS BUILT AND THE ANSWER WAS NOT "FEW", IT WAS NONE: 1429 gates, 67 printing rows of formatted numbers, ZERO writing anything a second reader could open. *** AND THE REGISTER THAT EXISTS TO ANSWER THAT QUESTION COULD NOT SEE ONE OF THEM. *** tools/ship/artefactWriters.mjs walks tools/ and skips /-selfcheck.mjs$/ BY CONSTRUCTION, so "no gate emits an artefact" was never a finding about gates -- it was a fact about the walk, and it read clean. That file's own header names the species: "a register that knows one shape of a thing reports everything else as absent". v3609 fixed it in the DETECTOR, replacing a declaration shape with a behavioural one, and left it standing in the WALK one level up. THE EXCLUSION IS NOT REMOVED AND THAT IS A DECISION: artefactWriters answers "which TOOL builds an artefact a page needs", a gate is not a build step, and widening a census built for thirty files to fourteen hundred would change what its rows mean -- so the blindness is MEASURED in the new gate instead, as a number rather than a clean-looking zero. NEW tools/ship/gateReport.mjs is a place to PUT a table a gate already computed and nothing more: no formatting, no verdicts, no opinion about what a row means, and the terminal stays the primary surface. shipyard-selfcheck is wired first because its three tables are what the last four rounds argued from, instruments.html renders any emitted report, and NEW GATE tools/ship/gateReport-selfcheck.mjs ratchets the population whose argument dies with the terminal at 66, which may only shrink. *** EMITTING IS OFF BY DEFAULT AND THE REASON IS ANOTHER GATE'S RULE. *** registerResidue holds that emitters are dry-run because "a stale artefact from an accidental run is worse than none", with an exemption for an artefact that is a PURE FUNCTION OF THE TREE -- and a gate's table is not one, since the shipyard gate's device rows need a GPU and the same gate honestly produces fewer rows on a box without one. So SWEK_GATE_REPORT=1 turns it on and every report records WHAT WAS NOT MEASURED. *** THE MANIM RULE IS APPLIED TO THE REPORT ITSELF, WHICH IS THE ONLY PART THAT MAKES THIS MORE THAN A DUMP. *** The shipyard gate holds its own report to the values its CHECKS ran on: sabotaging the table to carry 1.5x the shipyard figure takes it red naming both numbers, so a table edited for the page cannot quietly disagree with the gate that made it. THREE PREDICTIONS WERE WRITTEN BEFORE BUILDING AND THE FIRST WAS WRONG FOR THE ROUND'S OWN REASON: I expected wiring one gate to trip artefactWriters or registerResidue, "because a new artefact without a row is exactly what those ratchets are for". Neither fired, and the reason IS the finding -- the register cannot see a gate, so it could not object. The second was avoided rather than tested: I expected the emitter to want to write on every run and idempotent() to refuse it, and making it opt-in meant idempotent() never ran, which is a prediction dodged and is said so here. The third stands: 66 is a ratchet nobody pays down quickly, and the answer to "a ratchet that cannot move is a list of grievances" is that each wiring is one line in a gate that already has the table. *** FOUR THINGS THIS ROUND GOT WRONG, ALL FOUND BY RUNNING RATHER THAN READING. *** reports() returned index.json as a report -- a register counting its own listing as an instance of the thing it registers, the sixth time this tree has caught that species. The value check looked for an exact figure in the page's rendered text and found 0.0000045892996283214416 drawn as 4.589e-6: THE CHECK WAS RIGHT AND THE PAGE WAS WRONG, because a table of seventeen-digit floats is unreadable but one that has thrown them away is a rounding of the argument rather than the argument -- every cell carries its exact value in a title now, and all 92 reach the DOM to the digit. The sweep closing was written as since21 while origin/main already had one, and IN A JAVASCRIPT OBJECT LITERAL THE LATER KEY SILENTLY WINS: the other branch's closing vanished, its swept count with it, and the only symptom was "1 STILL UNSWEPT". That is the third key collision of this session, so gateSweep-selfcheck gains a check that READS THE SOURCE for duplicate ordinals, because a runtime read sees one key where the reader sees two. *** AND THE FIRST RUN OF THAT NEW CHECK REPORTED 0 RED BECAUSE THE GATE WAS CRASHING: *** it used a constant the file does not define, so the process died before printing a single FAIL line and a count of failures came back zero. A COUNT OF FAILURES IS NOT A VERDICT UNLESS THE PROCESS FINISHED. Five sabotages, all logged where they belong (2/3/3/1/2 by name): the emitter ignoring its flag, which left a stale artefact within one run of turning the rule off; the page's exact-value titles emptied; the page pointed at an index that is not there; the shipyard's report carrying a number its checks did not; and the duplicated ordinal. NEW docs/EXPLAIN-ITSELF.md records the whole thread as four items with what each would measure -- this round is item 2, item 3 is the animation and comes next because without emitted data an animation is a caption with a timeline, and items 1 and 4 are the register and the claims keeping a rendering where they should keep the source, which is the same inversion three rounds of this session kept finding by hand. UNCHECKED AND SAID PLAINLY: whether the reports stay current, since they carry the date they were measured and nothing re-emits them on a ship; whether 66 is the right number, since the detector reads source rather than output and will miss a gate that builds rows without a formatter; and whether artefactWriters SHOULD see gates after all, which is an argument this round declined rather than settled. *** AND THE COLLISION THIS ROUND ADDED A CHECK FOR HAPPENED AGAIN ON THE WAY IN, WHICH IS THE FIFTH SUPERSEDE OF THE SESSION. *** claude/orrery-seeded-by-git-log reached main first with its own v4394 AND its own since24 closing, so this round moves to v4395 and its closing to since25 -- and this time the collision was a merge conflict a human can see rather than a silent overwrite, because both branches had written the same ordinal into the same file. Their round is merged here and their gate takes the tree to 1431. The tree stands at 1431 gates. Full changelog on docs/CHANGELOG.md.
