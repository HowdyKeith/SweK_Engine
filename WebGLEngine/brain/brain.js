...
const BRAIN_BUILD = "v4463";   // v4463 -- *** render/panini.js HAS QUOTED ITS OWN PRIMARY SOURCE SINCE v2571 AND THE TREE NEVER BUILT WHAT THE QUOTE NAMES. *** lazarus-pkgs/panini's USAGE.md, carried verbatim in that file's header: "It is A LINEAR PERSPECTIVE VIEW OF A CYLINDRICAL IMAGE -- the cylindrical analog of THE STEREOGRAPHIC PROJECTION OF A SPHERE." Its line 41 says it again in its own words. So the tree has held the CYLINDRICAL member of that family for 1,892 versions and has never held the SPHERICAL one -- which is the "little planet", and which nothing here could produce. NEW render/stereographic.js builds it. Nothing is ported: the projection is four hundred years old and the construction is three lines of similar triangles, u = 2x/(1-z), v = 2y/(1-z), with no trigonometry in it at all. *** THE PRIMARY CHECK IS AN IDENTITY AGAINST THE MODULE THAT NAMED IT, NOT SELF-CONSISTENCY. *** panini.js gated itself on "d = 0 must reproduce x/-z TO THE LAST BIT" rather than on "the wide view looks nicer", and this does the same one level up. On the horizon -- where a cylinder and a sphere ARE the same surface -- paniniProject at d=1 and stereoProject are the same function, measured to 4.18e-11 over 180 azimuths, because Panini's S*sin(th) at d=1 IS the half-angle formula 2*tan(th/2). And it holds at d = 1 ALONE: at d = 0.5, 2 and 4 the two miss by 14 to 23 whole units, which is what stops the identity being a tautology satisfied by any two increasing functions. *** AND "ANALOG" IS EXACT ON ONE CURVE AND WRONG BY 88 DEGREES OFF IT. *** Stereographic is CONFORMAL -- a right angle on the sphere comes out a right angle on the screen -- asserted through the ANALYTIC Jacobian: angle preserved to 1.4e-14 deg, magnification isotropic to 4.4e-16 and equal to stereoScale(). Panini at d=1 is not, and the worst direction is not an outlier: it flattens a right angle to TWO DEGREES at the NADIR, because its height term y/hypot(x,z) diverges at the poles and panini.js returns null at exactly vertical for that reason. A LITTLE PLANET LOOKS STRAIGHT DOWN, so the cylinder fails hardest at precisely the orientation the picture is built on. A reader who took "cylindrical analog" to mean "basically the same" would reach for the cylinder and get a smear where the planet should be. THREE CORRECTIONS THE BUILD PRODUCED, LEFT VISIBLE. (1) The first draft of that finding said 35 degrees. That number was measured over six directions and the gate runs seven; the seventh is near-vertical and reads 87.998. A frozen number taken over a different sample than the check runs is exactly v4462's own defect, one round later, in a new file. (2) A FINITE-DIFFERENCE version of the conformality check reads 4.5e-5 deg against the analytic 1.4e-14 -- eleven orders of magnitude blunter, and large enough to call a real 1e-5-sized defect conformal. The blunt instrument is RECORDED and the sharp one is what the gate stands on. (3) The horizon landmark is EXACTLY 2 through stereoProject and ONE ULP SHORT through stereoRadiusFor, because Math.PI/2 is not pi/2 and tan of it is not 1. Same landmark, same algebra, different last bit -- which is why the construction avoids trigonometry and why the gate asserts landmarks through the exact path and holds the helpers to a tolerance. THE LITTLE PLANET ITSELF: nadir at the centre, horizon at radius 2 exactly, zenith approached and never reached. world/procPlanet.js already bakes an EQUIRECTANGULAR texture and nebulaSkybox, cubeBake and skyRenderer all deal in the same spherical domain, so the consumer for this projection existed before the projection did. NEW tools/ship/stereographic-selfcheck.mjs, fifteen checks in five sections. THIRTEEN SABOTAGES, ALL RED BY NAME, both files md5-identical before and after. *** TWO OF THEM COST ZERO RED ON THE FIRST PASS AND BOTH WERE FINDINGS ABOUT THE CHECK RATHER THAN THE CODE. *** Turning the nadir map into a REFLECTION changed nothing, because the handedness check computed a determinant from a HARD-CODED COPY of the rotation written out again inside the gate -- a second copy of a function cannot disagree with the first -- and both landmarks are blind to a mirror by construction, since both have y = 0 after the rotation. That check's own comment PREDICTED the landmarks would survive a mirror and then failed to test for it. It probes littlePlanetDir itself now, by signed volume, plus lengths and angles. And changing a constant inside the GLSL cost zero red because the shader check was a handful of regexes that happened not to cover the one that moved: a regex over shader text tests the regexes. #118 settled that idiom for this tree -- "the JS and GLSL Ashima simplex are NOT the same function, and no gate has ever compared their VALUES" -- so the shader is now mechanically rewritten into JS and compared numerically: 0.00e+0 over 240 directions and 240 plane points, with a build failure treated as a FAIL rather than a quiet skip. WHAT IS NOT CLAIMED: that a GPU agrees. The rewrite is textual, a real driver may differ in precision or in normalize(), and the shader's actual output still needs a screenshot on the rig -- the same limit panini.js declared and this inherits rather than pretending past. Nothing in main.js calls this yet; it is a module and a gate, not a wired effect. The tree stands at 1485 gates. Full changelog on docs/CHANGELOG.md.
// v4462 -- *** #129 ASKS WHAT IS MISSING BESIDES THREADS IF THE VBA TRANSMITTER WERE THE SweK RUNTIME INSTEAD OF NODE/BUN, AND MEASURED, THE QUESTION HAS ITS SCALE INVERTED. *** Counted over the 3,814 runtime source files in this tree (vendor, node_modules and dist excluded), COMMENT-STRIPPED: threads are 22 files, 0.6%, RANK 11 OF 12 -- the second-smallest gap of the twelve, not the defining one. ES modules are 3,523 files at 92.4% and first-class functions 3,404 at 89.3, which is *** 155 TIMES MORE OF THIS TREE THAN THREADS ARE. *** A runtime with threads and no closures could run 0.6% of what a runtime with closures and no threads could, so the answer to "what is missing besides threads" is: nearly all of it, and threads barely register. *** AND THE GAP IS INVERTED FROM INTUITION IN THE OTHER DIRECTION TOO. *** The two capabilities anyone would guess are hardest to find outside a browser -- a network stack and a GPU renderer -- are the two the VBA side already has. What is missing is the mundane substrate underneath: modules, closures, async, typed arrays, which is 90% of the tree and none of which a Winsock library supplies. *** THREE EVIDENCE CLASSES, HELD APART BY A CHECK AND NOT BY A HEADING. *** measured / language / archive are claims of three different strengths, and the failure mode of a capability table is a tidy one where "VBA has no closures" and "the transmitter's HTTP server works" look equally established. The census is MEASURED and re-derived on every run. VBA's language properties are LANGUAGE facts that would hold for an empty workbook. Anything about the archive is ARCHIVE, the weakest thing in the file, and no VBA row is ever tagged measured. *** THE SABOTAGE PASS FOUND THE HOLE THAT MATTERED, AND IT WAS THE ONE THE WHOLE FINDING RESTS ON. *** Flipping `closures as values` to `has: true` -- crediting VBA with the single capability this round is about -- WAS A SILENT PASS. The check only asked whether a row declared an evidence class, and a flipped row keeps the class it already had. A label is not a check. Every `has: true` row now names a `via` corroborator that must be FOUND: either a token appearing in this tree's own .bas/.cls/.frm files, or a marker vba/archiveManifest.mjs really holds. Crediting VBA with closures now requires inventing a VBA file that uses them. *** POINTING THE ROWS AT BYTES IMMEDIATELY CORRECTED TWO OF THEM. *** (1) performance.now claimed "QueryPerformanceCounter through the WinAPI declares the archive already carries" -- and there is NO QueryPerformanceCounter in any VBA in this tree. What modGPUBrain.bas actually declares is kernel32 GetTickCount, at ~15.6 ms, which is a whole frame of quantisation at 60fps: the capability at a resolution that cannot time the thing it would be used for. (2) fetch/XHR was filed as an ARCHIVE claim about the transmitter's servers, and the client half needs no archive at all -- modGPUBrain.bas drives WinHttp.WinHttpRequest.5.1 asynchronously with a pile-up guard and modFPSControl.bas does the same through MSXML2.ServerXMLHTTP.6.0, both readable from this box. *** SO ONLY ONE ROW IN THE TABLE STILL RESTS ON THE ARCHIVE, AND IT IS WebGL *** -- there is no GL call, no WGL context and no D3D of any kind in the in-tree VBA. *** AND THE INSTRUMENT IS A MAXIMAL FALSE POSITIVE FOR ITSELF. *** Section 2 grades the comment strip, which moves the headline row: 11 of 33 "threaded" files were prose ABOUT threads, a third of the row the item is built on. But a string literal is prose the strip cannot reach, and vba/runtimeGap.mjs HOLDS THE PATTERNS TABLE -- every regex's literal text sits inside it. So this round's own two files match ALL TWELVE of their own patterns. Derived by recounting with those two paths excluded rather than argued: without them WebAssembly is 20 and threads 21, so threads are second-smallest outright; with them the two tie at 22 and threads hold rank 11 on the stable sort alone. The tie is an artefact of my own note strings, and it is said rather than hidden. That is the sixth instrument in three rounds to measure itself instead of the code. TWO MORE FROZEN NUMBERS WERE WRONG AND UNCHECKED, IN A FILE WHOSE HEADER COMPLAINS ABOUT EXACTLY THAT: languageRowsAbsent read 8 against nine real rows, and the gate re-derived the whole census and then compared only three of its twelve rows to it, so nine frozen counts were decoration -- asyncAwait had already drifted by one. All twelve are compared now, and the gate caught the drift TWICE MORE before this round could ship: merging v4449 and v4450 moved four rows, and then TEN concurrent versions (v4452 to v4461) moved five rows and thirteen files and pushed the ratio from 154 to 155. A table read back rather than re-derived would have shipped the first set of numbers three times over. NEW tools/ship/runtimeGap-selfcheck.mjs, twelve checks in four sections. THIRTEEN SABOTAGES, ALL RED BY NAME, both files md5-identical before and after. WHAT IS NOT CLAIMED: that any of this RUNS. No Excel has ever run against SweK_VBA_v3499 from this box, excel.html and server.html still say so, and the census's patterns are proxies -- a regex over stripped source, whose false-positive class this round measured on itself rather than assuming away. The tree stands at 1484 gates. *** AND THE ROUND'S OWN NUMBER COLLIDED. *** This was built as v4451; a concurrent session shipped a different v4451 -- "a ceiling for the avatar" -- and reached main first, so this ships as v4462 and its sweep-ledger ordinal moved forward past ten concurrent closings to since80. That is the same collision this tree's own ledger ordinals are checked for, one level up, and the first insert of the moved entry landed OUTSIDE SWEEP_SINCE_V4297 entirely -- gateSweep-selfcheck caught it as "1 STILL UNSWEPT" rather than letting a ledger entry that parses but is not in the ledger pass for one that is. Full changelog on docs/CHANGELOG.md.
// v4461 -- *** THE ROUND'S FIRST CORRECTION IS TO ITS OWN MEASUREMENT, AND IT IS THE REASON EVERYTHING ELSE HERE EXISTS. *** I verified nine gates with `node "$f" >/dev/null 2>&1; echo "$(basename $f) exit=$?"` and reported ALL NINE EXIT 0 in a commit message. The command substitution runs during the same word expansion and resets $? to basename's status before $? expands, so every gate printed exit=0 -- INCLUDING THE TWO THAT WERE RED. The meter was broken, not the gates, and the shape is this round's whole subject: A CHECK THAT REPORTS A PROXY FOR THE FACT. It is found six times below, three of them in checks I wrote this round. *** CITED SOURCES: THE TWO OWED UPSTREAMS WERE CLONED AND THEIR LICENCE FILES READ FIRST-HAND, AND ONE CAME BACK DIFFERENT FROM ITS OWN HEADER. *** DaveFace/UnrealRetroShaders at bc2e5654 -- LICENSE 21 lines sha256 8e04615e7298, README 87 lines sha256 5ee544dcbd4d, HEAD author "Dave Watts" agreeing with the copyright line -- is grantorHoldsRights: FALSE, on the author's own words: README line 16 says "Some functions are based on stuff I've found online ... where I can remember", and the Credits section names who: "YUV colour conversion / Bayer implementation by Jazz Mickle". *** THOSE ARE EXACTLY THE TWO TECHNIQUES v4442 DECLINED TO TAKE, FOR A COMPLETELY UNRELATED REASON *** -- Bayer was already in fx/dither.js and YUV/posterise are aesthetic, so no gate could fail on them. A split made on FALSIFIABILITY landed on the same line as the LICENCE boundary, and the tree took only what the grantor holds by accident of reasoning rather than by having looked. That is recorded as luck, because it was. knightcrawler25/GLSL-PathTracer at 291c1fdc is clean: LICENSE 21 lines sha256 449495ccec87, and src/shaders/common/disney.glsl -- the file actually read, 350 lines sha256 24de7964b6e1 -- carries the MIT block in its own header, so the grant does not have to be inherited from the root. Vendoring was permitted and still refused. Debt 51 -> 49, equal to the baseline. *** FIVE PINNED CONSTANTS STANDING IN FOR CLAIMS, AND THE SECOND ENCUMBERED ENTRY IS WHAT EXPOSED THE FIRST. *** reachedLicences asserted `enc.length === 1`; the claim underneath was never "there is one" but "the ENCUMBERED category, defined at v4200 from a hypothetical, gets USED by reading". Replaced by substance: every encumbered entry must name who does hold the rights, and must be REACHED and non-redistributable, because an entry cannot be encumbered and shippable at once. *** AND MY REPLACEMENT WAS WRONG ON ITS FIRST RUN AND ACCUSED AN INNOCENT ENTRY *** -- written from the one new note in front of me, it red-flagged projapati66/Svg-IsometricCityAnimation, whose note says "neither of which the grantor holds", the same assertion in words I had not thought of. Reading that red as the EXAMPLE'S fault would have been the defect a second time; the accepted forms are an enumerated list now and the failure message prints it. registryOrphans went red because MY OWN v4460 commit registered seven physics instruments whose modules expose no reportLines(), taking the mechanical remainder from 13 to 20 against a ceiling of 15. Raising the ceiling was the ceremonial move; the seven have real doors instead, each re-measuring its module's frozen record LIVE, because a frozen record beside a live measurement is the only arrangement where drift is visible. smallSteps re-bisects the crossover at four budgets and lands on v4441 at ratio 1.000 for all four, and prints `saturated` beside every value because at N=2 the bisection returns its own bracket, which is the ABSENCE of a crossover wearing a number. subsurface's searchlight collapse is exact to 0.0e+0 across two decades of d. btdfDomain leads with vndfMass 1.000000, error 1.26e-7 -- the instrument checking itself before it is allowed to convict anything -- and finds 46% of the as-written transmission lobe on directions the transport cannot produce. microsurfaceWalk prints the Kulla-Conty error WITH ITS SIGN, -7.3% at F0 0.5 and +4.0% at F0 0.04, reproducing v4446 live. dielectricWalk gets R+T = 1.0000 on every row and prints Walter's number FROM THE RECORD rather than importing transmission.mjs, because an instrument that imports its defendant is not an instrument. bssrdfSample reports uniform-samples-to-match beside the variance ratio, and importance sampling LOSES at d = 5 and d = 20. albedoEstimator reproduces v4438's two lists, which DIFFER, and says so: a conservative rule flagging more than is materially wrong is the rule working. Having doors, all seven then QUALIFIED for the bench with page: null -- the same gate's other direction, caught in the same run -- and now carry instrument-bench.html. *** orreryEjecta WAS TWO ARRIVALS IN ONE RED WITH OPPOSITE ANSWERS, WHICH ONLY A RATCHET FROZEN BY NAME COULD TELL APART. *** tools/ship/orreryUniverse-selfcheck.mjs reached four bodies at once and goes to NOT_IMPORTERS: it is the gate on the vendor CENSUS, its subject IS the vendor tree, and it imports nothing from vendor/ -- the counter-argument, that it really does readdir those paths, is written into the file rather than omitted. tools/ship/slugWgsl-selfcheck.mjs is ACCEPTED into fonts: it reads vendor/fonts/IBMPlexSerif-Regular.ttf and parses it as the input its assertions run on, so ejecting vendor/fonts leaves it nothing to test. A count would have said "fonts went 3 -> 4" and hidden both. *** AND THE SAME EMPTY-DIRECTORY PROXY SAT IN TWO GATES. *** "There are TWO directories named vendor" counted directory ENTRIES in copiedOutsideVendor and orreryEjecta alike; an EMPTY untracked ai-bridge/vendor/ dated Aug 17 made both read three. Git cannot track an empty directory, so it exists in a working checkout and in no fresh clone -- which is also why copiedOutsideVendor looked green in a bisect worktree and red at HEAD, and why I first believed my own commit had caused it. The finding is "vendored CODE sits where the scanner does not look"; both count by content now and both print every candidate with its file count, so an excluded one is visible rather than dropped. *** THE RELEASE LEDGER DEMANDED A FOURTH BASELINE RAISE AND GOT A STRUCTURAL FIX INSTEAD, WHICH IS WHAT ITS OWN RITUAL TEXT ASKS FOR. *** v4460 is published -- the first correct publish in this arc -- and the ledger did not know, so it was refreshed from the API and v4460 written in. That cleared exactly ONE name and left seven owed against a budget of three, because `owed` counted every unreleased version NUMBER while `latest` sat computed six lines above it and never consulted. A box downloading releases/latest at v4460 is running v4459's code and v4452's; no separate build for those will ever exist and nobody would download one. Counting version numbers where the rule is about CODE IN SOMEBODY'S HANDS is the round's own defect sitting inside the gate that polices the round. A version beneath the newest PUBLISHED release is superseded now, and the forgiveness cannot swallow the rule: the floor can only be raised BY PUBLISHING SOMETHING, where a baseline is a number a person types, and both are reported apart. Driven both ways from ONE fixture -- same main v4111..v4444, same floor v4000, differing only in what was published -- and sabotaged both ways, the over-forgiving direction included. Owed is now none; the fleet runs what is built. THIRTEEN SABOTAGES, ALL GRADED ON EXIT CODES RATHER THAN ON A COUNT OF FAIL LINES, because a crashing gate prints zero of them. UNCHECKED AND SAID PLAINLY: nothing verifies a grantorHoldsRights: TRUE against its upstream, so flipping DaveFace's to true would clear both licence gates in silence -- inherent to a record of judgements, and worth knowing it is there. The seven new reports were read by me and by no gate; registryOrphans checks that they PRINT, not that they are right. *** THE SWEEP'S REWRITE OF sweep-timings.json WAS DISCARDED AND THE ROTATION'S WAS KEPT, AND THE DIFFERENCE IS MEASURED RATHER THAN PREFERRED. *** Re-running the quick sweep here rewrote the file and sweepCoverage went red naming six gates lost from budget, slugWgsl 58 -> 6889 ms -- readings taken under 8-way contention, which is the starvation quickSweep-selfcheck already warns about, so they were thrown away. Step 3b's rotation re-times SERIALLY and its numbers go the other way: 76 gates run, 62 now UNDER budget, wgslLayout 3919 -> 650 ms and labCensus 3950 -> 633. Those are FASTER than the readings they replace, so they are an improvement to the fleet's budget rather than one slow box imposed on it, and they are written. The rotation returned 4 red -- winPathGuard, homography, box3dFilter, wasmSupport -- and ALL FOUR ARE ALREADY IN redCensus with a reason, so none is a new finding and nothing was registered to get green. *** AND THE ROTATION IMMEDIATELY MADE sweepCoverage RED, WHICH IS THAT GATE DOING THE ONE JOB IT WAS BUILT FOR. *** Section 10 requires each of the twelve evicted-but-fast gates to be either still at the reading that evicted it or NAMED as returned; the rotation returned a seventh, box3dFilter, evicted at 3763 ms and running in 185 -- a factor of twenty -- so the row fired until it was named. It is red and was red the whole time it was invisible; naming it does not fix that and does not pretend to, it stops the eviction being the reason nobody sees it. The row's TITLE also carried TWELVE, TWENTY-TWO and SIX as typed words beside the same numbers derived in the body, so the headline read SIX while the sentence under it read seven -- the sixth pinned constant of the round, in the message of the check that catches pinned constants. All three now come off the record. The 14 gates in redCensus.RED_AT_V4279 are untouched. And the ledger refresh was read through the repository API rather than by refreshReleases.mjs --write, whose fetch the sandbox proxy answers with HTTP 401; the rig should still run --write, and if it disagrees the rig wins. The tree stands at 1483 gates. Full changelog on docs/CHANGELOG.md.
