...
const BRAIN_BUILD = "v4375";   // v4375 -- *** THE SHIPPED LADDERS, PRICED FOR THE FIRST TIME -- AND THEY ARE TELLS, NOT APPROXIMATIONS. *** v4374 derived LOD thresholds from a measurement and closed by naming what it had not done: the shipped pages still carry typed thresholds and were not rewired. This round prices them, and does not rewire them either, and the reason is the finding rather than an omission. BOTH PAGES BUILD THEIR LADDER FROM THE SAME FLAT QUAD. orrery-gpu.html declares quadMesh(6) near and quadMesh(1) far; universe-gpu.html declares quadMesh(5), quadMesh(2) and quadMesh(1). render/gpuDriven.mjs quadMesh lays a quad from (-1,-1,0) to (1,1,0) and subdivides it, writing z = 0 for every vertex, so subdividing moves nothing off the plane and changes no silhouette -- and neither page brings its own render pipeline, so both draw with the default flat look, which reads p and color and nothing else: no normal to shade with, no vertex stage to displace. What differs between rung and rung is the COLOUR each was given: two greens in the orrery, three blues in the universe, no two alike, which is not an accident. MEASURED over eight distances, each rung priced twice -- as it ships, and with one colour across the whole ladder so only geometry can differ: as shipped the worst rung differs by up to 5,476 pixels, and with one colour by 20 (orrery) and 63 (universe). Those residues are the RASTERISER and not the shape -- a subdivided flat quad splits its interior into triangles whose shared edges land a few pixels differently at some sizes -- which is why render/lodBudget.mjs ladderKind draws its line at the POLICY's 64 pixels rather than at an exact zero. Both ladders classify as TELL. *** AND ASKING THE POLICY FOR A THRESHOLD EXERCISES THE PATH v4374 BUILT AND HAD NO REAL DATA FOR. *** The orrery's single priced rung is free at every metric measured, so the largest measured is returned as a BOUND and flagged as one rather than reported as a crossing. The universe's two priced rungs are both free, both bounds land on the same metric, they do not fall, and the LADDER IS REFUSED OUTRIGHT -- which is the right answer: a ladder whose rungs cost nothing has no fidelity ordering between them, and returning a pair would be two numbers with nothing behind them. *** THE SUBDIVISION IS VERTEX WORK ALMOST NO PIXEL CARRIES: *** 400 instances at the orrery's own threshold of 0.012 draw 20,680 triangles where the same picture -- to within the 63 pixels the per-rung pricing measured -- needs 800, which is 25.9x. On this software rasteriser a triangle is nearly free, so that saving is a COUNT and not a time, and it is said here rather than assumed. AND THE PROSE BESIDE THE CODE SAYS SOMETHING ELSE: orrery-gpu.html's ladder carries the comment "a finer disc up close, a coarser one far away". quadMesh builds a SQUARE, and the finer one is not finer in any way a picture can show. The comment describes an approximation ladder; the code builds a tell. THIS IS NOT REPORTED AS A DEFECT AND IS NOT ONE. Three distinct colours chosen for three rungs is how a person watches a ladder work, and removing it would be a product decision rather than a correction, which is why this round measures and rewires nothing. What it settles is that "LOD threshold" names TWO DIFFERENT THINGS in this tree: the distance a TELL changes colour, which can be typed with impunity because there is no fidelity to get wrong, and the distance an APPROXIMATION may coarsen, which v4374 showed can be derived. Only the generated ladder is the second kind. Two sabotages: the tell/approximation distinction removed so every ladder reads as an approximation (2 red, one per page), and the budget read as 0 inside the classifier so the rasteriser's own residue counts as approximation error (2 red). *** AND THIS GATE'S OWN FIRST RUN WAS WRONG THREE WAYS, ALL OF THEM THE GATE'S. *** Its probe drew through a two-rung scene whose rungs tied on triangle count, and gpuDriven RANKS a ladder by its meshes rather than by the order given, so it was handed either one and every subdiv-1 rung read as though its geometry differed by thousands of pixels; the `const lods` line was read with a regex that stops at the first "]", which a colour array closes too, so section 1 reported zero rungs; and a script meant for the gate wrote its text over render/lodBudget.mjs, which was restored from the last commit rather than retyped. All three are logged in the gate. UNCHECKED: gpu-rig-check.html's [0.025, 0.04], whose ladder is quadMesh too but whose page is a rig probe rather than a scene; whether the tell should be a debug TOGGLE rather than the shipped default, which is a product decision and not a gate's to make; what the tell costs on a real rig rather than on SwiftShader; and every number the rig has not signed. The tree stands at 1419 gates. Full changelog on docs/CHANGELOG.md.
