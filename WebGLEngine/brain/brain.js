...
const BRAIN_BUILD = "v4301";   // v4301 -- *** TWO IDEAS, BOTH MIT, BOTH ruvnet, NEITHER'S CODE HERE; AND TWO REGISTER GATES THAT WERE RED ON main BEFORE THIS ROUND TOUCHED ANYTHING. *** First the finding, because it is the kind this tree keeps: citedSources-selfcheck read a debt of 51 against a baseline of 50, and copiedOutsideVendor-selfcheck failed three lines, both on a clean checkout of v4300. The first is v4299's world/gitEconomy.mjs citing Kalcode/spaceprojectsim (MIT) in its header with no register entry -- the exact shape #53 and #137 recorded, a third time; registered now, citedPaths naming the file, debt back to 50. The second is one of the SIX regressions the v4297 sweep named: v4281's world/licenceBodies.mjs keys its matcher on the MIT permission sentence and its gate carries the sentence as a fixture, so a census frozen at "15 notices, 14 under vendor/" read 17 and stayed red for twenty rounds. The gate now excludes QUOTERS by name -- files that hold the clause as data -- and lets a registered copy declare noticeInFile, so the historical 15/14 claim is still asserted and a copy that carries its own notice, as MIT asks, is registered rather than counted. Five of six v4297 regressions remain. NOW THE ROUND. batvu (ruvnet/batvu, MIT): the tree had a speaker and a processed microphone and had never asked how long a sound took to come back. audio/rangefinder.mjs is PURE -- samples in, ranges out, no AudioContext -- a 17.5-20.5 kHz Hann-windowed chirp from the strict sine, an FFT matched filter over physics/fft.js with an analytic envelope, an ordered-statistic CFAR detector, and the direct path as the clock. batvu's figures are checked as arithmetic: 10 log10(BT) = 11.76 dB against its 11.8; c / 2B = 5.72 cm against its 9.6 cm achieved, and this tree's own measured clean separation is 8 cm, between the bound and four times it. Synthetic echoes at known delays come back within 6 mm over 25 positions from 1.0 to 4.4 m; noise alone yields 2 alarms in 20 runs; and the same echo filtered with a 9-12 kHz reference is NOT found, which is the control that says the matched filter is doing the finding. Three things the first draft got wrong, each measured rather than assumed: cell-averaging CFAR with a 6+32 window missed a 27 dB echo because the compressed pulse's own skirt sat in its training cells (now a median over windows sized in pulse widths); a 9 dB threshold gave 24 false alarms in 20 noise-only runs because a Rayleigh local maximum clears 9 dB one time in 250 (now 13 dB); and every clean run reported a 9 dB target at 0.80 m that was the direct path's own tail, because a 5 ms chirp is 0.86 m long -- so the blind zone is the chirp's length, 0.94 m, and batvu's 0.6 m floor is stated as needing direct-path subtraction this tree does not do. audio/rangefinderLive.mjs is the browser half, asking for the microphone with echo cancellation, noise suppression and AGC OFF because each would remove the very signal; window.rangefinder.ping() is the door. No real-room number is claimed anywhere; the sandbox has no microphone. rvQR (ruvnet/rvQR, MIT): the vendored encoder had drawn one symbol carrying a URL since v525 and nothing here could decode a symbol at all. ui/qrDecode.mjs decodes a module matrix -- format BCH, unmask, zigzag, de-interleave, Reed-Solomon over GF(256) with Berlekamp-Massey, Chien and Forney, segments -- and is proven INVERSE to the encoder over all 160 version-and-level pairs at full capacity, in 4.8 s. Its two tables are the encoder's, parsed from the vendored source by the gate rather than trusted, and carried with the full MIT notice, registered DERIVED with noticeInFile. 24 flipped modules come back exact with the corrections counted; 400 are REFUSED -- the decoder throws rather than returning plausible bytes, which a sabotage of the corrector showed is the line that matters. ui/qrChannel.mjs is rvQR's protocol shape: a manifest at sequence 0 with length and SHA-256, numbered data frames, out-of-order and duplicate arrival free, and assemble() that names the gap or the hash mismatch and never returns a short buffer. Its throughput is table arithmetic: 256 bytes x 10 fps = 2,560 B/s is rvQR's "about 2.5 KB/s", and its 40 KB demo is 161 frames at 10 fps, 16.1 s, against "about 16 seconds". window.qrChannel.show(bytes) puts the frames on screen; the CAMERA HALF -- locating a symbol in a video frame -- is not in this tree and is said so in every file. Eight sabotages, all red by name, two to eight lines each. Caught while writing rather than by sabotage: v19-L holds 795 codewords, not the 792 I typed from memory; v1-L carries 17 bytes in byte mode, not 19; v2-H holds 16 codewords; 16.1 s does not round to 17. Also from main since v4298: v4299 and v4300 (Levels 11-14) closed five open items -- GPU-driven rendering, the strength field, WebGPU texture binding, the shader-complexity scale, and the song-as-terrain idea shipped at v4280 -- and the task list now says so. Unchecked and said plainly: any real room; a moving speaker; whether a phone honours the three OFF flags; kanji and structured-append modes; the frame rate a real screen and camera sustain. The tree stands at 1382 gates. Full changelog on docs/CHANGELOG.md.
