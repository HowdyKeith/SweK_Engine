...
const BRAIN_BUILD = "v4416";   // v4416 -- *** v4290 REFUSED TO PORT THE PATH TRACER AND GAVE A REASON. THE REASON IS TRUE IN GENERAL AND FALSE ON THE ONE SCENE THE TRACER WAS BUILT TO BE TESTED ON. *** pathTracerWgsl.mjs states it plainly: "The tracer runs in f64. A GPU runs in f32. Those are different renderers, so did-the-port-work has no answer until somebody says what agreement was supposed to look like." So v4290 ported the two DECIDABLE pieces -- the generator and a coverage kernel -- and left the transport alone. This round asks what that refusal left open: IS THERE A SCENE ON WHICH THE f64 RENDER IS EXACTLY REPRESENTABLE IN f32? THERE IS, AND IT IS THE FURNACE -- pathTracer.mjs's OWN DESIGNATED TEST SCENE. A sphere is convex, so a cosine bounce escapes to a constant sky and a camera ray is worth exactly rho or exactly 1; a pixel is the mean of spp of those, which with a DYADIC albedo and a POWER-OF-TWO spp is a dyadic rational needing under 24 mantissa bits. NOTHING ROUNDS. BOTH PRECONDITIONS MEASURED NECESSARY, NOT ASSUMED: rho in {0.5, .25, .75, 1} gives 0 non-exact pixels of 576 and rho in {0.3, 0.1, 1/3} gives 163, 163, 159; spp in {1,4,16,64} gives 0 and spp in {3,5,10} gives 26, 36, 39. NEW physics/render/pathTracerGpu.mjs puts the Lambertian transport loop on a real GPU and it is BIT-IDENTICAL to the CPU across 11,072 pixels in seven configurations -- the claim v4290 said could not be made. *** AND THE CONVEXITY ARGUMENT IS A THEOREM ABOUT REAL NUMBERS THAT f32 BREAKS. THE FIRST RUN SAID SO AND IT COST 120 WRONG INTERIOR PIXELS TO FIND OUT. *** 152 of 576 differed and 120 were INTERIOR, not silhouette; the values were BELOW rho and dyadic -- 0.421875 is (11 x 0.5 + 5 x 0.25)/16, five of sixteen samples bouncing TWICE. The bounce ray was re-hitting the surface it left, because occlusion.mjs's eps = 1e-6 was chosen against f64 and sits BELOW THE f32 NOISE FLOOR at these magnitudes. The theorem holds; its PRECONDITION -- the origin lies exactly on the surface -- is what f32 breaks. *** TWO REPAIRS WERE WRONG BEFORE THE THIRD WAS RIGHT AND BOTH WRONG ONES ARE KEPT. *** (1) A bigger absolute eps: swept 1e-6 -> 152, 1e-5 -> 12, 2.5e-5 -> 0 at 24x24, which looked settled and was not -- at 32x32 two interior pixels returned 0.49609375, which is 0.5 - 0.25/64, ONE sample in sixty-four self-hitting again. A THRESHOLD TUNED ON ONE FRAME SIZE IS TUNED ON NOTHING. (2) A "relative" eps scaled by length(P - centre), which is worse than wrong -- it is a NO-OP that looks principled, because at a bounce origin P is ON the sphere so that length is EXACTLY THE RADIUS every time; it scaled by a constant and returned counts byte-identical to the absolute sweep, which is the only reason it was caught. A CORRECTION THAT CHANGES NO NUMBER IS NOT A CORRECTION. (3) MOVE THE ORIGIN OFF THE SURFACE: o = P + N*eps makes the self-hit geometrically impossible instead of filtering it afterwards. THE SIGNATURE OF A STRUCTURAL FIX IS THAT THE TUNING PARAMETER STOPS MATTERING -- eps 1e-5, 1e-4 and 1e-3 all give ZERO differing pixels, and a threshold has exactly one good value. *** AND WHAT MAKES THE COMPARISON POSSIBLE IS EXACTLY WHAT MAKES IT WEAK, WHICH IS THE FINDING AND IS SHIPPED AS A CHECK RATHER THAN A CAVEAT. *** The derivation never mentions the SAMPLER: a ray's value depends only on whether it hits, because every bounce escapes to the same constant. The furnace is bit-exact BECAUSE it is sampler-blind. So the gate PLANTS A BROKEN COSINE SAMPLER and measures that the furnace certifies it BIT-EXACTLY -- 0 of 576 differing -- while the gradient sky catches the same plant at 7.310e-2 against a clean f32 floor of 3.917e-6, a separation of 18,660x that is measured rather than chosen. pathTracer.mjs's own v3487 comment said the same thing from the other side: "A SUITE THAT GRADED ONLY THE FURNACE WOULD CERTIFY A BROKEN SEEDING SCHEME." NEW physics/render/pathTracerGpu-selfcheck.mjs, eleven checks in four sections; section 4 is a check on the other checks. WHAT THIS DOES NOT CLAIM: that the whole tracer is on the GPU -- NEE, microfacets, Fresnel, energy compensation, roulette and stratification all stay on the CPU and are named absent rather than discovered missing; that bit-exactness reaches past the furnace -- the gradient run differs on all 576 pixels and the file says so with a number; and that f32 is enough for a shipping renderer, which is not asked here. Four sabotages, 2/2/1/1 RED by name, file md5-identical. The tree stands at 1448 gates. Full changelog on docs/CHANGELOG.md.
