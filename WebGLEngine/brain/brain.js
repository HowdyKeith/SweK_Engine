...
const BRAIN_BUILD = "v4336";   // v4336 -- *** A COMPUTE PASS THAT READS ONE, AND A SABOTAGE THAT PROVED NOTHING UNTIL IT WAS FIXED. *** The correction first, and it is this round's own: sabotage U -- the storage buffers mapped by ORDER instead of by role -- went 0 RED on its first run. A sabotage that goes 0 red is a finding, not a pass, and this one said the check beside it was empty: the shell listed the WRITTEN buffer first, which is the order three happens to emit, so position and role agreed and nothing was being tested. Worse, the claim printed beside it asserted that a positional mapping "would have bound them backwards" -- an assertion nobody had run. The shell now declares its INPUT first, the order render/gpuDriven.mjs's own cull pass uses, the sabotage bites for 2 red (6 device errors from assigning a read-only binding, every element reading periodic), and the claim says "measured as sabotage U" instead of predicting. The round: v4331 built a compute transplant that only ever WROTE, and every real pass in this tree reads buffers too. render/tslSource.mjs computeShell's storage entries now take an `access`, because three declares every buffer it touches as read_write whether the graph writes to it or not -- so the SHELL is where read-only is stated -- and transplantCompute matches a generated buffer to a shell entry BY ROLE, which one the body assigns to. render/physicsTsl.mjs makeChaosMaskTsl is the second pass: it reads the sweep's own buffer node and writes 1 where the exponent is positive. Measured on WebGPU: two dispatches on ONE frame's encoder, the second bound to the first's buffer, and the mask it writes is the SIGN of the sweep's own output on all 64 elements, 41 of them chaotic, no device errors. The keyed part: every element the mask calls periodic above r = 3.8 lies inside [1 + sqrt(8), 3.857] -- three of them, at r = 3.8286, 3.8381 and 3.8476 -- which is the PERIOD-3 WINDOW, whose lower edge render/lyapunovWgsl.mjs owns exactly as 1 + sqrt(8) = 3.828427. A sign test on a buffer another pass wrote, landing on a constant this tree already had. *** AND MAIN WAS RED WHEN THIS ROUND STARTED, ON A FIX THAT HAD ALREADY SHIPPED. *** v4330 corrected world/orreryFleet.mjs's recorded commit count for `three` from 2 to 1 and wrote a paragraph saying why; a later merge took the PARAGRAPH AND REVERTED THE VALUE, so main shipped a file whose prose said "corrected to what git says" directly above the number it had not corrected, orreryFleet-selfcheck was red on main, and no register named it. Re-applied, with the merge's own lesson recorded beside it: a note and the datum it describes living in one file is exactly what lets a merge keep one and drop the other. Sabotaged and measured: the role mapping reverted to positional (2 red, above), and the second pass reading its OWN buffer instead of the first's -- a one-word typo -- refused by name before the device sees it (1 red: the graph then touches one buffer and the shell names two). Not built and said: a workgroup-shared or ATOMIC pass, which three can emit and this transplant has never seen (the fleets' cull pass counts into an atomic<u32> and nothing here does); an INDIRECT dispatch, where the count a pass runs at is itself in a buffer; a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample, which the device cannot serve at all; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; and every number the rig has not signed. The tree stands at 1411 gates. Full changelog on docs/CHANGELOG.md.
