...
const BRAIN_BUILD = "v4328";   // v4328 -- *** A SATELLITE IS AN IMPORTER, AND RE-BAKING THE ORRERY AFTER FORTY-FIVE ROUNDS TURNED FOUR GREEN GATES RED. *** #68 asked for the ES satellites, asteroids and planets in the orrery, driven by the git log, so that two runs match unless the repo changed. The planets landed at v4189; this is the rest, and three of the item's own premises were wrong. planetSurface's 'one Math.random to remove' is the comment claiming the file HAS none -- zero real calls. 'Adding %H is one line' was already done. And the obvious asteroid belt, one rock per commit touching a body, is DEAD ON MEASUREMENT: twelve of fifteen vendored bodies have been touched by exactly one commit in the repository's life, the busiest by two, against 858 commits. Vendored code does not change; the code that USES it does. So a satellite is an IMPORTER -- the count is world/orreryEjecta.mjs's measurement, not a number chosen -- carrying the commit that LAST touched it, which is what makes 'one new commit changes exactly one thing' true and measurable rather than asserted: the gate edits one importer's hash and counts what moved (one, of 127). Altitude is log1p of the importer's bytes by repoHeightfield's rule, the period is kepler's imported not restated, eccentricity and inclination are REFUSED as orreryView refused them. The debris ring is the paperwork, so grass, keyhunt and slug finally LOOK like the finding v4266 made and could not draw. orrery.html renders it at planet zoom and the gate counts the pixels, because sabotage D showed the caption alone stays right when the canvas is empty. *** AND THE ROUND'S REAL COST WAS ELSEWHERE. *** The fleet enumerates bodies from the tree while everything else reads orrery.json, which was baked at v4189: fifteen bodies against fourteen, a dependency the orrery had never drawn, and box3d and htmx PAPERED since #61 while the page still printed two unpapered. Two gates had been red on the register saying exactly this for forty-five rounds. Re-baking fixed those two and broke four others, and every one of the four was a number typed against a quantity derived from the body count and held true by a snapshot that had stopped moving: a manual ship's index, a drawn-body total, an all-code example body that gained a LICENSE, and a probe whose undo re-derived the value instead of remembering it. All four fixed by deriving. The unpapered ratchet drops 2 -> 0 and becomes EQUALITY rather than a bound, v4258's lesson. SatelliteFleet's seven Math.random calls now draw from an optional seeded stream, default unchanged. CORRECTED BY THE NEW GATE ON ITS FIRST RUN: it named vendor/box3d/ in a check and became box3d's twenty-second importer -- the seventh instance of that trap here -- and the fix is not an eighth exclusion but building the needle from parts, so the file is not an exception. Four sabotages, 2/2/3/1 red by name. UNCHECKED: whether a sky of seventy craft around one planet reads as anything. *** AND THE MERGE WITH main TAUGHT THE ROUND ONE MORE THING. *** Three rounds landed while this was written, so the number moved from v4325 to v4328 -- and the fleet bake went red on its own commit, because a satellite carries the LAST commit that touched its file and main.js is an importer. A bake of last-commits cannot be instantaneously current: the commit that ships a round cannot know its own hash. So POPULATION drift (a body, an importer, a size) fails and COMMIT drift is reported beside the head the snapshot was taken at, which is orreryBake's own reasoning about positions applied one level up. The tree stands at 1408 gates. Full changelog on docs/CHANGELOG.md.
