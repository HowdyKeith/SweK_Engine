...
const BRAIN_BUILD = "v4321";   // v4321 -- *** THE ROADMAP'S FIFTH STEP, BUILT: THREE LAWS AS NODES, THE SAME KEYS FOUR WAYS. *** render/physicsTsl.mjs writes swk_lyapunov's exponent (two TSL Loops of the logistic map, the mean log-slope) and the Heidler return-stroke current (a closed form of t) as TSL functions any node material can take, beside v4319's blackbody; the constants are the modules' (imported, not retyped), the uniforms labelled. tslPhysics-selfcheck holds them to the keys the WGSL and GLSL twins are held to, read off pictures the shader is never handed the answer to: through three's TSL path on WebGPU and on its WebGL2 backend, the Lyapunov key at r = 4 decodes to ln 2 over the whole picture (median 0.693226, within 8e-5), the period-3 window is dark and r near 4 bright, and the lightning's peak over i0 reads 0.99998 at the true eta and 1.0667 at the published one; then render/tslSource.mjs transplants the fragments three emitted into gfx/device.js pipelines and BOTH backends read the same numbers to the last digit -- generated physics in the device's own shell. The emitted pairs go to tools/ship/tsl-emitted-physics.json and the WGSL corpus compiles them as generated code (four generated entries now). One new gate. MERGED: origin/claude/stale-numbers-sweep (SwiftUIShaders 41 of 41 ported, the gate sweep, the red census refreshed; its v4317-v4321 commit-message numbers are superseded by this tree's markers, which go 4320 -> 4321 here). NOTED, NOT MERGED: origin/main has moved by five commits from other sessions numbered v4301-v4305, which collide with this branch's v4301 onward -- whose numbers stand when the branches meet is a decision, not a merge. Sabotaged and measured: the Lyapunov log's 2 dropped (9 red: the exponent reads 0.000077 on every path and backend), the Heidler shape unsquared (5 red: 0.85081 and 0.9076 for 1 and 1.0667) -- the twins' own sabotages of v4315, reproduced in TSL and caught four ways each. Not built and said: a race's hull painted by a TSL node (the fleets are device pipelines), and the cost of a 448-iteration Loop through three, which nobody timed. The tree stands at 1398 gates.
