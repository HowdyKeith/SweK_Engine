...
const BRAIN_BUILD = "v4137";   // v4137 -- cflBind-selfcheck asserted maxSpeed > 1e4 on a runaway that is chaotic by construction: 6.76e+8 here (identical over five runs and two Node versions), 120 on Keith's Node 24 rig, same code. An exponentially unstable run's size after N steps is set by when the instability takes hold, which last-bit differences move -- so the threshold was asserting that two engines agree about a chaotic trajectory. Its three conjuncts were all the same condition, since finiteButBroken is DEFINED as allFinite && maxSpeed > 1e4. The claim never needed the magnitude: it now asserts the ACOUSTIC courant (soundSpeed*dt/h = 3.0, fixture constants) and allFinite, and reports maxSpeed. Sabotage-confirmed three ways, including that feeding it the rig's own 120 now passes. Full changelog on docs/CHANGELOG.md.
