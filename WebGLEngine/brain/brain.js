...
const BRAIN_BUILD = "v4126";   // v4126 -- a generated double-clickable .command launcher for the NTFS mounter. v4125's `sudo -n` button never handles a password but fails closed unless you ran `sudo -v` elsewhere first; a .command is handed to Terminal.app, so Terminal asks for the password -- same guarantee, no homework, and it is named the recommended path rather than offered as an equal alternative. Feeds no menu index to the script, so the stale-selection risk mount() has to verify against simply does not exist. Approval happens twice outside this server: Terminal's password prompt and the script's own "Continue? [y/N]". Volume names are POSIX single-quoted because a USB stick can be named `'; rm -rf ~ #`, proven as behaviour by running a hostile name through real bash and asserting nothing executed. Carries the launchd PATH fix this tree already learned twice; deliberately omits xattr de-quarantine (fs.writeFileSync never sets that attribute) with the difference documented. Gate: ntfsMounter-selfcheck, 37 checks. Full changelog on docs/CHANGELOG.md.
