...
const BRAIN_BUILD = "v4274";   // v4274 -- *** THE ORRERY'S SHADER STAGE IS ON THE PAGE NOW, AND THE ROUND'S REAL LESSON IS THAT MY CHECK OF IT WAS WRONG TWICE. *** v4273 built ui/orreryPost.mjs, proved it attaches and draws, and shipped it with nothing able to ask for it. orrery.html now has an overlay canvas, a "signal" toggle beside the surface button, and a frame hook that runs the effect AFTER drawSystem finishes -- uploading mid-draw would sample a half-built system. It is off by default, ignores the pointer so every click and scroll still reaches the orrery, and shows the REASON when it cannot attach rather than sitting dead, which was v4267's whole subject. *** BUT THE CHECK THAT IT WORKS WAS FOOLED TWICE AND SABOTAGE D FOUND BOTH. *** Draft one screenshotted the page before and after and required a difference -- on an ANIMATING orrery, so any two frames differ and the check measured the clock. Pausing fixed that and D still passed: merely revealing the overlay layer perturbs compositing. Measured directly, a never-drawn #fx captures at 110,787 PNG bytes against 128,266 drawn, a ratio of 1.2, and element screenshots composite what is behind them so they cannot isolate it either. gfx/device.js builds its context with `opts.contextAttribs || {}`, so preserveDrawingBuffer is off and any readPixels after presentation is guaranteed zero. *** SO THE HONEST DIVISION IS: SECTION 5 PROVES THE CONTROL WORKS, SECTION 3 PROVES THE RENDER WORKS -- the gate owns both canvases there and reads in the same task -- and neither is asked to do the other's job. *** The claim was narrowed rather than a cleverer capture invented. Sabotage A 1 red, B 1 (2,031 lit texels to 0 while draw() still reports success), C 2, D 1 in source with the screenshots still green, E 1, F 1. Full changelog on docs/CHANGELOG.md.
