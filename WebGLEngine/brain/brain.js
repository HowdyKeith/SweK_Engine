...
const BRAIN_BUILD = "v4338";   // v4338 -- *** WORKGROUP-SHARED MEMORY, AND A SECTION THIS TRANSPLANT HAD BEEN DROPPING ON THE FLOOR. *** The finding first: three emits a compute shader in sections, and its "// locals" section -- where var<workgroup> declarations live -- was never carried across by render/tslSource.mjs transplantCompute. Nothing had noticed because no graph had used shared memory; the first one that did produced a module whose body named an array nothing declared, and the device refused it by name. computeShell now takes `shared` ([{ name, element, length }] becomes var<workgroup> name: array<element, length>), the transplant renames three's own WorkgroupArray_NNN to the shell's name -- a generated identifier in a shipped module is a name that changes when three does -- and a count, element or length that disagrees is refused before the device sees it. The round: render/physicsTsl.mjs makeChaosReduceTsl counts the same chaotic elements as v4337 but the way a real reduction does. Each lane writes its 1 or 0 into the array the workgroup shares, the group waits at a barrier, and lane 0 sums 64 slots and contributes ONE atomic increment for all of them. Measured on WebGPU at 1024 elements: the total is 670, exactly the per-lane tally's answer, with SIXTEEN atomic operations instead of 670 -- one per workgroup rather than one per positive element. render/wgslSpec.mjs's own parseWorkgroupVars reads array<u32, 64> = 256 bytes out of the generated module, which is the first shader nobody wrote that that scanner has ever had to read. *** AND THE BARRIER IS MEASURED, NOT CITED. *** The same transplanted module with workgroupBarrier() removed reads 40 against a truth of 670, every run out of five: lane 0 sums the shared array before the other sixty-three lanes have written into it. The check asserts that it is wrong -- and says in its own detail that if it ever goes GREEN on other hardware, a device whose lanes happen to finish in order, THAT IS A FINDING AND NOT A FIX: WGSL guarantees nothing there without the barrier, and the answer is to record where the failure stopped being observable rather than to drop the check. Sabotaged and measured: the shell's var<workgroup> declaration left out of its prefix (4 red -- the module names an array nothing declares, the device refuses it, the total reads 0, and the scanner finds nothing), and the rename of three's generated identifier skipped (3 red -- the shell declares "lane", the body still says WorkgroupArray_593, and the device refuses that too). Not built and said: an INDIRECT dispatch, where the count a pass runs at is itself in a buffer -- gfx/device.js cannot do it at all today, it has drawIndexedIndirect and no dispatchWorkgroupsIndirect, and it is the last thing standing between this transplant and REGENERATING a real render/gpuDriven.mjs pass rather than demonstrating one; a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; and every number the rig has not signed. The tree stands at 1411 gates. Full changelog on docs/CHANGELOG.md.
