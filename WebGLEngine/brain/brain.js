...
const BRAIN_BUILD = "v4322";   // v4322 -- *** THE ROADMAP'S LAST TWO "NOT YET"S, AND MAIN MERGED IN. *** (1) A RACE PAINTED BY A TSL NODE. render/physicsTsl.mjs makeLyapunovLookTsl writes the Chaos race's look -- the hull's own coordinates as r and the seed, the exponent as the shade, lit by the normal, times the vertex colour -- as a TSL graph with the fleet's own knobs baked into the Loop; three emits it in WGSL and GLSL; and render/tslSource.mjs transplantIntoShell() carries the fragment into the FLEET'S OWN SHELL -- the look's vertex stage, its Cam struct, its VOut -- by the semantics three's vertex shader wrote beside each varying (varyings.nodeVaryingN = uv | normal | color, read by varyingSemantics()), the shell's names put in their place (v.local, v.n, v.color; cam.light, cam.chaos), refusing by name a varying the shell does not carry, a uniform its struct lacks or whose type differs, a texture. tslRace-selfcheck: the fleets scene with the Chaos fleet's pipeline swapped for the generated one draws the hand-written Chaos race on 36,864 of 36,864 pixels, worst 0, on WebGPU and on WebGL2, the pick still naming the Chaos ships; the fixture in three's shape is JSON data, so the gate bears no shader language for the census to count. The generated look joins the corpus (tools/ship/tsl-emitted-race.json, five generated entries now). (2) THE LOOP'S COST, HANDED TO THE RIG. tsl-rig.html runs on the machine it is opened on: the keys read off pictures through three's node path on each backend the box has (ln 2, the Heidler peak, Wien's column), badTv generated against hand-written to the byte, and the 448-step Lyapunov loop timed through three and through the transplanted gfx/device.js pipeline; tslRig-selfcheck loads it here at a small size (SwiftShader's numbers, said so) and reads tools/ship/tsl-rig.json back when a rig saves one -- RIG-PENDING until then, and the page never presents a WebGPU canvas, because presenting one loses the device on the headless shell (measured, twice). (3) MAIN MERGED. origin/main's five commits from other sessions (v4301-v4305: the rangefinder and QR channel, the song globe, the avatar in depth, the licence verdicts, the cheap-tree ship gate) are in; where both sides had ported the SwiftUIShaders the sweep's port (41 of 41, green here) was kept, this tree's markers stand, both changelog runs are kept in order. Three of main's gates moved their records by what this branch added, each with the reason written where the number is: the MIT-notice count (vendor/three-webgpu/LICENSE, 15 -> 16), the orrery's importer baseline (tsl-selfcheck NAMES vendor/three/ in a check and a scanner counts the mention; krbn-lyapunov.html), and the gate-sweep register (the 26 gates added since v4317 swept and named, 25 green and one red on arrival -- avatarZWander, main's own thirty-second avatar observation, red on origin/main's tree in the same shell too). Two new gates. Sabotaged and measured: the varying semantics swapped (4 red: the GLSL refuses to compile, a vec4 into a vec3, before any picture), the look's light dropped (3 red: three drops the dead normal varying and 689 hull pixels part from the hand-written race), and the rig grader blinded to the Heidler peak -- 0 red the first time, because a removed check has nothing to catch when the data is right; a CPU section grading a record that lies was added and the same sabotage reads 1 red. Not built and said: the page drawing the generated Chaos look (three is 3 MB and no page loads it beside the orrery); three's camera in a graph (still refused); linear-filtered sampling on the device path; and every number the rig has not yet signed. The tree stands at 1407 gates.
