...
const BRAIN_BUILD = "v4093";   // v4093 -- labResults-selfcheck re-frozen after a key-by-key diff of old vs fresh baseline, not a blind re-freeze. Six MOVED values, all already explained: figureeight's -1 sentinel to real drift values (v3852/v3973 documented behaviour), xpbd/kernel.restDensity vanishing (v3973's stowaway-field removal), and blackhole/onset.captureOnsetR/captureOnsetRBracket drifting ~0.01 -- investigated fresh, zero commits to either source file since the mirror, bit-identical on repeated local runs, so cross-platform float drift near an already-documented fragile bisection boundary, not a bug. Everything else is device-roster growth (75->129 devices) since this baseline was last frozen, distinguished from the six real moves by the same diff. Full changelog on docs/CHANGELOG.md.
