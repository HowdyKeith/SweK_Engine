...
const BRAIN_BUILD = "v4257";   // v4257 -- *** THE LEDGER RECORDED WHAT WAS READ AND NOT TAKEN; NOTHING RECORDED WHAT WAS TAKEN. *** world/reachedLicences.mjs says so in its own docstring, and the bytes that DID enter the tree had no register at all. New world/vendoredLicences.mjs covers them, and the census is stranger than #61's 'box3d and htmx': asking which directories lack a file called LICENSE flags FOUR and THREE OF THE FOUR ANSWERS ARE WRONG -- vendor/fonts is papered under IBMPlexSerif-OFL.txt, vendor/keyhunt vendors nothing (a technique reference that says NO CODE WAS COPIED), vendor/wasm is OUR OWN AssemblyScript output, and ui/vendor is a second vendor directory papered in its file header. The one real gap was htmx, whose minified bundle carries no banner at all -- ten licence-word hits are all the substring 'submit' -- so its 0BSD grant had to come from upstream at the pinned tag. #121 is closed too: the reached register held exactly TWO spdx values, MIT and AGPL-3.0, with 'Apache' and 'BSD' appearing nowhere, and now holds Apache-2.0 and BSD-3-Clause -- the first Apache entry being img2threejs, which has shaped shipped code since v3337 and was never recorded. *** AND THE ROUND FOUND WHY SUCH THINGS GO UNNOTICED: *** the licence gate SCANS 4,059 files and --affected connects it only to the ones it IMPORTS, because affected.mjs walks the import graph; verify.mjs does not name it either. v4247 added a registered repo's name to ui/gazeDwell.mjs, shipped ALL GREEN, and left that gate red. Full changelog on docs/CHANGELOG.md.
