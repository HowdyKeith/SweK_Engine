...
const BRAIN_BUILD = "v4378";   // v4378 -- *** THE THRESHOLD v4377 SHIPPED WAS DERIVED ON A RASTERISER THE PAGE DOES NOT RUN ON. *** The correction is the round. v4377 froze the disc ladder's pricing, derived metric = K / width from it, and put that derivation in two shipped pages -- and the record was measured on WebGPU. gfx/device.js has TWO rasterisers, and the gate's own output said so in passing on the line that proved the pages load: "webgl2 - orbits by the CPU twin". The orrery on this box runs WEBGL2. So v4377 shipped one rasteriser's number to a page drawing with another, and could not have told anyone what that cost, because only one had ever been asked. MEASURED, same ladder, same widths, same box: K is 7.72 / 6.54 under WebGPU and 7.59 / 6.43 under WebGL2 -- 1.6% and 1.8% apart. Fed back into the WebGL2 record, the WebGPU threshold implies a cost of up to 66.6 pixels against the 64-pixel budget it was derived from, 4.1% over. That is a small overspend and the size is not the point: the point is that it was UNKNOWN, and that nothing in v4377 could have produced the number. render/lodRecord.mjs now carries BOTH rasterisers -- 2 backends x 4 widths x 10 distances x 2 rungs -- and render/lodBudget.mjs discLadderThresholds takes a `backend` and answers for that one. A backend the record never priced is REFUSED rather than served a plausible number, because answering for a rasteriser nobody measured is the exact mistake being corrected. With no backend named it answers with the conservative K across every rasteriser it holds, which is inside the budget on all of them: fed back on every backend, width and rung, the worst of sixteen cases is 64.0 pixels against a 64-pixel ceiling. Both pages now pass device.backend, and the gate checks the page derived for the rasteriser its own route line says it got. *** AND ONE SABOTAGE IS WORTH READING FOR ITS SHAPE. *** Ignoring the named backend, so every caller gets the conservative pair, costs 4 red -- and the THRESHOLD it returns is still safe, because the conservative K is by construction inside the budget everywhere. What breaks is not the number but the answer to a different question: the page stops being able to say which rasteriser its threshold came from. A safe wrong answer is still wrong, and four checks say so. The other, serving a plausible pair for a backend nobody priced instead of refusing, costs 1 red. *** WHAT THIS DOES NOT ESTABLISH, AND IT IS THE HONEST LIMIT: *** both rasterisers measured here are SwiftShader's, one through WebGPU and one through WebGL2. Their 1.7% gap is a LOWER BOUND on how much a rasteriser can move this number, not an estimate of what a rig's real GPU would do -- a hardware rasteriser differs from a software one by more than two software paths differ from each other. The record is re-freezable by tools/ship/freezeLod.mjs on any box, which is the mechanism by which a rig could sign it, and until one does the numbers here remain this sandbox's. Also unchanged and named: gpu-rig-check.html's typed [0.025, 0.04], whose ladder is a rig probe rather than a look; the colour tell, which both pages keep because watching a ladder work is what it is for; and whether 64 pixels is the right budget, which is a judgement about eyes this tree cannot make and has put in one place instead of five. The tree stands at 1419 gates. Full changelog on docs/CHANGELOG.md.
