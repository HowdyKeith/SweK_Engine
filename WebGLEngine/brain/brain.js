...
const BRAIN_BUILD = "v4319";   // v4319 -- *** TSL ENTERS THE TREE, MEASURED BEFORE IT IS TRUSTED. *** Keith: TSL would expand what SweK can do, and nothing here ran it -- the vendored three is r160 (no TSL entry point), main.js draws with WebGLRenderer, and the tree's two "TSL references" are algorithms rewritten in GLSL with their origin credited. docs/TSL-ROADMAP.md sets six steps down and this round builds the first three and starts the fifth. (1) VENDORED. vendor/three-webgpu carries three 0.178's three.webgpu.js, three.core.js, three.tsl.js and LICENSE (3.2 MB beside r160's 1.2 MB, which main.js and every three.js page keep), with ONE edit, named in its README and checked by gate: three.tsl.js's bare 'three/webgpu' import rewritten to './three.webgpu.js' so pages and the harness load it by path. three 0.185 was tried first and refused on this shell's Chromium (its texture views carry a swizzle the browser does not know); 0.178 runs unpatched. The two builds must never meet in one page, and tsl-selfcheck scans every page for that. (2) IT RUNS HERE, ON BOTH BACKENDS. tsl-probe.html and tsl-selfcheck section 2: WebGPURenderer on WebGPU and, with forceWebGL, on its WebGL2 backend, a TSL colour node rendering the uv gradient, read back through readRenderTargetPixelsAsync -- and two facts a caller needs, measured rather than assumed: the readback hands rows top-first on WebGPU and bottom-first on WebGL2, and 0.178's WebGPU readback undersizes its staging buffer at widths whose rows are not 256-byte aligned (32 px raises a validation error and reads nothing; the gate keeps to 64 and says so). Presenting to a canvas on WebGPU loses the device on this headless shell, as gfx/device.js's offscreen flag already knew, so every picture graded is a render target. (3) badTv AS TSL, TO THE BYTE. render/badTvTsl.mjs writes Ashima's simplex and Turner's tear once as TSL nodes (nine constants compared as text to the WGSL's, the frequencies and gains interpolated from badTvModel.mjs, the cube in the original's order, select in TSL's argument order which is the reverse of WGSL's). tsl-selfcheck section 3: on WebGPU the picture three's WGSL builder makes, row-mirrored, is our hand-written WGSL's on 4,096 of 4,096 pixels with worst difference 0; on WebGL2 three's GLSL builder against our GLSL, the same; unmirrored the two agree on no pixel (the mirror three's uv convention implies, THREE_PASS_RELATION, is real); and every pixel is the CPU model's texel. One graph, two compilers, and each is the pair the tree wrote by hand. (4) PHYSICS AS TSL NODES, STARTED. render/blackbodyTsl.mjs: Planck's shape and Wien's root by a TSL Loop of 24 Newton steps as Fn nodes; section 4 reads the brightest column off both backends at x_lambda = 4.965114 within a column and x_nu = 2.821439 for n = 3, the root in the blue byte. One new gate (tsl, four sections) and a probe page. Sabotaged and measured: select's arguments swapped (4 red, 768 pixels wrong on both backends), the cube flattened (5 red, 3,456 pixels), the TSL Newton started at x = 1 (4 red, the trivial root in the blue byte), and a constant retyped to twelve digits, which the pixels cannot show (1.9e-13 is below f32) and the first text check missed because the header quotes the constant too -- the check now reads the vec4 line, and the log says so. Not built and said in the roadmap: TSL as a SOURCE for gfx/device.js (reading the builders' WGSL and GLSL out and feeding the device pipelines -- the step that turns every hand-written pair into a generated one), the Lyapunov and Heidler functions as nodes, and the main scene, which nothing on the ladder needs. The tree stands at 1396 gates.
