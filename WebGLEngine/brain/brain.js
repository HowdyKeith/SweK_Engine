...
// v4390 -- *** ONE OPERATOR FOR EVERY LITERAL MANUFACTURES SURVIVORS, AND CHOOSING IT BY THE CONSTANT'S ROLE FIXES THREE OF SEVEN. *** v4389 ran the mechanical scanner for the first time and found that five of its seven survivors were about the OPERATOR rather than the tree -- integer tick counts and a JSON indent, nudged by three percent and filed as constants nothing is checking. It recorded that as a hand annotation called `plausible` and said so plainly. tools/mutate/operators.mjs is that annotation becoming a function. THREE ROLES: a SCALE (non-integer, or in multiplicative context) keeps the relative nudge; a COUNT (an integer default, a tick offset) is set to ZERO and stepped by ONE, because a 3% nudge is not a defect anyone could ship and those two are; a FORMAT argument -- JSON.stringify's indent, toFixed's places -- gets NO MUTANT and is reported SKIPPED, which is a different and honest answer to "is this number checked". *** ARITHMETIC BEATS DEFAULT AND THE ORDER IS LOAD-BEARING: *** `const dt = opts.dt || 1 / 30` is both, and read as a default it would be set to 0 -- a zero timestep, caught instantly by everything, telling you nothing about the 1/30. *** THE CONTROL IS A CONSTANT WHOSE ANSWER WAS KNOWN BEFORE THE RUN. *** mutate.mjs's hand-picked table sets box3dLockstepNet's redundancy to 0 and has been CAUGHT every time the suite has run; if the role operator had not caught it, the classifier would be wrong in the one case with an independent answer. It caught it, after ONE gate. RE-SWEPT, per constant over the same eleven: 3% only gave 4 checked and 7 survivors; by role gives 6 CHECKED, 4 SURVIVORS, 1 CORRECTLY SKIPPED. maxCatchup and redundancy are caught the moment they are set to zero; the JSON indent is not a number anyone should check. *** AND ALL FOUR REMAINING SURVIVORS ARE THE SAME SHAPE, WHICH IS THE ROUND'S REAL FINDING. *** v4389 found the lockstep timestep survives because the gates that reach it are DIFFERENTIAL -- two peers compared to each other, so a constant they share moves both sides of the equality. That was not one curiosity: inputDelay SET TO ZERO survives too, and so does the history-window offset, and so does shipHalf. Lockstep schedules inputs ahead by inputDelay ticks and at zero both peers misbehave identically. *** SO THE RULE IS SHARPER THAN "A DIFFERENTIAL GATE IS BLIND TO A SHARED CONSTANT": IT SEES ONE ONLY WHEN SOMETHING BREAKS THE SYMMETRY. *** Both catches came from the same gate, physics/box3d-lockstep-loss-selfcheck.mjs, which injects PACKET LOSS -- redundancy governs what survives a lossy channel and maxCatchup whether a peer can ever catch up, so under loss the peers stop being symmetric and diverge for real. That is a statement about how to WRITE the missing check, not just about which numbers lack one. THE CLASSIFIER SHIPPED A BUG IN ITS FIRST DRAFT AND THE FIX IS RECORDED: it read the role off `context`, which findConstants TRIMS and TRUNCATES for printing, while `col` indexes the untrimmed line. So `1 / 30` read as a bare integer and JSON.stringify's indent was not seen to be inside the call. scan.mjs now carries `code` beside `context` -- the same family as v4389's length-preserving string stripper: a column is only meaningful against the text it was measured on. AND THE CLASSIFIER IS A SYNTACTIC GUESS THAT IS ALREADY KNOWN WRONG: shipHalf reads as a COUNT and is a physical half-extent. The role is wrong; the operator it chose (a body of half-extent 0) is still a better question than 30 -> 30.9, and the record says so rather than smoothing it. FORMAT is the one role that must not be wrong, because it makes a constant disappear from the experiment entirely, which is why it is a named list of eight calls rather than a rule. Four sabotages, 2/4/1/1 red by name, three files md5-identical; B is the instructive one -- every SCALE collapses into COUNT and dt would be caught FOR THE WRONG REASON, which is worse than surviving. UNCHECKED: the four survivors are still unguarded. This round sharpens the question and fixes nothing in the lockstep pair. The tree stands at 1428 gates. Full changelog on docs/CHANGELOG.md.
const BRAIN_BUILD = "v4391";   // v4391 -- *** CAMERA-RELATIVE RENDERING IS THE STANDARD ANSWER TO FLOAT32 AT DISTANCE AND IT IS NOT A FIX: IT BUYS A FACTOR OF 2.2. *** v4388 shipped the shipyard and named its own unchecked item -- "whether float32 error at distance is better fixed by a CAMERA-RELATIVE RENDER than by this indirection, which is the alternative nobody measured here". A design shipped without its alternative measured is an argument rather than a finding, so this round measures it. Four encodings, one body, a camera thirty voxels off it, and the view transform ACCUMULATED IN FLOAT32 at every step the way a vertex shader does rather than summed in float64 and rounded once, which reports a kinder number: (A) a world-absolute vertex through a view matrix with the translation baked in; (B) camera-relative, the classic render-relative-to-eye, world vertex with the camera differenced in float64; (C) the shipyard, a claim-local vertex with the pose carrying (body - eye); (D) double-single, the world coordinate carried as a high/low float32 pair, which is Cesium's trick for a globe. MEASURED, worst vertex in eye-relative space, in voxels: at a million, A 7.5e-2, B 3.4e-2, C 6.3e-6, D 4.6e-6; at 8.4 million, A 1.11, B 0.55, C 6.3e-6, D 4.6e-6. *** B IMPROVES ON A BY 2.0 TO 2.5 AT EVERY DISTANCE FROM A THOUSAND TO EIGHT MILLION, AND I PREDICTED IN WRITING THAT IT WOULD BE ORDERS OF MAGNITUDE. *** The reason is that the two errors are not the same size: render-relative-to-eye removes the CANCELLATION in the matrix multiply and cannot touch the QUANTISATION already in the stored coordinate, and for a body at distance the quantisation is the whole error. B's residual sits at 0.53 to 0.55 of a float32 spacing at every distance -- the vertex buffer holds a world number and the nearest representable one is up to half a spacing away before any arithmetic happens. BOTH REAL FIXES CHANGE THE STORAGE RATHER THAN THE ARITHMETIC, and they agree with each other to within 1.4x while being FOUR ORDERS below B at a million, so the choice between them is COST and not accuracy: the shipyard needs a per-body origin and a pose matrix, which a body that moves already has; double-single needs twice the attribute bytes and no pose at all, which is what STATIC far terrain can use and a shipyard has nothing to offer. *** AND THE ROUND CORRECTS ITS OWN PREDECESSOR. *** v4388 measured the shipyard's STORED error as exactly zero and wrote "exact" without saying which kind. The RENDERED error is 6.3e-6 of a voxel -- flat in distance, small, and not zero, because a rotation and three float32 accumulations sit on top of the storage. A reader who took "exact" to mean "exact through the pipeline" was wrong by that much, and both numbers are now named as different claims in the module's own header. THE TWO RIVAL ENCODINGS LIVE IN voxel/shipyard.mjs RATHER THAN IN THE GATE, because a rival implemented inside the check that judges it is not a rival; the page shows all three live beside the shipyard's own figure, so a reader meets the alternative where the claim is made. *** THREE SABOTAGES, AND THE THIRD COST 0 RED, WHICH IS THE ROUND'S SECOND FINDING. *** splitDouble returning a zero low word takes the flat-in-distance check red; eyeRelative differencing in float32 collapses the factor and takes the camera-relative check red; and DELETING THE PAGE'S THREE RIVAL ROWS COST NOTHING, because the check tested for the WORD "camera-relative" and the sentence explaining that camera-relative is not the fix still contained it. A CHECK FOR A WORD IS SATISFIED BY THE PROSE THAT EXPLAINS IT -- which is exactly v4383's shader-census defect, committed on this session's own gate three rounds after writing it up. The page publishes the three figures as a data attribute now and the gate recomputes them from the module; re-sabotaged by having the page compute camera-relative itself instead of calling the module, it goes red naming both figures. UNCHECKED AND SAID PLAINLY: whether the ordering survives a PROJECTION matrix, since this measures eye-relative space and stops there, and a near plane at 0.1 against a far one at 8e6 has its own depth precision story that this round does not open; whether a GPU's float32 rounds the same way Math.fround does on every driver, which is a claim about hardware this tree cannot make from node; and the cost side of the comparison, which is stated from what each encoding needs rather than measured in bytes or draw calls on a real scene. *** AND THIS ROUND WAS NUMBERED v4390 UNTIL IT TRIED TO LAND, FOR THE THIRD TIME IN THIS SESSION. *** claude/orrery-seeded-by-git-log reached main first with its own v4390, so this one supersedes forward rather than reusing the number; its round is merged here and its gate takes the tree to 1428. The tree stands at 1428 gates. Full changelog on docs/CHANGELOG.md.
