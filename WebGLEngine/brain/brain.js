...
const BRAIN_BUILD = "v4262";   // v4262 -- *** FIND THE CONSUMER BEFORE TAKING THE SOLVER: THE CONSUMER EXISTS, AND IT DISQUALIFIES. *** Backlog #133 put the discipline in its own title, so this round is the search done as a measurement and the answer is REFUSED -- nothing vendored, nothing ported. A local solver returns ONE coordinate of Mx = b without forming the vector, and needs three things at once: M diagonally dominant, a consumer wanting k << n coordinates, and n large enough. *** THE TWO PROPERTIES ARE IN DIFFERENT FILES. *** fluid/multigrid.mjs's 5-point Poisson operator is dominant in EVERY row (worst ratio exactly 1.0000 at 8x8, 16x16, 32x32) and flip2d's pressure projection needs all 16,384 cells -- a divergence-free projection touches every one. And tools/roundhouse/beamBind.mjs, the ONE genuine single-coordinate consumer in the tree -- solve(K, unit(N,i2))[i1], two numbers wanted of 320 computed -- solves a matrix at 0.3333 dominance with ONE ROW OF 160 dominant, IDENTICALLY at n = 8, 20, 60 and 160, so refinement never approaches the precondition and there is no larger version that qualifies. The third candidate is the trap the item names: the import graph WOULD fit ((I - alpha P) is row-dominant at 1.1765, all 3,467 rows) and NOTHING ASKS IT ANYTHING -- gateReach does BFS reachability and a tree-wide scan finds ZERO files computing an influence score, so it is recorded as a HOLE rather than a roadmap. Full solve budgets a replacement would have to beat: 2.95 ms for all 3,465 module coordinates, 200 ms for a 128x128 pressure field. *** THREE OF MY OWN CLAIMS WERE WRONG ON THE GATE'S FIRST RUN and were corrected from the matrices rather than by loosening the check: *** the beam's worst row is the FREE END (1 against 3) not the interior stencil (6 against 10); (I - alpha P^T) is dominant by COLUMNS not rows (0.0075 by rows); and the influence-score scan was counting this round's own files. Kept: dominance(), because "will an iterative method converge on this operator" is a question the tree could not previously answer about itself and it has four different answers. Sabotage C -- a fabricated consumer, #133's exact failure mode -- went only 1 RED, so every named consumer now carries a consumerEvidence line the gate greps for in the file it names; 2 RED after. Five sabotages, 8/2/2/1/1 red. Full changelog on docs/CHANGELOG.md.
