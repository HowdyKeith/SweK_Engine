...
const BRAIN_BUILD = "v4252";   // v4252 -- *** #60's WALL WAS NOT A WALL, AND THE PIXEL TEST THAT FOUND IT ALMOST TOLD ME THE OPPOSITE OF THE TRUTH. *** v4232 measured 0.0% skippable and named the last holder domAnimation, a CSS animation in the chrome; a four-second window could not go further. Spending v4250's deterministic clock: window.dayNight.t advances 5.5556e-5 per frame -- EXACTLY 1/18000 of a cycle, the identical rate at 120, 1200 and 6000 frames -- so THE SUN MOVES ON EVERY FRAME and dayNightCycle's ANIMATES verdict stands. A screenshot-tile diff had reported ZERO scene change at 1200 frames and the tempting reading, 'the scene is still', is wrong: a change below one 8-bit step is still a change and it accumulates, 6000 frames taking the sky from afternoon to evening. Paused, it moves EXACTLY 0.000e+0 over 6000 frames, so the static scenario really is static -- and there the flag is still held dirty every frame by 14 running animations, 10 endless, an SVG mascot and a watermark. *** THE TWO CLOCKS ARE INDEPENDENT, MEASURED AS A NUMBER: *** a CSS animation's currentTime advanced 999.96 ms across 1001 ms of wall clock with the engine stepping ZERO frames, because CSS runs on the compositor's timeline and replacing requestAnimationFrame does not replace it. New engine/domScope.mjs classifies each animation CHROME/SAMPLED/GEOMETRY/UNKNOWN, every uncertainty resolving towards drawing, with a registry for the one real DOM-to-scene path (v4120's domToTexture). The probe's vote is UNCHANGED and the flag still ships disabled: this round measures and builds the rule, it does not turn anything on. Full changelog on docs/CHANGELOG.md.
