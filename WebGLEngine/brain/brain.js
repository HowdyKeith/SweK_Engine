...
const BRAIN_BUILD = "v4370";   // v4370 -- *** A KERNEL THAT ALREADY SHIPS, GENERATED, AND HELD TO IT BIT FOR BIT -- AND THE ROUND'S OWN CLAIM ABOUT WHAT THAT IS WORTH WAS WRONG. *** Every TSL transplant in this arc has been graded against a twin written in this tree for the purpose, or against a picture that hides a difference under 1/255. tools/roundhouse/hmcGpu.mjs's WGSL_HMC is neither: it is the batch leapfrog the swek-hmc-bench fleet job runs on real hardware, it predates this arc by a thousand versions, and it carries its own CPU mirror at f32 (Math.fround after every arithmetic op) plus the FLOOR that mirror measured for real devices. render/physicsTsl.mjs makeHmcLeapfrogTsl is that kernel as TSL nodes -- one invocation per chain, L kick-drift-kick steps over U = 0.5 (q-mu)^T Sinv (q-mu) -- transplanted into a gfx/device.js compute module through render/tslSource.mjs. Measured on WebGPU against the SHIPPED text on the same 64 chains: BIT-IDENTICAL on all 256 endpoint values. Not a tolerance, 0. And both passes land inside 1.371e-6 of the f32 mirror against a floor of 2.5e-5. *** THE CORRECTION, WHICH THE GATE MEASURED RATHER THAN REVIEWED. *** The module's header said that writing the half-kick as 0.5*(eps*g) instead of (0.5*eps)*g is algebraically identical and NOT bit-identical. Run, it is bit-identical on every one of the 256: 0.5 is a power of two, that multiply is exact, and re-association across it cannot lose anything. The rule was right and the example chosen to illustrate it was false. So the gate now carries a re-association that IS observable beside it -- the gradient distributed, i00*qx - i00*mu0 for i00*(qx - mu0) -- which moves 215 of 256 endpoints, and pins BOTH numbers instead of the rule of thumb. *** AND THAT SECOND NUMBER IS THE ARGUMENT FOR STATING THE CLAIM AS 0. *** The distributed rewrite is off by 1.192e-6: 21x inside hmcGpu's own 2.5e-5 floor and 42x inside the 5e-5 tolerance a real device is graded against. Every tolerance in this tree passes that sabotage. The bit claim is the only thing here that does not. THE AGREEMENT IS ALSO NOT TWO PASSES DOING NOTHING, which is the failure a perfect score invites: all 256 values moved off their inputs and the 64 chains land in 128 distinct coordinates. Three sabotages, measured: the gradient distributed in the GRAPH (2 red -- 41 of 256 still bit-exact, worst 1.653e-6, and inside the floor, which is the whole point); the pre-loop gradient dropped so the first half-kick uses zero (2 red, worst relative 6.998e-1, four orders past the tolerance -- the leapfrog carries g INTO the loop from before it, and the shipped kernel is what says so); and the read/write-count agreement removed from transplantCompute (1 red, and what it costs is in the message: an all-read_write shell stops being refused by name and throws "Cannot read properties of undefined" from deeper in the mapping instead). NOT CLAIMED, because it is not true: the kernel's full SIGNATURE. L and n are uniforms in WGSL_HMC and baked constants in the graph, because a TSL Loop wants a JavaScript bound -- lyapunovNodes set that precedent at v4321 with samples and warmup -- so this is the kernel's ARITHMETIC at the fixture's own L, with the shipped pass fed the same L through its uniform. A graph whose step count comes from a buffer is a round of its own. Also unbuilt and unchanged: cullLodWgsl({ occlusion: true }), the one variant left, which needs a mat4x4 uniform, two while-loops whose bounds are computed and a nested tile loop; a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph; and every number the rig has not signed. The tree stands at 1414 gates.
