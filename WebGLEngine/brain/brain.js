...
const BRAIN_BUILD = "v4271";   // v4271 -- *** gfx/device.js HAS PROMISED "write your render ONCE, run on either runtime" SINCE IT WAS WRITTEN, AND NOTHING HAD EVER RENDERED THE SAME EFFECT BOTH WAYS AND DIFFED THE PIXELS. *** Now something has: render/badTvDevicePass.mjs is one descriptor carrying WGSL and standalone GLSL, rendered on WebGPU and on WebGL2 against the same position-encoding source texture, and compared to render/badTvModel.mjs and to each other. *** 0 OF 4,096 PIXELS DIFFER, AND BOTH MATCH THE MODEL EXACTLY -- 0 of 255. *** *** AND THE ROUND GOT ORIENTATION WRONG TWICE BEFORE MEASURING IT RIGHT ONCE. *** v4270 proved the WGSL computes the model's coordinates to 3.2e-8 and could say NOTHING about which way is up, because a coordinate is a pair of numbers until something samples a real texture with it: the first rendered comparison was off by 126 of 255 and exact at (1 - v). Then this file argued in a confident comment that the two vertex stages must DIFFER, since WebGPU texture row 0 is the top while readPixels hands rows back bottom-first. Rendered both ways: all 4,096 pixels differed, in whole-texel steps growing with the row -- the signature of the backends evaluating the tear at different v. The readback flip already lives in the harness, so flipping again in the shader flipped twice. Both stages now flip identically. *** IT IS NOT COSMETIC: badTvSampleAt COMPUTES fract(v - time*rollSpeed), so a "wrong but symmetric" orientation ships an effect that rolls the wrong way and looks entirely plausible. *** Sabotage A un-flips the GLSL: 3 red, and the WebGPU side stays GREEN throughout, which is why a per-backend check would have called half a success a finished port. B reverses the uniform order: 1 red, none of it in the render, because the gate hands both backends the same array -- a pixel diff cannot see a contract nothing in the test obeys. C drops the cube from the GLSL: 3,456 of 4,096 differ. A control threshold was guessed again and measured: the tear is INTERMITTENT, 12 of 64 rows past one texel, worst 1.86 against the model's own maxTear of 1.93. Full changelog on docs/CHANGELOG.md.
