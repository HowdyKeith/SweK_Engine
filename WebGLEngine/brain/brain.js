...
const BRAIN_BUILD = "v4282";   // v4282 -- *** physics/render/roughDiffuse.mjs SHIPPED AT v4275 AND HAD NO CALLER FOR SEVEN ROUNDS. *** Its own closing note said so: "unchecked: ANY CONSUMER. physics/render/pathTracer.mjs still uses albedo/PI and this round did not change it." v4280 and v4281 both cited it BY NAME as the example of a module the engine does not have. This round wires it into the path tracer. *** THE BEHAVIOUR CHANGE IS OPT-IN AND THAT IS PROVED AGAINST GIT RATHER THAN ARGUED FROM THE DIFF: *** tools/ship/roughDiffuseWired-selfcheck.mjs extracts the committed v4281 tracer with git show, runs it in a child process and compares the hash of the float buffer -- both d66e8de3147f2a91, and an explicit sigma of 0 gives the same bits again. Oren-Nayar at sigma 0 has A = 1 and B = 0 so its BRDF IS albedo/pi to the last bit and one path could have served both; it does not, because "the arithmetic agrees" and "the same statements run in the same order consuming the same randoms" are different guarantees and only the second makes every existing image identical BY CONSTRUCTION. WHAT ROUGHNESS DOES, MEASURED IN A PICTURE FOR THE FIRST TIME: the peak falls 1.1387 -> 1.1063 -> 1.0887 -> 1.0774 across sigma 0 / 0.3 / 0.6 / 1.0, and it falls FIFTEEN TIMES FASTER than the mean (peak -5.38% against mean -0.37%). That ratio is the signature -- light moved out of the facing region toward grazing angles, the moon rather than a billiard ball; a model that merely darkened everything would move both numbers together. THE COMPENSATION MEASURED IN A RENDER: the gate generates an uncompensated tracer by rewriting diffuseTable(sig) to null -- the mean falls 2.48%, 7.68%, 11.99%; with the table, 0.37%. *** THE INTEGRAL LOSES 25.6% AND THE PICTURE LOSES 12.0% AND NEITHER CONTRADICTS THE OTHER *** -- a render dilutes the lobe's loss with multiple bounces, the light's geometry and the pixels that see sky. Quoting the integral figure as the picture's would have been the easy error. NEE now evaluates the real BRDF at the light; the table is cached by sigma because buildDiffuseTable integrates the lobe. *** AND THE STALE SENTENCE IS REPLACED BY A DERIVED ONE, NOT A CORRECTED ONE: *** roughDiffuse-selfcheck said "no consumer" for seven rounds after it stopped being true-in-spirit and would have said it forever, because prose does not go red. Section 5 now WALKS the tree and reports what it finds -- 3 files, 1 consumer, 2 gates. Five sabotages, none 0 RED: !(sig > 0) relaxed to sig <= 0 (the tidy-up a reviewer would wave through) 6 red, because undefined <= 0 is FALSE and every sphere that never heard of roughness falls into the new branch with a NaN sigma; the table nulled 2 red, the second being that peak and mean then fall together at "a factor of 1", turning the report prose into a check; Math.PI * dropped 3 red; azimuthCos pinned to 1 3 red and the ball gets BRIGHTER with roughness -- a rough surface that glows still looks like a rough surface. TWO UNBIDDEN REDS FIXED, BOTH FOUND BY SABOTAGE POINTING AT THE GATE RATHER THAN THE MODULE: the clamp was asserted as a REGEX and nothing ever rendered past it (sigma 10 and SIGMA_MAX must now agree to the bit), and the signature check FAILED WHILE PRINTING "a factor of 33" because the ratio of two negative deltas reads exactly like the healthy answer, so the sign is now part of the claim. A check that fails while printing a reassuring number is half a check. All sixteen path-tracer consumer gates still pass. *** AND THE GATE'S OWN HEADLINE CHECK DIED THE MOMENT THIS ROUND WAS COMMITTED: *** section 1 read the reference tracer with git show HEAD:, correct for exactly as long as it took to run git commit -- after that HEAD held the PATCHED file and the bit-identity check was a file compared with itself. A sixth sabotage reproduces it and the headline check PASSES, 37357 chars against 37357 at this round's own commit, with nothing in the run looking wrong; the only red is the control asserting the two sources DIFFER before anything asserts they agree. The reference revision is now FOUND rather than named -- walk the tracer's history newest-first and take the first revision that does not import roughDiffuse. A before-and-after whose "before" drifts forward to meet the "after" is not a comparison. Full changelog on docs/CHANGELOG.md.
