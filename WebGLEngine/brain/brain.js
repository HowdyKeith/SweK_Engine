...
const BRAIN_BUILD = "v4270";   // v4270 -- *** v4269 SAID "NOTHING HERE CAN EXECUTE WGSL" AND SHIPPED IT, AND IT IS FALSE. *** The claim was inferred from wgslSpec.mjs's true note that the build box has no GPU -- true of a node process, irrelevant to a browser -- and never tested. *** AND THE FIRST PROBE THAT WENT LOOKING REPRODUCED THE EXACT CONFUSION ui/webgpuProbe.mjs EXISTS TO NAME: *** it evaluated on about:blank, got navigator.gpu undefined under three flag sets, and would have blamed the box. about:blank IS NOT A SECURE CONTEXT. Served the same empty page over http://127.0.0.1 the adapter appears at once -- google/swiftshader -- and compiles and runs WGSL. So SwiftShader IS the WebGPU here. tools/ship/webgpuHarness.mjs makes that reusable, and render/badTvWgsl.mjs is the tree's first shader deliberately carried to WGSL: Ashima's simplex plus felixturner/bad-tv, graded NUMERICALLY on the GPU against render/badTvModel.mjs and agreeing to 3.2e-8, with the shipping @vertex/@fragment pair compiled by a real driver. Sabotage A swaps select()'s arguments -- still compiles, still tears, still looks like noise -- and agreement collapses to 2.1e-2. Sabotage B alters a constant's 15th digit and trips the TEXT check while the GPU stays bit-identical: the two checks are not redundant. *** AND v4269's COUNT WAS ALSO WRONG, BY 16: *** its GLSL marker was the #version directive, which no three.js ShaderMaterial pass carries because three prepends it -- so badTvPass, aquarellePass, grassField, solidTexture and atmosphere all read as no-shader. 118 becomes 134 (118 directive, 16 framework). The eighth self-count in eight rounds arrived too: the new marker was a regex LITERAL, so the module matched itself, and then the comment explaining the fix quoted the directive. Both assembled now. Full changelog on docs/CHANGELOG.md.
