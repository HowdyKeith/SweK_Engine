...
const BRAIN_BUILD = "v4365";   // v4365 -- *** A GENERATED img2threejs MODEL, DRAWN BY THIS TREE'S OWN RENDERER -- AND THE SHOWCASE HAS NO LICENCE, WHICH DECIDED THE SHAPE OF THE ROUND. *** The correction first, because it changed the plan before a line was written: the instinct was "their output is what our TSL transplant eats", and it is not. img2threejs targets three 0.169 with WebGLRenderer and MeshPhysicalMaterial -- 100 references to MeshPhysicalMaterial against 1 to WebGPURenderer -- so the GEOMETRY crosses and the MATERIALS do not, because render/tslSource.mjs transplants node graphs and these are classic three materials. And the second correction, found by looking rather than assuming: the img2threejs TOOLING repo is Apache-2.0, but img2threejs-showcase, which holds the generated models, carries NO LICENSE file and no license field in its package.json. So nothing from it is vendored here, the gate builds its own three.js tree with the shapes a generated factory has, and a real model runs only from .img2threejs/model.js, which .gitignore keeps out of the mirror. THE ROUND: render/img2three.mjs walks a three.js object tree -- fifty meshes with their own transforms and their own materials -- and returns ONE mesh in the shape render/gpuDriven.mjs packMeshes already eats, so a generated model draws through the FLEETS' OWN shipped lit pipeline with NO NEW SHADER TEXT IN THIS ROUND, on both backends. It does not import three: everything is read by duck-typing (isMesh, attributes.position.array, matrixWorld.elements, material.color.r), so the whole first section runs on the CPU with hand-built objects and no browser, a three version bump cannot silently change what it reads, and the module never becomes a second import path for the vendored three, which the two-copies rule forbids. *** AND three ITSELF IS THE TWIN. *** Every vertex of a mesh two groups deep, under a rotation and a non-uniform scale, lands where three's own localToWorld puts it -- worst difference 6.0e-8 over the whole mesh -- and the picture gfx/device.js draws is the same on WebGPU and WebGL2 on 36,864 of 36,864 pixels, worst channel difference 0, 3,307 pixels lit in 23 distinct colour cells from four merged materials. Normals go through the INVERSE-TRANSPOSE of the upper 3x3, and the check for it asserts the naive answer is far away rather than that the right one is close, because under scale(2,1,1) the two components of (1,1,0) SWAP: 0.4472/0.8944 against 0.8944/0.4472. *** AND WHERE THE EXACTNESS STOPS IS MEASURED RATHER THAN LEFT AT ONE FLATTERING CAMERA. *** Step the same camera back one notch and ONE pixel of 36,864 parts -- (65,87), 112/9/5 under WebGPU against 57/4/3 under WebGL2 -- and it is a boundary pixel, a rasteriser tie-break at a shared edge. That is v4328's finding again, which measured 467 pixels washed under WebGPU against 415 under WebGL2 on a line list and said a claim over rasterised edges is per-backend by nature; the gate renders BOTH cameras and reports both, and says that going green-with-zero on other hardware would be a FINDING and not a fix, as v4338 said of its barrier. *** WHAT THIS PICTURE IS NOT, SAID HERE RATHER THAN LEFT TO BE INFERRED: THE MODEL'S FINISH. *** Roughness, metalness, clearcoat, transmission, maps and every onBeforeCompile shader patch are dropped at the bridge. This is the form wearing the fleets' lambert, and material identity is most of what a generated model's own review gate scores, so nothing here is a fidelity claim against a reference image. Three sabotages, measured: normals by the world matrix instead of its inverse-transpose (1 red, by name and not by tolerance); the merged indices not rebased (3 red, and the shape says what broke -- only the first mesh's vertices are ever referenced, 858 pixels lit where 3,307 were, and the far camera's edge disagreement goes with them because a model mostly absent has less edge to disagree about); and the world matrix not applied to positions at all (4 red, including three's own twin). MEASURED IN THIS SESSION AND UNSIGNED BY THE RIG: img2threejs-showcase at commit b14415bd, createSonyWf1000xm3Model.ts, type-stripped with node's own module.stripTypeScriptTypes and its three import pointed at the vendored 0.178, built and crossed the bridge at 62 nodes, 50 meshes, 58,778 triangles, 26 MeshPhysicalMaterial / 15 MeshStandardMaterial / 9 MeshBasicMaterial -- section 3 re-derives those numbers the moment the file is present rather than reciting them. UNBUILT AND NAMED: whether that project's own review gate would notice a difference this tree can measure exactly -- its hard gate is silhouette IoU >= 0.85 and its soft signals run on a 64x64 luma grid, against 36,864 pixels at worst difference 0 here -- which is the next round and the reason this one exists; skinned meshes and morph targets, which a character factory emits and this bridge reads as their bind pose; and a physical-material shell, without which the finish cannot cross at all. Also unbuilt and unchanged: a real GPU's log() and exp() against SwiftShader's; the double texel fetch three emits around a Discard; and every number the rig has not signed. The tree stands at 1415 gates. Full changelog on docs/CHANGELOG.md.
