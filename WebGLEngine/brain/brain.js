...
const BRAIN_BUILD = "v4386";   // v4386 -- *** THE CHECK PAGE THAT WAS THIS KERNEL'S WHOLE ANSWER TO "UNTRIED ON A GPU" HAD NEVER RUN A SINGLE STEP -- AND IT PASSED THREE OF ITS OWN ROWS WHILE DOING NOTHING. *** physics/mpm/gpuKernel.mjs states its own status: "CORRECT ON AN INTERPRETER, UNTRIED ON A GPU", and names exactly what an interpreter cannot establish -- "It does not prove the shader COMPILES on real hardware, it runs invocations one after another so IT CANNOT SEE A RACE, and it does not reproduce a vendor's f32." Its answer was mpm-gpu-check.html, "until it has been opened once". Opened headlessly through node-webgpu at v4386, the page built four pipelines with layout:"auto" and handed each one a bind group of all five buffers. WebGPU derives an auto layout from the bindings THAT ENTRY POINT USES, and `clear` never touches binding 1, so the bind group was INVALID for it -- "binding index 1 not present in the bind group layout" -- and an invalid bind group means every dispatch of every stage is silently dropped. *** THE SYMPTOM IS WHY THIS NEEDED A GATE AND NOT JUST A FIX. *** The particles come back exactly where they started, so "two identical runs are bit-identical" is trivially true, "the fixed-point accumulators never saturated" is trivially true because nothing scattered, and "nothing pushed it sideways" is trivially true because nothing pushed it anywhere. The centre of mass read 6.500000000 -- the rest value, to nine decimals. Only the two rows that compare against the CPU would have gone red, and they would have read like a physics disagreement. One explicit bind group layout, valid for all four stages, is the fix; the page and physics/mpm/mpmDevice-selfcheck.mjs both use one. *** AND ONCE IT RUNS, THE INTERPRETER'S VERDICT HOLDS FOR HALF THE KERNEL. *** The module records it: "free fall, the collapsing column and the Drucker-Prager pile all agree with the graded loop to about 1e-7 relative over 15 steps." On a device the ELASTIC path reads 2.86e-8, 8.80e-8, 1.76e-7 and 2.27e-7 over 15/30/60/120 steps -- the cited number, confirmed on hardware. The PLASTIC path reads 2.08e-4, 2.84e-4, 9.28e-4 and 2.01e-3, ratios of 7263x, 3224x, 5278x and 8867x, and is OVER the page's own 1e-4 threshold AT EVERY STEP COUNT INCLUDING THE 15 THE INTERPRETER MEASURED. So it is not error accumulating over a longer run; it is a per-step cost of three to nine thousand times. *** AND ON THIS FIXTURE THE PLASTIC BRANCH IS ARITHMETICALLY THE IDENTITY. *** Measured on the CPU: free fall with the singular-value clamp ON and OFF agree to EXACTLY 0. The clamp never fires, because a block in free fall does not deform. What the branch still does is decompose F and rebuild it -- svd2 then fromSvd -- which round-trips exactly in f64 and does not in f32. The kernel is paying thousands of times its elastic error for a code path that, here, changes nothing; the collapsing column, where the clamp DOES fire, reads 1.71e-3, the same order rather than worse, which says the cost is the round-trip and not the clamping. THE TOLERANCE IS NOT WIDENED, DELIBERATELY: tools/roundhouse/magmapGpu.mjs states the rule -- "the reflex when that happened would be to widen the tolerance until it passed -- which is how a gate stops meaning anything" -- so the gate asserts what is TRUE, elastic inside and plastic outside with the cause localised, and leaves what to do about the plastic path to a round that decides it on purpose. WHAT THE DEVICE DID CONFIRM: the shipped kernel compiles with no errors; it earns the DISCRETE free-fall parabola on its own at com.y 5.263533344 against 5.263531250, error 2.094e-6, with a sideways drift of 3.672e-7; two identical runs are bit-identical on all 576 floats; and the fixed-point accumulators never saturated. Every one of those is a first, because before the layout fix the same rows were true of a kernel that never ran. Three sabotages, 1/3/3. The first is the state the tree shipped in, which cost nothing anywhere until now. The second is the page's own B-spline sabotage, which finally has a number: the parabola off by 7.006e-1 and the elastic path degraded to 1.08e-1 by 120 steps. *** THE THIRD IS THE MOST USEFUL: *** the P2G scatter's atomicAdd replaced by atomicStore destroys the accumulation -- elastic 1.11e-2, parabola off by 4.233e-1 -- AND LEAVES "two identical runs are bit-identical" GREEN. A destroyed scatter is still deterministic if the device always schedules the same way. That row detects ORDER-DEPENDENCE and nothing else, and this device has no order to vary, which is that row's declared limit arriving as a measurement rather than as a caveat. Also found and fixed in this gate: a check that searched the page for the text layout:"auto" went red on ITS OWN explanatory comment, which spells the thing it forbids -- the third disguise this session of a file that spells a marker and joins the census it is meant to police. It asserts on the pipeline call now. UNCHECKED: A RACE, still. Integer atomics make the node total order-independent BY CONSTRUCTION and two runs agreeing is consistent with that, but SwiftShader is one implementation and a real card schedules workgroups differently, so what has been shown is that this device does not expose one. Vendor f32 beyond SwiftShader's, which is exactly what the plastic figure would move with. 65,536 particles, the size this kernel exists for and which no CPU loop can adjudicate. And SPEED, which nothing here times. The tree stands at 1427 gates.
