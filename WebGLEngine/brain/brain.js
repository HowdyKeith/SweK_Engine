...
const BRAIN_BUILD = "v4305";   // v4305 -- *** 41 OF 41: THE THIRTEEN SwiftUIShaders THAT #35 SAID COULD NOT BE PORTED ARE PORTED, AND THE PREMISE WAS THE FALSE ONE. *** #35 stalled at 28 of 41 from v4234 because 'the upstream Metal source is not in the tree, there is no network, and the remaining thirteen are not named anywhere here'. The network half was false (v4276 found the proxy gates GitHub per repository), krispuckett/SwiftUIShaders was cloned (MIT, LICENSE (c) 2026 Kris Puckett, read first-hand for the first time and registered), and aurora, datamosh, disintegrate, etherealAura, liquidChrome, liquidMirror, magneticField, morphBreathe, pixelateStorm, shatter, shatterGlass, smokeReveal and underwaterCaustics were ported from its Metal into render/swiftShaderModel.mjs (CPU reference) and render/swiftShaderPass.js (GLSL ES 3.00). Every one of the thirteen reaches the sin-hash, so they are graded the way frosted and pixelateMosaic have been since v4234: in the configuration where the hash is multiplied out. Eleven have one; in it seven match the CPU model to the level and four to a texel on a real WebGL2 context, five are the identity and are asserted to be so on the GPU, six still change the picture there so the agreement is not two no-ops, and the nine adders honour the alpha convention over an alpha ramp. Two -- liquidChrome and shatterGlass -- have no hash-free configuration and are graded by shape alone, which the gate states rather than hides. Twelve of the thirteen carry pointScale and the three hash-free displacers are measured at 2x. CORRECTED WHILE WRITING: shatterGlass reaches the hash only through the shared Voronoi helper and the source-derived sin-hash set missed it, nine checks went red at once, and the derivation now follows the helper; datamosh's block-edge line fires within 0.48 points of a block boundary, which no pixel reaches at pointScale 1, so upstream's edge is a Retina-only feature. Three sabotages, 3/1/2 red by name, and the one limit of the alpha instrument (five adders are the identity in their hash-free configuration, so a GLSL-only deletion of their k would not be seen on the GPU) is written in the log rather than rounded up. UNCHECKED: how any of the 41 looks beside upstream's Metal on an Apple GPU; this box has SwiftShader and a CPU model. The tree stands at 1387 gates. Full changelog on docs/CHANGELOG.md.
