...
const BRAIN_BUILD = "v4278";   // v4278 -- *** THE SHADER IS THE AUTHORITY ON ITS OWN LAYOUT, AND TWO PLACES IN THIS TREE WERE ANSWERING SEPARATELY. *** gfx/device.js's WebGPU pipeline builds its uniform buffer at offsets computed from `d.uniforms`, a list the CALLER supplies in the caller's order, while the shader independently declares its own struct -- and `layout: "auto"` means WebGPU derives the real layout FROM THE SHADER. Nothing compared them. Reorder either and every write lands in the wrong field: the module compiles, the pipeline builds, the pass runs, the draw completes, and the picture is quietly wrong. There is no error in that chain to catch. render/badTvWgsl.mjs had the same duplication in miniature: KNOB_ORDER was a frozen literal hand-copied from `struct U`, carrying a comment saying it existed "so a caller cannot pack them in the wrong sequence by guessing" -- and it was itself a guess. Idea from redcamel/wgsl_reflect (MIT, (c) 2021 Brendan Duncan; the account is a mirror and Duncan is the grantor, registered REACHED). NONE OF ITS CODE IS HERE: it is 5,141 lines of TypeScript implementing a real WGSL scanner, parser, AST and reflector, and vendoring a compiler front end to answer one layout question is out of all proportion. render/wgslLayout.mjs is 200 lines and is a DECLARATION SCANNER, not a parser -- everything a parser would handle (type aliases, const-expression array lengths, @align and @size attributes) returns null rather than a confident wrong number. *** AND THE TREE ALREADY HELD TWO ANSWERS FOR vec3, WHICH IS HOW THIS WAS FOUND. *** render/wgslSpec.mjs's sizeOf says 16; gfx/device.js's table says size 12, align 16. WGSL's rule is that vec3<f32> ALIGNS to 16 and OCCUPIES 12, which is why a scalar can sit at offset 12 and { vec3f, f32 } is sixteen bytes rather than thirty-two. wgslSpec was never wrong for its own caller -- it totals workgroup storage, where over-counting is the safe direction -- and it is wrong as a layout primitive, which nothing said until now. So alignOf and sizeOfType are separate functions here that never fall back on each other. A SECOND buried question: the address space changes the answer. This module first returned 24 for struct U and device.js returns 32; both right, because a uniform struct's alignment has a floor of 16 and device.js's `Math.max(16, ...)` was implementing that rule without naming it. With the space named they agree exactly, on the size and on every field offset. *** AND A REAL DEVICE WAS ASKED RATHER THAN A NOTE WRITTEN SAYING NONE WAS AVAILABLE. *** WGSL has no sizeof, so the layout was measured behaviourally on SwiftShader: markers written into two elements of an array<T> storage buffer came back at float indices 3 and 4, putting `b` at byte 12 and the stride at 16. The draft closing note had claimed no device had checked -- written before trying, the same mistake v4275 made about the network and v4269 about WebGPU itself. WIRED, not shelved: KNOB_ORDER is derived from the shader (resolved through @group(0) @binding(0), not by looking for a struct named U, so renaming it does not silently disable the check), and gfx/device.js now REFUSES a positive host/shader disagreement with a message saying the draw would otherwise succeed. Sabotage A collapses vec3's size onto its alignment -- the exact confusion already shipped in this tree, one ternary, not a typo -- 5 red including the device contradicting the code. B swaps two struct fields: 1 red, and NOT the failure it used to be, because KNOB_ORDER followed the struct and checkHostUniforms reported zero complaints. C un-wires KNOB_ORDER back to the literal: 1 red on the guarantee rather than its current output. D deletes the device.js refusal: 2 red, both about the wiring. Two more reds arrived unbidden: a claim that swapping uniforms changes no offset (false -- the map is keyed by name, and that change IS the bug; the BUFFER is what stays identical), and the probe's compute attribute pushing backendParity's WGSL census 39 to 40, a gate's fixture counted as a shipped shader. The tempting fix was to bump the baseline; the right one was to stop self-counting -- and the comment explaining that spelled the attribute out and turned a marker check red on the sentence describing the rule. Tenth instance. Full changelog on docs/CHANGELOG.md.
