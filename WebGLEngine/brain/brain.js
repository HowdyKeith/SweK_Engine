...
const BRAIN_BUILD = "v4455";   // v4455 -- *** THE SAME QUESTION WENT TO TWO BRANCHES AT ONCE AND THEY CONVICTED THE SAME MISSING TERM FROM DIFFERENT INSTRUMENTS, WHICH IS WORTH MORE THAN EITHER ROUND ALONE. *** v4436 measured the transmission lobe creating 28% more light than arrived and said it could not say why; v4447 proved the excess real and named two suspects in writing, the height-correlated G2 and the refraction Jacobian. Keith put "find where Walter's BTDF over-counts" to both lines, and they arrived at Walter's missing chi+ independently -- the other from an energy bound in half-vector space, this one from a Monte Carlo walk binned by exit direction. The chi+-only integral came out 0.661386 there and 0.663694 here, from code sharing nothing but the D and the Fresnel under test. THE VERDICT ON v4447'S TWO SUSPECTS IS SPLIT AND A THIRD DEFECT NEITHER ROUND HAD THOUGHT TO SUSPECT IS THE ONE THIS FILE CONVICTS. Eq. 21 describes one microfacet doing one refraction, so the half-vector h_t = -normalize(etaI*wi + etaO*wo) must be a facet that EXISTS (h.n > 0) and is LIT (h.wi > 0). transmission.mjs evaluated outside both, in two places that look like defensive hygiene: halfVectorT's `h[2] < 0 ? -h : h` does not sanitise a sign, IT INVENTS A FACET POINTING THE OTHER WAY, and Math.abs(iDotH) does not sanitise a sign, IT LIGHTS A FACET FROM BEHIND. *** THE BUG IS TWO ABS()ES, AND AN ABS IS THE MOST INNOCENT-LOOKING THING IN A SHADER. *** Every term is correct; the energy comes from the DOMAIN, which no line owns. PROVED ON A BOUND RATHER THAN A FIT: with masking alone, eq. 21 times |wo.n| is ALGEBRAICALLY (1-F) times the visible-normal distribution, a probability density that integrates to 1 by construction. Measured at alpha 1, cosI 0.25, eta 1.5 -- VNDF mass 1.000000 (the instrument, graded before it is believed), the (1-F)-weighted ceiling 0.916252, the o-space integral with the domain enforced 0.917790 (so the change of variables is EXACT, to the quadrature), and AS WRITTEN 1.878929, which is 2.05x a probability. AND THE WALK PUTS EXACT ZEROS EXACTLY WHERE THE ABS()ES PAY OUT: v4447's dielectric walk restricted to paths that bounce ONCE AND THEN ESCAPE, binned by exit cosine over 300,000 paths, gives 0 in bin 0 and 0 in bin 9 -- not few, ZERO, twice -- while multiple scattering fills both (85 and 45038), and Walter puts 4.03% and 5.73% of its own total there. The first failure has a closed form rather than a tendency: the raw half-vector points down exactly when |wo.n| < (nAbove/nBelow)|wi.n|, which at this configuration is 0.16667, which IS bin 0. *** WHAT THIS ROUND GOT WRONG AND CORRECTED BEFORE IT SHIPPED, WHICH IS THE PART MOST WORTH KEEPING. *** Its first draft headlined "G is not the culprit", on the ground that four G choices swing the total five-fold (1.244 / 0.932 / 1.879 / 4.697) while the impossible-domain share barely moves (46.7 / 48.0 / 51.2 / 51.2 per cent). THAT MEASUREMENT SUPPORTS ONLY THAT THE TWO DEFECTS ARE SEPARABLE, NOT THAT THE SECOND IS ABSENT -- and section 6 of the same file contradicted the headline three paragraphs later by measuring the enforced lobe still 2.17x the walk's truth. The file disagreed with itself and the headline was the half that was wrong. G2 IS GUILTY SEPARATELY: the shipped form is the SAME-SIDE height-correlated one applied to two directions on OPPOSITE sides, where the Smith uniform-height derivation gives the BETA FUNCTION B(1+Lambda_i, 1+Lambda_o), and that is the other branch's contribution rather than this one's. NEW section 8 holds it against this file's walk, which shares no line of code with it: chiPlus and g2 "beta" together give 0.305984 against a walk truth of 0.306083, and hold at alpha 1 cosI 0.7 (0.738746 vs 0.741067) and alpha 0.4 cosI 0.25 (0.748956 vs 0.749767). chi+ alone is still 2.16x, and Walter's own separable G1G1 would have overshot by 58%, so neither obvious repair was the answer. THE JACOBIAN IS INNOCENT AND IT WAS THE LOUDEST-LOOKING SUSPECT: across the exit cone it runs 78.1 down to 1.89, a 41x swing against D's flat 0.3183, and it is EXACTLY RIGHT -- it is what makes the o-space and h-space integrals agree to 1.5e-3. NEW physics/render/btdfDomain.mjs and its gate, eight sections; transmission.mjs is DELIBERATELY NOT PATCHED BY THIS ROUND, so btdfDomain sits beside eq. 21 rather than in place of it. dielectricWalk now returns the exit direction, which is the one line that made the binning possible -- the measurement that was missing was not a harder one, it was the one nobody kept. DEFAULTS ARE UNCHANGED ON BOTH BRANCHES and no rendered pixel moved. WHAT IS NOT CLAIMED: that this round fixes the lobe, since enforcing the domain alone takes alpha 1 from 4.07x truth to 2.17x and the rest was the masking model; or that it is visible in production, since at alpha 0.05 the impossible domains carry under 1.3% at every incidence past cosI 0.1, which is why smooth-glass renders never caught it. SIX SABOTAGES, MEASURED 8/4/9/9/6/1-THEN-2 BY NAME AND NONE ZERO-RED, which is worth recording because v4443 and v4445 each found a check that re-derived the thing under test and graded its own copy, and v4435, v4436 and v4447 each found a check nothing reached. *** THE ONE THAT READ 1 FOUND A SECOND COPY OF THE OPTION OBJECT INSIDE ITS OWN SECTION: *** the fixed lobe was named twice, so reverting one left the multi-configuration control grading a lobe the sabotage never touched -- not a wrong answer, a NARROW one. There is one frozen FIXED object now and every check reads it. *** AND THE INSTRUMENT FAILS AT THE SMOOTH END, WHICH IS SAID RATHER THAN AVOIDED BY NOT TESTING IT: *** at alpha 0.05 a uniform cosine grid cannot resolve a near-delta VNDF and the mass reads 0.50, then 0.87, then 0.96 as the grid refines. v4432's rule reads that correctly -- a number that MOVES when you refine is the GRID, not the model -- and it is why the conviction is stated at alpha 1 and nowhere else. The trap this round fell into first is in the file too: the opening probe integrated the REVERSE transport direction and got 0.553045 where the forward convention gives 1.244351, differing by exactly 2.2500, eta^2 -- the same factor transmission.mjs's own header is about, arriving the same way a second time, and 0.553 looked far healthier than 1.244 did. The tree stands at 1480 gates.
