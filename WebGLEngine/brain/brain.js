...
const BRAIN_BUILD = "v4272";   // v4272 -- *** THE SHIPPING three.js PASS RENDERS badTv UPSIDE DOWN RELATIVE TO THE DEVICE PATH, EXACTLY. *** v4271 proved WGSL-on-WebGPU and GLSL-on-WebGL2 render the same 4,096 pixels and closed by saying what that did NOT settle: whether the device path matches what main.js actually draws. It does not. render/badTvPass.js rendered through a real WebGLRenderer, from the file main.js imports, disagrees with the device pipeline on EVERY pixel -- 255 of 255. Row-mirror it and the two agree at 0 of 255 across all 4,096. *** AND NEITHER IS WRONG: *** badTvPass reads three's uv attribute, where v=0 is the quad's bottom, which is the standard EffectComposer convention; the device pipeline uses framebuffer space because that is the one convention where WebGPU and WebGL2 agree. Both are internally exact against badTvModel, in opposite orientations. *** THE HAZARD IS THE ROLL, NOT THE MIRROR: *** badTvSampleAt computes fract(v - time*rollSpeed), so swapping one pass for the other flips the picture AND reverses the roll, and a mirrored ROLLING image looks like a working effect with the tape running backwards. So the round refuses to pick a winner -- deciding needs a consumer to decide FOR -- and instead freezes the relation as exported data, THREE_PASS_RELATION, checked against the live render every run. Sabotage A un-flips the device pass: 6 red, and the mirror moves to the other side, there being no arrangement where all three agree at once. B edits the recorded relation to claim agreement: 1 red, only in the line comparing record to run, which is why it is data and not a paragraph. C flips the three.js pass instead: 6 red, mirroring A, because the gate privileges neither file. Full changelog on docs/CHANGELOG.md.
