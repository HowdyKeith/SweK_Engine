...
const BRAIN_BUILD = "v4452";   // v4452 -- *** THE MACHINES WERE NEVER THE PROBLEM, AND NEITHER WAS THE CODE. *** Keith: "I don't know why the swek engine locally runs with an ip, and then all the gpu pages have to be re opened with localhost. all the machines run webgpu pages fully." navigator.gpu is exposed only in a SECURE CONTEXT. A browser treats localhost and 127.0.0.1 as potentially trustworthy over plain http; a LAN address like http://192.168.50.57:8787 is not, so on the address every OTHER machine uses to reach the box the property is simply absent. ui/webgpuProbe.mjs has diagnosed exactly this since v3981, with a three-route message that names the tunnel as the fix for other machines. *** WHAT WAS MISSING IS THAT ALMOST NOTHING ASKED IT. *** MEASURED: of the 31 pages in this tree that acquire a WebGPU device, SIXTEEN consulted the probe and FIFTEEN did not; the fifteen said some version of "navigator.gpu is undefined (no WebGPU)", which reads as a verdict on the browser rather than on the address bar. Now 31 of 31, by three routes -- the page's own probe, the new ui/originNotice.js tag, or a device from gfx/device.js. *** AND THE REASON THE SYMPTOM WAS SO HARD TO READ IS A SILENT FALLBACK. *** gfx/device.js's requestDevice found webgpu unavailable, quietly took webgl2 -- or the null backend -- and RETURNED A WORKING DEVICE. No throw, no console line, nothing on screen: the page loaded, did less than it should, and the only symptom was that reopening it on localhost fixed it. A FALLBACK THAT CANNOT SAY IT FELL BACK IS INDISTINGUISHABLE FROM A BROKEN PAGE. It now asks the probe BEFORE the fallback runs -- at the moment the caller wanted WebGPU and is about to get something else -- once per page, lazily imported so Node importers never pull a browser module, and injectably so a gate can drive it with no document. Every caller of that module is covered without being edited, which is six pages. ui/originNotice.js is a SIDE EFFECT AND NOTHING ELSE, and that is why it is a separate file: the nine remaining pages guard the device in nine shapes -- seven `if (!navigator.gpu)`, one WebGPURenderer, one neither -- and rewriting nine control flows to add a message is nine chances to break a page that works today. A tag before </body> cannot change what a page does. Their own error text is left alone, because a page saying "no adapter" under a banner explaining the origin is telling the truth twice rather than once. THREE PAGES DID GET THE REAL FIX, because they shared one misleading sentence verbatim. *** SEPARATELY, THE RENDER QA QUESTION, AND IT HAD A SHARPER ANSWER THAN EXPECTED. *** Asked whether render QA can test the new TSL and WebGPU pages: the population is fine -- 426 pages discovered, 27 excluded each with a reason, and every GPU-shaped page among them. The mechanism was not. --have has existed since v3171 so a box STATES what it has and every page it cannot judge is NAMED with the capability it is missing, before the browser opens it, keeping "this box has no adapter" apart from "this shader is broken" -- v3120's law. renderQaBridge.run() destructured only/updateBaselines/base/video/all, so THE BUTTON COULD NOT SEND IT and the law held only from a terminal. That is v3563's own sentence one flag along -- "A FRONT DOOR THAT CANNOT REACH A FLAG THE TOOL DOCUMENTS IS HALF A DOOR" -- written in this same file while fixing --all and leaving --have beside it. Threaded now, read from ONE input by both run paths so a sweep and a single-page run cannot disagree about what the box is claimed to have, and blank still skips nothing because a green run over fewer pages than you think is the failure that looks most like success. The webgpu detector also learned the IMPORT tell it already used for webgl2: gfx-device.html and nebula-device.html hold their WGSL in a string and get the device from gfx/device.js, so they derived as needing nothing and would have been RUN AND FAILED on a box with no adapter -- the exact collapse the flag exists to prevent. 36 of 38 GPU-shaped pages classified now; the two that are not only MENTION WebGPU in prose, verified by running the classifier rather than by reading them. WHAT IS NOT BUILT AND IS SAID PLAINLY: the capability set is still TYPED rather than detected. render-qa's own gate has called that "STILL NOT DONE" since v3171 and it remains so, because detection means launching the QA browser and asking it, Playwright is not installed in the sandbox this was written in, and shipping a browser-launching probe nothing here can run is the untested code this tree refuses. Five sabotages, MEASURED 1/1/0-then-1/1/0-then-1 by name. *** BOTH ZEROS WERE DEFECTS IN THE NEW GATES RATHER THAN IN THE CODE, WHICH IS THE FIFTH AND SIXTH TIME THIS SESSION. *** Dropping the insecure-origin guard so the banner fires on any failure went ZERO RED, because the once-per-page flag had already fired in the case above and short-circuited the one that mattered -- a check passing because of a flag rather than because of the thing it names; gfx/device.js exports a reset for gates only and the sabotage is now red by name. And removing the detector's import tell went ZERO RED because NOTHING ASSERTED IT -- the widening that fixed two pages could have been undone in silence. AN EARLIER MEASUREMENT IN THIS ROUND WAS ALSO WRONG AND IS CORRECTED HERE: the first coverage numbers reported (377 pages, 28 GPU-shaped, 13 probing) were taken while the container had silently rolled back to v3904, so they described a tree eight rounds old. Re-measured on the real tree: 426, 38, 16. *** AND THE RATCHET WAS RAISED A SECOND TIME, WHICH IS THE THING TO WATCH RATHER THAN THE THING TO CELEBRATE. *** v4452 shipped owing two releases -- v4450 and v4451 -- because v4449's rule refuses a new version while the last one is unreleased, and neither had been published. Asked directly, Keith chose: "publish v4452 after merge, write off 4450 and 4451". The reasoning is the same one that applied to v4449 and it is sound -- v4452 contains everything both of them do, so one release hands the fleet all three rounds and a separate build for either would be an artifact nobody downloads. WHAT IS LOST IS NAMED: no box will ever run exactly v4450 or exactly v4451. *** THE WRITTEN-OFF RUN IS NOW THREE VERSIONS LONG AND THAT IS A STRUCTURAL SIGNAL, NOT A PREFERENCE. *** A ratchet whose floor rises every round has been switched off politely. The honest reading of three in a row is that THE SHIP AND THE PUBLISH HAPPEN ON DIFFERENT MACHINES: rounds are built in a sandbox with no credentials to publish, so the previous version is owed by construction every single time, and answering that with a write-off each round makes the gate ceremonial. releases.json now carries priorRaises so the sequence is visible in one place, and its raisedWhy tells the next round that reaches for that line to FIX THE STRUCTURE INSTEAD -- publish as part of the ship, or let the ratchet ask about what reached MAIN rather than what reached the changelog. *** A CORRECTION THAT COST KEITH A PRESS: I told him twice that v4450 "carries the Run 3 to 6 button" and it does not. *** Measured after he reported publishing and the API showed nothing: `Run 3` appears ZERO times in main's ui/githubPanel.js and once on the branch -- the one-press runner landed in v4451, which was never merged. Main offers the numbered route as separate presses, step 3 then step 4. Three independent reads confirmed the publish never reached GitHub at all: list_releases newest v4438, get_release_by_tag v4450 a 404, and list_tags newest v4438 -- NO TAG EITHER, so it did not get as far as the tag push the rig does in the same action. A wrong instruction about which build has which button is worse than no instruction, because it sends somebody looking for a control that is not there and leaves them doubting the machine. The tree stands at 1478 gates. Full changelog on docs/CHANGELOG.md.
