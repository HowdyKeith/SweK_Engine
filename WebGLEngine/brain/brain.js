...
const BRAIN_BUILD = "v4383";   // v4383 -- *** A SHIPPED KERNEL THAT HAD NEVER RUN, RUN -- AND ITS OWN CHECK PAGE WOULD HAVE PASSED A BROKEN RIEMANN SOLVER. *** simulation/euler/eulerShader.js opens by confessing it: "This has never run: there is no GPU where it was written. So it ships with an adjudicator, and it is deliberately scoped to something that can be got RIGHT blind rather than something impressive that would be a coin flip." It had no gate at all -- only euler-gpu-check.html, a page for a human with hardware -- and tools/ship/coverageTriage.mjs classified it HARDWARE: "Needs a real GPU, like the bench pages, so it cannot be gated in a sandbox." *** THAT SENTENCE WAS TWO CLAIMS WEARING ONE. *** It cannot be TIMED in a sandbox, which is still true and is why this round makes no timing claim anywhere. It cannot be CHECKED there was true of a sandbox with no device and stopped being true at v4359, when node-webgpu arrived. tools/ship/eulerGpu-selfcheck.mjs compiles the kernel, runs 4,490 dispatches of it and adjudicates it against its own CPU twin in twenty seconds. The triage entry moves to GATEABLE and is LEFT IN PLACE with what survived of its old verdict written into it, because a classification that was overturned is worth more in that list than one that was merely right. *** THE PAGE'S TWO KEYS, HEADLESS FOR THE FIRST TIME. *** (1) The device and the f64 CPU twin are the same solver -- both first-order Godunov, same HLLC flux, 400 cells, t = 3, a shared conservative dt from the CPU's own CFL so the two are asked the same question -- and they agree to 3.217e-7 RELATIVE over 562 steps, with 274 of 400 cells differing at all and a worst absolute of 1.6455e-6 on a peak of 5.1152. That is f32 epsilon accumulated against an f64 reference, which is the right order and is not zero; zero would have meant the comparison was reading one solver twice. THE PAGE ACCEPTS 2%, WHICH IS 62,172x LOOSER. (2) The device finds the strong-shock density ratio at 5.9679 against the exact (gamma+1)/(gamma-1) = 6.000 that the kernel is never told, and the CPU twin reads 5.9679 -- the same to four decimals -- with both missing by 0.534%. That gap is first-order truncation, which this module chose deliberately, and the two solvers agreeing on it is a stronger statement than either landing near 6 alone. *** AND THE LOOSE TOLERANCE IS NOT A CURIOSITY: IT IS LOOSE ENOUGH TO PASS A REAL BUG. *** Sabotage A breaks the HLLC wave-speed estimate so it stops taking the min and max over BOTH states. The device then disagrees with the CPU by 4.587e-3 -- 0.459%, comfortably inside the page's 2% -- and the shock ratio survives it too, so euler-gpu-check.html WOULD HAVE PASSED IT ON BOTH ROWS. A tolerance set by guessing, because the thing could not be run, accepts a broken Riemann solver; replacing the guess with the measurement is what lets a check see it, and that is the argument for the round rather than a nice extra. The page's OWN sabotage was also run for the first time -- the equation of state 2% wrong, on the argument that "gamma sets the compression a shock can reach, so this should move 6.000 somewhere else entirely" -- and it moves the peak to 7.8257, 30.43% off, caught by a factor of fifteen. The argument was right and now it has a number. Three sabotages, 1/2/1, and the contrast between the first two is the point: a boundary condition that is wrong is obvious (the outflow clamp dropped for a wrap gives 8.045e-1 and a shock ratio of 6.7687), and a wave speed that is wrong hides under a guessed tolerance. Also here: this is the arc's first 2D workgroup -- the kernel is @compute @workgroup_size(8, 8) -- and gfx/device.js already took a [x, y, z] dispatch, so nothing had to change for it. UNCHECKED, and the first one is why the kernel exists: SPEED. SwiftShader is a software rasteriser, so what is shown is that the kernel is CORRECT and not that it is fast, and no rig has signed a number. A real GPU's sqrt(), min() and max() against SwiftShader's, which is the one place an f32 kernel with a square root in its flux could part from this run. The MUSCL-Hancock path, which the CPU has and the kernel deliberately does not -- the module argued first order was the right thing to write blind, and whether second order is now worth writing is a question this round opens and does not answer. And simulation/lbm/lbmShader.js, which carries the SAME confession in the same words and still has no gate. The tree stands at 1423 gates.
