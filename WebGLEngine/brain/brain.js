...
const BRAIN_BUILD = "v4129";   // v4129 -- the nebula skybox's stars were flat-topped lattice cells (every direction in a cell got the same brightness), which is what made them read as square voxels; a radial falloff from the cell centre fixes it without touching the seamlessness the lattice exists for. The radius was MEASURED: my first pick, 0.42, shrank stars below a texel and thinned the field (2313 lit -> 659); 1.0 keeps the footprint and takes differing-adjacent-pairs from 11% to 99%. Writing the gate found a second bug -- passing starRadius:undefined restored the squares, since a spread copies an explicitly undefined key over the default. rig.html's human-only panel removed at Keith's ask, with every link verified reachable elsewhere first and RIG_ONLY itself kept in rigRunner.js. Gate: nebulaSkybox-selfcheck, 17 checks. Full changelog on docs/CHANGELOG.md.
