...
const BRAIN_BUILD = "v4366";   // v4366 -- *** WHAT A SILHOUETTE GATE CANNOT SEE, IN PIXELS -- AND BOTH OF THIS ROUND'S FIRST TWO RUNS WERE WRONG. *** The corrections first, because they are the round. (1) Every silhouette IoU in the first run read exactly 1.0000, including one measured against a shape with half of itself removed. The cause was this gate's own clear colour: (0.03, 0.03, 0.04) is one unit blue, and their foreground rule is saturation > 0.16 with luma < 0.94 where saturation is (max - min) / max -- which near black is 20% -- so every background pixel was foreground and the mask covered the frame. That is a faithful property of their rule, it is now a measured check rather than a footnote, and every render here clears to a neutral grey. (2) With that fixed, deleting the model's outermost part read IoU 0.2202 and a scale delta of 2.1194, and THAT was an artefact of this gate too: each variant was normalised to its OWN bounds, so removing the outermost part re-framed the whole model. Held to the base model's centre and radius, the same deletion reads IoU exactly 1. The first reading would have been quoted as the gate WORKING, which is the more dangerous direction to be wrong in. THE ROUND: render/divineEye.mjs is a port of img2threejs's deterministic review signals (https://github.com/img2threejs/img2threejs, Apache-2.0, forge/stage4_review/divine_eye.py and diagnose_render.py) -- their two HARD gates, silhouette IoU >= 0.85 on a 224 grid and an AREA scale delta <= 0.08, plus global SSIM and Sobel edge overlap on their 64 and 96 grids. Their constants, their arithmetic, their unusual choices kept, because a port that improves the thing it measures measures something else. *** AND THIS ROUND IS NOT DISCOVERING THEIR BLIND SPOT: THEY WROTE IT DOWN. *** divine_eye.py carries a comment headed "RESOLUTION CEILING, and it is a hard limit on what this module can ever report" -- a small feature "is not scored badly, it is ABSENT before any comparison happens" -- and their gates reference records the case that found it: a finished face and the same model with its face DELETED both scored 0.8803, identical to four decimals. What this tree adds is the number. MEASURED over 65,536 pixels, on the v4365 bridge, against a base render that is bit-identical to itself: recolour a component ENTIRELY and 2,203 pixels change at IoU exactly 1 and scale delta exactly 0; delete an interior badge, 649 pixels, exactly 1; delete a whole component that the outline does not depend on, 1,532 pixels, exactly 1; ADD a detached part, 713 pixels of new object in the frame, exactly 1 -- discarded before any comparison happens, because their mask keeps only the largest 4-connected blob and returns the 34.2% it dropped as a WARNING, which is not a failure. All four at once: 4,448 of 65,536 pixels, 6.8% of the frame, three separate defects, and both hard gates read their perfect scores. Eight of the seventeen changes swept score IoU EXACTLY 1 and scale delta EXACTLY 0 -- not within tolerance, identical -- and every one of them leaves the outline alone. *** AND THE OTHER HALF, WHICH IS WHY THIS IS A CALIBRATION AND NOT A COMPLAINT. *** Where the gate can see, it is SHARPER than its own arithmetic predicts: a flat-projection reading of an area delta puts the 0.08 ceiling at sqrt(1.08) = 1.0392 linear, and the sweep catches 1.03 while passing 1.02, because the projection is perspective and the bounding box is quantised to 224 cells. And the SMALLER change is the one caught: deleting the strip moves 713 pixels against the knob's 1,532 and is refused, because the strip is the model's extreme in x and its removal is a bounding-box event -- its IoU of 0.8524 clears the 0.85 floor on its own and the SCALE gate is what stops it. What these gates measure is not how much moved but WHERE. THE CONDITIONS ARE THE FRIENDLIEST THE GATE WILL EVER SEE and that is said in the gate rather than left to be noticed: these are two renders of one model from one camera, so the baseline IoU is exactly 1, while their gate scores a render against a PHOTOGRAPH where IoU never starts at 1. A blind spot measured here is a floor on the blind spot there. AND IT IS FOUR OF THEIR SIGNALS: pHash, bilateral symmetry, blowout parity, flat-region ratio, tonal parity, objectness, the CIEDE2000 hue work, the self-uncertainty routing and the VLM layer are unported, so no verdict here is their verdict -- it is what their HARD gates let through, which is the thing their own contract says nothing downstream may overturn. Three sabotages: the luma grid widened from 64 to the render's own size (1 red, and only 1 because the hard gates are mask statistics that never read it); the largest-blob filter dropped (4 red -- BUT 0 RED FIRST, the third sabotage this session to prove a check nothing was exercising, because every render compared had one connected blob until the detached-part variant was added for it); and the mask's saturation clause dropped (1 red). Also this round: render/img2three.mjs unitMesh takes a `from` model, so a variant is not re-framed as well as changed. UNBUILT AND NAMED: whether these numbers hold against a photograph rather than a second render, which needs a reference image and a licence to keep it; their microscope path, which is what they say answers feature-scale fidelity and which this round does not port or test; and every number the rig has not signed. The tree stands at 1416 gates. Full changelog on docs/CHANGELOG.md.
