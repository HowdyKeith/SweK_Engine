...
const BRAIN_BUILD = "v4268";   // v4268 -- *** SIX LICENCE VERDICTS LIVED OUTSIDE THE TREE, AND ONE OF THEM WAS WRONG. *** Open items #100 and #132 assert UNPAPERED for six repositories -- advanced-threejs-tsl-webgpu-rendering, gi-voxels, Repo-Explainer, threejs-procedural-terrain, ar-globe, gaze-aware-3d -- and NOT ONE of those names appeared anywhere in this repository: 4,572 files scanned, 0 hits. 1,342 gates could not see the claims and could not have caught them. claimCheck-selfcheck names exactly this as the staleness it cannot reach. *** AND THE REASON IS STRUCTURAL, NOT FORGETFULNESS: reachedLicences.mjs WOULD HAVE REJECTED THEM. *** Its three postures all presuppose somebody opened the source, and validateEntry requires licenceExists to be a boolean -- so the only way to file a repo nobody has looked at is to assert true or false about it, and false is an accusation this tree has made wrongly four times (fireworks.js, the fonts, quickjs, HeatDistortionEffect). world/namedNotChecked.mjs is the missing third state: UNCHECKED, no licence fields allowed, mayTake false for all six with a reason that says the gap is OURS and not theirs. *** AND #100 IS ANSWERED BY A SOURCE THE TREE ALREADY HAD: *** "the only TSL reference" is false -- render/solidTexture.mjs has opened with boytchev/tsl-textures (MIT, Pavel Boytchev 2024) since v4243, cited in one header and entered in NO register, which is the shape of #137 and #53 recurring a third time. Now registered, marked SECOND-HAND because this round had no network. #132 states four and lists five; neither reading reaches four and the module refuses to pick. The gate went red on its author twice: the sixth self-counting scan in six rounds, and a control threshold of 2 that was guessed and is 1. Sabotage A 3 red, B 2, C 2 -- measured, exit codes read; C exposed a hardcoded "4" in the gate's own message. Full changelog on docs/CHANGELOG.md.
