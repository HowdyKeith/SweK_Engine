...
const BRAIN_BUILD = "v4139";   // v4139 -- stabilityBind-selfcheck's header ended with five frozen decimals inside the sentence that refuses frozen readings, and two of them stopped reproducing on Keith's Node 24 rig, reddening claimTrace's ratchet 3 -> 5. They are not portable: the fixture is deliberately unstable, and an unstable SPH energy ratio is set by how the divergence develops. Bit-identical here across Node 20 and 22. The readings moved below the imports -- out of the region claimTrace reads as claims, into the one that holds context -- and the first draft of the note explaining that quoted the five it was removing, taking the header to eight claims. Re-frozen with the documented switch, diff checked: total 141 -> 135, untraceable 64 -> 61, nothing else moved. Full changelog on docs/CHANGELOG.md.
