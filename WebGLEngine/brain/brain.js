...
const BRAIN_BUILD = "v4135";   // v4135 -- reverts one line of v4134 that closed Keith's rig: swek_free_port.bat's netstat loops went back to findstr :%PORT% from the quoted /C: form, which is safe on a plain command line (its sibling uses it) and fragile inside for /f ('...'). The port ARGUMENT -- the actual fix -- stays. The lesson recorded rather than just the patch: this box cannot execute a .bat, so the gate READS the file and says so, which caught that the form changed but not that it stopped matching; bundling untestable polish into a fix turned one improvement into an outage. Also confirmed while here: server.js boots clean on an arbitrary PORT and answers /health, so nothing server-side was implicated; and the cloth-soak job runs in 3.9s exit 0 -- the "minute" Keith saw is a declared upper bound, not a runtime. Full changelog on docs/CHANGELOG.md.
