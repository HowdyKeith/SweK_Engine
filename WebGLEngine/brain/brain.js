...
const BRAIN_BUILD = "v4373";   // v4373 -- *** GENERATED GEOMETRY, GENERATED LODs, A GENERATED CULL -- ALL THREE IN ONE SCENE, AND THE JUDGE TURNS ITSELF OFF ON THE FAR RUNGS. *** Each link had been proved alone and none of them joined. v4365 brought a generated three.js object tree onto gfx/device.js; v4362 to v4364 wrote render/gpuDriven.mjs's cull as a TSL graph and held it to the shipped pass, but only ever BESIDE it -- the generated module had never decided what draws; and the LOD ladder was never generated at all, since every scene built on the bridge declared one rung and a hand-written stand-in triangle. WHY A GENERATED LADDER IS A DIFFERENT THING FROM A DECIMATED ONE, which is the whole point: a mesh file has one resolution and its coarse levels must be simplified OUT of it, an approximation of an approximation carrying the simplifier's own error on top. A model that exists as CODE has no such problem -- call the factory again with a smaller budget and the coarse rung is built by the same construction as the fine one. img2threejs's own mesh codec already carries the idea (a Quality type with named levels, a ?quality= override, buildModel beside buildModelProgressive) and nothing consumed it. render/img2three.mjs buildLadder is what consuming it looks like: one factory at three budgets, every rung flattened and framed by RUNG 0 rather than by itself, with three invariants returned as numbers rather than thrown -- triangles must fall strictly, no rung may reach outside the finest, and the material families must match. MEASURED: 24 / 10 / 5 segments give 1200 -> 220 -> 70 triangles (100%, 18.3%, 5.8%), every rung inside the finest by 0.0e+0, three meshes at every budget. *** AND render/gpuDriven.mjs GAINED A HOOK SO THE DECISION CAN COME FROM OUTSIDE IT. *** `cull: { wgsl, entryPoint, bind(pipe, buffers), write(u) }` lets a caller bring the compute pass that decides what draws, the way `pipeline`/`bind` already let one bring the render shaders: the scene creates and owns every buffer, hands them to bind() once, and calls write() each frame with the SAME forty floats packCullUniforms has always produced -- so a pass whose binding shape is different (a generated one, whose frustum is its own uniform rather than a member of struct Cull) is drivable without gpuDriven knowing anything about it. Refused by name: a cull without bind/write, a custom cull with occlusion (the two-phase path builds a second pass this hook does not describe), and a custom cull on WebGL2, where the route culls on the CPU and there is no pass to replace. THE RESULT: 96 instances on a three-rung generated ladder, the scene drawn once by cullLodWgsl and once by makeCullPassTsl transplanted into a device module -- 65,536 of 65,536 pixels IDENTICAL, worst channel difference 0. And the ladder was actually CLIMBED, which the picture alone could not say: both passes sort the instances 32 / 32 / 32 across the three rungs, so a matching picture is not one rung quietly taking everything. *** WHAT EACH RUNG COSTS IS NOW A MEASURED CURVE AND NOT A TYPED CONSTANT. *** At 5.8% of the triangles rung 2 moves 9,354 pixels of 65,536 up close and 531 far away -- 17.6x fewer -- and no rung is ever free at any distance measured. That is what an LOD threshold chooses between, priced. *** AND THE CALIBRATION ROUND'S JUDGE CAME APART HERE, IN THE OTHER DIRECTION. *** v4366 measured what img2threejs's hard gates cannot SEE. This round found where they report something that is not there. Below 0.035 foreground coverage their mask is REPLACED by "every pixel with alpha", which on an opaque render is the whole frame, so on the two far rows BOTH masks read coverage 1.0000 and the silhouette IoU reads EXACTLY 1 with a scale delta of EXACTLY 0 while 384 and 531 pixels differ -- the gate is not approving the coarse rung, it has stopped having a silhouette to compare. Worse is the TRANSITION: one row has the reference at 0.0400 coverage and the render at 1.0000, one side in the fallback and one not, and it scores IoU 0.0402 with a scale delta of 14.12 on a 2,175-pixel change. That is two mask REGIMES being compared rather than two pictures, and their own code says so -- as a WARNING, which their contract never lets fail a build. render/divineEye.mjs now surfaces that regime as a value (fellBack) rather than a string nobody reads. TWO SABOTAGES, and the second red of the first one is the finding: normalising each rung to its OWN bounds costs 2 red, the framing check AND the mask-regime measurement, because rungs scaled back up to the same box never fall under the coverage threshold -- a dishonest framing does not merely mis-score a model, it hides the conditions under which the judge stops working. The other, never writing the custom cull's per-frame uniforms, costs 2 red and 6,859 pixels. A THIRD ATTEMPT WAS MALFORMED AND IS LOGGED AS SUCH: it commented out the rest of the line it edited, so the gate CRASHED instead of failing a check, and a crash is not a red. STILL HAND-WRITTEN IN THIS SCENE and named rather than left to be noticed: the render shaders (the fleets' own lit pair), and the thresholds themselves, which are now PRICED but not yet chosen -- choosing them needs a policy for how many pixels a rung may cost, which nobody has stated. Also unbuilt: the ladder against a real img2threejs factory, whose Quality levels this stands in for and which is not vendorable because the showcase repo carries no licence; the occlusion variant, refused by the hook rather than supported; and every number the rig has not signed. The tree stands at 1418 gates. Full changelog on docs/CHANGELOG.md.
