...
const BRAIN_BUILD = "v4330";   // v4330 -- *** THE SPLIT, ITS PRICE WAS A THIRD OF WHAT I QUOTED, AND THE NUMBER MOVED UNDER IT. *** This shipped as v4329 and is v4330: the orrery branch merged into main while this round's sweep was running and took 4329 with it (that branch had already moved 4328 -> 4329 for the same reason, one round earlier). Nothing here is renumbered twice and nothing is reused -- main was fast-forward-refused, main merged in, this round supersedes forward, which is the rule working rather than an accident. AND THE MERGE FOUND A RED THAT WAS ALREADY ON main: that round's new orreryFleet-selfcheck re-takes its per-body commit counts from git and refuses a record that disagrees, and it disagreed at main's own head -- `three` recorded as 2 where `git log -- WebGLEngine/vendor/three` returns 1, red on origin/main in a clean worktree before this branch touched anything, and named by no register. Measured before it was blamed. The number is corrected to what git says, the record still names v4329 as the round that took it, and the finding it supports is untouched because htmx and box3d are still 2. Also re-baked: orrery-fleet.json records importer byte sizes, and this round changed two of them (server.html, main.js). The correction first: v4328's note priced this split at "four numbers and a list entry" in render/backendParity.mjs's census. It cost ONE LIST ENTRY AND NO NUMBERS -- GLSL 145, WGSL 56, BOTH 13, GLSL-only 132, WGSL-only 43, before and after -- because render/physicsTsl.mjs carried shader markers ONLY through the shells, so it left the census in the same move that brought the new file in. The estimate was made without checking which side the markers were on, and it is corrected in render/fleetTsl.mjs's own header rather than quietly dropped. The round: render/physicsTsl.mjs was named for physics at v4321 and had grown three fleet shells and five looks by v4328, flagged three rounds running. render/fleetTsl.mjs now holds the shells (a graph is transplanted INTO one), the looks built on them and the hand-written twins the graders use; physicsTsl keeps lyapunovNodes, heidlerNodes and the two KEYS whose pictures decode to ln 2 and the Heidler peak. A look that paints physics on a hull is a LOOK: it lives in the fleet module and imports its arithmetic from the physics one. AND THE SPLIT IS CHECKED RATHER THAN ANNOUNCED: tslPhysics-selfcheck asks the exact question that keeps them apart -- does the physics module contain shader text at all -- because a shell is a vertex stage written out in both languages, and a look drifting back would bring one with it. Sabotage P (a shell put back) is 7 red: 2 here by name and 5 in the backend-parity census as the markers arrive (glslBearing 145 -> 146, wgslBearing 56 -> 57, both 13 -> 14, the directive/framework split, and the shader-module list growing to eleven). Nothing was exempted and no exclusion list was written. THE OTHER HALF, ASKED FOR BY KEITH: server.html gains a RENDER TSL drawer -- "put all the related TSL links in there, including all the test links I should and want to click on". It holds the four pages (tsl-probe, tsl-rig, orrery-gpu, gpu-rig-check, MOVED by pageSections.mjs like every other drawer) and, written out by hand above them, the three QUERY VARIANTS the drawer mover cannot file because it files pages and not URLs: ?tsl=1 (four races painted by graphs), ?tsl=1&soft=1 (the Glyph race sampled instead of fetched) and tsl-probe ?webgl=1 (the same graph forced onto the other backend, to be clicked beside the first). tslRace-selfcheck section 3 checks those three links exist, because a mode of a page no link reaches is reachable only by somebody who already knows it -- the same failure pageReach has ratcheted against for dozens of rounds, wearing a query string. The panel names the two pages that OWE THIS TREE A NUMBER (tsl-rig.json, rig-timing.json) once each rather than twice: a page keeps one anchor with one hand-written title, so gpu-rig-check's own anchor gained the JSON it owes instead of a second copy appearing in the drawer. The nine-page render group that has waited in the UNPLACED register since v4315 is down to one, and universe-gpu.html stays there on purpose: it has no TSL in it, and following the other two because the three used to be a trio would be the subject filing that register exists to refuse. Sabotaged and measured: a shell put back into the physics module (7 red, above), the ?tsl=1&soft=1 link replaced with the plain page (1 red, by name), and the section's chip pointed at a button that does not exist (1 red, naming it). Not built and said: a MIPPED or anisotropic sample, which the device cannot serve at all; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; TSL COMPUTE, which is the largest unclaimed piece of the roadmap and the one place the tree's pair contract does not apply at all; and every number the rig has not signed. The tree stands at 1408 gates. Full changelog on docs/CHANGELOG.md.
