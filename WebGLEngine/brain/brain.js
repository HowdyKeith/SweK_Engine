...
const BRAIN_BUILD = "v4136";   // v4136 -- blackbodyBind-selfcheck has been hard-crashing since v4055 on an observable that round deliberately deleted as a tautology, and a SECOND reference to the same dead name sat inside a bit-identical check where undefined === undefined passed vacuously -- seven advertised, six actually asserted. Fixed, and names are now checked to exist before they are checked to match. Nothing caught it for eighty versions: its timing is on file at 560ms and the known-red list predates the break. Also: assumptionMap's timeout line named the device about to be built while labelling it "last", so "80/129 (last: twof)" meant twof was STARTING -- and twof is 178.1s of a 250.9s run, 71% of the gate. The progress line now distinguishes finished from starting, announces any device over 5s as it lands, and ends with where the time went. Full changelog on docs/CHANGELOG.md.
