...
const BRAIN_BUILD = "v4122";   // v4122 -- the SPH spatial grid, rewritten. The old "grids lose below 1000 particles" number was measuring a Map.get() and a closure per cell, not the grid: direct-indexed Int32Array head/next buckets over the cloud's own bounding box replaced the hash for typical extents (hash kept as a fallback, capped at 1<<22 cells), and GRID_CROSSOVER dropped from 1000 to 128 on re-measured numbers (200 particles 0.4x loss -> 2.6x win; 6000 particles 5.1x -> 19.5x). The harder half was the chaotic answer-key fixtures pinned to one neighbour-search order: a new REFERENCE_WALK constant pins materialKnobs.mjs and stability.mjs to brute force explicitly, but poolFixture.mjs's own settlePool() needed a PRECISE fix instead -- its "eightfold depth" fixture was already on the grid path under the OLD crossover, so blanket-pinning brute force there was itself a regression, caught via git stash against the untouched tree before it shipped. All six SPH gates verified on the final code, all pass. Full changelog on docs/CHANGELOG.md.
