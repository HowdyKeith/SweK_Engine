...
const BRAIN_BUILD = "v4300";   // v4300 -- *** LEVEL 14, THE RUNG ABOVE 13: *** the things Level 13's gates named as unchecked become checked, and the economy stops being a toy that can only get richer. (1) THE ECONOMY CLOSES ITS LOOPS. A market has a TREASURY and buys only what it can pay for ("cannot pay" is an event, not a silent sale on credit); RECIPES turn goods into other goods at every market that has the inputs (source + data -> binaries, source -> docs, binaries + docs -> data), so stock is made and used, not only moved; every ship pays UPKEEP per day; a ship that cannot pay and holds nothing is BANKRUPT and leaves the sky at radius 0. And the book still balances, twice: tons as before, and credits -- traders + treasuries - minted is the starting total, to the credit. *** THE FIRST RUN DEFLATED THE UNIVERSE: *** 200 days with upkeep as a pure sink and every one of 14 treasuries at a few credits, because haulers buy low and sell high, so money runs from markets to holds and out; the second run, with production minting the base value of what it made, still ended with 430,000 credits in 22 holds and the treasuries empty. So upkeep is now PAID TO THE MARKET the ship is at -- fuel and crew are bought somewhere -- and money only enters (minting) and moves. At the default upkeep nobody goes bankrupt and no treasury runs dry in a hundred days; at 900 credits a day 14 of 22 haulers go bankrupt and the ledger still closes. The core is generic now: makeEconomy(world) over any bodies with a position function; the orrery's world moves and is chased per tick, a still one is reached at a time fixed at departure. (2) THREE HUNDRED HAULERS IN THE ENDLESS SKY UNIVERSE, THEIR FLIGHT ON THE GPU. world/universeEconomy.mjs makes the 694 systems markets, stocked by what they hold (a star is source, a planet data and docs, a station binaries -- not accurate, and said so) and puts 300 haulers among them. The systems stand still, so a flight is a function of the clock alone: render/gpuHaul.mjs evaluates every ship's lerp from eight floats per ship that go up ONLY on the ticks a ship departs, docks or goes broke, and nothing is read back -- the CPU trades on the arrival time it computed. gpuUniverse-selfcheck section 4: a hundred days over 694 markets with every ton and credit accounted for, 14,250 trips docked at 173 distinct systems, 43,188 recipe runs, 146 markets broke of 694; the GPU's hauler positions are the sim's own to 2.2e-5 map units every thirty ticks; uploads happen exactly on the changed ticks (at 300 haulers that is every tick, and a crew of six shows 112 of 120). universe-gpu.html draws them as a third scene, names a hauler with its cargo and credits or a system with its prices, and shows the last trades. (3) THE TERRAIN GETS LIGHT. The normal at a texel is the central difference of its four neighbours in world units, the shade is ambient + (1 - ambient) * max(0, n . L), computed once per chunk at its centre texel and painted flat: red stays the centre height byte (exact, as Level 12 graded it) and green is the shade, so a pixel is still a readout the CPU model is held to, now of two numbers. gpuTerrain-selfcheck: all 64 chunk shades match the model within one level on both backends -- worst 0 of 255 -- across 46 distinct shades. (4) PICKING GETS A THIRD BYTE. Alpha carries 128 + the id's third byte, so ids to 8,388,607 decode and background stays alpha 0; graded on a synthetic pixel because nothing in the scenes reaches 65,536. (5) *** THE RIG GETS A QUESTION IT CAN ANSWER. *** Every WebGPU frame this sandbox ever graded went to an offscreen texture, because its headless shell loses the device on any canvas-targeted pass, and nothing has confirmed WebGPU presenting to a canvas on a browser with a compositor. gpu-rig-check.html requests the DEFAULT device, draws the gates' own scene (gridScene, three LODs, two-phase Hi-Z) INTO the canvas, reads it back through the canvas path, and compares per-colour pixel counts to tools/ship/rig-expected.json, which rigCanvas-selfcheck writes from the sandbox's offscreen render and then loads the page here to prove its comparison logic passes on WebGL2 before the page goes to a box nobody can watch from here. RIG-PENDING: open gpu-rig-check.html on Galaxina over localhost or https and read the verdict; PASS on webgpu is the fact this tree has never had. The parity baseline records render/gpuHaul.mjs (48 WGSL). Sabotaged five ways across four gates, every one red, in each gate's log. Still unchecked and said: a body that moved BEHIND something is still drawn by phase 0; whether the economy's parameters make it lively rather than merely balanced; and SPEED, everywhere, until the rig answers. The tree stands at 1380 gates.
