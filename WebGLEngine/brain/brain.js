...
const BRAIN_BUILD = "v4381";   // v4381 -- *** THE PARAMETER THAT MOVES img2threejs's VERDICT IS THE CARVE'S GRID -- AND THIS ROUND SHIPPED AS A QUARTER OF ITSELF BECAUSE MAIN GOT THERE FIRST. *** The bookkeeping first, because it is the honest half. This round was built as a four-section gate joining v4371's carve to v4366's port of img2threejs's hard gates, and main's v4372 landed mid-build having already done three of those sections, better. Named rather than quietly deleted: "their gates read perfect scores on a hull 90% too big, on a carving view" is v4371's reprojection result and v4372 dismissed it in the right words -- "1.000000 by construction and never at risk"; "the gate catches it from a view the carve did not use" is v4372's, measured properly there by MESHING the hull and rendering it through the lit pipeline, which found the same hull at the same 54.9% error REFUSED from one camera and PASSED from another; and "the judge and the sculptor share one blind spot" is v4372's reconciliation, "they are silhouette statistics being asked a volume question", which is more careful than the version this file first wrote. Three sections became one cheap citation check. *** WHAT SURVIVED IS THE PARAMETER v4372 DID NOT VARY. *** Same shape, same held-out camera, same method, only the carve's grid changing: a tube is PASSED at n=32, PASSED at n=48, REFUSED at n=64 and PASSED at n=96, and its volume error over those four -- 86.7%, 77.8%, 90.0%, 81.1% -- does not order the verdicts. *** AND THEIR SCALE GATE IS A FUNCTION OF THE GRID ALONE. *** At each resolution scaleDelta comes back IDENTICAL for three shapes whose errors span 0.9% to 90.0%: 0.000000 at n=32, 0.000000 at n=48, 0.083333 at n=64, 0.059524 at n=96. A hundredfold range of error, four numbers, and all four are properties of n. The mechanism is a bounding box: a visual hull's box is its object's box plus at most the quantisation of whatever grid the carve ran on, which rounds to nothing at 32 and 48 and to ONE VOXEL at 64 and 96 -- priced at 7 and 5 of their 224 cells, so 7/84 = 8.33% clears their 8% ceiling and 5/84 = 5.95% does not. *** THE PAIR THAT SAYS IT OUTRIGHT: at n=64 their gate REFUSES a hull 3.3% too big, and at n=96 it PASSES one 81.1% too big. *** Same gate, same camera, same method, two grids, and a factor of 24 in error pointing the other way. Beside v4372 that makes two free parameters -- where the camera stands and how fine the grid is -- and the reconstruction error moves the verdict least of the three. The uncomfortable practical consequence is stated in the gate rather than left to be inferred: a team that made their carve FINER to improve it would also make it stop being refused, and neither change is about the model. NOT A CRITICISM OF img2threejs, said before the numbers rather than after: their gate scores a render against a PHOTOGRAPH where IoU never starts at 1; here it scores a hull against its own input shadows, the friendliest case it will ever see, and their divine_eye.py documents its own resolution ceiling in a comment. Every verdict comes from render/divineEye.mjs compare(), their constants, on shapes painted on a NEUTRAL grey ground -- v4366's own correction, because their foreground rule reads near-black as foreground -- so their buildForegroundMask, their largest-4-connected-blob filter and their 224-grid resample all run for real. Three sabotages, measured 3/3/1, and the 1 is the round's own thesis arriving as a result: dropping a view from carve() makes the hulls worse and does NOT disturb the resolution dependence, because the dependence is a property of the grid rather than of the hull's accuracy. A sabotage that fails to move a claim is usually a finding about the claim; here it is the claim. AND A PROTOCOL FAILURE RECORDED IN THE LOG RATHER THAN TIDIED AWAY: the first sabotage was reverted with git checkout on a file that was still UNTRACKED, so the revert failed silently and that sabotage was still in place while the next two ran -- both reported 4 red, all of them the first one's. Re-run from a verified copy for the numbers above. A restore that is not checked is not a restore. AND A SECOND ONE IN THE SAME ROUND, THE SAME SPECIES: the merge that brought main's v4372 in left a conflict in brain/brain.js, and `git add -A` marked it resolved -- so `git diff --diff-filter=U` reported zero conflicts over a file that still carried its markers. It reached verify, where tools/check.mjs caught it as a SyntaxError at line 2881 and refused the ship. The staging command is what hid it and the gate is what did not; a conflict list taken after `git add -A` is not a conflict list. UNCHECKED: a REAL PHOTOGRAPH, which neither the carve nor the judge has met here; their five unported soft signals and the VLM layer, so no verdict here is their verdict -- it is what their HARD gates let through, which their own contract says no soft signal may overturn; SSIM and edge overlap, which flat two-tone silhouettes do not exercise and which v4372's meshed hulls are the instrument for; and WHERE between n=64 and n=96 the verdict flips, which is the same gap v4372 left between its two cameras. The tree stands at 1420 gates. *** AND IT SHIPPED AS v4374 AND IS v4381. *** Main took v4373 AND v4374 while this branch verified -- the third and fourth collision in five rounds on this pair -- so this renumbers FORWARD with a HEADROOM JUMP the way v3900 did rather than to the next free seat, because taking v4375 would only have set up a fifth.
