...
const BRAIN_BUILD = "v4283";   // v4283 -- *** THE PRE-FILTER BUILT TO PREVENT SILENT GREEN LIGHTS HAD TWO OF ITS OWN. *** tools/ship/affected.mjs matches ENGINE-relative paths; git diff --name-only prints REPO-relative ones (WebGLEngine/main.js) from EVERY directory, and HANDOFF.md documents feeding changed files to --affected. The two never composed, nothing checked the strings named files at all, and a path matching nothing selects no gates -- a run of no gates EXITS ZERO. Measured on this round's own previous commit: three real changed files select 0 gates unnormalised and 1 normalised, same files, only the spelling differs. A typo bought the same clean green in 0.0 seconds. *** THE SECOND IS WORSE BECAUSE THE PATHS WERE PERFECT: *** walk() returns ROOT-relative paths and the selection filter relativised them AGAIN, resolving against process.cwd(). From the engine root it was right; from the REPOSITORY root -- where git is run and where the ship ritual stands -- every path became "../tools/ship/..." and matched nothing. Zero gates, exit zero, from the directory you are most likely standing in: measured at 1 gate from the engine root and 0 from the repo root and 0 from the filesystem root. *** BOTH PRINTED THE EVIDENCE AND NEITHER WAS READ: *** "0 of 1355 gates reach 3 changed file(s) (0.1%)" is a count of zero beside a fraction that is not, because the ANALYSIS found the gate and the RUNNER dropped it. tools/ship/changedPaths.mjs classifies every path, normalises what it can OUT LOUD, and refuses what it cannot. *** IT DOES NOT MAKE "NOTHING REACHES THIS" AN ERROR *** -- that is the graveyard census's whole subject, and shaders/voxel.vert.glsl is checked to still report zero rather than being refused; the bug was that zero was reported for inputs nobody had established were FILES. A comma-joined list is refused rather than split, because a comma is legal in a filename and a selector that guesses under-selects. --budget took the same raw argv, so its "reachable gates dropped for time" warning could never fire. Also: selfchecks.mjs now refuses when selector and runner disagree, and that refusal's soundness was CHECKED (both walks hold the same 1356, differing only by the runner finding itself); and affected.mjs still contained the third walker its own v3041 note is about -- orphaned, called by nothing, with a skip pattern unanchored to a path segment that matched vendoredLicences-selfcheck.mjs as readily as vendor/. Deleted. *** AND v4282'S CLOSING OBSERVATION IS WITHDRAWN: *** the pre-filter did not cost more than the ship gate -- the SELECTION takes four seconds, and the twenty minutes was 97 gates that genuinely reach the path tracer, run without --budget. Six sabotages, none 0 RED; the cwd bug restored now produces a REFUSAL rather than a green run, and dropping "missing" from the refusal set reddens the check that the refusal NAMES the path, because a refusal for the wrong reason sends someone looking in the wrong place. *** ONE SABOTAGE DID NOT GO RED, IT HUNG: *** splitting the comma list let the runner fall through to a real run of 96 gates and the check sat past two minutes reporting nothing. --select-only was added for it and exits 3 rather than 0, because a flag that runs no checks and returns success is a fourth silent green. Full changelog on docs/CHANGELOG.md.
