...
const BRAIN_BUILD = "v4454";   // v4454 -- *** v4436 MEASURED THE TRANSMISSION LOBE CREATING 28% MORE LIGHT THAN ARRIVED AND SAID IT COULD NOT SAY WHY; v4447 PROVED THE EXCESS REAL AND RULED OUT THE INNOCENT EXPLANATION; THIS ROUND SAYS WHERE, AND IT IS NOT A TERM. *** v4447 closed by naming two suspects -- the height-correlated G2 and the refraction Jacobian -- and said the walk was now sharp enough to tell them apart. It is, and the answer is NEITHER. Walter eq. 21 describes one microfacet doing one refraction, so the half-vector h_t = -normalize(etaI*wi + etaO*wo) must be a facet that EXISTS (h_t.n > 0) and is LIT (h_t.wi > 0). transmission.mjs evaluates outside both, in two places that look like defensive hygiene: halfVectorT ends with `h[2] < 0 ? -h : h`, which does not sanitise a sign but INVENTS A FACET POINTING THE OTHER WAY, and Math.abs(iDotH) reaches Fresnel and the Jacobian, which does not sanitise a sign but LIGHTS A FACET FROM BEHIND. *** THE BUG IS TWO ABS()ES, AND AN ABS IS THE MOST INNOCENT-LOOKING THING IN A SHADER. *** Every term is correct; the energy comes from the DOMAIN, which no line owns. THE PROOF IS A BOUND THE FORMULA BREAKS RATHER THAN A FIT: with masking alone (G = G1(wi)), eq. 21 times |wo.n| is ALGEBRAICALLY (1-F) times the visible-normal distribution, and the VNDF is a probability density that integrates to 1 by construction. Measured at alpha 1, cosI 0.25, eta 1.5 -- VNDF mass 1.000000 (the instrument, graded before it is believed), the (1-F)-weighted ceiling 0.916252, the o-space integral with the domain enforced 0.917790 (so the change of variables is EXACT, to the quadrature), and AS WRITTEN 1.878929, which is 2.05x a probability. AND THE WALK PUTS EXACT ZEROS EXACTLY WHERE THE ABS()ES PAY OUT: v4447's dielectric walk restricted to paths that bounce ONCE AND THEN ESCAPE, binned by exit cosine over 300,000 paths, gives 0 in bin 0 and 0 in bin 9 -- not few, ZERO, twice -- while multiple scattering fills both (85 and 45038), and Walter puts 4.03% and 5.73% of its own total there. The first failure has a closed form rather than a tendency: the raw half-vector points down exactly when |wo.n| < (nAbove/nBelow)|wi.n|, which at this configuration is 0.16667, which IS bin 0. *** WHAT LAST ROUND SUSPECTED IS RULED OUT BY THE SAME MEASUREMENT. *** Swapping G2 for separable G1G1, for masking alone, and for no G moves the total 1.244 / 0.932 / 1.879 / 4.697 -- five-fold -- while the impossible-domain share barely moves: 46.7 / 48.0 / 51.2 / 51.2 per cent. A term that swings the answer five-fold without touching the defect is not the defect. The Jacobian is innocent too, and it was the loudest-looking suspect: across the exit cone it runs 78.1 down to 1.89, a 41x swing against D's flat 0.3183, and it is EXACTLY RIGHT -- it is what makes the o-space and h-space integrals agree to 1.5e-3. NEW physics/render/btdfDomain.mjs and its gate, seven sections; transmission.mjs is DELIBERATELY NOT PATCHED, so btdfDomain sits beside eq. 21 rather than in place of it and the two can be measured against each other. dielectricWalk now returns the exit direction, which is the one line that made the binning possible. *** WHAT IS NOT CLAIMED: that this fixes the lobe. *** Enforcing the domain takes alpha 1 from 4.07x the walk's single-scatter truth to 2.17x -- roughly half the excess -- and the rest is the masking model, which is a different round; the gate tests that gap out loud rather than quietly not testing it. Nor that it is visible in production: at alpha 0.05 the impossible domains carry under 1.3% at every incidence past cosI 0.1, which is why smooth-glass renders never caught it. FIVE SABOTAGES, MEASURED 8/4/9/9/6 BY NAME, AND FOR THE FIRST TIME IN SEVEN ROUNDS NONE WENT ZERO-RED -- worth recording, because v4443 and v4445 each found a check that re-derived the thing under test and graded its own copy, and v4435, v4436 and v4447 each found a check nothing reached. Every section here is pinned to a number arrived at by a DIFFERENT ROUTE: section 1 to a half-vector-space integral that never applies the Jacobian, section 4 to the shipped btdf itself, section 5 to a Monte Carlo walk sharing no line of code with the formula. Sabotage D is the sharpest -- it IS the defect this file convicts, re-applied as though it were the repair -- and it lands 9 red because section 2 grades against an ABSOLUTE ceiling rather than against the value it is supposed to differ from. *** AND THE INSTRUMENT FAILS AT THE SMOOTH END, WHICH IS SAID HERE RATHER THAN AVOIDED BY NOT TESTING IT: *** at alpha 0.05 a uniform cosine grid cannot resolve a near-delta VNDF and the mass reads 0.50, then 0.87, then 0.96 as the grid refines. v4432's rule reads that correctly -- a number that MOVES when you refine is the GRID, not the model -- and it is why the conviction is stated at alpha 1 and nowhere else. THE TRAP THIS ROUND FELL INTO FIRST IS IN THE FILE TOO: the opening probe integrated the REVERSE transport direction and got 0.553045 where the forward convention gives 1.244351, and those differ by exactly 2.2500, eta^2 -- the same factor transmission.mjs's own header is about, arriving the same way a second time. A non-reciprocal lobe punishes a swapped convention with a clean, plausible, wrong number, and 0.553 looked far healthier than 1.244 did. The tree stands at 1479 gates.
