...
const BRAIN_BUILD = "v4364";   // v4364 -- *** THE FLEETS VARIANT, THE FRUSTUM IN A UNIFORM -- AND THE SAME LESSON TWICE IN ONE ROUND. *** The correction first, because the round found it in its own first run: the generated pass agreed with the shipped one on all six per-region COUNTS and differed on 330 of its 552 RECORDS, because the graph wrote a constant 0 into the record's fleet field where render/gpuDriven.mjs writes the clamped fleet. Then, sabotaging the clamp, it happened again in the other direction: with the fleet index taken as given, the 110 instances asking for fleet 7 against a fleet count of 2 aim at region 21 and up, off the end of a six-region buffer, and the device folds those atomicAdds into the last region -- 129 becomes 193 and the six counts STILL sum to 552. Counts are a weaker claim than records, and this round was saved by the stronger one twice. THE ROUND: render/gpuDriven.mjs cullLodWgsl({ fleets: true }) is the configuration the orrery actually runs -- a per-instance fleet index in its own array<u32>, clamped to the fleet count the uniforms carry, and a region that is fleet * lodCount + lod rather than lod alone, which is the only place the region index is two-dimensional. render/physicsTsl.mjs makeCullPassTsl generates it, and measured on WebGPU against the SHIPPED text over two fleets and three LODs: the same instance count in every one of the six regions (38 / 103 / 81 / 58 / 143 / 129) and all 552 survivor records identical to the float, with the 110 clamped instances landing in fleet 1 in both passes. *** AND BOTH PASSES ARE DRIVEN BY ONE PACKING, WHICH REMOVES THE LAST PLACE THE TWO COULD HAVE BEEN FED DIFFERENT NUMBERS. *** render/tslSource.mjs computeShell gained `uniformArrays`: a uniform whose ELEMENT is a fixed-size array, which is the type struct Cull gives its six planes and what a storage buffer stood in for since v4362. So packCullUniforms is called ONCE and sliced -- floats 0 to 24 to the planes uniform, 24 to 40 to the scalar struct, the whole 40 to the shipped pass -- rather than a second copy being packed for the graph to read. WHAT THAT DOES NOT REACH, said rather than glossed: three emits a TSL uniformArray as its own uniform BINDING and not as a member of the scalar struct, so struct Cull is still TWO bindings for a generated pass. The bytes are the same forty floats; the bindings are two. And the graph is unmoved by where the frustum lives: the two transplants' bodies are the same text, because planes.element(i) is one node either way, which is what a shell owning the declarations is supposed to buy. *** A CORRECTION TO WHAT THIS TREE HAS SAID TWICE ABOUT WHAT IS LEFT. *** Both v4362 and v4363 named hizOccluded's `ptr<storage, array<f32>, read>` argument as the blocker for the occlusion variant. That is wrong: the pointer exists because the HAND-WRITTEN side factored the test into a function, and a graph inlines it and reads the buffer directly. What { occlusion: true } actually needs is a mat4x4 uniform, two while-loops whose bounds are computed, and a nested tile loop -- none of which any graph here has emitted. It is the one variant left, and it is now described by what it costs instead of by a difficulty it does not have. Three sabotages, measured: the fleet clamp dropped (2 red, the folded increments above); the uniform array's element-and-length agreement not checked (1 red, a shell declaring five for a graph reading six stops being refused); and the uniform array left out of the shell's prefix (1 red, and gfx/device.js is what says so by name -- "the shader declares no storage or uniform binding named planes" -- rather than a WGSL diagnostic). Section 10 deliberately KEEPS its frustum in a storage buffer and was not tidied: the buffer-crossing hazard it measures is only visible where the shell's declaration order differs from three's emission order, and moving the planes into a uniform makes the two agree and the hazard invisible. Also unbuilt and unchanged: whether the record SLOTS agree, which they cannot -- an atomicAdd hands them out in arrival order, so the claim is the sorted set and the per-region counts; a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph; and every number the rig has not signed. The tree stands at 1414 gates. Full changelog on docs/CHANGELOG.md.
