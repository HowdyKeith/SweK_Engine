...
const BRAIN_BUILD = "v4261";   // v4261 -- *** THE TREE HAS EIGHT IMAGE PASSES AND HAD NEVER ASKED ONE OF THEM WHAT IT DESTROYS, AND ASKING BROKE v4260. *** Every gate on those passes checks the maths against the shader it was ported from -- fidelity, which says nothing about what happens to the picture. v4260 built the instrument by accident: its frames carry their own index, so a known frame can go through a pass and the OUTPUT be asked which frame it is. render/effectLegibility.mjs sweeps each pass over its own defaults and reports the strength at which the frame stops being identifiable. FOUR CLIFFS, ALL DIFFERENT, none of them previously known: liquefy fails at k=0.25 (below its shipped strength), aquarelle at exactly k=1 (AT shipped), crt at 0.75, badTv at 3. *** AND THE CENSUS FOUND ITS OWN INSTRUMENT LYING TWICE BEFORE IT FOUND ANYTHING ABOUT AN EFFECT. *** (1) 1,248 encoded frames through four passes returned a CONFIDENT WRONG FRAME NUMBER 182 times, 14.58%: frame 1 read as 129 and frame 2 as 130, bit 7 flipped, because bit 7 and its parity block were NEIGHBOURS against the right edge and a warp pulling that edge flipped both while parity stayed consistent. A check bit next to the bit it checks is defeated by any spatially local corruption, which is what every image effect produces. Two mirrored bands, reversed and inverted: 182 -> 3. (2) badTv scored 312/312 SURVIVED at every strength, which read as harmless and was blindness: badTv shifts each ROW by its own amount and the decoder read two pixel rows -- at 3x the row it read was torn 2.17px while row 24 of the SAME BAND was torn 30.28px, nearly two 16px blocks. Three spread rows required to agree: 3 -> 0, and badTv falls honestly at 276/312. Silent errors 182 -> 3 -> 0, and the same fix took v4260s noise sweep from 13-of-256 silently wrong to 0. Re-measured against real VP9: 0 unreadable across three clips, so the stricter decoder costs nothing. A CLAIM WITHDRAWN: I documented liquefy as non-monotone in strength; the recoveries were the silent misreads, and all four curves are monotone once the encoding is right. Five sabotages, 4/3/1/4/3 red -- and my first attempt at sabotage A was worthless (it broke the file rather than restoring the old behaviour) and is recorded as such. Full changelog on docs/CHANGELOG.md.
