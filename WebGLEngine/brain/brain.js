...
const BRAIN_BUILD = "v4372";   // v4372 -- *** THE CARVE AS A COMPUTE PASS, A CLAIM THE LEAPFROG'S SHAPE COULD NOT MAKE, AND A DEFECT IN THE TRANSPLANT ITSELF THAT HAD STOOD SINCE v4336. *** v4371's gate closed by naming its own next step -- "the carve is n^3 per view in JavaScript, which is a compute pass wanting to happen and is not one yet". render/carveTsl.mjs is that pass: one invocation per voxel, the views looped inside the kernel so the grid is read and written once instead of once per view, transplanted into a gfx/device.js compute module by render/tslSource.mjs. *** AND IT IS A HARDER SHAPE THAN v4370's. *** That round held a TSL graph to a shipped WGSL kernel BIT FOR BIT because the kernel is smooth arithmetic: an ulp of disagreement stays an ulp and the claim survives it. This pass ENDS IN A floor(), which is discontinuous -- the CPU projects in f64, the device in f32, and a voxel landing within an ulp of a pixel boundary floors into a DIFFERENT PIXEL on the two machines, at which point it is not off by an ulp, it is solid on one and carved on the other. No tolerance expresses that; a voxel is or is not. So the twin is mesh/carve.mjs's own f32 mirror -- projectF32, one implementation with a rounder, which is tools/roundhouse/hmcGpu.mjs's leapfrogF64Flat/leapfrogF32 idiom -- and the device agrees with it on all 1,572,864 voxels across six fixtures, 0 differing. It agrees with the f64 carve on the same 1,572,864 too, and THAT is a measurement rather than an assumption: the discontinuity is real, 66 pixel flips in 17.3 million voxel/view pairs, and NOT ONE of them changes a verdict. A flip only bites where the two candidate pixels disagree in the mask, and being within an ulp of a PIXEL edge has nothing to do with being near a SILHOUETTE edge -- two unrelated boundaries have to coincide, and in 17.3 million trials they never did. *** THE DEFECT: render/tslSource.mjs READ `==` AS AN ASSIGNMENT FOR THIRTY-SIX ROUNDS. *** Its write-detector asked whether a body contained `<buffer>.value[ ... ] =`, and that pattern matches the first `=` of `==`. Every earlier pass in this arc bound a storage read to a var before testing it; this one compares inline -- `if ( masks.value[p] == 0u )` -- and was REFUSED BY NAME, "the pass writes masks and the shell declares it read". Nothing ever shipped wrong from it, because it refuses loudly rather than mis-declaring a binding, but it is exactly the species versionPreflight's header warns about: a guard that fires on legitimate work is a guard people learn to route around. `=(?!=)` is the whole fix, and the optional [-+*/] catches a compound assignment the old pattern also missed; `>=`, `<=` and `!=` were never at risk because their operator sits between the `]` and the `=` where the old pattern allowed only whitespace. Section 1 pins both directions -- a comparison is not a write, and a real write is still caught -- and needs no device to do it, so it stands where WebGPU does not. AND THE BOUND SURVIVES THE CROSSING: the device's hull contains the object on all six fixtures, checked on the hardware and not only in the mirror. v4371's out-of-frame finding reproduces there too, since the policy is a uniform: with outside:"clear" the device returns a hull 7.3% SMALLER than the object and stops containing it, voxel for voxel with what the CPU says, while the same fixture under the KEEP default contains it. SPECIFIED OPERATIONS ONLY IS A CHOICE HERE AND NOT AN ACCIDENT: the four trig values are computed on the CPU and arrive in a buffer, so the kernel is + - * / and floor, and three's cos() against SwiftShader's cos() never enters the claim -- an agreement this tree has never measured and now does not need to. Four sabotages, measured 2/3/4/4, and the interesting pair is not the loudest: the spin's dz term dropped moves 133,352 voxels and leaves containment intact, while the view stride dropped from the mask index moves only 17,128 -- ONE EIGHTH -- and breaks the bound on three of six fixtures, because reading the wrong mask carves material no view ever called empty. That is why containment is asserted separately from agreement rather than assumed to follow from it. NOT MEASURED, and it is the reason the pass exists: whether any of this is FASTER. The sandbox device is SwiftShader, a software rasteriser, so a timing taken here would clock a CPU pretending to be a GPU and would say nothing about the hardware this pass is for. The n^3-per-view cost stays an argument until the rig signs a number. Also unchecked: a real GPU's f32 against SwiftShader's, since the 66 flips are this device's and another vendor would move them though it cannot move the bound; a grid too big for one buffer, n=64 being 1 MB of u32 where n=256 would be 64 MB; and the segmentation problem, inherited unchanged from v4371 -- every mask here is still computed from a solid the tree defines. The tree stands at 1416 gates.
