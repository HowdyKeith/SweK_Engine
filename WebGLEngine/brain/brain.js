...
const BRAIN_BUILD = "v4388";   // v4388 -- *** VALKYRIEN SKIES 2's SHIPYARD, PORTED AS ARITHMETIC -- AND THE PRECISION ARGUMENT IT IS ALWAYS SOLD WITH IS FALSE IN FLOAT64. *** Keith raised ValkyrienSkies/Valkyrien-Skies-2 (LGPL-3.0, Java and Kotlin, Minecraft 1.20.1, Fabric and Forge). Its central trick is worth more than the mod: a moving structure's blocks are NOT stored where the structure appears. They stay axis-aligned on the integer grid in a ChunkClaim of 512 chunks, millions of blocks from spawn, and every interaction is transformed BACK into that space and answered there, so every system that assumes a grid keeps working unmodified. This tree already had both halves and never the indirection -- voxel/voxelDDA.js has been Amanatides and Woo on an axis-aligned grid since v2782, and box3d carries rigid bodies -- so new voxel/shipyard.mjs is ONLY the indirection: a claim allocator, a pose that inverts by conjugate rather than by solving, and a raycast that transforms the ray and hands it to THE ONE MARCHER THIS TREE HAS, passed in and refused if absent, so no second copy can drift from it. FOUR PREDICTIONS WERE WRITTEN DOWN BEFORE ANY MEASUREMENT AND THREE OF THEM WERE WRONG. (1) The round trip holding under 1e-9 held, at 1.5e-11 across 400 random poses over a full claim span. (2) *** THE ONE THE ROUND WAS BUILT ON DID NOT. *** I predicted that transforming the DATA each frame would accumulate error until, somewhere before ten thousand motions, a ray hit the wrong voxel. Measured: after 10,000 cumulative rigid motions the baked geometry is 1.9e-12 OF A VOXEL from truth -- twelve orders below mattering, growing as roughly the square root of the motion count from 2.8e-14 and never arriving. IN FLOAT64 THE SHIPYARD BUYS NO PRECISION AT ALL, and the gate now asserts that FAILURE as a measured number so a later round cannot quietly reinstate the folklore. (3) and (4) The claim arithmetic was out by exactly one binade twice -- 2047 claims at whole-voxel spacing and 127 at a sixteenth, against 1024 and 64 predicted -- and the prediction's WORDING was wrong as well: it said float32 "cannot represent" a voxel boundary at 2^23, when integers are exact in float32 all the way to 2^24 and what dies at 2^23 is every position BETWEEN two boundaries, which is where a click on a face lands. *** AND THEN THE ARGUMENT TURNED UP SOMEWHERE ELSE, WHICH IS THIS ENGINE'S SITUATION AND NOT MINECRAFT'S. *** Stored as float32 -- what a vertex buffer holds -- and moved out along +X, the world-baked body is off by 0.036 of a voxel at ten thousand, 0.28 at a hundred thousand and 2.0 WHOLE VOXELS at a million, after the same 2,000 motions that cost float64 nothing. It is not the motions, IT IS THE ADDRESS: float32 spacing grows with the coordinate and a stored world position pays it on every voxel. Claim-local, the same body is off by EXACTLY ZERO at 0, 1e3, 1e4, 1e5, 1e6, 4e6 and 8.4e6, because every integer and half-integer in 0..8192 is exactly representable in float32 (all 8,193 checked, not spot-checked) and nothing ever rewrites them. THE SHIPYARD CAN SIT MILLIONS OF BLOCKS OUT PRECISELY BECAUSE THE CLAIM ADDRESS IS NEVER A VERTEX -- the mesh goes to the GPU local, with the pose as a uniform. Minecraft's float64 server would never have shown this, which is why the round is a port of the idea rather than of the reasoning. WIRED, NOT HOSTED: ray-march-demo.html has cast one ray per pixel through voxelDDA since v2782, and the block that used to float in its world grid IS THE SHIP NOW -- taken out of voxelType() entirely, given a claim, and answered by the same marcher through the transform, with a spin slider, a distance slider that walks the body out to 8.4 million, and a readout of the two float32 errors from the module rather than from a caption. *** AND THE FIRST RUN OF THAT SECTION FOUND A REAL DEFECT IN THE PAGE WITH EVERYTHING CORRECT: ZERO SHIP PIXELS. *** The body sat at y = 30 where the old block had been, 36 degrees off the camera's forward axis against a vertical half-FOV of 30 -- correct and OFF-SCREEN. Found by rendering the frame in node and counting, moved to y = 22, and the live DOM now reports 529 ship pixels, the same number node predicts. A SOURCE CHECK WOULD HAVE CALLED THE OFF-SCREEN VERSION A WORKING PAGE, which is v4386's finding arriving one round later on a different page. NEW GATE tools/ship/shipyard-selfcheck.mjs, six sections, driven RED by three sabotages (2/3/1 by name, md5-identical after): the ray direction passed through worldToLocal so it picks up the body's translation, which is the commonest way to write a transformed raycast wrong and reads correct; worldToLocal rotating by the quaternion instead of its conjugate, which is silent because the two directions are the same arithmetic run backwards; and the ship put BACK into the world grid as well as its claim, caught only by the check that the body VANISHES when the indirection is switched off -- 794 pixels on and 590 OFF. *** AND v4386's RATCHET TOOK EFFECT ON THE VERY NEXT ROUND TO ADD A MODULE. *** The sweep closing names voxel/shipyard.mjs, which is exactly the sentence that held img2three and carve off the orphan census -- and it cannot hide this one, because the page imports it, so the ritual check stayed at 2 rather than rising to 3. UNCHECKED AND SAID PLAINLY: whether the shipyard is worth building into this engine's world, which is one page and one module with nothing streaming, colliding or persisting through it; a body's PHYSICS, since box3d is untouched and the pose is handed in rather than solved; whether float32 error at distance is better fixed by a camera-relative render than by this indirection, which is the alternative nobody measured here; and VS2's Krunch solver, whose own licence could not be confirmed from this session and whose constraint solver this round does not port. The tree stands at 1426 gates. Full changelog on docs/CHANGELOG.md.
