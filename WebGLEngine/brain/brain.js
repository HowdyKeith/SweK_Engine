...
const BRAIN_BUILD = "v4130";   // v4130 -- merged claude/tier-2-keys-patch-4lwhzk (58 commits, v4027..v4061): the knob census, the cost record, and 484 build + 116 sweep costs the tree was ALREADY referencing through a costRecord.mjs path whose file did not exist. The zip of patches was not the way in -- only 12 of 58 applied against a base 102 commits behind -- so the branch was merged from origin with a real three-way merge, 46 hunks resolved per file rather than by side. Two bugs the merge itself made were caught before commit: sameValue/choicesFor defined twice (SyntaxError) and a dropped costVsCalls import that is still called (ReferenceError at run time, invisible to node --check). tidalBind-selfcheck's disputed runtime was re-measured rather than trusted: 840 ms, ours, with the branch's ~6s kept as the wider machine-local bound. v4027..v4061 now name two different bodies of work; the collision is recorded, not renumbered. Full changelog on docs/CHANGELOG.md.
