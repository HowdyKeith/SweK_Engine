...
const BRAIN_BUILD = "v4374";   // v4374 -- *** THE LOD THRESHOLDS DERIVED FROM A MEASUREMENT -- AND THE POLICY THIS ROUND STATED FIRST WAS MEASURED WRONG. *** v4373 priced the rungs of a generated ladder and said plainly that the thresholds were priced but not yet chosen, because choosing needs a policy nobody had stated. This round states one, and the correction is the finding. THE DRAFT POLICY: a rung may be used wherever the pixels it changes are at most a FRACTION of the pixels the subject covers -- of the subject and not of the frame, on the reasoning, written into the module before it was checked, that a frame budget "is satisfied trivially by anything far enough away". MEASURED over a 27.5x range of angular size, from 0.125 down to 0.0045, that fraction does not move: rung 1 costs 57% to 75% of its own covered pixels and rung 2 costs 80% to 90%, spreads of 1.33x and 1.36x against 27.5x of distance. IT IS SCALE-INVARIANT, and the reason is not subtle -- a coarser rung shades differently across the WHOLE subject rather than only at its outline, so the changed count and the covered count scale together. A scale-invariant cost expresses no preference about distance, so no threshold can be derived from it at any budget, and render/lodBudget.mjs says so by failing closed and naming the rung rather than returning a number. AND THE SENTENCE WRITTEN AGAINST THE FRAME POLICY TURNS OUT TO BE THE ARGUMENT FOR IT: a distant model may use a coarse rung precisely BECAUSE its error is trivially small in absolute terms. That is what "you cannot see it from there" means, and it is the whole content of an LOD. So the policy is ABSOLUTE -- at most 64 pixels of the frame per instance -- and it is one number in one place rather than the five typed thresholds this tree currently carries across three pages. THE DERIVATION: render/lodBudget.mjs takes a record of [{ metric, changed, covered }] per rung and interpolates the crossing between the two measured samples that bracket the budget, returning the bracket with it so a threshold can be read back to the measurement it came from. From the priced ladder it derives 0.0515 and 0.0416, against the 0.055 and 0.03 v4373 typed -- 0.94x and 1.39x, so the first typed number was nearly right and the second was holding the finer rung 39% too long. IT FAILS CLOSED on the pattern tools/roundhouse/costRecord.mjs sweepBudgetFor set at v4361, and in four distinct ways, each named: a rung the record never priced; a rung never cheap enough at any metric measured; a ladder where any one rung fails, because a partly-derived ladder is a ladder with a typed number hiding in it; and thresholds that do not FALL, since gpuDriven tests them in order and a rising pair would skip a rung. A record whose cost does not rise with metric is flagged rather than interpolated through, and a caller who must have a number brings its own and gets it back marked `typed`, so a hand-chosen threshold stays visible as one. *** AND THE DERIVED PAIR IS DRIVEN, NOT JUST DERIVED. *** The derivation is node-side and the renderer is in the page, so the record crosses out and the thresholds cross back in through a second harness call parameterised by the first one's answer: 96 instances at 0.0515 and 0.0416 cost 1,406 pixels against rung 0 everywhere, inside the 6,144 the stated policy allows, with 22.9% of it spent -- and the typed pair, measured in the same units, spent 1,168 of the same 6,144. A bound assembled from single instances at single angular sizes, then measured whole, which is the only way to find out it holds. Two sabotages: the fail-closed dropped so a partly-derived ladder returns numbers anyway (2 red -- the refusal AND the typed-floor path, because a caller's floor is only reachable when the derivation admits it could not answer), and the policy's MODE ignored so 64 is compared as a fraction (1 red: read that way it is a budget of 6400%, everything is under it, and the module would have returned the largest metric measured as a bound -- a number, confidently, meaning nothing). UNCHECKED AND SAID PLAINLY: whether 64 pixels is the RIGHT budget, which is a judgement about eyes and displays this tree cannot make and has therefore put in one place instead of five; whether the derived pair holds at a camera or a subject it was not priced at, since the record is one model at one screen size; the shipped pages, which still carry typed thresholds and are not rewired by this round; the ladder against a real img2threejs factory, not vendorable because the showcase repo carries no licence; the occlusion variant, refused by the hook rather than supported; and every number the rig has not signed. The tree stands at 1418 gates. Full changelog on docs/CHANGELOG.md.
