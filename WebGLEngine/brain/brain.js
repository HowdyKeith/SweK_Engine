...
const BRAIN_BUILD = "v4277";   // v4277 -- *** TWENTY-SIX PERMISSIVE LICENCES IN A ROW, AND THEN ONE THAT IS NOT. *** world/licenceSweep.mjs grows to THIRTY-FIVE repositories actually cloned and actually opened, and the twenty-seventh broke the model the first twenty-six had quietly built. but0n/rvo2.js carries the University of North Carolina at Chapel Hill's academic licence and LOOKS LIKE A BSD NOTICE FROM TEN FEET AWAY -- copyright line, "Permission to use, copy, modify, and distribute this software and its documentation", an ALL-CAPS warranty disclaimer running two thirds of the file. Every surface feature matches. The grant reads "for educational, research, and non-profit purposes", and the next paragraph says commercial use "may be obtained by contacting" UNC. That is a licence you have to ASK FOR, and a run of twenty-six permissive files is exactly what makes a shape-matching reader feel reliable right up to the moment it is wrong. So `permissive` is a field on every entry now, and it is NULL rather than true wherever nothing was granted: silence must not read as a grant, which is why both declared-only entries have null and not the permissive spdx their package.json names. rvo2.js also has THREE PARTIES -- UNC holds the copyright 2008-10, package.json names Paul Wells as the porter, the account is a third -- and nobody downstream can grant more than the first party did. That is backlog #82's ENCUMBERED shape with the whole chain legible inside one repository. TWO MORE SHAPES: but0n/glTF-WebGL-PBR's licence file is 58 lines carrying THREE licences -- its own MIT, then a Third-Party Code section quoting glMatrix's MIT and dat.gui's Apache-2.0 in full -- so its own spdx UNDERSTATES what a consumer inherits; and but0n/vixel is the Unlicense, a public-domain dedication that grants more than MIT and imposes no attribution condition at all, the one case where this tree's paperwork discipline genuinely relaxes. but0n/THREE.js-PathTracing-Renderer is genuinely UNPAPERED and is the largest repository in the sweep to be so: 194 tracked files, a 190-line README, and not one licence word in any .md, .txt or .json. Size is not paperwork. Tally: 28 papered (24 MIT, 1 Apache-2.0, 1 Unlicense, 1 UNC academic, 1 unresolved), 7 not, of which 2 are declared-only. Three sabotages on the new section: rvo2.js filed as BSD-3-Clause and permissive 3 red; a declared-only entry granted permissiveness 1; the composite's third-party section deleted 1 -- the quietest of the three and the closest to what a tidying pass would actually do. *** AND THREE CHECKS THAT HAD BEEN GREEN AND CORRECT FOR TWO ROUNDS WENT RED ON CORRECT DATA, ALL FOR THE SAME REASON: A COUNT TYPED INTO A GATE IS A CLAIM WITH AN EXPIRY DATE. *** All three are derived now. A fourth was not stale but simply false -- the composite check claimed the quoted notices widen the sweep's spdx reach, and running it showed both sets the same size, because Apache-2.0 was already reached directly. The true statement is narrower and is what the gate says. Full changelog on docs/CHANGELOG.md.
