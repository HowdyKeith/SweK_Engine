...
const BRAIN_BUILD = "v4301";   // v4301 -- *** LEVEL 15, THE RUNG ABOVE 14: FLEETS IN THEIR OWN ARCHITECTURES. *** "Having completely different fleet architectures means we can make different alien race ship types." So a GPU-driven scene now takes FLEETS: every instance carries a fleet number, the cull writes one region per (fleet, LOD) instead of one per LOD, and each fleet brings its own meshes, vertex layout, shader pair and bind hook -- the draw loop is unchanged (use the fleet's pipeline, one indirect draw per region), the CPU twin culls per region, and the identity picture carries the fleet (blue packs lod + 8 x fleet, five bits, thirty-two fleets). A scene with no fleets is a scene with one, and every region index it computed before is the same number. render/fleets.mjs is nine races, each a re-skinner the tree already had, imported rather than retyped: Union (the Level 11 quad, the control), Wedge (a lit 3D hull, six flat faces; the user's own .glb or .obj replaces it), Pixel (a 16x16 sprite drawn in source, transparent texels discarded), Loft (that same bitmap lofted solid through fx/spritemesh/spriteMesh.js), Voxel (the flight radar's jet through gpu/voxelCreature.js, ui/planeMeshLayer.js's own generator, per-vertex colours), Holo (an SVG silhouette raised watertight by mesh/extrudePolygon.mjs, drawn with the swiftShaderPass HOLOGRAPHIC rainbow over a lambert hull and a scanline), KRBN (the hull is RAISED FIRST, hatching is drawn over it in Krbn's screen space, and tools/krbn/strokeLift.js lifts every stroke point onto the surface by ray-cast -- "we are able to krbn skin a model after we raise its points" -- drawn on a line-list, the topology gfx/device.js learned on WebGL2 this round), Glyph (tools/export/reskin.js's ASCII glyph quads pinned to the hull by area-weighted sampling, the ramp shared with tools/render-qa/asciify.mjs), and Cells (the hull seen through 8x8 screen cells, a glyph per lit shade -- an impression of ASCII, labelled as one). Every look is WGSL and GLSL over the same buffers; both backends draw all nine from the same records. THE USER'S OWN MODELS: userModelSources() reads the EV ship assignments (swek.esShipModels.v1) and the radar's plane models (voxelengine.planeModels), says which are loadable and why the rest are not (a sprite loft, a dead blob URL, an asset-library id), and loadUserModel() turns a .glb (physics/mesh/glb.mjs, pure) or .obj (parseObj, with `l` records for strokes) into a hull; orrery-gpu.html and universe-gpu.html build the Wedge, Krbn, Glyph and Cells races on it when one exists. Every owner in the git graph is a race (fleetForName hashes the owner), so the economy's traders fly as their races and a pick names the race and its architecture. *** THE GATE FOUND THE PICK LYING. *** gpuDriven's default pick shader drew every hull UNSPUN under looks that spin theirs, so the identity picture named pixels beside the colour (a lit hull's mean colour over "its" pixels read 71/76/85 against 140/149/167); each look now brings a pick shader with its own vertex motion, the identity encoding lifted from PICK_WGSL by pattern, and the sprite picks discard where the sprite does -- 0 dark pixels among the named on eight of nine races (Cells names the hull between its glyphs, by design and said). fleets-selfcheck: eighteen region counts the twin's on both backends, every named pixel a record of its own fleet, centre picks 100% on the six hulls with area, the nine mean colours pairwise apart by at least 50/765, both backends agreeing on every count and every footprint; the Krbn skin's 67 landed points all on a face of the raised hull and round-tripping through strokeLift's OBJ; a user's .glb and .obj through a mocked fetch. Sabotaged three ways, every one red (4, 7 and 1 lines), in the gate's log. Still unchecked and said: a heading in the record (the spin is the golden angle times the id, a stand-in); how the nine look on a screen; and the rig, still pending. The tree stands at 1381 gates.
