...
const BRAIN_BUILD = "v4127";   // v4127 -- the topic row re-filed at Keith's direction: four chips unpinned into the alphabetised run, Render QA + Rig Jobs into System Tools, Homebrew formula into Mac System, and two renames into the "PL: " family. All through mechanisms that already existed (CHIP_PINNED, CHIP_GROUPS) -- my first attempt hand-rolled a second copy of both and Keith's screenshot caught it, because the row is sorted at RUNTIME and the DOM order I was reading is not what anybody sees. The renames then broke chipOrder-selfcheck, which turned out to be grading three of five PL drawers against a hardcoded list and passing anyway because the two it never knew about happened to sort adjacent. The set is derived from the row now. Full changelog on docs/CHANGELOG.md.
