...
const BRAIN_BUILD = "v4337";   // v4337 -- *** AN ATOMIC PASS, AND A REGEX THAT LOST ITS BACKSLASHES TWICE. *** The correction first, and it is a tooling one this round found in itself: the browser script every section of tools/ship/tslPhysics-selfcheck.mjs runs is a TEMPLATE LITERAL, so a pattern written as /atomicAdd\(/ inside it arrives at RegExp as atomicAdd(s*...) -- the backslashes are eaten by the template parser and again by the string parser. Two attempts at building the non-atomic variant replaced NOTHING, left an atomicAdd on a plain u32, and the only reason it was caught is that the device refused the module by name ("no matching call to atomicAdd(ptr<storage, u32, read_write>, u32)") instead of running it wrong. It is done by index and plain string now, with no pattern to escape, and the reason is written where the next person will hit it. The round: render/gpuDriven.mjs's cull pass counts survivors into an indirect draw's instanceCount, which every invocation may increment at once, and nothing transplanted in this tree had ever done that. render/tslSource.mjs computeShell's storage entries now take `atomic: true`, declaring their elements as atomic<T>, and the two sides must AGREE -- three writes atomicAdd(&buf.value[i], ...) and WGSL takes that pointer only into an atomic<T> -- so a shell that forgot is refused by name rather than by the device's compiler, and a shell that declares one where the pass has none is refused too. The counter is also a buffer the pass WRITES while nothing assigns to it, so v4336's role detector learned to look for the atomic call as well. render/physicsTsl.mjs makeChaosTallyTsl counts the chaotic elements of the sweep into one number. Measured on WebGPU at 1024 elements over SIXTEEN workgroups: the tally is 670 and 670 of the 1024 elements of the buffer it read are positive -- exactly, five runs out of five, no device errors. *** AND THE ATOMIC IS MEASURED RATHER THAN ASSUMED. *** The same transplanted module with the atomic taken out by hand -- a plain read-modify-write on a plain u32 -- compiles, runs, and counts 156 to 171 against that truth of 670: 74% to 77% of the increments lost to contention, and a DIFFERENT wrong number every run. That is why the section runs at 1024 and not at 64: with one workgroup there is nothing to lose and a plain add would have passed, which would have been a check that proved nothing -- the same trap v4336's sabotage U fell into one round ago. Sabotaged and measured: the atomic-declaration guard removed (1 red -- both refusals stop, and what was caught by name is left to the WGSL parser at pipeline creation), and the tally graph built without .toAtomic() so it counts with a plain add (1 red, refused by name before the device sees it). Not built and said: an INDIRECT dispatch, where the count a pass runs at is itself in a buffer, which is the last thing standing between this and regenerating a real gpuDriven pass; workgroup-SHARED memory, which three can emit and no shell here declares; a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample, which the device cannot serve at all; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; and every number the rig has not signed. The tree stands at 1411 gates. Full changelog on docs/CHANGELOG.md.
