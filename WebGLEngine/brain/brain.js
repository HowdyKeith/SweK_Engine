...
const BRAIN_BUILD = "v4275";   // v4275 -- *** THE SPECULAR SIDE HAS GGX, SMITH, FURNACE TESTS AND ENERGY COMPENSATION; THE DIFFUSE SIDE WAS albedo/PI AND ZERO FILES MENTIONED OREN-NAYAR. *** Nine files here name GGX and thirty-one touch path tracing, so a rough surface and a polished one scattered identically in the one lobe where roughness is the whole point. physics/render/roughDiffuse.mjs closes that: Oren-Nayar's published 1994 form, reducing to Lambert EXACTLY (A === 1, B === 0, factor === 1, no epsilon) and reciprocal EXACTLY, plus an energy compensation MEASURED by this tree's own integration rather than quoted. The motivation as a number: plain Oren-Nayar loses 1.4% of the light at sigma 0.2, 15.6% at 0.6 and 25.6% at 1.0, worst case 34.5%. The table puts it back to 0.23%. *** AND WHAT THE ROUND REFUSES TO CLAIM: THIS IS NOT EON. *** The six suggested repositories -- EON-diffuse, OpenPBR, OpenPBR-viewer, snelly, fibre, Trinity -- were NOT opened; this session has no network, no licence was read and no analytic fit was consulted. All six are registered in world/namedNotChecked.mjs as NAMED and unchecked, which is what v4268 built that register for, and Trinity is flagged LIKELY DUPLICATE before anyone spends a round on it: four fluid solvers already ship here. The register gate was hardcoded to six entries and is now per-source, because a register meant to GROW should not be gated on its size. Sabotage A adds 1e-9 to a coefficient -- invisible to any image, 3 red, which is the argument for asserting === 1 over a tolerance. B drops the max/min from alpha/beta: reciprocity breaks and the surface GAINS energy, worst reading -120%, while still rendering a plausible rough surface. C 2 red. Full changelog on docs/CHANGELOG.md.
