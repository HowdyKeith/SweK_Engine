...
const BRAIN_BUILD = "v4054";   // v4054 -- the arrival: a warp cross of the system inside the fold tunnel, chained to v4053's planetary descent. rig/cinematicShot.js gains sequences, and chainLegs() DERIVES each later leg's start from the previous leg's end so a seam cannot cut -- measured 7.57e-7 units across the join with leg 1 deliberately mis-written, against a 1.01e+4-unit teleport when chainLegs is stubbed out. The warp leg really spans 20000 -> 140, which is the four-decade case v4053's header used to argue for logarithmic distance: half-way through, the camera is ~1732 units out where linear would still be ~10075. The tunnel runs over leg 0 only and its leg is timed to render/foldTunnel.js's own jumpDuration(); verified live, 0 of 47 descent frames still had it up. Full changelog on main.js's ENGINE_VERSION line.
