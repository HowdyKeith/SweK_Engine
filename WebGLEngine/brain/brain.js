...
const BRAIN_BUILD = "v4315";   // v4315 -- *** v4315 -- THE SWEEP COMES HOME, AND OUR OWN PHYSICS GOES TO LEVEL 11. *** (1) THE MERGE. origin/claude/stale-numbers-sweep (the stale-number sweep across the devices, swk_lyapunov, krbn-lyapunov.html, seven more SwiftUI ports: liquidChrome, pixelateStorm, magneticField, aurora, datamosh, smokeReveal, morphBreathe) is merged into this line. It had numbered its commits v4194 and v4298 through v4313 while never bumping the tree's own markers (its main.js still said v4296 and it wrote no changelog entries), so nothing in the tree collided: those numbers are superseded by this line's, which is the authority, and the merge's one conflict was the generated knowledge index, rebuilt. Its gates run green here: swiftShaders (the ln 2 key read off the GLSL, the 35 ports), redCensus. (2) swk_lyapunov AT LEVEL 11. render/lyapunovWgsl.mjs is the logistic map's Lyapunov exponent as a WGSL twin beside the GLSL, in three shapes: a compute probe in the corpus's shape (tools/ship/wgslCorpus.mjs runs it native and in the browser -- 2,048 of 2,048 identical), a full-screen KEY pipeline on gfx/device.js that writes the exponent in 16 bits across two channels on either backend, and a fleet LOOK. physicsShaders-selfcheck reads ln 2 off the compute probe (median over 128 seeds 0.693231, |err| 8.3e-5) and off the key pipeline on BOTH backends (16,384 pixels each, median 0.693226, |err| 7.9e-5), from a shader handed r as a coordinate; the period-3 window is dark on both and the two backends' column medians agree across the sweep. (3) THE LIGHTNING, AS A SHADER WITH A KEY. "We have real lightning code": physics/discharge/heidler.mjs, the Heidler return-stroke current, a closed form of t. render/heidlerWgsl.mjs is its compute probe (in the corpus, 2,048 of 2,048 identical native and browser); the GPU reads its peak over i0 as exactly 1 at the shape's true eta and 1.0667 at the published one -- the reference formula's own 6.7%, the module's v3193 finding, reproduced on the device -- and the whole waveform matches the f32 twin element for element to 1e-5. (4) THE CHAOS RACE. render/fleets.mjs gains a tenth race: the SVG hull with r across it and the seed up it, the exponent as the shade -- a ship that carries its bifurcation diagram and turns with it; fleets-selfcheck draws it with the other nine on both backends. (5) THE LIBRARY, SIFTED. docs/PHYSICS-SHADER-CANDIDATES.md ranks sixteen more models in the tree by how cleanly they would take the same treatment -- blackbody, diffraction, Fresnel, lensing, a current loop, random-matrix spacings, the infinite well, symplectic defect, Kepler, entropy as fragments; Ising, Brusselator, Kuramoto, percolation, Yee, Langevin as compute passes -- each with the exact key a gate would pin, and names what is NOT there (no dielectric-breakdown model; the Heidler current is the lightning physics the library has). docs/LEVEL17-PREVIEW.md gains the constructions only this infrastructure allows. Parity baseline: 141 GLSL, 51 WGSL, both 9, five dual-language shader modules. Sabotaged three ways, every one red (6, 3 and 1 lines), in the gate's log. Still unchecked and said: the Chaos race's exponent read off a hull (the fleets gate grades its identity and distinctness, not its arithmetic); the look's cheaper knobs against ln 2; a real GPU's log(); and the four Level 16 remainders (a WebRTC wire between two machines, two different browsers' hashes, a better learner, the rig), which are the next round's. The tree stands at 1386 gates.
