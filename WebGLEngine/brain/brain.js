...
const BRAIN_BUILD = "v4361";   // v4361 -- *** 4.79 HOURS OF MEASUREMENT, READ BY NOTHING, AND NINE BUDGETS TYPED BESIDE IT. *** #40 reads "corroborationCensus: twof is 73% of the whole sweep", and the first thing to check was its own number. Measured against the frozen record: twof is 58.8% of the census BUILD sweep (458.9 s of 780.4 s over 484 device/modes) and 23.5% of the LADDER sweep (4053 s of 17,251 s over 116 devices). Neither is 73%. One item, one number, two different sweeps -- and twof IS the dearest device in the lab on both, derived here by folding the 484 mode costs up rather than asserting the name, so the day something overtakes it the line says so. *** BUT THE CONCENTRATION IS NOT THE DEFECT. *** v4049-v4051 paid 4.79 hours to measure what every device's sweep costs, retired all ten "we stopped at 300 s" bounds, and froze 116 numbers with sweepAtLeast empty. And then: sweepCostFor() -- exported, documented at length, "when a proxy spans two orders of magnitude, the measurement is the model" -- had ZERO call sites in the whole tree. writeSweepCosts() likewise, so there was no path in the tree to refresh what was bought. And where the BUILD half got scaledCostFor at v4173 because "a frozen cost and a live deadline are different clocks", the sweep half never got the conversion. Orphaned at both ends and running on the wrong clock. *** MEANWHILE THE GATE THAT RUNS THE SWEEPS CARRIED NINE HAND-TYPED ROUND NUMBERS. *** Measured against the record they are inconsistent by three orders of magnitude, and the interesting one is not the waste: compose had a 200 s budget against 0.3 s measured (730x), galaxy 120 s against 0.4 s (323x), blackhole 200 s against 29.5 s (6.8x) -- and quantum 120 s against 100.5 s, which is NINETEEN PERCENT HEADROOM on the slowest of them. One hand set both. *** AND WHAT A DERIVED BUDGET BUYS IS NOT SPEED. *** An over-provisioned budget costs no wall time when the sweep finishes early -- it caps, it does not spend. What it costs is the ability to NOTICE: a 730x budget cannot tell that a device became a hundred times slower, so the cut that would have said so never happens. sweepBudgetFor() reads the measurement, converts it with the tree's own hostScale() rather than a second estimate of host speed, and applies a headroom that is the MEASURED staleness of the record (712.7/458.9 = 1.553, twof re-timed at v4173) times a stated 1.3 -- so every device now trips at 2.02x slower and one factor governs the lab instead of five. It FAILS CLOSED: a device the record has never priced returns no budget and names itself, because a silent guess is exactly what the typed budgets already were; a caller that must have a number writes its own floor down through sweepBudgetOr. A group is budgeted by its DEAREST member and not the sum, because knobLiveness's stopwatch is per device and a sum would hand the dearest one the whole allowance. *** SEVEN WERE DERIVED AND THE SEVENTH WAS WRONG, WHICH THE RUN FOUND AND NOT THE READING. *** Section 3f's device list is computed from a lab-wide scan; derived, its budget came to 8187 s, because the set can contain twof at 4053 s and knobLiveness spends per device. The gate was killed at forty minutes against a measured cost of 424 s. The record was not wrong -- the call site was asking a different question. Its 200 s is a CAP on a broad probe, not a prediction of what an exhaustive sweep costs, and reading one as the other is precisely the confusion being removed from the other six. So six ship derived, and the exception is pinned by name with what deriving it cost, beside widenStill and jointlyLive, which stay typed because nobody has ever measured them. Four sabotages, 1/2/2/1 red by name -- and sabotage D is a limit of the machine rather than of the check: hostScale reads 1.0 in this sandbox, so dropping the host conversion is invisible to every line except the one that passes an explicit scale, which is said in the log rather than left as a silent pass. UNCHECKED: whether the 1.553x staleness measured for ONE device holds for the other 115. Re-freezing is a four-hour run and its own round. The tree stands at 1414 gates. Full changelog on docs/CHANGELOG.md.
