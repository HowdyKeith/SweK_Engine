...
const BRAIN_BUILD = "v4396";   // v4396 -- *** #150's LAST TWO NAMES, AND A THIRD NOBODY WENT LOOKING FOR. *** v4385 shipped box3d joint motors and limits and left sensors/triggers and CCD; this is those, fourteen functions built and RUN natively against the real library. A sensor reports one begin and one end as a body passes through it, names the sensor and visitor by body index, and -- the property that makes it a sensor -- leaves the trajectory IDENTICAL to falling through empty space, y = -70.005989 both, against 0.399930 for a solid box in the same place. The trap the vendored header warns about twice is measured rather than quoted: enableSensorEvents is false by default EVEN FOR SENSORS and must be on at BOTH ends, and the same fall with the visitor's events off reports begins 0, ends 0 -- a live sensor, a body going through it, and nothing at all. Continuous collision is TWO switches and the truth table is stronger than the header: across all eight combinations at 500 m/s, the bullet flag does nothing against a static wall and nothing against a dynamic one either unless the world switch is also on. It is a second gate BEHIND b3World_EnableContinuous, not an alternative to it, and no sentence in the vendored header says so. One predicate reproduces all eight rows, which is one claim that can be wrong rather than eight chances to write down what I expected. *** THEN THE ROUND WAS WRONG THREE TIMES IN ONE SECTION, AND NONE OF IT WAS FOUND BY READING. *** Two bisections of the same tunnelling experiment, differing only in their upper bracket, returned 22.58 m/s and 30.08 m/s -- and a bisection cannot disagree with itself on a monotonic predicate. A dense scan says why: with the world switch off, pass-through ALTERNATES IN BANDS -- stopped at 20, through at 21, stopped at 22, through at 23-24 -- and 90 m/s stops while 13 m/s goes through. It is ALIASING, not a threshold, so the honest quantity is a RATE: 64 of 96 sampled speeds pass through with the switch off, 1 of 96 with it on. And that ONE is the third correction: at 34 m/s the body goes clean through with continuous ENABLED while 33, 35 and 36 all bounce, so the rule is NECESSARY AND NOT SUFFICIENT and the gate ships the counterexample rather than a caveat. *** AND THE THING NOBODY WENT LOOKING FOR. *** Two tunnelling runs at 640 and 1280 m/s landed on the same six decimals, which no physics does. It is b3WorldDef.maximumLinearSpeed, whose DEFAULT the vendored headers state nowhere because it is assigned in box3d's own .c: measured through the getter, 400 m/s exactly, and raising it to 5000 lets the same 2000 m/s request through. swk_body_set_velocity accepts any speed and reads back unchanged until the world steps. IT IS ALREADY IN THE ARTIFACT THAT SHIPS -- box3d's own default, nothing this round added -- and physics/esBox3d.js clamps a ship to its hull's speed and hands that straight to box3d, so ev/tools/es-arena.mjs's Fighter at 430 has always been silently flying at 400 in the browser. The engine's own clamp overridden by a library clamp it did not know existed. Five sabotages, 2/4/4/2/2 RED by name, three subject files md5-identical -- and sabotage D caught a gate of mine that required its own finding to stay broken: the hull check first asserted a hull EXCEEDS the cap, which would go red the day somebody fixed it, so it was rebuilt around the predictor with a control that brings the Fighter to 390 and confirms the gate stays green. The tree stands at 1432 gates. Full changelog on docs/CHANGELOG.md.
