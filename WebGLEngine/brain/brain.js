...
const BRAIN_BUILD = "v4376";   // v4376 -- *** THE BODIES ARE SQUARES, AND THE PAGE'S OWN COMMENT CALLS THEM DISCS -- MEASURED AGAINST A CLOSED FORM. *** v4375 priced the shipped LOD ladders, found them tells rather than approximations, and noted in passing that orrery-gpu.html's ladder carries the comment "a finer disc up close, a coarser one far away" while render/gpuDriven.mjs quadMesh builds a square. This round measures it rather than leaving it as a remark. A body covers 1,369 pixels where a disc of the same reach covers 1,060 -- a ratio of 1.2915 against the 4/pi = 1.2732 that a square over its INSCRIBED disc must give, DERIVED from the two areas rather than asserted, with the 1.4% excess being edge coverage at this size. There is nothing anywhere that could round a corner off: the default pipeline's fragment is `return v.color;`, with no discard and no distance-to-centre test, and the pages bring no pipeline of their own. THE LADDER THE COMMENT DESCRIBES IS NOW BUILT AND PRICED. render/gpuDriven.mjs discMesh is a triangle fan of n segments inscribed in the same [-1,1] square quadMesh spans, so it drops into a ladder in that one's place without moving a record or a threshold. Priced exactly as v4375 priced the shipped ladders -- every rung twice, as it would ship and with one colour across the whole ladder so only geometry can differ -- a 10-gon and a 5-gon against a 32-gon differ by up to 1,911 pixels, and the geometry-only cost FALLS with distance (6, 8, 27, 65, 147, 346, 1283, 1423 for the 10-gon across the sweep) instead of sitting flat. So render/lodBudget.mjs ladderKind classifies it APPROXIMATION where it classified both shipped ladders TELL -- the same classifier, the same 64-pixel policy, the same page's ladder shape, and a different answer because the geometry now differs. *** AND A REAL THRESHOLD IS DERIVABLE FOR IT: 0.0354 and 0.0286 *** -- interpolated between measured samples, where the shipped quad ladder had no crossing to report at all and the module said so by returning a flagged bound and then refusing the ladder. A 32-gon is already the disc to 0 pixels, so the ladder's fine rung is not what makes a body square; the shape is. *** WHAT SWAPPING WOULD COST, BOTH NUMBERS, SO THE CHOICE IS A CHOICE. *** A body would get rounder and SMALLER: an inscribed disc covers 0.7743 of the square, measured, against pi/4 = 0.7854. And the cheap end of the ladder would cost MORE, because an n-gon is n triangles where the quad is 2 -- so the far rung goes from 2 triangles to 5. What it would buy is a page whose comment is true and a ladder with fidelity to price. THE PAGES ARE NOT CHANGED BY THIS ROUND. Which shape a planet is belongs to whoever owns the look, and this round's job was to stop the answer being a preference: it is now two numbers and a classification. Two sabotages: discMesh circumscribing the quad instead of inscribing it (1 red -- the square-over-disc ratio stops matching 4/pi, because it is no longer that pair of shapes, and the check is against a closed form rather than a tolerance), and every rung built at 3 segments (3 red -- the shape claim, the classification, which becomes a tell again, and the derivation, which then has no crossing to report; one line and the round's whole argument goes with it). UNCHECKED and unchanged: gpu-rig-check.html's typed [0.025, 0.04]; whether the colour tell should be a debug TOGGLE rather than the shipped default, which is a product decision and not a gate's to make; what any of this costs on a real rig rather than on SwiftShader, where a triangle is nearly free; and every number the rig has not signed. The tree stands at 1419 gates. Full changelog on docs/CHANGELOG.md.
