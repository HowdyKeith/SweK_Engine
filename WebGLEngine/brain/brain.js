...
const BRAIN_BUILD = "v4302";   // v4302 -- *** A SONG AS A PLANET, AND THE NAMING TRAP CLOSED FROM BOTH SIDES. *** Keith: "could we wrap an audio file terrain into a globe?" world/songGlobe.mjs takes the SAME field world/songHeightfield.mjs makes for ?song=1 -- rows time, columns frequency, height loudness, silence water -- and wraps it: time goes round the equator so the song's end meets its start at the seam and a spinning globe plays the piece past you once per turn; frequency runs pole to pole, bass at the south and treble at the north; loudness is radius, R (1 + amp h), with silence at sea level R exactly. The poles are capped, not collapsed: the columns stop half a cell short and each pole is one vertex at that column's mean radius with a fan to it, because mapping the last column to a point would put grid vertices at one place with grid different radii. Pure arrays in, arrays out; song-globe.html turns them into a BufferGeometry under three.js with an orbit camera, a chooser and a dropped-file path, ?song=<url>, and the engine's own default sweep character for character. Every claim is arithmetic: a constant field is a sphere of radius R to the vertex and to 4 pi R^2 within 0.2%; a pure tone is a RING, one column on every meridian at the latitude binOfFrequency predicts, radius constant to 1e-12 all the way round; a rising sweep is a SPIRAL whose ridge column never falls with longitude and climbs 43 of 64 columns; the seam is row 0 to the bit; and in a real browser the page builds 16,514 vertices and 32,768 triangles for the sweep, the module's own counts, the renderer reports drawing them, and a picked 2 kHz WAV rebuilds the globe with its peak in the 2 kHz column. bSongGlobe sits beside Song Terrain on server.html and window.songTerrain.globe(url) opens it from the console. Four sabotages, and *** TWO OF THE FOUR WENT GREEN ON THE FIRST DRAFT: *** the seam repeating row 1 instead of row 0 passed the seam check, which compared the two seam columns with each other and both were row 1 -- it now compares against the field's own row 0; and water unpinned from sea level went ZERO RED, because a songHeightfield's silent cells already sit at its minimum, so the pin was unfalsifiable on that input -- a hand-made field with a lake above the floor reaches the branch, and the lake reads 1.125 instead of 1 without the pin. An input that never reaches the guarded branch makes a check unfalsifiable; that is the v4297 and v4301 finding a third time. #144, the naming trap: render/crtModel.js's mask() is the APERTURE GRILLE, a phosphor stripe pattern with a pitch and a depth; render/aquarellePass.js's mask is a DISSOLVE TEXTURE whose alpha decides where one image has given way to another. Same word, unrelated things, adjacent files. v4299 did the half that mattered most by naming the per-region strength a FIELD, so no third meaning was added; this round writes the distinction into both headers and aquarelleModel.mjs, each naming the other so a grep for "mask" under render/ lands on the disambiguation, and both gates -- crtPass-selfcheck and render/aquarelle-selfcheck -- assert it from their own side. Unchecked and said plainly: what the globe looks like; counts, radii, ridges and draw calls are asserted and the picture is not. The tree stands at 1383 gates. Full changelog on docs/CHANGELOG.md.
