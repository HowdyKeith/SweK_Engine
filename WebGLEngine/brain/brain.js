...
const BRAIN_BUILD = "v4299";   // v4299 -- *** ONE ROUND, THREE LEVELS, AND A NUMBER THAT HAD TO MOVE. *** These three levels were built on a branch as v4297, v4298 and v4299 while main took v4297 for the full sweep and v4298 for the song button. Never reuse a version number: two builds with one number and different bytes is what jams the fleet. So the three land here as ONE round, v4299, and inside it the old round names read as the levels they were -- Level 11 (Level 11: textures bind on WebGPU, the GPU culls and draws itself, complexity is a scale, strength is a field), Level 12 (Level 12: the GPU decides what is hidden and what is where, and the orrery, the terrain and the ladder stand on it), Level 13 (Level 13: the GPU-driven path becomes something a person can use, and the economy moves in). Each level's own entry follows, unedited except for that renaming; every number in them was measured on a real GPU in the sandbox before it was written. LEVEL 11: *** FOUR THINGS KEITH NAMED AS LEVEL 11, ALL FOUR MEASURED ON A REAL GPU BEFORE THIS WAS WRITTEN. *** (1) gfx/device.js's WebGPU backend BINDS TEXTURES. From v4273 to v4296 pass.texture() on that backend refused by name (and before v4273 it was `() => {}`), so the orrery's post stage -- the abstraction's first non-demo consumer -- was pinned to WebGL2. The bindings are now DERIVED FROM THE SHADER: render/wgslSpec.mjs parseBindings() reads every texture, sampler, uniform and storage declaration at pipeline creation, the bind group is assembled from what the pass bound by name, and a declared texture left unbound is refused with its name and binding number instead of drawn black. deviceTexture-selfcheck drives the SHIPPING module through both backends in one browser launch and diffs the frames: badTv against the CPU model, worst 0 of 255 on both, and 0 of 4,096 pixels differ between the two backends. ui/orreryPost.mjs no longer asks for webgl2 by name; its capable-backend list is read from the device's CAPABILITIES table rather than typed, and orreryPost-selfcheck now draws the effect through both routes: 1 channel value of 9,216 pixels differs, by 1 of 255 -- linear filtering rounding differently in one texel. Also found on the way: the stage allocated a canvas-sized texture EVERY FRAME on both backends and freed none; it now re-uploads into one texture through update(). (2) GPU-DRIVEN RENDERING: render/gpuDriven.mjs is a compute pass that frustum-culls every instance, picks its LOD from angular size, appends the survivors to a compacted per-LOD record buffer through an atomic, and writes the instance count into a drawIndexedIndirect command; the render pass then draws each LOD with one indirect draw and never learns how many survived. gfx/device.js grew what that needs and nothing else -- buffers with a declared usage, compute pipelines, a dispatch ordered before the frame's render pass, index buffers, an instance-stepped second vertex buffer, drawIndexed and drawIndexedIndirect -- and WebGL2, which has neither compute nor indirect, REFUSES BOTH BY NAME and draws the same scene through the CPU TWIN, cullLodCpu(), whose per-LOD records the shader is required to reproduce. gpuDriven-selfcheck: the probe agrees with the twin on all 768 instances; the counts read back from the indirect buffer are 33/33/34 against the twin's 33/33/34; the record sets match per LOD; all 100 visible instances show their LOD's colour at their projected centre on both backends; and the histograms of the two frames are identical, with 216 of 65,536 pixels differing, every one a quad edge on a pixel centre where the two rasterisers break the tie differently. *** THE FIRST GPU FRAME DREW NOTHING AND NOTHING SAID WHY. *** A uniform field named `meta` is a reserved word in WGSL; createShaderModule does not throw, the pipeline just does nothing, and the twin drew 100 instances against the GPU's 0. gfx/device.js now watches every module's compilation and refuses by name on the next use() or dispatch() with the compiler's line numbers. Also measured: a WebGL2 canvas defaults to MSAA and the WebGPU backend has none, which alone made 3,417 pixels differ; the WebGL2 backend now defaults antialias off, so parity is the setting both can keep. (3) SHADER COMPLEXITY AS AN ENCODED SCALE: render/shaderComplexity.mjs scores a shader statically -- samples, transcendentals, branches, atomics, barriers, and loop bodies multiplied by their literal trip count or an assumed eight -- and encodes the score on a log2 class so that only doublings separate shaders. badTv's WGSL fragment and GLSL fragment land in the same class (93 and 83, C6). rankLods() DERIVES the LOD order from triangles times complexity and sorts the thresholds to match, so LOD 0 is the most expensive level however the levels were typed; shaderComplexity-selfcheck shuffles eight shaders twenty times and gets one ordering. What is still typed is named there: ai/AutoQualityController.js's TIER_ORDER, whose tiers are knob sets rather than shaders. (4) EFFECT STRENGTH AS A SPATIAL FIELD: badTv and crt took scalar knobs only. FIELD_FRAGMENT_WGSL and FIELD_FRAGMENT_GLSL are DERIVED FROM THE SCALAR SHADERS BY SUBSTITUTION (a missing anchor throws at load) and add one texture, tField, whose red channel scales both displacements before the wrap; crtPass.js takes a uField and mixes toward the untouched source; render/strengthField.mjs makes the fields and samples them the way a NEAREST sampler does; the CPU models take the same strength. strengthField-selfcheck: a white field reproduces the scalar pipeline with 0 of 4,096 differing on both backends, a black field is the identity at worst 0 of 255, a band field tears inside the band and not outside at worst 0, and the crt radial field matches crtModel at worst 0 of 255. *** WHAT THIS BOX CANNOT DO, MEASURED FOUR WAYS: *** the headless shell loses the WebGPU device on ANY render pass that targets a canvas -- in the DOM or not, COPY_SRC or not, waited on or not -- and renders an offscreen texture correctly, so gfx/device.js gained offscreen: true and every WebGPU frame graded here went to an owned texture; presenting to the orrery's canvas on WebGPU is rig-only. TWO GATES WENT GREEN THAT WERE RED AT HEAD: backendParity, whose PARITY_BASELINE had been stale since v4272 with two GLSL and four WGSL files arriving unrecorded -- green at v4279, red since, and exactly the "went red" case v4296 called UNKNOWN -- is re-measured and recorded (138 GLSL, 45 WGSL, both 6, render/gpuDriven.mjs the third dual-language shader module); and gateQuality, which v4296's record lists as fixedInRound and which was red at HEAD on one prose-matching regex from v4287, is fixed with the whitespace-insensitive idiom. Still red and not mine: shaderCensus (in the standing list) and postChain (SSAO's fifth bloomPass draw), both red at HEAD before this round's files existed. Sabotaged, nine ways across the four new gates, every one red, results in each gate's log. The branch stood at 1370 gates after this level. LEVEL 12: *** LEVEL 12 WAS NOT WRITTEN DOWN ANYWHERE, SO HERE IS WHAT IT IS: *** Level 11 let the GPU decide WHAT to draw; Level 12 lets it decide what is HIDDEN and what is WHERE, and puts the three named consumers on that path. Four items, each measured on a real GPU before this was written. (1) DEPTH, AND HI-Z OCCLUSION. gfx/device.js's WebGPU backend had no depth attachment -- the WebGL2 backend has had DEPTH_TEST since it was written -- so the two agreed only on scenes built not to overlap. It now attaches depth32float by default (the one depth format a copy may read), and the frame readback carries the depth image as f32. render/gpuDriven.mjs builds a depth PYRAMID by compute -- level0 copies the depth texture, reduce max-reduces 2x2 with clamped edges -- and the cull pass rejects any instance whose nearest point is farther than the farthest thing in every block its projected bounding cube covers. Two-pass: this frame culls against LAST frame's pyramid. hiZ-selfcheck hands the CPU twin the GPU's OWN depth image: the GPU pyramid equals the CPU reduction TO THE BIT, 21,845 values over 8 levels, 0 differ; frame 1 draws 1/64 with no pyramid, frame 2 draws 1/48 with it, the 16 gone being exactly the 4x4 the twin puts in the wall's shadow, as a set; and *** THE CULLED FRAME IS PIXEL-IDENTICAL TO THE UNCULLED ONE, 0 OF 16,384 *** -- occlusion saved work and no pixels. WebGL2, with no compute, draws all 65 and the depth test hides the same 16; the two frames agree per colour with 106 edge pixels differing and none interior. Found on the way: one WGSL module with two entry points cannot serve two pipelines through this device, because the bind group is built from every binding the MODULE declares; the builders are two modules now, and the header says why. (2) GPU-RESIDENT ORBITS -- THE WebGPU ORRERY. render/gpuOrbits.mjs is a compute pass that places every body from its elements (axis, period, phase, radius, the numbers world/orrery.mjs derives from the vendor tree) and writes the instance records the cull reads: orbit -> cull -> indirect draw, three dispatches and no CPU per body per frame. *** THE TWIN IS world/orreryView.mjs positionAt(), TO THE LETTER. *** gpuOrbits-selfcheck: every GPU position is within 1.87e-4 of its axis from positionAt (worst on krbn; sin and cos are SwiftShader's low-accuracy path, tan measured 4.6e-5 at v4290), radii travel untouched, z is 0, the cull counts on GPU-written records are the twin's 7/8, and WebGL2's twin route agrees. orrery-gpu.html draws the baked system through the device, names its route in the HUD, and loads here on the WebGL2 fallback with the clock running and 8 + 7 of 15 drawn; the 2D orrery stays the product and is linked. makeGpuDrivenScene() takes a records SOURCE now -- a buffer another pass writes, with a cpu() twin -- and a consumer's own render pipeline. (3) GPU TERRAIN. render/gpuTerrain.mjs lifts a heightfield in the VERTEX STAGE -- textureLoad in WGSL, texelFetch in GLSL, never a filtered sample -- over GPU-driven chunk instances whose LOD is derived; each chunk paints its CENTRE texel's height as a flat colour, so a pixel names a texel. gpuTerrain-selfcheck: at all 64 chunk centres the pixel IS the centre texel's height byte, on both backends, 0 wrong; counts 8/56/0 equal the twin's; 60 colours; the backends agree per colour within 5 pixels. (4) THE LAST TYPED ORDERING, DERIVED. ai/AutoQualityController.js walked `TIER_ORDER = ["fast", "balanced", "quality"] // ascending difficulty` from v643. ai/qualityTiers.mjs sums the complexity of the shaders each tier's knobs switch on: fast(0) < balanced(1900) = quality(1900), and *** balanced AND quality TIE, AND THE TIE IS THE TRUTH *** -- their knob sets are identical, which the typed array hid; ties break by name and are REPORTED. main.js fetches the knob shaders and derives the order at boot; the derived order is the walked order today, so nothing changes until a knob separates the tiers, and then nothing needs retyping. *** ALSO ON THIS BOX: THE NATIVE DAWN PATH CAME UP. *** tools/ship/headlessGpu.mjs skipped for want of node-webgpu; `npm i -g webgpu` here (the skip reason's own instruction) and headlessGpu-selfcheck and crossBackend-selfcheck went from red to ALL GREEN -- 12 corpus entries identical across Dawn-native and Chromium, including Level 11's cull probe at 1536/1536 and this round's four compile-only shaders. Both are among the 37 standing reds; the register is untouched, because on Keith's rig they stay red until the same install, and a record that says green for a box is a record for that box. backendParity's baseline moved again and is recorded (139 GLSL, 47 WGSL, both 7, render/gpuTerrain.mjs the fourth dual-language module). Sabotaged eight ways across the four new gates, every one red, in each gate's log. Unchecked and said: a moving camera against last frame's pyramid, chunk seams between LODs, WebGPU presenting to a canvas on this box. The branch stood at 1374 gates after this level. LEVEL 13: *** LEVEL 13, DEFINED HERE AS THE RUNG ABOVE 12: *** Level 11 chose what to draw, Level 12 what is hidden and where; Level 13 closes the two gaps Level 12 named and adds what a user needs from the picture -- and then, at Keith's ask, lets the game's economy loose among the GitHub planets. (1) TWO-PHASE OCCLUSION. Level 12 culled against LAST frame's pyramid, so a fast pan could drop a newly revealed body for a frame; the unchecked note said so. Now what phase 0 rejected is re-tested against THIS frame's pyramid after phase 0 has drawn, and the survivors are drawn on top in the same tick -- gfx/device.js gained begin(), a render pass that LOADS colour and depth instead of clearing. hiZ-selfcheck section 3 moves the camera three units: a single phase leaves 228 pixels wrong and 12 bodies missing; two phases are pixel-identical to the unoccluded frame, phase 2 drawing exactly the 12. (2) GPU PICKING. The compacted record is eight floats now -- the input record, then (id, lod, phase) -- so identity survives compaction, and a pick pipeline draws identity instead of colour into an OFFSCREEN target (a per-frame `offscreen` on both backends: an owned texture on WebGPU, an owned framebuffer on WebGL2) and decodes one pixel. gpuPick-selfcheck: 92 of 101 visible bodies pick as themselves with their LOD on both backends, the 9 covered by a big near body pick THAT body -- depth decides, as for colour -- a gap picks nothing, and the presented picture is untouched by a pick. *** `meta` IS A RESERVED WORD IN WGSL, AGAIN, *** this time as an attribute name, and Level 11's compile watcher named it at the first use(). (3) THE UNIVERSE. world/universeBodies.mjs turns es-universe.json into records: 694 systems and 5,517 stellar objects, 6,211 in all, on the orrery's plane with objects lifted above their system by kind so a pick has one answer. universe-gpu.html draws them through the device with two-phase Hi-Z and names what the pointer is over. gpuUniverse-selfcheck: WebGL2's counts are the twin's (684/5,455 of 6,211 in view), WebGPU's two phases never exceed the frustum count and occlusion removes some, every one of Sol's twenty records picks as itself or as a body whose quad covers it from in front, and the page names a system under a sweep. Measured on the way: markers were sub-pixel from the whole-map vantage; the map was first put on XZ while the quads lie in XY; objects at 0.02 map units per in-system pixel put Sol's planets over Caph's star -- three wrong models the gate refused before the right one. (4) TERRAIN SEAMS CLOSED, AND WHY THAT NEEDED CULLING. skirtedQuadMesh() hangs a strip from each chunk edge the full height range down, double-wound, so no LOD needs its neighbour. The gate's first three drafts found NO crack -- from below, from the side, across a cliff -- and the picture said why: without backface culling a crack shows the higher chunk's underside, never the sky. gfx/device.js now takes cull and frontFace in the descriptor on both backends; the terrain sheet culls its underside; and from above the low chunk the crack between a fine staircase edge and a coarse straight edge shows 10,266 sky pixels, and with skirts 0. (5) *** THE ECONOMY IN THE GIT UNIVERSE. *** Keith: "could the simulator start their own trade routes among github planets? Not to be accurate, but just to start their own sea monkey life?" world/gitEconomy.mjs: every orrery body is a market in ev/esEconomy.js's own universe shape, stocked with source, binaries, data and docs counted from its files, priced off COVERAGE -- stock against need, no price table; every contributor git says crossed between these repositories (world/traderGraph.mjs) and a hauler per body is an esEconomy player with the game's unchanged buy() and sell(); docked, a trader takes the best margin per ton it can afford, flies to the moving body, sells, looks again; selling moves the price, the price moves the next trader. Idea reached via Kalcode/spaceprojectsim (MIT, David Clausen), a Rust + Bevy space economy whose markets price off coverage and whose ships chase margin; nothing vendored, the shape transferred. gitEconomy-selfcheck, a hundred days: 3,004 of 3,004 tons where the ledger says (stock + holds + consumed - produced), every trader's credits start + earned - spent, 768 events, every departure the best margin on offer within the 5% preference noise, 277 distinct routes because trading moved the prices, the same seed the same hundred days event for event, a different seed a different life. orrery-gpu.html draws the 22 traders as a second scene over the bodies (clear: null), the log shows the last trades, and the pointer names a trader with its cargo and credits or a body with its prices. The toy is exact about what it does and says what it is not: infinite market credit, nothing built, nobody bankrupt. Sabotaged seven ways across three new gates and two grown ones, all red, in each gate's log. Landed on main beside the sweep's and the song button's gates, the tree stands at 1379 gates.
