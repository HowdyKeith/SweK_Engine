...
const BRAIN_BUILD = "v4286";   // v4286 -- *** v4285 FILED SSAO AND THE COMPOSITE AS READ-OFF-SOURCE AND ONE OF ITS TWO REASONS WAS SIMPLY WRONG. *** "SSAO needs a depth buffer that is not the colour buffer" -- SSAO_FS has exactly ONE sampler and the harness binds one texture; it was measurable the whole time and nobody tried. The other reason was real: COMPOSITE_FS takes five samplers and with one bound they all read the same image, so any measurement would have been of a fiction. renderGlslToPixels now takes textures:{samplerName:texelFn} and binds each to its own unit, additively. SSAO MEASURED: its reach is 0.95*uRadius/max(1,depth) texels and the footprint TRACKS the uniform -- uRadius 2, 5, 10 give radius 2, 5, 9; at 20 it reads 16 because the 32px frame clips it, not the pass. *** AT uRadius 10 THE APRON IS 9 TEXELS, LARGER THAN AN 8x8 TILE *** -- fusing it there means loading 26x26 texels for 64 outputs. THE COMPOSITE MEASURED PER INPUT: uBloom 0 and uSSAO 0, uSceneDepth 1 with exactly 8 pixels moved (the 3x3 outline Sobel ring) -- *** AND MINUS ONE WITH THE OUTLINE OFF, because the Sobel never runs. *** A pass's footprint depends on which FEATURES are enabled, so an apron must be sized for the feature set and not the file. *** AND THE METHOD ITSELF REPORTED CONFIDENT NOTHING: *** perturbFootprint always poked its texel WHITE, and against SSAO_FS that measured NO footprint at any radius -- white reads as depth 1.0 and the shader skips depth >= 0.999, so the poke landed on the one value the pass ignores. A perturbation the shader discards is evidence about the POKE. The poke is a parameter now and the white result is kept as a CONTROL. A SECOND METHOD DEFECT FOUND THE SAME WAY: measuring heat displacement returned "radius 0" while uHeatRadii and uHeatStrength had silently failed to bind (float ARRAYS; the harness sets scalars and vectors), so zero was the footprint of a shader whose heat was switched off by the binding failure. perturbFootprint now VOIDS any run with unresolved uniforms rather than returning a number, and heat is recorded in UNMEASURED rather than folded in. Every FUSION entry now carries evidence "measured"; the verdict is unchanged -- god rays alone forbids single-dispatch fusion -- but rests on five passes rather than three. Six sabotages: binding every extra sampler to unit 0 makes uBloom read radius -1, which is exactly what v4285 would have measured and what the fiction looks like. *** AND ONE WENT 0 RED BECAUSE THE GUARD IT ATTACKED WAS NEVER REACHED: *** deleting the void-on-unresolved guard changed nothing, since no measurement produced an unresolved uniform -- a guard no test reaches, inside a gate. The heat path is now run and the REFUSAL is the assertion; redone it goes 2 red. One check was rewritten rather than repaired: it asserted the string "0.0035" appeared in the composite entry, true while that entry was read off source; it now asks for the EVIDENCE instead of the literal. NOT DONE and asked for: wiring the fused pass into a real WebGPU render target, which is its own round and whose payoff is memory traffic this sandbox cannot measure. Full changelog on docs/CHANGELOG.md.
