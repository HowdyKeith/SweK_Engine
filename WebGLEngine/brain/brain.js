...
const BRAIN_BUILD = "v4363";   // v4363 -- *** THE CULL PASS ITSELF, BOOKKEEPING AND ALL -- AND THE PASS BROKE THE BUFFER MAPPING THAT CARRIED THE LAST SIX ROUNDS. *** The correction first, because this round found it in its own build rather than in review: render/tslSource.mjs has mapped a graph's storage buffers to a shell's BY ROLE AND ORDER since v4336, and it is wrong here. three declares its buffers in the order the BODY FIRST USES them, not the order the graph created them -- this pass reads the per-instance extras before the frustum planes, so the mapping crossed two array<vec4<f32>> that nothing else tells apart. Measured rather than argued: the same module with its labels stripped draws NOTHING, 0/0/0 against 96/246/210, because the frustum test is reading the extras. The fix is that a TSL storage node which is .label()ed is emitted under that name, so the graph NAMES its buffers and the transplant maps by name, with the roles still checked by name rather than inferred from position; role-and-order stays the fallback for a graph that names nothing, which is every earlier section. THE ROUND: render/tslSource.mjs computeShell gained a STRUCT element -- a storage entry may give `struct: { name, fields: [{ name, type, atomic }] }` instead of an element, the shell declares that struct, and the atomic lives on a FIELD, because array<Cmd> is not an atomic buffer; one of Cmd's five members is. That was the one capability standing between v4362's cull DECISION and the whole of render/gpuDriven.mjs cullLodWgsl(), and render/physicsTsl.mjs makeCullPassTsl is now that pass entire: the count guard, the day-t clock gate, the frustum, the ladder, an atomicAdd into the region's indirect draw command, and three vec4s per survivor written at a region-major offset. Measured on WebGPU against the SHIPPED text on one 768-instance scene: the same instance count in every region (96 / 246 / 210) and every one of the 552 survivor records identical to the float, then again with the clock gate on -- 58 / 135 / 122, the 237 bodies not yet vendored on day 3 held back by both passes alike. AND THE STRUCT IS THE SHIPPED ONE, WHICH THE FILE CAN SHOW RATHER THAN ASSERT: the shell's declaration is the same text render/gpuDriven.mjs writes, and the four non-atomic Cmd fields the CPU seeded come back untouched, which is exactly where a wrong layout would have shown -- the atomicAdd landing on indexCount instead. WHAT IS NOT CLAIMED, BECAUSE IT IS NOT TRUE: the SLOT a survivor lands in. An atomicAdd hands slots out in whatever order the lanes arrive, so the claim is the per-region counts and the sorted set of records, and the gate says so instead of sorting quietly. Three sabotages, measured: the struct-layout agreement dropped so the shell is taken on trust (1 red -- and what it would cost is not a compile error but the CPU writing one layout while the module reads another at the same binding); the by-name mapping removed (2 red, the pass drawing nothing); and the record offset with its region term dropped (2 red, and this is the one that separates the bookkeeping's two halves -- the per-region counts stay exactly right because the atomicAdd is untouched, and all 552 records differ because three regions now write over each other from slot 0). Also found and fixed in this gate: the template-literal backslash trap for the third time this session -- a regex written inside the browser script loses its backslashes twice, so three checks silently measured nothing until they were rewritten as plain splits. STILL HAND-WRITTEN and named rather than left to be noticed: the occlusion and fleet variants of cullLodWgsl, where hizOccluded reads a depth pyramid through a ptr<storage, array<f32>, read> argument no graph here has emitted and a region becomes two-dimensional; and struct Cull's `array<vec4<f32>, 6>` planes field, a fixed-size array in a UNIFORM struct, which is why the six planes still arrive in a storage buffer. Also unbuilt and unchanged: a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph; and every number the rig has not signed. The tree stands at 1414 gates. Full changelog on docs/CHANGELOG.md.
