...
const BRAIN_BUILD = "v4124";   // v4124 -- an install button for vinimlo/galaxy-profile (GPL-3.0), never vendored: cloning a public repo onto the user's own machine and running it as its own process is not distributing it, the same reasoning voxtral's engine and webrtx's build already use. Pinned to a reviewed commit (the first install button here to pin a commit rather than an artefact digest, since this is source that gets executed rather than a binary that gets verified), built into a real venv (a bare pip install collided with an apt-managed system package on this box), config.yml pre-filled from upstream's own example rather than guessed at, and a supplied GitHub token sabotage-verified to never touch disk. Gate: galaxyProfile-selfcheck, 24 checks, all pass. Full changelog on docs/CHANGELOG.md.   // v4123 -- type-checking opted into file by file via "// @ts-check" + tsc --noEmit, not a TypeScript conversion (no build step anywhere in this tree, and a-Shell's jsc runs portableSuite.mjs directly on iOS with no Node at all). Five files annotated: render/crtModel.js, render/crtPass.js, ui/domToTexture.js, ui/voxtralBrowser.js, ui/webrtxBrowser.js, picked for JS/GLSL and DOM boundaries and state machines. Real bugs found: a possibly-null createShader() used unchecked, a Node-vs-Element cloneNode() gap, a missing getContext("2d") null guard, a SharedArrayBuffer-including type where crypto.subtle.digest needs a real ArrayBuffer, and dead fallback code TS proved unreachable. New tools/ship/tscResolve.mjs resolves the tsc binary the way playwrightResolve.mjs resolves a browser; tools/ship/typecheck-selfcheck.mjs builds an explicit file list from the "// @ts-check" pragma rather than running "tsc -p tsconfig.json" (which auto-parses every .js including shaders/voxel.frag.glsl.js and hard-fails), translates tsconfig.json's own compilerOptions into CLI flags rather than retyping them, and is sabotage-verified against a deliberate type error. Full changelog on docs/CHANGELOG.md.
