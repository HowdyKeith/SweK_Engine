...
const BRAIN_BUILD = "v4458";   // v4458 -- *** BLEND STATE ON gfx/device.js, BY NAME, ON BOTH BACKENDS -- AND THE ROUND OPENS WITH TWO THINGS THE SWEEP SAID THAT NEED READING CAREFULLY. *** The quick sweep run before this commit reported two NEW reds. One was tools/ship/budgetEvidence-selfcheck.mjs, and it was an ORDERING ARTEFACT rather than a defect: the sweep runs that gate at 78 ms, before the sweep has written the timing record for the gate this round added, so the evidence gate reads a file that does not yet name it; run after the sweep finished it is green (0 red), and the record ships in sweep-timings.json. The other was tools/ship/releaseLedger-selfcheck.mjs, and it is REAL: main is four versions ahead of the releases page against a budget of three (v4452, v4453, v4455, v4456 unpublished, and v4457 now on main). It went red the moment v4457 reached main, it is not this round's to clear -- the sandbox cannot publish (the releases API answers 401) and the ship skill says the rig publishes -- and it is NOT recorded in redCensus, because a lag gate written into the register of known reds would be the escape hatch v4453 refused to leave open. If this ship's verify refuses on it, that refusal is the gate working. THE ROUND: until now a device pipeline carried topology, cull and frontFace and no blend at all, so every draw through gfx/device.js landed opaque whatever alpha the fragment wrote -- invisible because every consumer drew opaque geometry or a full-screen post effect, and fatal for Slug text (text/slugShaderWgsl.js, v4457), which returns colour PREMULTIPLIED by coverage and needs (ONE, ONE_MINUS_SRC_ALPHA). The descriptor now takes `blend: none | premultiplied | alpha | additive` (BLEND_MODES). The words are the contract: WebGL2 sets them at use() beside cull with blendFuncSeparate; WebGPU puts them on the colour target; the null backend records the word on the pipeline; an unknown word is refused by name, once, before any backend sees it, the same shape as BUFFER_USAGES; and render/gpuDriven.mjs renderPipelineDesc carries it the way it carries topology. MEASURED by tools/ship/deviceBlend-selfcheck.mjs, which drives the SHIPPING module through both real backends: an opaque quad over the whole target, a translucent one over the left half at a nearer depth, one frame per mode. THE KEY IS THE BLEND EQUATION ITSELF, evaluated in f64 on the two colours and quantised to bytes, not a picture from either backend -- so a backend blending with the wrong factors is wrong against it and not merely different from its twin. Every byte on every frame is within 1 of the equation on both backends (worst 1 on none, premultiplied and additive, 0 on alpha), the two backends are IDENTICAL on all four modes (pair worst 0), the untouched half is exact, and the four modes produce four different colours, which is the control that says the equation checked something. Three sabotages, each applied to gfx/device.js, run, and restored byte for byte: WebGL2 never enabling BLEND, 6 red at worst 127 of 255; the WebGPU target built without its blend, 8 red; and the premultiplied dst factor mistyped as one, 4 red at worst 77 -- *** AND THAT THIRD ONE IS THE POINT OF HAVING THE EQUATION: BOTH BACKENDS AGREED WITH EACH OTHER ON THE WRONG FACTOR, *** so the backend-to-backend diff alone would have passed it. The seven other device gates (deviceTexture, gpuDriven, gpuPick, fleets, badTvDevicePass, deviceKind, deviceBridge) are green against the edited module. FOUND WHILE BUILDING, AND FILED ON THE NEXT ITEM RATHER THAN WIDENED INTO THIS ONE: the WebGL2 backend ignores depthWrite (the WebGPU pipeline honours d.depthWrite; the WebGL2 one has no gl.depthMask), and the device's texture path uploads rgba8unorm only where the Slug atlas is rgba16float and rg16uint. Both sit in front of drawing text through the device; docs/TSL-ROADMAP.md step 7 item 3 names them. NOT CLAIMED: presenting to a canvas on WebGPU (this box loses the device on any canvas-targeted pass, so the frames went to the owned offscreen texture); any blend equation but add; and a blend on a render target texture rather than the frame. The tree stands at 1483 gates. Full changelog on docs/CHANGELOG.md.
