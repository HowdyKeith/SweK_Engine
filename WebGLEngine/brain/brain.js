...
const BRAIN_BUILD = "v4371";   // v4371 -- *** THE SECOND SCULPTOR, AND THE JUDGE SCORES IT 1.000000 WHILE IT IS 90% TOO BIG. *** v3337 took img2threejs's JUDGE (render/silhouette.mjs, render/perceptual.mjs) and refused its numbers; v4255 took its first SCULPTOR (mesh/lathe.mjs) and found the judge nearly worthless there -- a revolved profile's front view IS the profile mirrored, so an L-bracket scored 0.62 against a vase's 0.99. This round takes the other technique img2threejs names, multi-view silhouette carving, and the same finding comes back sharper by the whole remaining range. mesh/carve.mjs intersects orthographic silhouette cones into a visual hull. It assumes nothing about the shape, and its error has a KNOWN SIGN: the hull contains the object, asserted over all 36 fixture/view-count pairs rather than observed once. *** AND REPROJECTION IoU IS EXACTLY 1.000000 ON ALL SIX FIXTURES, whose volume errors span 0.0% to 90.0%. *** A hull reprojects to the silhouettes it was carved from by construction, so the number was never at risk; holding a view OUT and scoring against it does not repair the problem either -- three of the six still read exactly 1.000000 against a view nobody carved from. *** THE LATHE HAD ONE NUMBER THAT TESTED ITS ASSUMPTION. THIS HAS NONE, AND THAT IS PROVED RATHER THAN CONFESSED. *** A cube with a sealed cavity casts a shadow IDENTICAL to a solid cube's on 48 of 48 directions across yaw AND elevation while its volume differs by 1000 voxels, so every function of the silhouettes -- named or unnamed -- returns the same value for both. That is a counterexample to the existence of such a number, not a failure to have found one; detecting the cavity needs depth or shading, which is what img2threejs's optional Depth-Anything stage is for. *** MORE VIEWS IS NOT BETTER; THE RIGHT VIEWS ARE. *** The cube is EXACT from two views at 0 and 90 degrees and 25.0% too big from THREE at 0, 60 and 120 -- so monotonicity is stated over NESTED view sets, where it holds on every fixture, and not over the count, where it is simply false. A tube is 90.0% too big from sixteen azimuths and EXACT from two plus one view from above. *** THREE PREDICTIONS THIS ROUND WROTE DOWN AND THEN MEASURED WRONG. *** (1) The first silhouette was a ray march down each pixel's centre against a carve testing each voxel's centre; those are not the same point, and THE CUBE MEASURED -0.7% -- a hull SMALLER than the object, which a visual hull cannot be. Shadow and carve are exact duals now; sabotaging that back costs 6 red and drops reprojection IoU from 1.000000 to 0.920000. (2) A row-fill was added because a real silhouette is a filled region and gaps would make the hull too tight. It adds EXACTLY ZERO pixels on all six fixtures at two angles, and turning it on fills the bore of a tube's top view -- an annulus is not row-convex. It ships off, exported, with fillGain() to say so with a number. (3) The cup was put in expecting one top view to recover its bore. A cup has a FLOOR: the top shadow is a disc of 448 pixels against a full disc's 452, and sixteen azimuths plus a top view still read 53.4% over. What decides recoverability is not the size of the concavity but whether a ray passes CLEAN THROUGH it, which is why the tube joined the fixture set. *** AND A SABOTAGE THAT WENT 0 RED TURNED OUT TO BE THE FIX. *** Flipping the out-of-frame policy cost nothing because no fixture had a single voxel projecting past the border -- dead code carrying a written justification that had never been executed. Given a fixture that reaches it, the justification is wrong: clearing an unseen voxel breaks containment by 7.0%, because a pixel off the edge is a view with nothing to say rather than a view saying "empty". The default is now KEEP, section 9 exists to reach the branch, and the honest cost is stated -- the hull extends to the grid's edge wherever nothing looked. Two numbers earned against closed forms: the sphere's two-view hull is the Steinmetz bicylinder and walks in to 4/pi (1.2456, 1.2586, 1.2757 at n = 32, 64, 96, gaps 0.0276 > 0.0146 > 0.0024); and the cross's 32-azimuth hull reads 30.0%, 35.0% and 42.2% against its continuous convex hull's 40.0%, CROSSING it -- so "the discrete hull approaches the continuous one from below" was written into the module header on the strength of one resolution and deleted after three. Four sabotages, measured 1/6/1/6, plus a fixture sabotage that went 0 red and is recorded as the claim confirming itself rather than dressed up as a catch. *** AND A SEPARATE FINDING, UNRELATED TO THE CARVE AND FOUND BY RUNNING A GATE BY HAND: vendor/three-webgpu HAS BEEN UNDECLARED SINCE v4319. *** three 0.178's MIT LICENSE was copied in the same commit, so nothing was ever unpapered on disk; what was missing was the record. tools/ship/vendoredLicences-selfcheck.mjs has been RED on every verify for fifty rounds and reported by none of them, because it takes 15 s and verify's quick sweep caps at 3 s -- a standing red nobody runs is a check nobody has. Declared, and the gate is green. UNCHECKED, exactly as mesh/lathe.mjs's gate says of itself: a REAL PHOTOGRAPH. Every silhouette here is computed from a solid this tree defines, so nothing has met the segmentation problem img2threejs stages 1 and 2 spend most of their effort on. How much a WRONG mask costs is not measured either -- the one-signed bound holds against a TRUE shadow, and a shadow that is too small carves real material away. Also unwired: no page calls mesh/carve.mjs, so the judge and both sculptors still meet only inside gates; and the carve is n^3 per view in JavaScript, which is a compute pass wanting to happen and is not one yet. The tree stands at 1415 gates.
