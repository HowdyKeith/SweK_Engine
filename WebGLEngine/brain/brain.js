...
const BRAIN_BUILD = "v4328";   // v4328 -- *** THE INK LAYOUT, AND WHAT TWIN-GRADING IS BLIND TO. *** The correction first, and this round found it in its own sabotage rather than in a later one: every "on every pixel, worst 0" line in tslRace-selfcheck grades the GRAPH against a hand-written twin, and the twin is built from the SAME shell -- so a mistake in the shell moves both halves of the comparison together and the byte claim does not move at all. Sabotage N drops the ink shell's topology and 36,864 of 36,864 pixels still agree; what catches it is the named assertion on the descriptor (topology === "line-list") and nothing else. That is now written into the gate's header beside the claims it qualifies. The round: render/physicsTsl.mjs inkLookShell is the third and leanest shell -- the Krbn race's strokes on a LINE-LIST, the flat layout's p and colour, no normal and NO UV at all, not even the hull's own x and y standing in for one as the lit shell does, and its shipped fragment is the whole of `return v.color;`. It is also the first shell whose topology is not the default, and the descriptor carries "line-list" out to gfx/device.js. makeInkTsl washes the stroke's colour toward its own luminance by a labelled `ink` uniform and reads nothing else, three emits exactly ONE varying for it, and a graph reaching for a uv here is refused by name. Measured, section 8: on WebGPU and on WebGL2 the Krbn strokes painted by the GENERATED pipeline are the hand-written twin's picture on 36,864 of 36,864 pixels (worst 0), the wash parting from the fleets' own flat strokes on 467 pixels and 415 respectively, the drawing on the screen and the pick still naming the Krbn ships. THE SECOND LIMIT, measured in the same numbers: 467 and 415 are not a discrepancy to fix, they are the two backends rasterising a line differently -- each is byte-exact against its own twin, and no cross-backend pixel claim about a line-list is made here or can be. orrery-gpu.html?tsl=1 now paints FOUR races from graphs: the Chaos hull breathing, the Pixel quad lit by the lightning, the Glyph race drawing its own shipped look, and the Krbn strokes washing on a line-list. With this the fleets' three vertex layouts -- lit, sprite, flat -- have all been crossed into. Sabotaged and measured: the shell's topology dropped (4 red, and the picture claim silent, which is the finding above) and the graph's wash dropped (2 red -- 467 pixels and 415, worst 6; small because a wash toward luminance is small on ink this dark, and caught on every one of them). Not built and said: a MIPPED or anisotropic sample, which the device cannot serve at all -- it makes one sampler per filter mode, repeat addressing, no mip chain; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; that render/physicsTsl.mjs now holds three shells and five looks that are not physics, and the split it wants is the next housekeeping round (it will move the fleet shells into their own module and cost the backend-parity baseline four numbers and a list entry, which is exactly what that census is for); and every number the rig has not signed. The tree stands at 1407 gates. Full changelog on docs/CHANGELOG.md.
