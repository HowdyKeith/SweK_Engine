...
const BRAIN_BUILD = "v4269";   // v4269 -- *** THE TREE IS WebGPU-CAPABLE AND CAN MOVE ALMOST NOTHING TO IT, AND THE RATIO WAS NEVER COUNTED. *** gfx/device.js is a unified WebGL2/WebGPU device promising "a demo writes its render ONCE and runs on either runtime" -- but shaders are the one thing it cannot unify, so a render only travels if its pipeline supplies BOTH languages. Counted, comments stripped: 118 modules ship GLSL, 38 ship WGSL, and FIVE ship both -- 4.2%. Of those five, three are pages; only two are shader modules. So the portability abstraction is satisfied by two modules out of 118, and it has TWO consumers, both its own demos. *** AND THE FAILURE WAS NOT GRACEFUL: *** the WebGPU backend read d.shaders.wgsl bare, so a GLSL-only pipeline -- 113 of 118, the common case -- reached createShaderModule as code:undefined, a driver-shaped error naming neither the pipeline nor the missing language. Now it refuses by name, and the gate proves the guard FIRES by calling the backend against a stubbed device rather than grepping for the message. For the orrery specifically: ui/orreryDraw.js is canvas 2D with no shader stage, so no effect in this tree can touch it; the route off 2D needs text/slug*.js for its four fillText calls, and text/slugShader.js is 337 lines of GLSL with zero WGSL. That is the blocker -- one file, a number instead of a hunch. REFUSED: transliterating it this round. The negative-zero bit extraction, the 1/65536 double-root fallback and the abs() in calcCoverage are load-bearing, and NOTHING HERE CAN EXECUTE WGSL. *** AND THE SELF-COUNTING TRAP IS SOLVED BY CONSTRUCTION, NOT BY A SEVENTH EXCLUSION LIST: *** neither the module nor its gate contains a shader marker -- they are assembled from fragments -- and that is asserted as a check. Sabotage A 5 red, B 5, C 4, measured. Full changelog on docs/CHANGELOG.md.
