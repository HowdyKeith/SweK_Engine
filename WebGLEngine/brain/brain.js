...
// v4381 -- *** THE GPU BRAIN'S MLP LAYER AS A NODE GRAPH, BIT-IDENTICAL TO THE KERNEL THAT RUNS IT -- AND THE CLAIM HAS TO SPLIT BY OPERATION CLASS. *** brain/mlp.js is a hand-written WGSL batched MLP: y = act(W*x + b), one dispatch per layer, activations ping-ponging, weights uploaded once. brain/brain.js builds one from buildLayers() and a second from the attack layers at batch caps 64 and 256; blobBrain.js runs a third. The TSL transplant arc spent v4322-v4370 proving generated WGSL matches a hand-written twin and had never been pointed at it. render/brainTsl.mjs is that kernel as nodes, transplanted through tslSource into the kernel's own four buffers, and MEASURED ON A DEVICE: BIT-IDENTICAL, 0 differing cells across four layer shapes and all three activations. *** WHY THIS TWIN IS WORTH MORE THAN THE ONES BEFORE IT. *** v4335 found the weakness in every earlier byte claim here -- the twin is built from the SAME SHELL, so a shell mistake moves both halves together and sabotage N dropped a topology with 36,864 of 36,864 pixels still agreeing. brain/mlp.js predates this arc by a thousand versions, was written to run policies rather than to be compared with anything, and shares no code with it. *** AND THE CLAIM CANNOT BE ONE CLAIM. *** hmcGpu could say "specified operations only" and earn a bit claim against a CPU mirror; this pass cannot, because sigmoid is 1/(1+exp(-x)) and WGSL gives exp() an ULP budget rather than correct rounding. So: none and relu are BIT-IDENTICAL to an f32 Math.fround mirror (0 cells), sigmoid is NOT (2 of 8 cells, worst 5.96e-8) -- and the same sigmoid layer is still bit-identical to the KERNEL, because same device, same exp. A round that took one tolerance wide enough for the sigmoid would have reported "agrees" and thrown away the exact claim the other two carry. *** THE ROUND'S OWN GRAPH WAS WRONG ON ITS FIRST RUN, AND THE DAMAGE WAS NOT WHERE REASONING PUT IT. *** The kernel opens with `if (o >= P.nOut || r >= P.batch) return;` because an 8x8 workgroup over-dispatches; the graph dropped it, reasoning that a compute(N) is exact. It is not -- N is rounded up to whole workgroups of 64, so a 96-cell layer runs 128 invocations and an 8-cell layer runs 64. Measured, it corrupted not the 32 out-of-range cells but ONE IN-RANGE CELL, the last, by 3.43 and 0.93: an out-of-bounds WGSL store is not required to be dropped and this device CLAMPS it to the final element. The two fixtures that are exact multiples of 64 were perfect, which is precisely how a missing guard hides from round numbers. *** AND THE REDUCTION ORDER IS LOAD-BEARING WHILE THE SHIPPED POLICY IS BLIND TO IT. *** Summing the products first and adding the bias last is algebraically identical and moves 1,520 of 2,448 cells (62%, worst 3.815e-6) over layers with real biases -- so the kernel's accumulation order IS its arithmetic. The same rewrite on brain/policy.js's attack policy moves NOTHING: 0 of 256, because all sixteen of its biases are zero and adding zero first or last is exact. Grading the reduction against the real consumer alone would have passed a wrong accumulation order and called it a bit claim, so the gate runs both fixtures and says which sees what. TWO COUNTS THIS ROUND WROTE DOWN AND HAD TO CORRECT: "five modules ship through it" came from a grep that matched comments (rl/dockPolicy.js says a policy "drops straight onto" BatchedMLP and imports nothing) -- it is two, counted by import; and the flattening's bijection check stayed GREEN under the transpose sabotage, correctly, because a transpose IS a bijection and a structural check cannot see a permutation. Four sabotages, 3/4/3/3 red by name, and sabotage A found a defect in the GATE: every detail line printed "0 differing cells" beside its own FAIL because the strings were typed rather than counted. Also repaired: gateSweep-selfcheck's list of sweep closings, typed out by hand for twelve entries, now reads the frozen record's own keys and sorts them numerically -- "since10" sorts before "since2" as a string. UNCHECKED: whether a generated pass is worth SHIPPING into brain/mlp.js. It is neither faster nor shorter; what it is, is a second expression of one kernel that a change to either half now has to keep. The tree stands at 1421 gates. Full changelog on docs/CHANGELOG.md.
// v4382 -- *** THE PHYSICS WORLD CAN FINALLY BE ASKED WHAT A RAY HITS, AND A SECOND IMPLEMENTATION SAYS IT IS RIGHT. *** physics/box3d/box3d_shim.c has carried bodies, filters, three joint types, impulses, transforms, contacts and a deterministic record/replay with divergence detection for hundreds of rounds, and never a cast -- while the vendored box3d has had b3World_CastRayClosest the whole time. swk_world_cast_ray adds it: origin plus translation, closest hit, returning a BODY INDEX like every other entry point rather than the b3ShapeId the library hands back, resolved through the same B3_ID_EQUALS scan the contact reader uses, with -1 for a miss because 0 is a real body. *** AND THE ROUND'S OWN PREMISE WAS WRONG, WHICH COUNTING FIXED. *** It was filed as "several modules roll their own ray intersection"; a loose grep found five files and three were gates or comment matches. Counted properly, comments stripped, there is exactly ONE ray implementation in this tree -- mesh/meshBVH.mjs -- with three consumers, and multiplayer/wadLevelHost.js's castRay() DELEGATES to it. That is a better finding than the one it replaced and it sharpens what the round is worth: the tree had one ray, and box3d's cast is the SECOND it has ever had, which is exactly what makes each able to check the other. Eleven rays on all six axes plus an oblique one, four boxes of deliberately unequal shape, the same solids as hulls in box3d and as twelve triangles each in the BVH: agreement on the body, the distance and the face EVERY TIME, worst distance gap 2.400e-7 and worst normal gap exactly 0 -- box3d in f32 over hulls, the BVH in f64 over triangles, neither written to agree with the other, and one shared miss so that "agrees" is not agreement about nothing. *** THE WINDING TABLE HAD A FACE BACKWARDS AND ONLY ONE RAY IN NINE COULD SEE IT. *** boxTriangles' +y face was wound inward. Seven of the first probe rays travelled along x or z and every one agreed with box3d on body, distance AND normal; the eighth pointed down, hit the slab's top, and the winding gave (0,-1,0) against box3d's (0,+1,0) -- a gap of 2.0 on a unit vector, not a rounding question. The hit point and the distance were right in that case too, which is why nothing else noticed. So the gate no longer takes a ray set on trust: it checks that all twelve triangles of a box face away from its centre, on a box with three unequal half-extents so a cube's symmetry cannot hide a swap. A ray set is a sample; that is the property. *** AND THE SEGMENT CONVENTION FAILS SILENTLY, WHICH IS MEASURED ON THE DEVICE RATHER THAN WARNED ABOUT. *** box3d covers origin -> origin + TRANSLATION and reports a fraction OF THAT TRANSLATION; mesh/meshBVH.mjs covers origin -> origin + dir * maxT and reports a distance. A caller handing box3d a unit direction gets a cast one unit long. Run: the same ray hits body 0 at fraction 0.45 with a translation of 20 and returns -1 with a translation of 1 -- no error, no complaint, only an absence. translationFor() and distanceOf() write the conversion once, and translationFor deliberately does NOT normalise, because a direction of length 2 with a range of 3 means three lots of that vector. The row layout is the shim's: SWK_RAY_STRIDE is published through swk_ray_stride() as swk_contact_stride() set the precedent, rayCast.mjs names the fields, and the gate compares the two rather than letting a packed layout be declared twice. Four sabotages, 2/2/2/1 red by name, the two shim ones REBUILT natively before running so what went red is compiled physics. Sabotage C is the instructive one: a hard-wired body index still agreed on 4 of 11 rays, because a third of them genuinely hit body 0. UNCHECKED: the WASM packaging -- build-box3d-native.sh needs only cc and cmake so the physics is built and measured here, but getting this into vendor/box3d/box3d.wasm needs emsdk and stays rig work, exactly as #125's note said of the filter. Also unchecked and now named: shape casts, sensors, and joint motors, which box3d has and this shim still does not. The tree stands at 1422 gates. Full changelog on docs/CHANGELOG.md.
const BRAIN_BUILD = "v4383";   // v4383 -- *** THE COUNTER THIS TREE SET AS THE TRIGGER FOR ITS BIGGEST ARCHITECTURAL DECISION WAS COUNTING THE WORD, NOT THE LANGUAGE. *** v4380's register audit found tools/ship/shaderCensus-selfcheck.mjs saying FOURTEEN where its register entry said 4, and deferred the judgement to a round of its own: that gate has held since v3274 that a hand-written shader pair is cheaper than an IR "while few files carry both languages -- if this count climbs toward twenty the arithmetic inverts, and THAT is when to re-open llvm-tutorial-book's three-stage shape: parse, lower, emit per target". Answered here, and the answer is that the number was never real. The census classified a file by testing its RAW SOURCE for six tokens, and two of them -- GLSL's storage qualifiers -- are ordinary English: render/bloomFused.mjs counted as a shader PAIR on the sentence "attribute any difference to the SAMPLING", the English verb. MEASURED: FOUR of the fourteen author no GLSL at all, SEVENTY-TWO of the hundred and sixty-nine called GLSL-only carry no shader source of any kind (main.js and brain/brain.js among them, on changelog prose about HTML attributes), and the correction runs BOTH WAYS -- TWO REAL GLSL PASSES WERE MISSING ENTIRELY, three.js ShaderMaterial passes that carry no version directive because three prepends it. *** AND THE BETTER INSTRUMENT WAS ALREADY IN THE TREE, ONE DIRECTORY OVER. *** render/backendParity.mjs classify() has read this tree's shader languages correctly since v4269, badTvPass and the framework-GLSL population included; two shader-language censuses stood here with different answers and the one the IR trigger read was the worse one. The census owns no detectors now -- it walks, classify() decides, singleSource's move. THE HONEST FIGURES ARE 10 BOTH, 23 WGSL-ONLY, 99 GLSL-ONLY; the trigger is twenty and it has not fired. The compiler stays unbuilt, and step 4 of docs/TSL-ROADMAP.md is the version of that answer which costs nothing: the three-stage shape re-opened without this tree maintaining a parser, an IR or an emitter, because three's node graph IS the IR and its two builders ARE the emitters. *** THE HABIT WOULD HAVE BEEN RIGHT FOR THE WRONG REASON, AND A SABOTAGE SAID SO. *** This tree's instrument rule is "a check about code strips comments first", and I wrote that up as the fix before testing it. Turning classify()'s stripping OFF costs this gate ZERO RED: the pair count does not move at all and five single-language files change hands. Comments were never what made the count wrong; the MARKERS were, and a version directive cannot occur in a sentence where a-t-t-r-i-b-u-t-e is a sentence by itself. *** REMOVING THE SELF-EXCLUSION CAUGHT A SECOND SELF-COUNT IMMEDIATELY. *** v1 named its own filename in the census's skip list, so the gate's "a file that MENTIONS both is not a file that IMPLEMENTS both" check could not fail -- and nobody learned that pairShape's split pattern, a regex literal holding all three stage attributes, still made the census a WGSL file by its own detectors. The name came out, the second self-count appeared on the next run, the markers are assembled now, and the check is live. A NAME IN A SKIP LIST HIDES EVERY REASON IT WAS ADDED, not just the one somebody wrote down. Two checks the gate did not have: the inversion point asserted SEPARATELY from the baseline, because two thresholds under one assertion is how a drifting counter gets quietly re-based to wherever it landed; and the discredited raw-text detectors re-created inside the gate with the census held to DISAGREEING with them by 76 files, which goes red naming the phantoms if raw-text classification ever comes back, and whose own text says that reaching zero phantoms means the check has stopped exercising anything. Three sabotages: the census reverted to v1's detectors (exit=1, 3 red, and the pair count read 15 because the census counted itself again), a stage-attribute literal put back into the census (exit=1, 1 red, the self-count check alone), the comment stripping turned off (0 RED -- the finding above, not a pass). shaderCensus-selfcheck is GREEN and pruned to FIXED_SINCE_V4279: the third standing red this session to turn out to be unopened mail after vendoredLicences at v4371 and rigJobs at v4379, and the first whose recorded LINE was itself the defect rather than a symptom of one. UNCHECKED and said plainly: whether classify()'s markers miss a shader dialect neither census tests for; the pairShape drift numbers, still reported rather than gated because the split is crude; and the honest v3274 figure, which cannot be recovered and is not guessed at -- the baseline records 10 measured today and says so. *** AND THE FIRST GREEN RUN COST THE GATE ITS PLACE IN THE QUICK SWEEP, WHICH IS A COST TO FIX AND NOT TO ACCEPT. *** prose() is the full lexer and the walk sees 2,265 files, so the gate went from 279 ms to 3,541 ms under the 8-way sweep -- over the 3,000 ms budget, out of the sweep that runs every round and into the full one that runs rarely, which is most of the way back to a red nobody opens. A file whose RAW text holds no marker cannot hold one in its code or its comments either, both being subsets of it, so it is skipped whole: 0.48 s, and the same 10 / 23 / 99 / 5. *** AND THIS ROUND HAS BEEN NUMBERED THREE TIMES: v4381, THEN v4382, NOW v4383. *** claude/orrery-seeded-by-git-log landed its own v4381 while this round was verifying, and its v4382 while this round was verifying the renumber -- so this one supersedes forward twice rather than reusing either number, the same call that branch made when it renumbered v4380 to v4381 on the way in. Both of its rounds are merged here and its gates take the tree to 1422; every superseded number in this round's own files, gate headers and register entry moved with it. THE RACE IS THE FINDING, NOT THE RENUMBER: two branches shipping into one numbering space discover the collision only at the push, after a ten-minute verify has already been spent on the losing number. AND THE KILL THAT WAS MEANT FOR THE STALE VERIFY MATCHED THE SHELL ISSUING IT, because the pattern was the command line and the command line contained the pattern -- the same self-match this round's census was fixed for, one layer down, ten minutes after writing it up. The tree stands at 1422 gates. Full changelog on docs/CHANGELOG.md.
