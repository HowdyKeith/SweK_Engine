...
const BRAIN_BUILD = "v4448";   // v4448 -- *** v4434 NAMED THREE PATHS OF ui/pageFxOverlay.js IT DID NOT DRIVE. DRIVING THEM FOUND ONE MORE LEAK, AND THE OTHER TWO CAME BACK CLEAN. *** THE RECORDER BUTTON ARMED A 20,200 ms TIMEOUT AND closePageFx CLEARED NONE OF THEM: measured, click Rec then close and 0 of 1 timers are cleared. When it fires it calls refreshBar() against a DETACHED bar and THROWS NOTHING -- it rebuilds the toolbar on a dead node, silently, and holds the whole openPageFx closure (state, cv, bar, the voxel grid) alive for twenty seconds after the overlay is gone. That is v4434's leak in a different costume: registered with no handle to cancel it by. closePageFx clears every armed timer now, and the callback carries an `if (!host) return` so a missed clear is harmless rather than invisible. *** THE OTHER TWO ARE NEGATIVE RESULTS AND ARE REPORTED AS SUCH. *** shatterTransition removes its own canvas in 136 frames -- 2.1 s at 16 ms a frame, which is its own cutoff and not a guess -- two overlapping transitions both finish and both clean up, and an onDone that THROWS neither escapes nor strands a fullscreen canvas over the page. It has NO cancel handle, so a reader advancing pages fast runs two independent loops; both terminate on their own clock, which makes that a fact about the design and not a leak. And initVoxelGL is ASKED FIRST, returning null when getContext('webgl2') does, so the 2D renderer is a genuine fallback engaged by GL's own answer rather than the default. *** THE FIRST READING OF shatterTransition SAID THE CANVAS IS NEVER REMOVED, AND THAT WAS THE HARNESS. *** Its loop takes dt from performance.now(), so rAF driven synchronously leaves dt at roughly zero and the 2.1 s cutoff is unreachable -- 400 frames in, the canvas was still there. A NEGATIVE RESULT IS WORTH NOTHING UNTIL THE INSTRUMENT CAN MOVE, so the gate now owns a clock and one frame is one 16 ms tick. That is the fourth instrument in two rounds to measure itself instead of the code. *** AND ONE SABOTAGE READ ZERO RED BY REPEATING THE BUG INSIDE THE CHECK FOR THE BUG. *** The guard check fired every armed timer and passed if none THREW -- but running refreshBar() against a detached bar throws nothing, which is precisely why the leak was invisible in the first place. Dropping the guard cost NOTHING. It measures MUTATION now: the detached bar must still have the same 8 children and the same markup after every timer has fired. tools/ship/pageFxOverlay-selfcheck.mjs grows from twelve checks to nineteen, in seven sections. NO NEW GATE FILE, and the sweep ledger's own invariant is added.length === swept -- so a closing accounts for gates that APPEAR and *** THE LEDGER CANNOT SEE A GATE GROW. *** This round's entry is recorded as a zero-sweep closing carrying the widening in a field the accounting does not read, rather than forcing a malformed entry to make the point. WHAT IS STILL NOT CLAIMED: that GL resources are released -- nothing in fx/voxelize/voxelRender.js disposes anything, so reclamation rests on the detached canvas being collected, and this harness has no GL and cannot measure context lifetime, so that is recorded as a fact about the file rather than asserted as a leak; or that jsdom is a browser, since its 2D context here carries pixels and draws nothing. Twenty sabotages, all RED by name, three files md5-identical. The tree stands at 1475 gates. Full changelog on docs/CHANGELOG.md.
