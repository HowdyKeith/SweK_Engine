...
const BRAIN_BUILD = "v4351";   // v4351 -- *** AN INDIRECT DISPATCH, AND A SECOND SABOTAGE IN THREE ROUNDS THAT PROVED NOTHING. *** This shipped as v4339 and is v4351: main jumped from v4338 to v4350 while this round's sweep ran -- the other branch's own preflight caught a fifth number collision and skipped clear of it -- so 4339 through 4349 are unused and this round supersedes main's head rather than filling a hole below it. A marker that went backwards would read as old code wearing a new label, which is the failure the version check exists for. The correction first: sabotage AB -- the "this buffer must have usage indirect" guard removed from gfx/device.js -- went 0 RED on its first run, exactly as v4336's sabotage U did. The gate only ever handed dispatchIndirect a correctly-made buffer, so the guard was unreachable and the check beside it was empty. A check that hands it a plain storage buffer was added, and only then did removing the guard cost anything (2 red). Twice in three rounds, which is a pattern rather than an accident: a refusal is not tested by code that never triggers it. The round: gfx/device.js has always had drawIndexedIndirect, so the GPU could decide how many INSTANCES to draw -- and the number of INVOCATIONS was still a JavaScript number. pass.dispatchIndirect(pipeline, buffer, byteOffset) reads three u32 (workgroupsX, Y, Z) when the command runs, not when it is encoded; WebGL2 refuses it by name as it refuses every compute call, and the null recorder records it. render/physicsTsl.mjs makeDispatchSizerTsl is one lane that reads a count another pass produced and writes those three numbers. Measured on WebGPU, four passes on one frame's encoder: the sweep writes 1024 exponents, the tally counts 670 of them atomically, the sizer divides by the workgroup size, and the mark pass runs 704 invocations -- 11 x 64, none past the edge -- with NOTHING coming back to the CPU in between. Seed the same buffer with 64 or 200 instead and the same encoded command runs 64 or 256 invocations: it is the buffer that decides, not the caller. *** AND THE SHELL OWNS THE DECLARATIONS, WHICH IS WHAT MAKES A READ-ONLY VIEW POSSIBLE. *** three declares the tally as atomic<u32> in the SIZER's module too, because the flag lives on the node rather than on the use; the shell declares it a plain read-only u32 there and the device runs it. One buffer, two views -- a binding is memory, and the atomic is how a shader touches it. Sabotaged and measured: dispatchIndirect dispatching a fixed 1x1x1 instead of reading the buffer (2 red -- 64 invocations where the buffer said 704, and the seeded runs stop moving), and the usage guard removed (2 red, after the check above was added). WITH THIS, EVERY SHAPE render/gpuDriven.mjs's CULL PASS HAS IS TRANSPLANTABLE: a read, a write, an atomic count, workgroup-shared memory behind a barrier, and a dispatch sized by a buffer. Not built and said plainly, because it is now the only thing left in this arc: no round has written the CULL ITSELF as a TSL graph and held the fleet's own picture to it -- the eight sections demonstrate each shape beside the real pass rather than replacing it. Also unbuilt: a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; and every number the rig has not signed.  *** AND THE SAME PER-BODY COUNT WENT WRONG ON main A THIRD TIME. *** world/orreryFleet.mjs records `three: 2` where `git log -- WebGLEngine/vendor/three` returns 1. Corrected at v4330, reverted by a merge and re-applied at v4336, reverted and re-applied here -- three times, each time red on main with no register naming it. It is a mechanism, not an accident: the value lives on two branches and the one that rewrites that file most often carries the stale number, so every merge from it puts the wrong one back. This branch cannot fix the other one; what it can do is say the number out loud in the file, which it now does, so the next merge conflict is read rather than resolved by side. The tree stands at 1413 gates. Full changelog on docs/CHANGELOG.md.
