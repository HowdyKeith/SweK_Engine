...
const BRAIN_BUILD = "v4251";   // v4251 -- *** CUT A ROD SQUARE-ON AND YOU GET A CIRCLE; CUT IT AT 60 DEGREES AND YOU GET AN ELLIPSE EXACTLY TWICE AS LONG AS IT IS WIDE -- MEASURED 2.004 AGAINST A PREDICTED 2, WITHOUT THE MATERIAL EVER BEING TOLD THE ANGLE. *** render/rebar.mjs is a rebar cage and a bedding-plane stack as solid fields of (x,y,z): no rod list, just a modulus, so the cage fills space. The exposed section matches r/cos(phi) at 0/30/45/60/70 degrees to 9.49e-5 m against a 1.00e-4 m sample spacing, its WIDTH stays 0.01196 at every angle while only its length grows, rod spacing on the cut matches pitch/sin(theta) to 3.99e-3 m, and the same closed form predicts bedding stripes on a second material to 0.1245 m. The gate's own first predictor was wrong by an exact factor of two and said so. Four sabotages, each grep-confirmed applied and restored md5-identical: half-rods escaped four of five shape checks and were caught by absolute size alone, and the width control was reached by none of them -- rebarAt takes no normal, so no edit to the module can make width vary with cut angle. Full changelog on docs/CHANGELOG.md.
