...
const BRAIN_BUILD = "v4386";   // v4386 -- *** THE SHIP RITUAL WRITES THE SENTENCE THAT HIDES THE MODULE THE ROUND JUST BUILT. *** v4380's register audit left three findings and v4383 repaired one; this is the second. tools/ship/referenceKind-selfcheck.mjs has been red on a ratchet since v4084 -- "the prose-rescued population may only SHRINK", 222 against a ceiling of 181 -- and its register line was an annotation about SWEEP BUCKETING rather than about the gate, so the number nobody had read was a live, rising one on a check whose own words are "A RISE MEANS A NEW ORPHAN IS BEING HIDDEN BY A SENTENCE". Read here. THE FIRST THING THAT HAPPENED WAS THAT MY PLAN DIED ON THE GATE'S OWN OUTPUT: I predicted in writing that most of the 222 would be comment-only and designed the round around splitting prose from code, and the gate already reports that route as measured and refused -- "only 31 of 191 are comment-only, the rest are blurbs and where-fields in string literals" -- along with two others. RUNNING A RED GATE BEFORE DESIGNING ITS FIX COST ONE COMMAND AND SAVED THE ROUND. What the population actually contains, measured: THREE OF THE MODULES ON IT ARE THIS BRANCH'S OWN -- render/img2three.mjs, render/divineEye.mjs, mesh/carve.mjs -- and all three have exactly ONE rescuer, tools/ship/gateSweep.mjs. Those are the SWEEP CLOSINGS, one paragraph per round that adds a gate, REQUIRED BY THE SHIP SKILL, each reading "X-selfcheck guards render/X.mjs". So a round that builds a module, gates it and follows the ritual has documented its own module out of the orphan census, and nobody chose it: it is this gate's "documenting a module is the single commonest way to hide it" with the documentation MANDATED. FIXED BY THE ROUTE THE GATE NAMES AS PROGRESS -- WIRE IT -- AND THE PRECEDENT IS EXACT: render/ssimWindowed.mjs came off the unwired register at v3693 because ssim-compare.html imported it, so render/divineEye.mjs is imported by that same page now, and the population is 221. *** AND THE PAGE EARNS THE MODULE RATHER THAN HOSTING IT, WHICH IS THE DIFFERENCE BETWEEN WIRING AND GAMING A NUMBER. *** Its subject is "what a whole-image number cannot say" and the port answers with two more numbers that cannot say it, both measured here across all four defect sizes the page offers. I PREDICTED IN WRITING THAT THE PORT'S 64x64 DOWNSAMPLE WOULD DILUTE THE DEFECT AND READ HIGHER THAN global ssim(), AND IT DOES NEITHER: the two agree to within 0.00048 at every size and the sign of the difference is not even consistent, because a box average preserves the mean, variance and covariance a single-window SSIM is built from. THE SIMPLIFICATION THAT LOOKED EXPENSIVE COSTS NOTHING. The other one costs everything: the port's 96x96 Sobel edge overlap reads 1.00000 on the SMALLEST defect and 1.00000 on the BIGGEST while the two in between read 0.99954 and 0.99977, so it does not even ORDER them -- 94.5% of cells are edges at threshold 0.12 on a full-frame sinusoid, and inverting a region moves no Sobel response at all because 255-v has the same gradient MAGNITUDE as v. A number that reads a perfect 1.0000 with a quarter of the image inverted is exactly what that page exists to show, and it is reported beside its edge DENSITY because without the density it is unreadable. *** THE NEW RATCHET'S FIRST DRAFT WAS DEFEATED BY A SENTENCE, WHICH IS THE FOURTH TIME IN THIS THREAD THE DEFECT HAS LANDED ON THE FILE STUDYING IT. *** Written as "the closing is the module's ONLY rescuer", it stayed GREEN under the sabotage that removed the import and left the page's two prose paragraphs standing -- because the paragraph made the page a second rescuer. A check about modules hidden by a sentence, hidden from its own subject by a sentence, caught by sabotaging it rather than by reading it; the property is now "the closing is ONE OF the rescuers". Three sabotages, all logged in the gates they belong to: the two rows deleted with the import left in place (exit=1, 2 red -- and THE SOURCE CHECK PASSED, v4379's finding reproduced exactly, which is why that section LOADS the page instead of reading it); the import removed with the prose left (exit=1, 3 red in divineEye-selfcheck and the ritual ratchet at 3 against 2, the leading slash in "/render/divineEye.mjs" being the only thing that separates the import from the paragraph); and the first draft of the ratchet, above. ALSO MEASURED: the FOURTH route to the changelog-versus-load distinction, never among the three tried before although v3223's law has been on the books since -- 39 of the 221 are rescued ONLY by a register. It does not reach 181 either, and four routes failing is itself the answer: 221 is a real diffuse population, not one hidden class waiting to be subtracted. UNCHECKED AND SAID PLAINLY: the ratchet is NOT lowered and the ceiling is NOT raised -- wiring one module moved it by one, render/img2three.mjs and mesh/carve.mjs are still gate-only and named, no page builds a three.js object tree to flatten, and inventing one to move a number would be the opposite of what the wire route means; whether these two numbers hold on a photograph rather than a synthetic pair; and whether graveyard's own scan should learn any of this, which is still the replacement this thread has refused four times for reasons it wrote down. The tree stands at 1424 gates. Full changelog on docs/CHANGELOG.md.
