...
const BRAIN_BUILD = "v4264";   // v4264 -- *** THE CONSUMER IS REAL AND THE OFFERED ALGORITHM IS WRONG FOR IT, WHICH IS A DIFFERENT ANSWER FROM v4262's. *** Backlog #138 arrived as "take novalain/gpgpu-odd-even-transition-sort"; v4262's rule says measure the consumer first. The consumer is unambiguous -- Gaussian splats are alpha-blended with the depth buffer off, so they must be drawn back to front, and BOTH engine/SplatRenderer.js and render/SplatRenderer.js re-sort every splat on every camera move. Both ran the identical line: Array.from(idx).sort((a,b) => keys[a]-keys[b]), boxing every index into a plain Array once per frame and paying a call per comparison. MEASURED: 3.25 ms at 10K splats, 45.66 at 100K, *** 255.87 ms at 500K -- 3.7 fps, 16x over a 60 fps budget *** -- while render/SplatRenderer.js's HEADER CLAIMS ">500K splats interactively" and its own comment beside the sort says "for typical N=32K it's fine; we accept the cost". The file disagreed with itself and the header was the false half. engine/ called the same line a "CPU radix-style sort", which it was not. render/splatSort.mjs replaces it with a real two-pass 16-bit LSD radix sort on an order-preserving float-to-uint mapping: *** 14.43 ms at 500K, 17.7x faster, 69 fps, inside the budget *** -- and producing the SAME PERMUTATION index for index, asserted against the old sort at nine sizes rather than eyeballed, so the render is unchanged by construction. THE REFUSAL, priced in compare-exchanges rather than big-O: odd-even transposition needs n passes of n/2, so 500K splats is 1.25e11 compare-exchanges against 9.5e6 -- *** 1.3e4x more work. *** It wins only with O(N) processors and a browser has none. What was taken is the premise -- that this sort should stop being a comparison sort -- and not the algorithm. *** SABOTAGE D WENT 0 RED AND IT WAS THE ONE I HAD NAMED "the sabotage that would ship": *** allocating the scratch per call is invisible to a timing check, because even allocating a 65,537-entry histogram every frame stays inside the budget. Speed was the wrong instrument for an allocation claim; the gate now sentinel-fills the caller's scratch and asserts the function wrote into it. Sabotage C also broke differently from my prediction and is recorded as it ran. Five sabotages, 5/3/2/1/1 red. Full changelog on docs/CHANGELOG.md.
