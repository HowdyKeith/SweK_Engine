...
const BRAIN_BUILD = "v4350";   // v4350 -- THREE ROUNDS' WORTH, AND THE THIRD ONE ENDS A RED THAT HAS BLOCKED EVERY SHIP THIS SESSION. (1) THE HELPER v4333 ADDED HAD EXACTLY ONE CALLER, WHICH IS THE SHAPE IT WAS WRITTEN TO REMOVE. cubeBake's six-face walk was lifted out of the skybox and then used by the skybox and nothing else, while proceduralStar and planetSurface went on carrying their own copies of the same loop -- a helper written for one caller is the fault this tree keeps finding in other people's code, committed here one round after arguing against it. Both adopt it now, and the interesting half is WHY THEY HAD NOT. proceduralStar fits and simply had not been changed. planetSurface DOES NOT FIT: it writes FOUR targets -- albedo, normal, roughness, height -- out of ONE surfaceSample per texel, and a single-target helper would have made it sample four times, roughly four times the work TO LOOK TIDIER. A helper that makes its caller worse is not a refactor, so the primitive is now bakeCubemapTargets(size, channelCounts, sample) -- one walk, one sample per texel, N buffers -- and the original bakeCubemap is a thin wrapper over it rather than a second copy. The gate pins the sample COUNT (6*7*7 calls for 6*7*7 texels), that the wrapper equals a one-target call byte for byte, and the planet's four digests. Measured across star, surface and skybox at several sizes and seeds: every digest identical before and after. (2) A BAKE THAT READS THE TREE CAN NO LONGER DELETE WITHOUT SAYING SO. v4335 ran orreryBake --write in this sandbox, which does not carry the rig-only build artefacts, and the bake dropped native/gate_probe, gate_probe.c, libbox3d.a and shim.o -- orrery.json from 3,767,005 bytes to 1,226,434, REPORTING "15 bodies" EITHER WAY. It surfaced two gates downstream in playerShip, because the git economy is built from orrery.json and a smaller world has different ports, so a five-tonne purchase stopped conserving. tools/ship/bakeShrinkGuard.mjs now refuses a write that loses a NAMED entry, and the choice of identities over bytes is the whole design: bytes false-fault on a legitimate shrink AND miss a loss that happens while growing, whereas what actually went wrong is that names disappeared. The walk is shape-agnostic, so it serves orrery.json (bodies as an array) and orrery-fleet.json (bodies as an object) without being taught either, and a deliberate removal passes with --allow-shrink, which PRINTS the refusal it overrode. Driven against the real accident: the guard refuses the exact v4335 write, names all four files, exits 1, and leaves orrery.json byte-identical. (3) *** AND THE STANDING RED WAS NOT A MISSING GPU. *** pathTracerWgsl has failed every verify this session and was reported each time as environmental. It is, and the skip reason was more precise than the summary it was being folded into: "node-webgpu is not installed here -- `npm i -g webgpu` (a Vulkan ICD IS present at /opt/pw-browsers/chromium-1194/chrome-linux/vk_swiftshader_icd.json)". The driver was already on the box, inside the Playwright bundle; the tree's own headlessGpu.mjs already records the licence position and a MEASURED adapter of google/swiftshader. One package later the gate is ALL GREEN, and its own frozen numbers match the run to the digit -- maxDirErr recorded 9.474e-8, measured 9.474e-8. bloomFused, the other native-path gate, stops SKIPPING and runs its real dispatch. THE DISTINCTION THAT MUST NOT BE LOST: this is a SOFTWARE adapter, so it settles "does this execute and agree with the CPU twin" and settles NOTHING about the rig's floats. The three fleet-kernel verdicts in deviceOwed are still owed and still need Galaxina. Also: a sabotage of the new guard went 0 RED and is recorded as a finding rather than dropped -- replacing `if (before == null) return null` with `before = {}` changes nothing, because an empty baseline has no identities to lose either, so my check was aimed at a line that decides nothing. It was replaced with one that reaches a branch that does (a "tolerate a few losses" threshold, which would have passed v4335's four). Not checked and said plainly: the npm install lives in THIS container and does not persist -- a fresh session starts red again until it runs `npm i -g webgpu`, and that is a environment question rather than a tree one. Nothing here was rendered on real hardware. gates: this round adds TWO -- bakeShrinkGuard and versionPreflight -- and the merge with main brings its own, so the count is not this round's arithmetic to claim; enumerateGates says 1413. The tree stands at 1413 gates. *** AND A FOURTH THING, ADDED BECAUSE THE MERGE THAT LANDED THIS ROUND WAS ITS FOURTH COLLISION. *** SESSION_START.md's rule 3 -- never reuse a version number, because "two builds with the same number but different bytes is exactly what jams the peer auto-update fleet-wide" -- WAS ENFORCED BY REMEMBERING IT, and in one session it was broken four times by two lines numbering from their own trees: v4327-v4329, then v4331 and v4332, then v4336. main's own history carries two renumbers for it and this branch carries two more. That is a rule with no mechanism, so tools/ship/versionPreflight.mjs is the mechanism: before a number is spent it reads what origin/main actually carries and refuses a number less than or equal to it, naming the next one that would work. verify.mjs calls it, because a rule written down twice and enforced zero times is what this round is about. All four collisions are replayed in the gate against the main that existed at the time, and all four are refused; a deliberate jump for headroom (v3900's own habit) still passes, because this guards REUSE and not tidiness. It does not lock or reserve, and says so: two sessions can still pick the same free number in the same minute, and only a lock could stop that. What it catches is the case that actually happened every time -- main moved while a round was being built and nobody re-read it. *** AND THE FIRST DRAFT OF THAT GUARD STOOD ASIDE ON EVERY RUN. *** `git show origin/main:...main.js` threw ENOBUFS, because main.js carries the whole round note on one line and is over 2 MB against execFileSync's 1 MB default -- and the catch reported it as "origin/main is not readable here (no such ref, or not a clone with a remote)". A GUARD THAT ANSWERS "nothing to compare" FOREVER, IN WORDS THAT SOUND LIKE A REASON, committed inside the file written to stop a rule going unenforced -- the same species as v3940's browserSafety skip naming a cause that was not true. It was found by running it against a tree whose main was plainly readable and READING THE ANSWER rather than the exit code. The buffer is raised, a read that fails now reports why, and the gate pins the distinction: a buffer failure can never again be reported as a missing ref. *** AND THE NUMBER JUMPS TO v4350, WHICH IS THIS TREE'S OWN HABIT RATHER THAN A NEW IDEA. *** The preflight above was written, wired into verify, and then CAUGHT A FIFTH COLLISION WITHIN THE HOUR: main shipped its own v4338 while this round's v4338 was being verified, and the guard refused it by name ("THE SAME NUMBER, which is two builds with one number and different bytes. Supersede FORWARD: v4339 or later."). Renumbering by one into a line that ships roughly every ten minutes, from a verify that takes twelve, is a race this round has now lost four times. v3900 already solved it and wrote the reason down -- "version numbers were jumped there for headroom rather than reused, because reusing a number jams peer auto-update" -- so this round takes the headroom instead of the next seat. The guard permits it explicitly: it refuses REUSE, not distance, and its gate pins that a deliberate jump still passes. That the fix and the demonstration landed in the same round is not tidy, and it is the strongest thing that can be said for the check: it did not have to wait for the next session to prove it. Full changelog on docs/CHANGELOG.md.
