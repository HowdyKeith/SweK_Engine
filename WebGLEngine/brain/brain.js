...
const BRAIN_BUILD = "v4331";   // v4331 -- *** THE COMPUTE STAGE, AND A CLAIM THIS TREE COULD NOT MAKE. *** The roadmap has called TSL compute the largest unclaimed piece for four rounds; it is built, and the first thing it did was refuse the sentence every round since v4320 has ended with. render/tslSource.mjs computeShell() declares what a gfx/device.js compute module needs -- storage buffers bound BY NAME, one uniform struct after them -- and transplantCompute() carries three's emitted compute shader into it: the generated NodeBuffer_529 becomes the shell's buffer, objectStruct becomes its uniform struct, three's naming is gone, and the module validates and runs. render/physicsTsl.mjs makeLyapunovComputeTsl sweeps r across [3.4, 4.0] into a storage buffer with the SAME lyapunovNodes() the keys and the looks use; render/lyapunovWgsl.mjs lyapunovComputeWgsl is the hand-written twin, the module's own lyapunov() in the same shell, so the twin is shipped code. THE ONE PLACE THE PAIR CONTRACT DOES NOT APPLY, said rather than discovered: WebGL2 has no compute stage, gfx/device.js refuses a compute pipeline without WGSL by name, and there is no GLSL half to hold this to. What it can be held to is the twin and the CPU, and both are. *** AND THE CLAIM IS NOT "TO THE BYTE", BECAUSE THE SUBJECT IS A CHAOTIC MAP. *** Measured on a 64-element sweep, on WebGPU: the generated pass and the hand-written one are BIT-IDENTICAL on every element whose exponent is negative -- 22 of 22, at both sample counts -- and they part on the same FIVE chaotic elements at both counts, by 2.5e-5 after 12 iterations and 4.5e-2 after 448. Two modules compiled separately may round a multiply-add differently, and on a chaotic orbit that one ulp is the whole difference by the end; the rate at which it grows IS the Lyapunov exponent this pass computes, which physics/chaos/logistic.js is a module about. So the gate asserts bits where bits are meaningful, measures the divergence where they are not, and claims NO bound against the CPU's f64 on the chaotic elements -- a tolerance wide enough to cover 0.37 would assert nothing. On the periodic elements it agrees with f64 to 9e-4, and r = 4 reads 0.695892 where the CPU's own 384-sample value is 0.693418 and ln 2 is 0.693147: a finite-sample number at the most chaotic point in the sweep, and no bit claim is made there. THE FINDING THAT WOULD HAVE COST A ROUND: three emits `enable subgroups;` and a @builtin(subgroup_size) it never uses, because its own renderer asks the adapter for that feature -- gfx/device.js never did, and left in, the device refuses the whole module (sabotage S: 6 red, 12 uncaptured device errors, every element zero). The transplant drops both. Also measured first, in the probe rather than in the gate: seed 0.5 lands on the logistic map's fixed point at r = 4 and the exponent reads ln 4, a right answer to a question nobody meant to ask, so makeLyapunovComputeTsl refuses that seed by name. Sabotaged and measured: the subgroups directive left in (6 red, above) and the storage rename skipped so the module keeps three's generated buffer name (6 red -- the CPU line by name, and a buffer nothing is bound to reading zero on the device). The generated pass and its hand-written twin both join the WGSL corpus. Not built and said: a compute pass that READS a storage buffer as well as writing one (this one takes a uniform and writes an array; the fleets' own cull pass reads three); workgroup-shared or atomic work, which three can emit and this transplant has never seen; a MIPPED or anisotropic sample, which the device cannot serve at all; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; and every number the rig has not signed. The tree stands at 1408 gates. Full changelog on docs/CHANGELOG.md.
