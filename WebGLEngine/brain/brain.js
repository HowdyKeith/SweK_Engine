...
const BRAIN_BUILD = "v4453";   // v4453 -- *** THE RULE HAD BEEN SWITCHED OFF POLITELY, AND THE THIRD SIGNATURE IS WHAT MADE IT VISIBLE. *** Keith, after writing off v4450 and v4451: "fix the structure before v4453." v4449's ratchet said YOU MAY NOT SHIP A NEW VERSION WHILE THE LAST ONE IS UNRELEASED. That is exactly right when the ship and the publish happen on ONE machine, and it is UNSATISFIABLE HERE: rounds are built where there are no credentials to publish, and the rig publishes afterwards, by hand. So the previous version was unreleased AT EVERY SHIP, BY CONSTRUCTION -- the gate went red at v4450, v4451 and v4452, and each time the only reachable answer was to raise the baseline. THREE RAISES IN THREE ROUNDS IS A GATE COLLECTING SIGNATURES RATHER THAN ENFORCING ANYTHING, and the escape hatch being sanctioned is what made it invisible: every individual write-off was reasonable and the sequence was not. TWO CHANGES, NEITHER OF WHICH LOOSENS WHAT THE RULE PROTECTS. *** FIRST, THE QUESTION IS ASKED ABOUT MAIN, WHICH IS WHAT v4449'S OWN WORDS ALWAYS SAID AND NOT WHAT IT MEASURED. *** Its text reads "a version that reaches MAIN and never reaches the releases page is a version nobody outside this repo will ever run" -- and it then counted the WORKING TREE's changelog, which gains a round's entry in the same commit as the version bump, long before anything merges. A round sitting on an unmerged branch therefore counted as debt the fleet was owed, when nobody could download it and nobody was missing anything. It reads origin/main's changelog now. THE FALLBACK IS STRICTER AND NEVER LOOSER: if origin/main cannot be read the working tree's list is used, which is a SUPERSET, so a missing tool makes the check OVER-report debt and say which it did. *** SECOND, THE HARD ZERO BECOMES A STATED LAG BUDGET: main may run at most N versions ahead of the releases page. *** N is 3, and the number is argued rather than picked: the failure this whole mechanism was built for is 3 releases across 261 versions, which is EIGHTY-SEVEN TIMES this budget, while two or three rounds between publishes is a workflow rather than a defect. AN UNSTATED BUDGET IS NOT AN INFINITE ONE -- a ledger with no lagBudget FAILS the gate, because a floor with no ceiling is the v4413 defect wearing this file's clothes. *** AND THE BUDGET LIVES OUTSIDE baseline ON PURPOSE. *** The baseline forgives versions already gone by and cannot be back-filled; the budget bounds the ones still to come. If one edit could move both, the escape hatch that swallowed this rule three times would swallow it again one level up, and the gate refuses a maxVersionsBehind found inside baseline by name. Six sabotages, MEASURED 3/3/1/0-then-0-then-1/1/0-then-1. *** THE FOUR-THEN-SIX ZEROS ARE THE FINDING, AND BOTH WERE THE SAME BLINDNESS. *** Swapping the module to read the working tree while REPORTING "origin/main" passed twice. The first check asked `owedSource === "origin/main" || owedDegraded`, which is a restatement of what the thing under test says about itself and a lying label satisfies it. The second compared the count against an INDEPENDENT read of main -- better, and still unfalsifiable, because at the moment it ran THE BRANCH AND MAIN WERE IDENTICAL and no count can separate two identical lists. Closed by making the reader injectable: the gate hands in a main that DIFFERS from the tree, against a fixture ledger with a low enough floor that `owed` comes back NON-EMPTY, and reads which list the arithmetic used. An empty owed-set proves nothing about where it came from. *** AND A NOTE ON HOW TWO ZEROS LOOKED LIKE ZEROS: *** the sabotage runs counted lines matching "^  FAIL", and A GATE THAT CRASHES PRINTS NONE OF THEM. One reported zero in this session was a syntax error in the sabotage itself, read as a gate failing to notice. Sabotages are graded on the EXIT CODE now. WHAT IS NOT CLAIMED: that anything is published. This round makes the gate honest about a workflow it could not describe; it does not put a build on the releases page, and v4452 is still the version the fleet is owed. WHAT IS ALSO NOT DONE, AND IS THE REAL END OF THIS THREAD: the publish still happens on a different machine from the ship. A budget makes that survivable rather than solved, and the thing that would solve it is a ship that can publish -- which needs the credential where the round is built, and that is a decision about secrets rather than about code. The tree stands at 1478 gates. Full changelog on docs/CHANGELOG.md.
