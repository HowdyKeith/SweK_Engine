...
const BRAIN_BUILD = "v4265";   // v4265 -- *** TRAP 3 MEASURED AT LAST, AND THE CONTROL INVERTED THE FINDING. *** tools/ship/swiftShaders-selfcheck.mjs has said since v4234 that "NO comparison here renders at a device pixel ratio other than 1, so trap 3 is argued from the source and never measured". It is measured now: all 17 pointScale-carrying shaders rendered at ps=2 on a real WebGL2 context and compared against the CPU model at ps=2. RESULTS: pointScale changes the picture in EVERY ONE of the 17 (worst change 11 to 166 levels, none inert), so the parameter is load-bearing wherever it is carried and not decoration; 11 agree with the model at BOTH 1x and 2x; and *** NOTHING AGREES AT 1x AND BREAKS AT 2x -- the port carries trap 3 correctly. *** *** THE FIRST RUN SAID OTHERWISE AND WAS WRONG. *** Without a 1x control it reported FIVE shaders "DISAGREEING at 2x" -- glitch, melt, thermal, inkBleed, frosted -- which reads as five ports that are right at 1x and wrong on a Retina display. Re-run with the same comparison at ps=1, every one already disagreed at 1x: they are the known sin-hash set (glitch 12, melt 114, thermal 110, inkBleed 89, frosted 8) and blaming the point scale would have been a fabricated defect. wavePool went the other way, reading 0 at 2x and 5 at 1x, so a single scale can flatter as well as accuse. A MEASUREMENT AT ONE SCALE IS NOT A COMPARISON. Sabotage B reproduces the false finding exactly by hard-coding the control away; sabotage A drops echo's pointScale multiply and the section names echo as the one shader that agrees at 1x and breaks at 2x -- a defect no earlier version of this gate could have seen, because every comparison rendered at ps=1. *** AND THE BATCH ASKED FOR COULD NOT BE A PORT: *** the upstream Metal source is not in this tree and this sandbox has no network, so the 13 unported shaders are not even NAMED anywhere here. Porting a shader from its name is invention, so the count stays 28 of 41 and the round is a measurement instead. Full changelog on docs/CHANGELOG.md.
