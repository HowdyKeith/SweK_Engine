...
const BRAIN_BUILD = "v4384";   // v4384 -- *** THE JUDGE HAS EXISTED FOR NINE HUNDRED ROUNDS AND HAS NEVER SCORED A REFERENCE IT DID NOT MAKE ITSELF. *** render/silhouette.mjs was built at v3337 around img2threejs's rule that a HARD gate cannot be averaged away, and its thresholds were deliberately left null because nobody here had measured one. v4255 built mesh/lathe.mjs and wrote the problem into its own header: revolving a profile makes an object whose front view IS the profile mirrored, "so the front-view IoU against the source image is high almost by construction, and is close to worthless as evidence". Both statements have stood unreconciled for a hundred and thirty rounds -- a veto with nothing honest to veto. The missing piece was never a better rasteriser, it was A REFERENCE FROM SOMEWHERE ELSE, and world/songHeightfield.mjs (v4280) has been sitting on one: an STFT column is magnitude against frequency, which is a radius as a function of height, which is a lathe profile. mesh/songLathe.mjs revolves it. *** WHAT MAKES A SPECTRUM WORTH LATHING IS NOT THAT IT IS PRETTIER THAN A PHOTOGRAPH, IT IS THAT IT HAS AN ANSWER KEY. *** Two references, neither produced by the geometry pipeline: a CLOSED FORM (a rect-windowed sum of bin-centred sines has |X[k]| = A*N/2 at each tone's bin and zero elsewhere -- arithmetic, no transform runs) and a NAIVE O(N^2) DFT sharing not one line with physics/fft.js's radix-2, which needs no closed form and so reaches sweeps and chords. Measured on a three-tone chord at N=256: max |fft - analytic| = 3.176e-13, max |fft - naiveDFT| = 4.943e-13, and the two lathed silhouettes agree at IoU 1.000000 -- the same pixels, and render/silhouette.mjs's own hard-gate combiner ACCEPTS at a limit of exactly 1, the first threshold this tree has ever earned rather than refused. *** AND THE CONTROL IS IN THE GATE BECAUSE IT IS THE CHECK THAT LOOKS MOST LIKE PROOF AND CARRIES THE LEAST. *** A solid of revolution has the same silhouette from every azimuth -- true of ANY lathe, so it says nothing about the audio. Reverse the spectrum, putting every tone at the wrong frequency: rotational invariance still reads 1.000000 while the IoU against the closed form collapses to 0.419142. *** WRITING THAT CONTROL FOUND A DEFECT IN v4255. *** A companion check claimed lathe.mjs's asymmetry() would be blind for the same reason and CAME BACK RED at 0.590698 on a solid that is symmetric by construction: silhouetteMask rasterises at pixel CENTRES and asymmetry mirrored pixel INDICES -- two functions in one file, one pixel apart, never crossed because v4255's fixtures are index-space predicates that happen to be index-symmetric. With the mirror about centres it reads 0.000000. Fixed with an explicit convention rather than a silent switch, since the index convention is right for profileFromMask's axis and moving it would have moved every number two green gates already record. That number is the one lathe.mjs's header calls THE ONLY ONE THAT TESTS THE ASSUMPTION. *** THE CROSSING PUTS A NUMBER ON A SENTENCE songHeightfield COULD ONLY WRITE IN WORDS. *** Its header says a tone moved half a bin gets smeared by the rectangular window; a smear has no size until something measures it. Lathed: IoU against the closed form goes 1.000000, 0.882668, 0.733083, 0.598592 as the offset goes 0, 0.1, 0.25, 0.5 -- monotone, so it is the window and not an accident, and forty percent of the silhouette is the window's fault. Volume is checked against a frustum stack whose constant is (n/2)sin(2pi/n) AND NOT pi: at 64 facets the polygon form agrees to 1.88e-8, which is Float32Array storage noise, while pi is out by 1.61e-3 -- a tolerance 85000x looser, wide enough to swallow a real fault. And the readback is lossy with a closed form of its own: rasterising the solid and reading the profile back recovers the HALF-CELL INTERPOLANT, not the profile, because a pixel row samples the frustum at its own midpoint -- 50.794% error against the profile against 0.794% against the interpolant, a 64-fold gap between unexplained loss and understood loss, and what it loses hardest is the isolated pure tone. Four sabotages, 2/10/3/2 red by name, md5-identical after. Sabotage A is the instructive one: self-normalising the column leaves section 4 reading 1.000000, because scaling both paths by their own peaks cancels exactly -- a reference independent of the TRANSFORM is not independent of a global SCALE, and only the scalloping curve and the sweep readback catch it. UNCHECKED: a real song, which has no answer key at all. Also unwired: no page consumes mesh/songLathe.mjs, so the sculptor's only consumer is still a judge -- what changed is that the judge is no longer marking its own homework. The tree stands at 1423 gates. Full changelog on docs/CHANGELOG.md.
