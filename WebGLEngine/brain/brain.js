...
const BRAIN_BUILD = "v4256";   // v4256 -- *** v4249 SAID THE RAGDOLL FIX COULD NOT BE ASKED FOR THROUGH THE SHIM. IT WAS ALREADY THE DEFAULT, AND THE WORK WAS NEVER RIG WORK. *** #125 was filed on the assumption that touching box3d_shim.c needed a toolchain this sandbox lacks. box3d is C17 with no dependency beyond libm and cc/cmake are present, so it BUILDS AND RUNS AND MEASURES here -- only the WASM packaging needs emsdk. Once run, the header answers outright: b3JointDef.collideConnected is a bool on the shared base and b3DefaultJointDef() is `{ 0 }`, so jointed neighbours have NEVER collided. Measured natively: three overlapping boxes give 24 contacts = 3 pairs x 8, and jointing two pairs leaves EXACTLY 8 -- the non-adjacent pair. So what v4249 measured was non-adjacent self-collision, and box3d's own header names that case and prescribes a unique negative groupIndex per ragdoll. New swk_body_set_filter takes it to 0 contacts / 0.0000 impulse, with the control that group +1 leaves it UNCHANGED at 8 / 0.0442 -- the SIGN does the work, not the filter's presence. The whole public header closure is now vendored with its MIT LICENSE and commit 8441b4a0, closing the box3d half of #61, and the shim compiles offline against it. THE GATE FOUND A LATENT DEFECT: build-box3d-wasm.sh hardcodes its export list while the clang script auto-discovers, and FIVE functions were missing from it -- including swk_contact_count and swk_contacts, which v4249's own measurement runs on. Full changelog on docs/CHANGELOG.md.
