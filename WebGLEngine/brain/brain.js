...
const BRAIN_BUILD = "v4459";   // v4459 -- *** THE CORRECTION REACHED FOUR COPIES OF THE PROSE AND NOT THE PRINT STATEMENT, AND THE PRINT STATEMENT IS THE ONLY COPY ANYBODY RUNS. *** v4458 recorded composeValidate.reportLines() as manufacturing a finding; v4459's first act was to retract that in the module header, the changelog, main.js and brain.js. reportDoors' own reportLines() went on publishing "and TWO return a report anyway, one of them a manufactured finding" the whole time -- and so, it turns out, did the NOT DONE sentence at the end of this very note, which said "the two fabricating formatters are recorded rather than changed" in all three of the places the correction had just been applied to. *** FIVE COPIES, FOUR CORRECTED, AND THE ONE THAT EXECUTES WAS NOT AMONG THEM. *** Both are fixed here and the summary line is built from a RETURNS_BARE_BECAUSE map now, so a reason cannot be corrected in one place and shipped from another. The same front door was also printing a hand-typed eighth row, "tools/roundhouse/knobLiveness.mjs   never", UNDERNEATH the seventh row its own cost table derives at 38.0s -- a duplicate and a retired claim, sixty lines below the paragraph explaining that "never returns" was an observation at ninety seconds promoted to a property. The printer types no member name of its own now: every row is derived from the table. AND THE COST RECORD COULD NOT SAY "NOT MEASURED": cheapFlag: null stood for both "measured, no cheap path" and "never measured at all", so the column is four explicit states (IMPOSSIBLE / MEASURED / NONE / UNMEASURED) and IMPOSSIBLE is DERIVED from the live signature in both directions. *** THAT DERIVATION FAILED ON ITS FIRST RUN AND CORRECTED THE ROUND THAT WROTE IT. *** doorKinds is `export async function reportLines()` with NO PARAMETER too, so v4458's `cheapFlag: "{ live: false } -- HAS NO EFFECT", cheap: 72.3` -- the only entry carrying a flag, a verdict AND a number, and therefore the best-supported-looking row in the table -- was an argument passed to a function with nowhere to put one: the bare call timed twice, 71.0 and 72.3, 1.3s apart. AND SABOTAGE S WENT 0 RED, WHICH IS THE ROUND: moving knobLiveness from UNMEASURED to `MEASURED, cheapFlag: "totalBudgetMs", cheapSeconds: 38.0` satisfied every shape check and nothing failed, because A CONTROL AGAINST "AN UNMEASURED ENTRY CANNOT LOOK MEASURED" DOES NOT CATCH A FABRICATED MEASUREMENT -- which is exactly what the 72.3 was, in the gate written to catch it. So MEASURED stopped being a claim: the record carries the argument itself, the gate applies it, times it and compares the line counts every run (levelClaim 18 lines in 0.00s against 19 bare, orphanTriage 5 against 154), and a member of NEVER_CALL may not be MEASURED at all, because a cheap path nobody can afford to take is one nobody has verified. Eight sabotages: the never row returning 2 RED, the manufactured-finding summary 1, doorKinds' flag 1, levelClaim typed IMPOSSIBLE 1, the fabricated measurement 0 THEN 3 after the repair, a formatter losing its reason 1, a line count off by one 2, the cheap flag ceasing to be a different question 1. ALSO IN THIS ROUND, FROM THE ROUND BEFORE IT: vacuity.mjs names the 0-RED sabotage as ONE SYMPTOM WITH FOUR CAUSES -- an empty collection under every(), an unreachable branch, a guard downstream of the classification it depends on, and a harness that damaged what it measured -- and gives overNonEmpty and emptyOfNonEmpty so the empty case cannot be forgotten; a tree-wide scan for the first cause was REFUSED rather than shipped, because 948 of 1482 gates use the shape and 64% flagged is a census nobody would read. UNCHECKED AND SAID PLAINLY: the BTDF defaults are still not flipped and no rendered image moves; the retired-claim check is a match on two exact words, so a withdrawn claim REPHRASED passes it, kept because it is the check that would have caught the two that actually happened; a wrong cheapArg makes the gate SLOW before it makes it red, since the clock is an assertion and not a budget; knobLiveness-selfcheck stands at its 7 pre-existing reds; definitionGates stands at 23 and 269, verified identical at HEAD~1; and the release backlog is still owed on the rig, where releaseLedger reads 5 against a budget of 3. The tree stands at 1483 gates. Full changelog on docs/CHANGELOG.md.
