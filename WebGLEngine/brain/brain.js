...
const BRAIN_BUILD = "v4121";   // v4121 -- CRT toggle on the ES/EV pages. Two things to keep. FIRST: I had told Keith ev.html was a WebGL page needing preserveDrawingBuffer, and grepping ev.html for getContext finds only "2d" -- six times. The WebGL lives in the ev/ modules it imports. A grep of the PAGE is not a survey of what the page RENDERS WITH, and I reported a conclusion from one. SECOND, the useful shape of the proof: a CRT overlay that reads black is indistinguishable from a dark scene, and the EV system view is genuinely black space. So the gate checks lit fraction AND peak brightness (0.31% lit, peak 637/765) -- the fraction alone would have called a working read a failure, or a failed read a starfield. And it asserts the flag against getContextAttributes() on the LIVE context, not against the source line that asks for it, because a browser may refuse an attribute and reading the file back would never notice. Gate: crtToggle-selfcheck, 30 checks. Full changelog on docs/CHANGELOG.md.
