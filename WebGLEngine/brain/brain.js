...
const BRAIN_BUILD = "v4362";   // v4362 -- *** THE FLEET'S OWN CULL DECISION, GENERATED -- AND THE FIRST DISAGREEMENT WAS THE GATE'S FAULT, NOT THE GRAPH'S. *** This shipped as v4361 and is v4362: the other branch renumbered ITS round to 4361 and landed it while this one verified, which is the second real collision in three rounds and the reason main grew a preflight for it. The correction proper, because the round found it in its own first run: the generated cull and the shipped one agreed on 552 of 768 instances, and the 216 that differed were EXACTLY the culled ones. A difference confined to the frustum-dependent verdicts names its own cause -- render/gpuDriven.mjs frustumPlanes() returns 24 FLOATS, not six vec4 arrays, so `planeF.set(planes[p], p * 4)` filled the plane buffer with nothing and only the plane tests could notice. The graph was right and the harness feeding it was wrong; one line, and the shape of the disagreement is what found it. The round: nine sections and six rounds of this arc have transplanted every SHAPE render/gpuDriven.mjs's cull pass uses -- a read, a write, an atomic count, workgroup-shared memory behind a barrier, a dispatch sized by a buffer -- each beside the real thing. This is the arithmetic itself. render/physicsTsl.mjs makeCullLodTsl is cullLod() as nodes: six frustum-plane tests, a distance to the eye, an angular-size metric of radius over distance, and a threshold ladder returning a LOD or -1 for not-in-view. Every draw in the GPU-driven scene passes through that decision. Measured on WebGPU against the SHIPPED pass (cullProbeWgsl, the same text the real cull composes) over the probe's own 768-instance scene: the same LOD on 768 of 768, and the metric identical to the last bit -- worst difference 0, not a tolerance. AND THE AGREEMENT IS NOT TWO CONSTANTS MATCHING: 216 instances are rejected by the frustum and the 552 survivors land 96 / 246 / 210 across the three LODs, so the two passes agree while distinguishing 768 instances four ways. TWO DELIBERATE DIFFERENCES, BOTH VISIBLE IN THE SHELL AND BOTH SAID: the six planes arrive in a storage buffer rather than in struct Cull's `array<vec4<f32>, 6>` field, because render/tslSource.mjs computeShell builds a uniform struct from scalar and vector fields and a fixed-size array field is a shell feature this round did not build; and the plane loop sets a flag across all six rather than returning on the first rejection, which is the same verdict written as arithmetic instead of control flow. *** WHAT IS STILL HAND-WRITTEN IS THE OTHER HALF, AND IT IS NAMED RATHER THAN LEFT TO BE NOTICED: *** the bookkeeping -- an atomicAdd into array<Cmd>, a STRUCT of five fields with one atomic, and three vec4s per survivor written at a region-major offset. computeShell declares array<T> for a scalar or vector T and has no struct element, so that half is not transplantable yet; the occlusion and fleet variants are unbuilt too, and hizOccluded reads a depth pyramid through a pointer argument no graph here has emitted. Sabotaged and measured: the plane loop shortened to five (2 red -- the six-plane loop is gone from the module and the verdicts part on whatever the sixth plane rejected), and the ladder's comparison inverted (2 red -- 216 of 768 still agree, exactly the culled ones, which never reach the ladder). *** AND THE ORRERY RECORD CAME BACK A FOURTH TIME. *** world/orreryFleet.mjs records `three: 2` where git returns 1; corrected at v4330, re-applied at v4336 and v4351, reverted by a merge each time and reverted again here. v4351's note asked that the next merge conflict be read rather than resolved by side, and the next merge resolved it by side. Notes do not survive a merge; only values do. So this round stops asking and writes down what would actually fix it: either that branch takes the one-character correction into its own copy, or the per-body counts stop being a hand-typed literal and become a bake output -- which costs the drift check its meaning and is therefore a trade, not a cleanup. Until one of those, the line returns as 2 and the gate goes red on main about every other round. Also unbuilt and unchanged: a real GPU's log() and exp() against SwiftShader's; a MIPPED or anisotropic sample the device cannot serve; the double texel fetch three emits around a Discard; three's camera and model matrices inside a graph; and every number the rig has not signed. The tree stands at 1414 gates. Full changelog on docs/CHANGELOG.md.
