...
const BRAIN_BUILD = "v4460";   // v4460 -- *** TWO GATES THAT COULD NOT FAIL, AND THE SECOND ONE HAD BEEN GREEN OVER A WRONG ANSWER FOR FORTY-TWO SHIPPED VERSIONS. *** universeWire-selfcheck computed the cross-engine economy key, WROTE it to tools/ship/universe-hash-expected.json, and then asserted against the file it had just written; the browser half read AGREES because it fetched a file the same run had produced a second earlier. The whole chain agreed with itself -- ALL GREEN, exit 0, and the gate sits in no red register because it was never red. v4316's own sabotage log named the defect AND the fix -- "the file another engine checks against is the one COMMITTED, not the one a page computes" -- true of the intent and false of the mechanism for 144 rounds. The committed key read 2ac2a467 and the tree computes df581d2d, five fresh processes for five. *** AND MY FIRST TWO ATTRIBUTIONS FOR WHY IT MOVED WERE BOTH WRONG, READ OFF A DIFF HUNK RATHER THAN MEASURED. *** I wrote that dependencies changing SIZE moved it, and radiusFor really is the cube root of a body's byte count, so it reads true; the sabotage reproducing the historical change went 0 RED, and measuring it showed why -- 964 bytes added to each of the FIFTEEN bodies in turn moves the hash NOT AT ALL, because `bytes` sets a body's drawn radius and the economy integrates none of it. THE PLANT THAT "FAILED" WAS THE ONE SAYING THE EXPLANATION WAS WRONG, and it is kept as a permanent negative control that must stay 0 RED with the same negative asserted as a number. Bisected file by file and then field by field, the real causes are: v4414 rewrote every body's ARRIVAL DATE, 14 of 15, and a body's orbit is set by its age, so THE WHOLE SKY MOVED; and v4416 added PROVENANCE.txt ATTRIBUTION FILES to six vendored dependencies, where stockOfFiles turns every file into cargo at max(1, round(bytes / 4096)) -- all six are under 1 KB, so each rounds to ZERO and is lifted to a full ton. *** SIX TONS OF `docs` APPEARED IN THE ECONOMY BECAUSE SOMEBODY WROTE DOWN WHO OWNS THE CODE: 4,049 bytes of attribution, six tons of freight. *** Neither round was about the economy and neither did anything wrong; the defect is the gate. It COMPARES now, writing is behind --write, --write itself passes only if the drift record already names the hash being baked, and the bytes are re-read after the comparison so "this gate did not touch the file it grades" is asserted by mechanism -- a comparison against a file the comparer may have written is not a comparison. THE OTHER GATE: sweepCoverage's standingReds is careful in ONE direction -- a nonzero exit code beside a killed process is not a failure -- and nobody ever asked the other direction. A ZERO beside an over-budget reading is not a pass either: quickSweep writes codes[gate] only for the gates it ran, so an over-budget entry keeps whatever status it had the last time it was cheap enough to run. 371 over-budget entries carry code 0 and 360 are stamped "unknown -- before v4408"; run one at a time, TWENTY-TWO ARE RED, EIGHTEEN OF THEM IN NO REGISTER, and TWELVE OF THE TWENTY-TWO NOW FINISH UNDER THE 3,000 ms BUDGET THAT EXCLUDES THEM -- box3dFilter is recorded at 3,763 ms and runs in 89. Among them quickSweep-selfcheck itself, whose fixture names three "cheap green gates" of which two have since gone red. *** AND MY FIRST INSTRUMENT THERE WAS THE WRONG ONE, WHICH THIS TREE ALREADY HAD IN WRITING. *** quickSweep-selfcheck prints "a parallel FAILURE on its own is unconfirmed, not red"; I ran an 8-way parallel pass anyway and quoted its number to myself before a serial pass existed. Two parallel passes of the same 371 gates disagreed on FIVE, and the serial re-run resolved 21 of 43 to GREEN ALONE -- a 49% false-red rate in my own measurement, in the round about false verdicts. The 22 split 17 real assertion failures, 3 needing a GPU this box lacks and 2 network-dependent, and the split is in the record rather than folded into a headline. New: standingGreens, undatedVerdicts, verdictClasses as a partition, and STALE_GREENS_V4460 with every count checked against its own parts. Eighteen sabotages across the two rounds; three went 0 RED and every one was a finding. TWO OF THOSE THREE ARE MECHANISMS vacuity.mjs NAMED ONE ROUND EARLIER AND WERE COMMITTED IN THE ROUND AFTER IT -- the empty collection (sweepCoverage's uncoded class is empty in this tree, so folding it into green changed nothing) and the unreachable branch (backfillStamps guarantees every entry has a stamp, so the fallback never fires) -- and THE FIRST FIXTURE WRITTEN TO REACH THE SECOND STILL DID NOT REACH IT, because it gave every fixture entry a stamp too. An unreachable branch inside the fixture written to reach it, the third time this session. ALSO CLEARED, all pre-existing: vacuity-selfcheck had no sweep closing and vacuity.mjs no registry entry, both owed by the round before v4459; and v4457's slug-wgsl entry declared its page IN A COMMENT with the field left undefined, which is the exact state instruments-selfcheck exists to catch. UNCHECKED AND SAID PLAINLY: df581d2d is Node/V8 on this box and the cross-engine answer is still a person on Firefox or Safari reading the page's line, unchanged from v4316; none of the 22 red gates is FIXED by this round, only named, and the 144 entries recorded nonzero and the whole killed bucket were not re-run; the BTDF defaults are still not flipped and no rendered image moves; and the release backlog is still owed on the rig, where releaseLedger reads 7 of a budget of 3 AT THIS SHIP -- the gate excludes the version being shipped, so v4460 makes it 8; the backlog is v4452, v4453, v4455, v4456, v4457, v4458, v4459. The tree stands at 1483 gates. Full changelog on docs/CHANGELOG.md.
