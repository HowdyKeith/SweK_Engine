...
const BRAIN_BUILD = "v4263";   // v4263 -- *** THE TREE HAS TWO LICENCE REGISTERS AND CODE FELL BETWEEN THEM, OUT OF COMPLIANCE RATHER THAN MERELY UNFILED. *** world/orrery.mjs answers "what did we VENDOR, and is it papered?" over the subdirectories of the TOP-LEVEL vendor/; world/reachedLicences.mjs answers "what did we READ AND NOT VENDOR?". A COPY that does not live under vendor/ is in neither by construction, and nothing in 4,262 rounds had listed that population. It is two files: shaders/ashimaNoise.js (Ashima Arts' GLSL simplex noise, whose own header says "unmodified apart from whitespace", used by badTvPass, aquarellePass, solidTexture, fireMesh and the noise gates) and ui/vendor/qrcode.mjs (Kazuhiko Arase, 2,237 lines) -- *** the second IN A DIRECTORY LITERALLY CALLED vendor and still invisible, because the scan is of the top-level one. *** BOTH WERE OUT OF COMPLIANCE: MIT requires the copyright notice AND the permission notice in every copy, and each carried the copyright line plus a POINTER -- a licence name, or a URL. A pointer is not an inclusion. Measured: 15 MIT permission notices existed in the tree, 14 under vendor/ and the 15th a packaged dependency, so NO engine-source copy outside vendor/ had one; verified against git show HEAD rather than memory. Worse, shaders/ashimaNoise.js STATED the obligation and stated it WRONG, calling attribution "the licence's one requirement" -- understating what it owed in the file whose job was to discharge it. Both notices now sit beside the bytes they cover, LABELLED as reproduced from the standard MIT form rather than fetched, because this sandbox has no network and a licence written from memory and called "verbatim" is v4203's mistake committed again. The nine DERIVED works (aquarelle, doomFire, odometer, the ashima CPU translation, three TheLongSilence techniques) are recorded with their attributions and are NOT called a compliance gap: whether a re-implementation is a "substantial portion of the Software" is a legal judgement, not a grep result. *** SABOTAGE D WENT 0 RED: *** seenBy, the field holding the round's central structural claim, was read by no check -- it is now computed from both registers with the recorded value required to agree, 3 RED after. Five sabotages, 4/2/1/3/4 red. Full changelog on docs/CHANGELOG.md.
