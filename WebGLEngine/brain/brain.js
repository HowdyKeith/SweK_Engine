...
const BRAIN_BUILD = "v4314";   // v4314 -- *** LEVEL 16, THE RUNG ABOVE 15: OTHER PEOPLE, OTHER MINDS, AND A UNIVERSE THAT PERSISTS. *** Numbered v4314 rather than v4302 because the tree's rule is "never reuse a version number, supersede forward" and origin/claude/stale-numbers-sweep already carries v4298 through v4313 of its own (colliding with main's v4298-v4300 and this branch's v4301); the next free number above every branch is this one. (1) A UNIVERSE THAT IS A SEED AND A JOURNAL. world/gitEconomy.mjs keeps the steps as run-lengths and every intervention (gift, credits, stock, commits) at the tick it applied; log() is the whole universe, replay() on a fresh economy reaches the same state hash, and the hash sees every integer the sim owns (stocks, treasuries, prices, cargo, credits, where each ship is -- the gate mutates each one and demands a different hash), positions left out on purpose because two browsers' sines may differ in the last bit. A log with one intervention dropped replays to a different hash and says so. orrery-gpu.html saves the journal to localStorage and a restore reloads and replays it, reporting whether the replay reached the saved hash. (2) TIME IS GIT TIME. Sim day 0 is the day the first body was vendored (git's date, from orrery.json); each market opens on its own body's day, its hauler joins with it, the contributors are there from day 0; no ship trades at a market before its day; the books close across the openings; two histories from one seed are one history and a history replays from its log. The sky stays today's (orbits as built) and the calendar is git's -- said so. (3) TWO BROWSERS, ONE UNIVERSE. world/economyLockstep.mjs puts the economy behind the physics' own lockstep net (physics/box3dLockstepNet.js, unchanged): an input is a list of interventions sent inputDelay ticks ahead with redundancy, a hash per tick checked against every peer's. Over a memory wire delayed two pumps, reordered, duplicated and losing 10% of messages, two economies reach 300 ticks with the same hash at every tick and no desync; at 60% loss the pair STALLS at tick 9 rather than forking; a peer that intervenes outside the wire is caught by both sides at the next tick, by name. Two TABS of orrery-gpu.html (?peer=a, ?peer=b) share one universe over a BroadcastChannel: alone a peer does not move; with the other tab open both run in step and a's commits reach b's journal. (4) THE BRAIN AS A TRADER. world/traderPolicy.mjs seats the docking brain's own policy network (brain/rl/dockPolicy.js FlightPolicy) in a hauler's chair: it scores every candidate leg the economy offers (a good, a destination, both prices, the distance, the treasury -- what greedy sees, no more) and takes the best; trained by the docking brain's evolution strategy over whole economies. THE NUMBER, ON HELD-OUT SEEDS: greedy 69,262 credits over 60 days, the learner 86,821 -- learning beats greedy at 125% (168% on its training seeds, which flatter it), 145 parameters, 14 iterations, two seconds; tools/ship/trader-policy.json carries the parameters and the numbers. The gate does not require the win; it requires the number. (5) THE UNIVERSE REACTS TO GITHUB. world/commitsFeed.mjs turns commit records into "commits" interventions: each commit a ton of source made at that repository's market and its base value minted, journaled, replayed, deduplicated; a repository that is no body here is skipped with its reason. The live source is a fixture and says so -- the ai-bridge does not yet expose commits. THREE GATES: universeJournal, economyLockstep, traderPolicy. Sabotaged nine ways across the three gates: eight red as measured, one (the hash without cargo) green in the lockstep gate and red in the journal gate's coverage probe, recorded as such. Still unchecked and said: a real WebRTC wire between two machines (ev/p2p.js is the plug; the rig has the browsers); two different browsers' hashes; whether a better learner would win; the sky under history. The tree stands at 1384 gates.
