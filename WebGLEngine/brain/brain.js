...
const BRAIN_BUILD = "v4330";   // v4330 -- *** THE THIRD LICENCE STATE HAD A COLOUR SINCE v4189 AND HAS NEVER HAD A BODY. *** #48, "Little Prince micro-planets: cast the Khronos catalogue as the inhabitants". world/orrery.mjs names three states -- CAPTURED, UNPAPERED, REACHED -- and ui/orreryDraw.js has carried #33ccff for the third since the renderer was written, over an orrery whose fifteen bodies are all directories under vendor/. report() has printed "reached 0" every time it has ever run. The claim a legend entry with nothing in it makes is that this repository reaches nothing, and two registers in the tree hold 189 such sources: world/reachedLicences.mjs's 39 and gpu/khronosSamples.mjs's 150 models. *** AND THE PRODUCER FOR THIS WAS ALREADY THERE, ACCUSING THEM. *** reachedLicences.asBodies() says in its own doc-comment "for world/orrery.mjs"; it was written at v4198 and its only caller was its own gate, so nobody ran the sentence. Run: buildOrrery(asBodies()) -> 39 UNPAPERED, 0 REACHED, because the field that decides the state is `reached` and the shape emitted `vendored: false` -- true, and about a different question. Thirty-nine sources filed as "vendored here with no licence provenance", including the twenty-two whose licences are OPEN and four quoted verbatim two files away, from the function written to prevent exactly that. One field, measured both ways. *** UNBOUND IS A DERIVATION, NOT A STYLING CHOICE. *** A captured body orbits because it was taken in; a source read and not taken never entered, so it is on a flyby -- it passes once and it leaves, and the orbit's SHAPE carries the licence fact. world/orreryReached.mjs places them on the zero-energy conic by Barker's equation solved in closed form through Cardano, and it agrees with kepler.js's own stepRK4 to a worst relative error of 8.8e-15 over 200,000 steps. Perihelion is the only length derived, from the one quantity a not-taken source has here: how many files in this tree exist because of it (takenPaths + citedPaths, 0 for 22 of the 39, 7 for gibbok/animatelo, which therefore comes closest). No radius is derived at all and that refusal is checked -- you cannot see how big a thing you did not take, and a Khronos model's variant count is Khronos's publishing decision, not a size. *** AND THE OBVIOUS TEST FOR "UNBOUND" IS WRONG, WHICH IS THE ROUND'S REAL FINDING. *** kepler.js's semiMajorFromEnergy returns Infinity when E >= 0, so `a === Infinity` looks like the check. A parabola has E EXACTLY ZERO, and exactly zero is the one value a float cannot be trusted to land on: measured over 20,000 states built from the exact parabolic formulae, 4,592 of them -- 23% -- read E < 0 and got a huge FINITE axis, with a worst |E|*q of 6.9e-16, which is rounding and nothing else. So the classification compares against mu/q, the problem's own energy scale, and the sign is never read. *** THE MEASUREMENT OF WHICH MODELS THIS TREE REACHES FOR WAS WRONG TWICE BEFORE IT WAS RIGHT. *** First answer 148 of 150, because the scan included gpu/khronosSamples.mjs and the catalogue counted its own catalogue -- the eighth instance of that trap here. Excluding it: 13, and those are still coincidence, because a Khronos model list is largely the vocabulary of a 3D engine (DirectionalLight 63 as three.js's light class, Box 30 in a BZFlag map parser, Cameras 26 as a UI label, Cube 21, Triangle 12). The narrow rule -- a file that IMPORTS the catalogue naming a model as a complete quoted string -- gives ONE: the Fox, in glb_viewer.html. Gates are excluded as a rule and not as a convenience, which is what keeps this round's own gate from being an exception. 134 of the 150 have never had a licence read and are drawn in their own colour rather than the forbidden one, because failing closed on vendoring is not the same as accusing. Four sabotages, 2/4/4/1 red by name, and TWO OF THEM WENT RED IN THE GATE RATHER THAN THE CODE: section 4's first draft sampled five states and passed a restored sign test by luck at 77%, and the pixel check counted the unread-licence grey at tolerance 26 and scored 309 with nothing drawn -- it was measuring the page furniture. Both replaced by measurements that move. UNCHECKED: whether 189 arcs around a 15-body system reads as context or as clutter. *** AND THE ROUND'S OWN GATE CROSSED A BOUND SOMEBODY ELSE HAD SET. *** gateAxioms-selfcheck asserted that fewer than 20 gates touch a graphics context; #48's browser section made it the twentieth and the check went red, correctly. The repair a bound invites is to edit 20 to 21, and nothing then says which gate moved it -- v4258's finding that a bound is satisfied more easily the looser it gets. So the set is now NAMED and compared for equality: a new browser gate adds itself in the same commit, and a gate that STOPS needing a context is reported too, which a `<` could never do. Confirmed by removing one name: it goes red naming the arrival. The tree stands at 1409 gates. Full changelog on docs/CHANGELOG.md.
