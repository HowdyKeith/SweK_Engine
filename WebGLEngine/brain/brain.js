...
const BRAIN_BUILD = "v4303";   // v4303 -- *** THE SHIP GATE NOW RUNS THE CHEAP TREE, ALL SIX v4297 REGRESSIONS ARE REPAIRED AND RECORDED, AND THE FIRST RUN OF THE NEW SWEEP FOUND SIX MORE REDS -- ONE OF THEM MINE. *** #134, open since v4257: verify.mjs named one selfcheck and ran the physics suites, so a gate that went red in a round shipped under ALL GREEN until somebody swept by hand, which at v4297 was eighteen rounds. This round measured before choosing. All 1,383 gates, 8 workers, 20 s cap: 616 s wall. By bucket, 713 gates finish under 0.5 s (151 s serial), 866 under 1 s (260 s), 1,026 under 3 s (550 s), 1,206 under 10 s; 125 do not finish in 20 s. So tools/ship/quickSweep.mjs runs every gate under a 3 s budget from a timings file it rewrites each run, 8-way, re-runs every red alone (gateSweep.classify is the rule, not a copy), and reconciles against the red register -- redCensus.RED_AT_V4279, UNCONFIRMED_SLOW, the v4297 slow bucket and unmeasured list, 101 gates; the six regressions are deliberately NOT in it. A KNOWN red is listed; a NEW red fails the ship. Measured: 1,028 gates in 91 s, 999 green, 26 known red, 0 false red. verify.mjs calls it, SWEK_QUICKSWEEP=0 skips it while iterating and is not a way to ship, and the ship skill says so. THE SIX v4297 REGRESSIONS: backendParity, gateQuality and staleness were green again by v4300 and no round had said so -- the register rotting in the good direction, v4279's failure mode; copiedOutsideVendor was repaired at v4301; windowsImport is repaired here (roughDiffuseWired-selfcheck imported its scratch module by raw path, pathToFileURL now); postChain is repaired here and the repair is the interesting one -- it said "750" in five places, v4288 added four lines above the SSAO draw, and the gate was red for fifteen rounds over a number that had been right once; the SSAO line is now the line that disappears when SSAO is turned off, confirmed by bloomPass.js binding ssaoProg within twelve lines above it, never typed. gateSweep.REGRESSIONS_REPAIRED records all six with the round that did each, asserted as a set against SWEEP_V4297.regressions. THE FIRST SWEEP'S OWN FINDINGS: 43 phase-1 reds, 33 in the register, 10 outside; alone, four were starvation (fetchCap, gateConfigWrites, splatSort, steamdeckLaunch -- the 36% again) and six were real. gateSweep-selfcheck was MINE: it compared the frozen v4297 record's swept count with the LIVE tree and went red the round after it shipped, when v4298 added a gate -- the gate written to catch regressions was one for five rounds; the record now carries enumeratedAt and the live tree is only asserted to have grown. serverButtons asserted no *universe*.html exists and v4300's universe-gpu.html, the Endless Sky universe on the GPU, matched; it now asserts no GITHUB-universe page. typecheck went red at v4299 on the strength field's untyped parameters in @ts-check files (TS7006, then TS2339); typed. orreryEjecta's baseline moved 68 to 69 because song-globe.html imports three. moduleHistory's "every mention is a comment" needle was a substring, so "y.js" matched "MemoryHeatmapOverlay.js" and the gate's own message; a path needle now, and the check no longer demands the fixture row exist when the stripper handles the comment. staleness is red mid-round by design and green after the ritual. AND THE SCANNER CAUGHT ME FIRST: moduleHistory's missing-import walk reported tools/ship/tools/ship/quickSweep.mjs -- verify.mjs's new import() used a cwd-shaped path where import() resolves against the module, so the sweep would have reported COULD NOT RUN at the first ship; fixed before it ever ran. #87, the phone front door: phone.html lands on a Home tab of tiles built at runtime from its own menu and Tools list, never a second list; the saved tab is a Resume tile rather than an auto-jump; ?tab= still deep-links; and ui/pageGauges.js, the dock the other 62 non-render pages mount, is now on phone.html, view.html and avatarpreview.html, with the sticky header and nav pushed below it by the dock's measured height -- the census of phone-facing pages is read from phone.html's own links. phoneFrontDoor-selfcheck boots it on a 412x915 viewport: 14 tiles for 15 buttons, 11 for 11 tools, the dock ends at 119 px and the header starts at 126. Four sabotages, all red by name; one browser check stayed green under sabotage C and the log says why. #139 is not built: api.github.com is 403 through the proxy, repository pages render contributors client-side, and the GitHub tools are scoped to this repository, so whether any two vendored bodies share a contributor is a rig question, recorded on the item. Unchecked and said plainly: the 357 gates over the 3 s budget are covered by the full sweep and by nothing at ship time; a regression in a 40-second gate still waits for a hand sweep. The tree stands at 1385 gates. Full changelog on docs/CHANGELOG.md.
