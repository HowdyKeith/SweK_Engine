...
const BRAIN_BUILD = "v4333";   // v4333 -- *** MERGED WITH main AT v4330, AND THE MERGE CHANGED THIS ROUND'S OUTCOME, SO IT IS RECORDED HERE RATHER THAN LEFT FOR A READER TO NOTICE. *** While this branch ran, main shipped its own v4327-v4330 off the orrery line, so THREE NUMBERS EXISTED TWICE with different bytes -- the exact thing rule 3 forbids, and main's own history renumbered twice for it. These rounds moved forward to v4331-v4333; nothing was reused and no content changed to fit a number. Two real decisions fell out of it. (1) main independently built a RENDER TSL drawer and put gpu-rig-check and tsl-rig in it, while v4332 had put both in WebGPU: one page, one drawer, so main's filing wins. tsl-rig is the TSL rig, that drawer was named for these pages, and v4332's own note said the render group was waiting on a drawer nobody had named -- it has been named, so the note is answered rather than overruled. (2) THAT FREES TWO SLOTS AND magmap-bench TAKES ONE. v4333 kept it out at 15 of 15 and said the cap was the only reason; the cap moved, so the third fleet bench joins the two this round placed, and all three are reachable. The WebGPU drawer is 14 of 15. Also merged forward rather than re-decided: both sides invented a THIRD sweep closing in the same week, main's for its fleet gate and this branch's for the two gates v4331 added. main's shape wins -- `closings` is a list, so a round adds an entry instead of another named term -- and this branch's becomes since4. The existence check that v4331 fixed (a closing could name a deleted file and stay green) is re-applied over `closings`, because main's copy never had it and it now covers since2, since3 and since4 alike. *** AND THE MERGE'S OWN RE-BAKE BROKE A GATE THAT HAD NOTHING TO DO WITH THIS WORK, WHICH IS THE FINDING. *** orreryFleet went red at the merged head on file sizes, so both orrery bakes were re-run. orreryFleetScan is safe here -- it re-measures importers and sizes, 127 entries before and after. `orreryBake.mjs --write` IS NOT: it reads the tree to build the bodies, and this sandbox does not have the ones that exist only on the rig, so the re-bake SILENTLY DROPPED native/gate_probe and its neighbours and took orrery.json from 3,767,005 bytes to 1,226,434. Nothing said so; the write reported "15 bodies" either way. It surfaced two gates away, in playerShip-selfcheck, because the GIT ECONOMY IS BUILT FROM orrery.json -- a smaller world has different ports, so a buy of 5 t stopped conserving and the ledger check refused. That gate passes on origin/main and failed at this head, which is what proved the damage was the merge's rather than main's. orrery.json is restored to main's bake and only the fleet scan is re-run. THE RULE THIS LEAVES: a bake that reads the working tree must not be run from a tree that is missing files, and this box is missing files by construction. The verify gate cannot catch it, because a smaller world is a VALID world -- what caught it was a gate two steps downstream refusing an arithmetic that no longer closed. Keith: "add hmc-bench and ising-bench too if they do not have a panel that it is in already." They did not, and THE REASON THEY DID NOT WAS A CONFLATION THIS TREE ALREADY BUILT A REGISTER TO PREVENT. Both sat in UNPLACED -- the "deliberately not filed, and here is why" list -- under the reason "OWES A DEVICE VERDICT ... needs one real-GPU run; no verdict has ever arrived", and so did magmap-bench, which v4328 left out ON THAT SAME REASONING AND SAID SO OUT LOUD. That reasoning was mine and it was wrong. tools/render-qa/deviceOwed.mjs (v3339) exists to separate exactly these two questions, and its own header says these pages "appear only as links in a pageSections drawer, which records WHERE a page lives and not WHETHER IT HAS EVER SAID ANYTHING" -- being in a drawer was always expected and never implied a verdict. THE TREE ALREADY HELD THE PROOF: floor-atlas.html is in Sampling & Methods AND in OWES_VERDICT, both at once, and has been for a thousand versions. The old reading was also self-defeating in a way the entry itself recorded: keeping a page out of every drawer leaves typing its filename as the only way to open it, which is why "no verdict has ever arrived" sat INSIDE the reason for hiding it. The register is the accountability; the drawer is the address. deviceOwed still owes exactly what it owed -- all five entries, unchanged -- and hmc-bench and ising-bench are now reachable by clicking. AND THE GATE REFUSED MY FIRST FIX, USING KEITH'S OWN WORDS. Three benches into a drawer of thirteen is sixteen against a cap of fifteen, so I moved celltrack-viewer-gpu.html out to the Cell Tracking drawer, which sits at 1 of 15, calling it a subject filing. pageSections-selfcheck went red: "celltrack holds ONLY celltrack, on purpose -- Keith: 'celltrack is a big button item, it is it's own very large science project'", and paramecium, flesh and mc-flesh were pulled OUT of it because grouping BY SUBJECT rather than by machinery is "the keyword probe that has misled this project three times". So the move was the fourth instance of a mistake the gate was written to catch, and it caught it. Reverted: celltrack-viewer-gpu is filed by MACHINERY and stays in the WebGPU drawer, which is the rule this tree files by and the rule v4328's own panel note claims to follow. WHICH LEAVES THE THIRD BENCH OUT ON ARITHMETIC RATHER THAN ON REASON, and its UNPLACED line now says that instead of the wrong thing it used to say: magmap-bench belongs beside the other two and is out ONLY because the drawer is at 15 of 15. A cap is not a judgement. It is the first page in if the cap moves or a member leaves. consistency-fleet.html keeps its place in UNPLACED and loses the same false reason -- it is unfiled because a consistency-board pair run across two peers is not a subject any drawer is about, which is an open decision rather than a debt. Driven in Chromium against the real page rather than asserted from the markup: the chip renders, the panel is hidden until its own chip is clicked and open after, the slot holds 15 anchors, hmc-bench and ising-bench arrive carrying THE TITLES SOMEBODY ALREADY WROTE FOR THEM ("HMC against its own answer key", "Ising against its answer key") because the mover moves the node rather than rebuilding it, pageSections reports no registry entry without an anchor, and there are zero page errors on load. page-index.json was re-derived, not hand-patched. The panel now states the distinction where a reader will meet it: three of its pages owe a real-GPU verdict, and opening one does not by itself verify anything -- deviceOwed is the register, the drawer is the address. Not checked and said plainly: still nothing opened against a real adapter here, so the drawer's promise remains a promise; this box has no GPU, which is the same reason pathTracerWgsl stays red and the same reason those three verdicts are still owed. gates: this round added none, and none was needed -- the gate that already existed refused the wrong fix by name. THE COUNT MOVED ANYWAY, and not by this round: the merge with main brings its fleet gate alongside the two v4331 added, so the tree stands at 1410 gates. Full changelog on docs/CHANGELOG.md.
