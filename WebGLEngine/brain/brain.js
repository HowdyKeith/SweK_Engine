...
const BRAIN_BUILD = "v4134";   // v4134 -- this file's own bug is in here: the port beacon was read ONCE at startup into a const, so a brain that resolved 8787 at boot kept dialling it after the engine came up on another port -- "GPU Brain says offline" while the brain was running and correct. BRIDGE is a let now, re-resolved when a POLL FAILS rather than on a timer, with BRAIN_BRIDGE still always winning and any move announced. Alongside it: START_NODE_Engine.bat now derives its port from PORT the same way server.js does, instead of guarding 8787 while the server bound something else and asking the production server on 8787 to exit. And the real-terrain fly-in gained a satellite layer and an ADS-B handover, after measuring that the old shot peaked at altitude 200 while airliners sit at 190 -- there was nowhere for satellites to be. Gates: portAgreement-selfcheck, arrivalLayers-selfcheck. Full changelog on docs/CHANGELOG.md.
