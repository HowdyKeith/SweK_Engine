...
const BRAIN_BUILD = "v4325";   // v4325 -- *** WHAT "TO THE BYTE" CANNOT SEE, AND THE SECOND SHELL. *** The correction first: every round since v4320 has said the generated pipeline draws the hand-written one "to the byte", and it does -- but the picture is eight bits a channel, and this round measured what that hides. Two rewrites of the hand-written twin's arithmetic that are algebraically equal and NOT bit-equal -- ((i0 / eta) * shape) / i0 as (i0 * shape) / (eta * i0), and exp(log(r) * x) as pow(r, x) -- moved no byte at all: 36,864 of 36,864 pixels still agreed, worst 0. Read the claim as what it says, the PICTURE agrees on every pixel, not as bit-identical arithmetic; it is written into tslRace-selfcheck's header now. Then the round itself: the shell transplant was written for the lit layout and had only ever been used on it, so "any shell" was an untested word. render/physicsTsl.mjs heidlerSpriteShell() is the SECOND -- the fleets' own shipped sprite vertex stage (p, color, uv; no normal, and a uv that is a real attribute rather than the hull's own x and y) with the {{DISPLACE}} hook and its local added, held to that shipped text in both languages -- and makeHeidlerSpriteTsl() is the graph that suits it: the Heidler return-stroke current across the quad's own uv, the same key physics/discharge/heidler.mjs is held to, its knobs packed as two labelled vec4s the way the fleet's looks pack theirs. render/tslSource.mjs: the rename of three's locals is the SHELL's map now (DEFAULT_LOCALS, `locals` per language) and vertexDisplacement() says which locals a displacement reads, so a graph that moves vertices along the normal into a layout that has no normal is refused BY NAME rather than renamed into a variable that layout never declares -- as is a fragment that reads the normal there. tslRace-selfcheck section 5: on WebGPU and on WebGL2 the Pixel race painted by the GENERATED sprite pipeline is the hand-written twin's picture on 36,864 of 36,864 pixels (worst 0), 4,018 pixels away from the race's own bitmap look, about a thousand of them lit by the lightning, and the pick still names the Pixel ships. orrery-gpu.html?tsl=1 now swaps two races into two different shells, each in its own try, so a failure in one leaves the other generated. Sabotaged and measured: the sprite shell's varyings swapped (3 red -- a wrong map is a type error before it is a wrong picture, the page throwing "'xyz': vector field selection out of range"), the graph's uv.y fade dropped (2 red -- 3,416 pixels part from the twin, worst 240), and the transplant ignoring the shell's own locals (1 red -- the refusal simply disappears). Not built and said: the INK layout (a line-list, whose topology no shell here carries and whose fragment has no uv at all to read), a graph that samples a texture into a shell (the fleets' own sprite look does; the transplant refuses textures), three's camera and model matrices inside a graph (still refused -- the shell owns the transform), and every number the rig has not signed. The tree stands at 1407 gates. Full changelog on docs/CHANGELOG.md.
