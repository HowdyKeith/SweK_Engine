...
const BRAIN_BUILD = "v4323";   // v4323 -- *** TWO MORE HAND-OFFS FROM TSL TO gfx/device.js, EACH TO THE BYTE. *** (1) LINEAR SAMPLING. A texture with a LinearFilter changes what three emits -- textureSample through a sampler where a nearest texture was read with textureLoad -- and render/tslSource.mjs already carried three's sampler to the device's one `samp`; tslSource-selfcheck section 3 now proves it on a checkerboard source: the generated pipeline's linear badTv is the hand-written linear pass's on 4,096 of 4,096 pixels (worst 0) on WebGPU and on WebGL2, the picture really blended (a checkerboard's blue landing between its two values), and three's own linear render, row-mirrored, identical to the device's on both -- two samplers, one filter, no difference. (2) THE PAGE. orrery-gpu.html?tsl=1 loads three (vendor/three-webgpu, 3 MB, only then) on the SAME backend as the device, emits the Lyapunov look from the TSL graph in that backend's language, transplants it into the fleet's own shell (transplantIntoShell takes one language now -- a page has one renderer and one device; the gate emits both) and swaps the Chaos fleet's pipeline before the scene is built; the HUD says so, window.__universe.tslLook records the language and that the fleet's pipeline IS the generated descriptor, and tslRace-selfcheck section 3 loads the page here: GENERATED on the HUD, WGSL on WebGPU, no page errors, the identity picture still naming Chaos ships. One correction from building it: the HUD object first captured the flag by value before the swap ran (null forever) -- a getter now. Sabotaged and measured: the transplant no longer renaming three's sampler to the device's (4 red: on WebGPU nothing draws, 0 of 4,096 agree; WebGL2 binds by texture name and stays green), and the page building the descriptor but not swapping it in while the HUD still says GENERATED (1 red: the page records whether the fleet's pipeline IS the descriptor, and the gate refuses the HUD's word without it). Not built and said: three's camera inside a graph (a vertex-stage transplant, still refused); and every number the rig has not signed (tsl-rig.json, rig-timing.json). The tree stands at 1407 gates.
