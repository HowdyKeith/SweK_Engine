...
const BRAIN_BUILD = "v4133";   // v4133 -- the clone button took the DEFAULT branch and never said so, which is why the rig ran v4116 for days while this tree was at v4132: origin/main is v4116 and everything since lives on a feature branch. Nothing was broken -- cloneEngineSource always honoured a ref and always read the version from the cloned tree -- the question was simply never asked, and the result never mentioned the copy was older than the engine asking for it, with both numbers already in its own return. Now there is a branch picker (listSourceBranches + a panel dropdown that passes the ref) and a stale warning that leads rather than trails. The version shown per branch is parsed from the commit SUBJECT and is named versionGuess for that reason; the authoritative one is still read out of the tree after the clone. A public read also survives a rotten token now, scoped to READS so no write ever retries anonymously. Gate: cloneSource-selfcheck stubs fetch and runs the real lister, sabotage-confirmed three ways. Full changelog on docs/CHANGELOG.md.
