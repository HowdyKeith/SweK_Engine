...
const BRAIN_BUILD = "v4504";   // v4504 -- *** THE CORRECTION FIRST, AND IT IS ABOUT NUMBERS. *** This is the branch's first changelog entry since v4458, and it rolls up forty-six ledger rounds that tools/ship/gateSweep.mjs records as since80 through since126 with `at` labels v4459 through v4504. In the same weeks main, from another line of work, shipped its own v4462, v4463 and v4478 through v4481 with different content, and its merge notes counted five such ordinal collisions; so where one of those numbers appears in this branch's ledger and in main's changelog, they name DIFFERENT WORK, and the ledger's `at` on this branch is a round label rather than a shipped version. Main has moved past this branch's base by those commits, which means the ritual's fast-forward is refused by rule and this ship STOPS AT THE PUSH: the merge is not this ship's to make, and the fleet's releases page is untouched by it (v4457 and v4458 are still unpublished; the lag gate is red, and that refusal is the gate working, as v4458 said). THE STRETCH, in the order the ledger holds it. gfx/device.js: texture formats by name with nearest forced on the integer format and depthWrite honoured on WebGL2 (v4459); a declared-but-unused binding refused by name instead of drawing nothing (v4461); a presented canvas read back three ways (v4462); mipmaps on both backends, a per-level blit where WebGPU has no generateMipmap (v4464); optional features negotiated and reported (v4489); and a real defect found under the ticker and fixed, per-draw uniforms on WebGPU where the last uniform() had won every draw in a frame (v4497). Slug text on the device: render/slugDevice.mjs exact against text/slugEval.js through a rasteriser model fitted from the fragment (v4460), ev/esShipLabels.js as the first consumer with an automatic raw fallback (v4463), GPOS pair kerning read (v4485), three OFL faces vendored and papered (v4486), atlases pre-packed at build time with a hash gate (v4487), word wrap (v4488), a rig page for fragment cost (v4490, rig-pending), curved text by strips with a per-strip Jacobian (v4491), bidi shaping and CJK fallback measured and recorded as wont (v4492), the per-frame reupload measured and the cheap fix built (v4493), Slug's fragment as TSL nodes through the widened transplant (v4483, v4484), rotated and perspective text held to a perspective-correct model (v4496), a 3D ticker of glyph bodies on box3d (v4497), one glyph outline morphing into another through vendor/morphicons (v4498), a fill texture inside the glyph with the Doom Fire first (v4500), the fire-filled glyph melting to a puddle with a correction to v4500's clamp where the flipped sample sat at exactly 1.0 (v4501), the ticker on fire with a napalm trail held to a composited key (v4502), a glyph shattering into nine shards each carrying its sub-rectangle so the shader never learns the glyph was cut (v4503). The physics kernels on the device: XPBD's four shader files that nothing had loaded for eighteen hundred rounds now run (v4465); two gates that said there is no GPU here since v3282 corrected, MPM and HMC on Dawn (v4466); one compute path, computeRun (v4467); the probe convention lab-wide with a written contract (v4468); the step loop (v4469); the GPU Brain's kernels exported and graded in the corpus (v4470); a TSL Loop with a bound read from a buffer, which step 6 had called impossible for 140 rounds on an assumption (v4471). The 3D orrery: a lit icosphere (v4473), an orbit camera (v4475), scene labels by the cull's own metric (v4477), and the promotion REFUSED by a table of ten picture facts the 2D page holds (v4478). The git terrain: the treemap as a device heightfield (v4479), Worley biomes as a compute pass with a bit-exact twin (v4480), biome colour and water in both languages (v4481), erosion measured and recorded as a wont-do-yet (v4482). And three passes with CPU twins: the little planet over procPlanet's bake (v4499), the zoom blur toward any centre with GODRAYS_FS run raw beside it, its luminance and depth gates and its 128-to-31 weight measured in numbers (v4504, where the gated threshold was first set BELOW the scene's brightest pixel and the clamp sabotage was blind twice before the scene's edges were painted). Two probes wait on the rig: three-probe.html for a newer three (v4494) and textureBytes for the asset library (v4495). UNCHECKED HERE, said plainly: everything marked RIG-PENDING above; the quick sweep's budgetEvidence red, which is an ordering artefact green on re-run; the fourteen redCensus knowns; and the ledger lag itself. The tree stands at 1526 gates. Full changelog on docs/CHANGELOG.md.
