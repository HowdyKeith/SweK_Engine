...
const BRAIN_BUILD = "v4449";   // v4449 -- *** THE FLEET DOWNLOADS releases/latest, AND FOR 261 SHIPPED VERSIONS IT HAS DOWNLOADED THREE OF THEM. *** Keith: "we want you publish a release per shipped version, so the fleet runs what is actually built." Measured against the GitHub releases API before anything was written: the tree built v4448, the newest published release was v4438 -- TEN VERSIONS BEHIND -- and of the 261 versions in docs/CHANGELOG.md, exactly 3 were ever published (v4296, v4300, v4438), which is 1.1%. *** THE DOWNLOAD CHAIN WAS NEVER BROKEN, AND THAT IS THE FINDING. *** githubBridge.fetchEngineBuild pulls releases/latest, scanDownloads finds the asset, the installer applies it, and every step has had a gate since v3907 -- whose own header already said the remaining work was "on the PUBLISHING side, a release per version, tagged, with the properly-rooted zip as its asset". It pulled v4438 because v4438 was the newest thing anybody published. The ship ritual had eight steps and NOT ONE OF THEM SAID PUBLISH, so publishing happened when somebody remembered, and 1.1% is what remembering looks like over 261 rounds. Step 7 now asks for it. *** PUBLISHING STAYS ON THE RIG AND CI STILL DOES NOT DO IT, WHICH IS A DECISION THIS TREE ALREADY MEASURED. *** The zip is not byte-reproducible: the packer walks a live tree, so commit dbc0855 produced 26,775,683 bytes on the CI runner, 27,424,068 on the rig and 27,766,762 in a third checkout, and ten CI runs died on `gh release create` before v4068 stopped trying. A CI publisher would replace the artifact the rig verified with different bytes for the same commit. So the new machinery records and enforces rather than publishes: tools/ship/releases.json is the ledger of what is actually on the releases page, refreshReleases.mjs rewrites it from the API, and releaseLedger-selfcheck.mjs asks offline -- the tree's own rule that a gate needing GitHub goes red when the wifi does -- whether the LAST shipped version was published. *** THE RATCHET IS ABOUT THE PREVIOUS VERSION, NOT THIS ONE, AND THAT IS NOT A SOFTENING. *** Checked the naive way, "ENGINE_VERSION must have a release" is red throughout every correct ship, because verify runs before the commit and the release is published after the tag; a gate red for the whole of every correct ship is one people learn to ignore, which is how this tree acquired a red register nobody read for thirty-nine rounds. YOU MAY NOT SHIP A NEW VERSION WHILE THE LAST ONE IS UNRELEASED is false only when somebody actually skipped one. Versions at or below the frozen v4448 baseline are written-off debt and cannot be back-filled -- a zip built today for v4301 would carry bytes v4301 never had. *** ONE SABOTAGE OF FOUR FOUND NOTHING AND WAS THE MOST USEFUL. *** Deleting the newest release row moved the reported lag from 10 to 148 and turned ZERO checks red, because the lag is reported rather than asserted: a release the fleet was already running could vanish -- upstream delete, bad merge -- and the gate would have shrugged. Closed with a two-number ratchet the refresh raises, and the same sabotage is now exit=1 with both lines red by name. I also PREDICTED sabotage 1 would name 144 owed versions and wrote that into the log before running it; the gate said 117, because I did the arithmetic in my head over a changelog with gaps in it. Both corrections are in the gate's header rather than quietly fixed. WHAT IS NOT CLAIMED: that any release carries a usable asset -- the ledger records TAGS, and .github/workflows/release.yml is what waits for the zip, unzip -t's it, runs verify_zip.py and sweeps it for credentials on the PUBLISHED archive; that the fetch half of refreshReleases works, since this sandbox's proxy answers the releases API with HTTP 401 and only the pure parse could be exercised, with a real payload; or that v4449 itself is published, because publishing needs the rig and this round can only ask for it. The tree stands at 1476 gates. Full changelog on docs/CHANGELOG.md.
