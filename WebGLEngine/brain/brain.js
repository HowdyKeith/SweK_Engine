...
const BRAIN_BUILD = "v4324";   // v4324 -- *** THE ROADMAP'S LAST LOCAL "NOT YET" -- AND THREE'S CAMERA NEVER HAD TO CROSS. *** The roadmap said a graph with three's camera in it was refused; what a moving graph actually needs is its DISPLACEMENT, and three writes that as statements on positionLocal BEFORE its camera and model matrices (positionLocal = position; positionLocal = ( positionLocal + ( normalLocal * ... object.amp ... ) ); then the varyings, then modelViewMatrix). render/tslSource.mjs vertexDisplacement() lifts those statements and the uniforms they touch; transplantIntoShell() carries them into a host shell whose vertex stage says {{DISPLACE}} -- the fleet's own transform stays the fleet's (the record's placement, the turn, the device's viewProj), three's names become the shell's (positionLocal -> pl, normalLocal -> nl, position -> p, normal -> n, object.amp -> cam.amp), and a shell without the hook, or a displacement reading a uniform the shell's struct lacks, or an unlabelled one, refuses by name. render/physicsTsl.mjs: makeLyapunovLookTsl({ breathe }) adds a position node (the hull along its own normal by amp * (sin 4x + 1)) with amp a labelled uniform, and lyapunovLookShell() is now a TEMPLATE -- its Cam struct generated from its uniform list plus any extra, its vertex stage the fleet's with {{DISPLACE}} where a graph's motion lands, empty when the graph moves nothing, or a hand-written line when a gate wants a twin. tslRace-selfcheck section 4: on WebGPU and on WebGL2 the Chaos race BREATHING by the generated vertex stage is the hand-written twin's picture on 36,864 of 36,864 pixels (worst 0), about 720 pixels moved from the still race, and with amp 0 the generated stage draws the still race exactly -- the displacement is the only difference; the fragment-only transplants (sections 2 and 3, and the page) still hold to the byte through the templated shell. orrery-gpu.html?tsl=1 now breathes: amp animated from the frame clock through the fleet's bind, the HUD saying the hull breathes by the graph's position node. Sabotaged and measured: the displacement dropped from the transplant (6 red: 0 pixels move, ~720 part from the twin on both backends) and three's normalLocal left unrenamed (5 red: a variable the shell never declared, refused by the validator and the device). Not built and said: three's camera and model matrices themselves (a graph that reads them stays refused; the shell owns the transform); a race whose layout is not the lit one (a sprite or ink shell would need its own template); and every number the rig has not signed. The tree stands at 1407 gates.
