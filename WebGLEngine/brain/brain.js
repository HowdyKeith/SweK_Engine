...
const BRAIN_BUILD = "v4260";   // v4260 -- *** REPRODUCIBLE IS NOT ACCURATE, AND MEASURING A VIDEO INPUT SPLIT THE GOAL IN TWO. *** v4188 pointed the whole shader chain at a live camera, which no gate can ever hold still. render/videoFrames.mjs (activetheory/activeframe, MIT -- the premise, not the code) steps a video FILE by seek-and-wait so frame 47 is frame 47 on every run, wired to window.videoFrames.open/run/check. FOUR HEADLESS-CHROMIUM PROBES, and the third and fourth changed what the file claims. PLAY + rAF disagrees with ITSELF: 17 of 20 ticks differ between two identical back-to-back runs. Seek-and-wait repeats exactly: 20/20 frames and 20/20 pixel digests identical. *** AND THAT SAME PERFECTLY-REPEATABLE RUN WAS ON THE WRONG FRAME 19 TIMES OUT OF 20, *** because the clip was 20.3 fps and the plan assumed 30 -- deltas 0,-1,-2,...,-21. Frame-accurate seeking IS achievable when the timing really matches (47/47 at 10 fps, 46/47 at 30) and the failure is always a STEP, never noise, so driftProfile separates rate / drop / offset -- a drop misreported as a rate error sends you to recalibrate fps, which cannot fix it. A HYPOTHESIS I GAVE UP: I expected boundary seeks to be ambiguous in the decoder; they came back 6/6 exact. The midpoint keeps its place for the reason that WAS measured -- floor(t*fps) misreads 137 of 2000 boundary times at 25 fps, 38 at 30, and *** 0 at 24 fps, *** the rate that would have hidden it. So the pixels are the only authority: frames carry their own index, and rVFC mediaTime disagreed with them in 11 of 12 comparisons, always by +1, ON THE FILE THAT SEEKED 47/47 CORRECT. identify() returns a margin, and the gate records that the margin GROWS as the drift entrenches -- it warns about a boundary, it cannot detect a wrong rate. A BUG CAUGHT BEFORE SHIPPING: an all-black frame decoded as a confident frame 0; two sync blocks fix it, and re-inserted it goes 4 RED while the 0..255 round trip stays green. cameraTexture.js now READS the rVFC metadata it discarded since v4188, giving the live camera a dropped-frame count. Sabotage A went 1 RED, which was too few -- section 7 swept only 30 fps and 20 indices, none of them among the boundary times floor() misreads; widened to five rates over 400 frames each, now 2 RED. Five sabotages, 2/4/1/2/2 red. Full changelog on docs/CHANGELOG.md.
