...
const BRAIN_BUILD = "v4379";   // v4379 -- *** THE LIST OF THINGS ONLY KEITH CAN DO HAS BEEN UNREACHABLE FOR 250 ROUNDS, AND THE GATE THAT SAID SO WAS IN THE RED REGISTER. *** Five rounds running, v4374 to v4378, this branch closed its note with "every number the rig has not signed" and never once put the job where he would find it -- which is the exact failure tools/ship/rigJobs-selfcheck.mjs's own header describes: "work that needs Keith's hardware gets described at length in a changelog and then lives nowhere he would ever find it." Going to fix that turned up something larger. v4129 removed the "Only a human at the rig can do these" panel from rig.html at Keith's request, KEPT the list in ai-bridge/rigRunner.js on the stated grounds that fourteen items of recorded reasoning about what each chore UNBLOCKS are "expensive to write and impossible to reconstruct", and forwarded the reader to "that file's own note for where it surfaces now". THAT NOTE DOES NOT SAY, BECAUSE IT SURFACED NOWHERE. Measured at v4379: fifteen entries, served on /rig/list every time the bridge answers, rendered by ZERO pages. The only other mention in the tree is a comment in server.html describing the history. AND THE GATE WAS RED ABOUT IT THE ENTIRE TIME. rigJobs-selfcheck's check named rig.html -- "the page renders title, why and how for each" -- so it went red the moment the panel was removed, and the red register recorded that line as a fact about a deleted panel. Read rather than filed, it said something else: the record is unreachable. That is the second standing red in three rounds of this session's neighbourhood to turn out to be a message nobody opened, after vendoredLicences at v4371. THE FIX, IN THREE PARTS. server.html gains an "Only at the rig" panel with a tab to open it -- the front door, not the page Keith cleared, and his panel stays lost. The gate's check no longer names a page: it asks whether ANY page renders the list, so the surface can move again without going stale twice. And the register entry is REMOVED because it is fixed, not widened. *** AND A STATIC SCAN WAS NOT ENOUGH, WHICH TWO SABOTAGES SAID BEFORE THE LIVE CHECK EXISTED. *** Dropping the payload in the renderer, and dropping WHY from the row it builds, both cost 0 RED against a scan of the page's source, because the strings are still in the file -- and a source that MENTIONS the list is precisely what shipped for 250 rounds. The gate now serves the page with a stub /rig/list, loads it, and reads the panel out of the LIVE DOM: fifteen titles, fifteen unblocks, fifteen hows. Both sabotages then cost 1 red each. It also checks the panel says the bridge is not running rather than sitting on "checking..." forever, because a front door that lies quiet is how this started. THE JOB ITSELF IS NOW ON THE LIST, and it is the one five rounds deferred: re-freeze render/lodRecord.mjs on real hardware. What it unblocks is stated as the list requires -- two shipped pages derive their LOD thresholds from a SOFTWARE rasteriser, and v4378 measured the two software backends 1.7% apart in K, which is a lower bound on what hardware would differ by rather than an estimate of it. How is one command, `node tools/ship/freezeLod.mjs`, on a GPU-backed browser, and tools/ship/shippedLadder-selfcheck.mjs then holds the pages to whatever that machine draws. A panel without a tab would have been the same failure wearing a different hat, so the tab is there and every one of server.html's 47 panels has one. UNCHANGED: the fourteen older entries, which were already well-written and needed a reader rather than an edit; rig.html, which keeps the shape Keith asked for; and every number the rig has not signed -- but the asking is now somewhere he can click. The tree stands at 1419 gates. Full changelog on docs/CHANGELOG.md.
