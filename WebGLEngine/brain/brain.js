...
const BRAIN_BUILD = "v4259";   // v4259 -- *** THE DOOR HAS ONLY EVER OPENED INWARDS, AND THE READER IS WHAT GRADES THE WRITER. *** world/schematicLoader.js has read .schem/.schematic/.litematic since v456 and nothing in 3,800 rounds ever WROTE one, so a build could come into this engine and never leave. world/schematicWriter.js is the other half: a big-endian NBT sink, LEB128 varints, gzip, and a Sponge v2 .schem WorldEdit can //paste -- wired to window.schematic.save(), which reads a box out of the world with world.voxelAt. Because parseSchematic was ALREADY HERE, the gate compares every voxel instead of opening the game: 105 voxels over a non-cubic 7x5x3 fixture, 0 mismatches, with transposed / constant / single-voxel controls proving the comparison can fail. THE ROUND TRIP CLOSES IN ONE DIRECTION ONLY, measured not glossed: voxels->schem->voxels is exact over the nine materials the loader can emit, while schem->voxels->schem kept 9/18 block names (18 real Minecraft blocks collapse to 9 buckets, cobblestone/granite/andesite/bedrock/deepslate all landing on stone). Of the engine's FOURTEEN voxel codes only 9 survive a re-import -- rubble, flowing water, ice, screen and memory have no branch in mcNameToVoxel -- so they are a separate table, named for what Minecraft RENDERS. *** A BUG THIS ROUND SHIPPED AND THEN CAUGHT: *** MC_NAME_FOR is not injective (WATER 10 and FLOWING_WATER 11 are both minecraft:water), the palette deduped by voxel ID, two indices collided on one name, and every plain WATER block re-read as STONE. Re-inserted as a sabotage it goes 4 RED in section 5 and *** 0 RED in the round trip ***. Same shape twice more: dropping the varint continuation bit is 3 RED in section 2 and 0 RED in sections 3-7 (no palette here ever exceeds 13, so every index fits in one byte and a writer that cannot encode 128 passes the whole round trip), and dropping the forced-type table is 5 RED in section 6 and 0 RED in sections 3-5 (the loader's Number() does not care that Width went out as INT; WorldEdit would). Five sabotages, 3/4/3/5/7 red. Full changelog on docs/CHANGELOG.md.
