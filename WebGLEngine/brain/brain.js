...
const BRAIN_BUILD = "v4316";   // v4316 -- *** v4316 -- THE FOUR REMAINDERS, TAKEN AS FAR AS ONE BROWSER AND NO GPU CAN TAKE THEM, AND SAID SO. *** Level 16 closed with four things unchecked: a real WebRTC wire between two machines, two different browsers' hashes, whether a better learner would win, and the rig. (1) THE WIRE. world/universeWire.mjs turns a page's params into the lockstep peer's wire: ?peer=a alone is the BroadcastChannel (two tabs); ?peer=a&webrtc=<rendezvous>&room=<room> is ev/p2p.js's WebRTC transport (two machines, the ai-bridge rendezvous for the handshake, the data channel for the rest), adapted to the same send/onMessage shape and dialled with a longer input lead. The transport is injected, so universeWire-selfcheck drives the mapping with a mock -- the rendezvous named, the room, the peer, the refusals -- and the real wire is RIG-PENDING by name: this sandbox has one browser and no rendezvous. (2) THE KEY ANOTHER BROWSER CAN CHECK. The state hash of a fresh git economy after 400 ticks from seed 7, under the sky of one named day (the orbits depend on the day the ages are measured against -- the first key DIFFERED between Node and the page for exactly that reason, and now names its day), is written by the gate from Node to tools/ship/universe-hash-expected.json; orrery-gpu.html computes its own and prints AGREES or DIFFERS beside it. Here it AGREES on Chromium, which is V8, which is Node -- plumbing, and the HUD says so; the cross-engine answer is a person on Firefox or Safari reading the same line. (3) WOULD A BETTER LEARNER WIN? traderPolicy-selfcheck section 3 trains three more times -- Level 16's narrow net for 14 iterations, the same net for 30, a wide [16,16] net for 30 -- and grades the SPREAD on held-out seeds: 125%, 98% and 122% of greedy. So the Level 16 win is inside the spread of trainings: some beat greedy, some do not, and wider is not better by more than the spread. Reported as the spread, not the best run; tools/ship/trader-policy-spread.json carries the numbers. (4) THE RIG'S NUMBER. gpu-rig-check.html grows a timing table -- ms per frame for both routes at the universe's size (6,241 records) and ten times it (62,500), counts awaited so the GPU has finished -- and a person on the rig saves its JSON as tools/ship/rig-timing.json; rigTiming-selfcheck loads the page here (WebGL2 through the twin: 1.1 ms and 11.0 ms per frame on SwiftShader, which is not a GPU and is labelled as such), grades the table's shape, and reads the rig's file when it exists, printing the ratio between the routes. Without it: RIG-PENDING, and no sentence in this tree may say the GPU-driven path is fast. Sabotaged five ways across the two new gates, every measurable one red (3, 1, 1, 1 lines; one, the rig's second route, measurable only on the rig and said so), in the gates' logs. Still unchecked and said, now with a page load each on the rig: the WebRTC wire itself, the second engine's hash, and the speed. The tree stands at 1388 gates.
