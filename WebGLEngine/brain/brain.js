...
const BRAIN_BUILD = "v4254";   // v4254 -- *** THE TREE'S ONLY PATHFINDER IS 8.24% LONGER THAN A STRAIGHT LINE, AND STRING-PULLING IT IS A NET LOSS IN TIGHT GEOMETRY. *** worker/botPathfinder.worker.js has been the navigation since round 216 with NO gate of any kind; this one drives the real worker by shimming self, not a reimplementation. Its 8-connected path matches cos(t)+(sqrt2-1)sin(t) at nine angles to 8.88e-16, peaking at 1.082392 = sqrt(4-2*sqrt2) at exactly 22.5 degrees, with 0 and 45 degrees measuring exactly 1.000000 as the built-in control. New nav/funnel.mjs adds the string-pulling that grid A* has never had. THREE FINDINGS, and the third reverses the round: a corridor built straight from an 8-connected path is PINCHED TO ZERO WIDTH at every diagonal, so a funnel over it provably recovers nothing until diagonals are expanded through an orthogonal neighbour; on open ground it then recovers 38-66% of the excess with 0 portals missed; but on a wall with one gap the STAIRCASE never enters a wall (0 of 724 samples) while the funnelled path through the SAME corridor enters one at 18 of 616, because A* only ever tests a cell's CENTRE and the staircase was safe by luck of construction. Inset the portals until it is as safe and it is LONGER than the staircase, 319.59 m against 318.39 -- the whole saving was the safety margin. My own funnel first returned the straight line while missing 46 of 49 portals, and the inverted-orientation sabotage reported a PERFECT 100% recovery while missing 63: a length test alone would have ranked both broken versions above the working one. Full changelog on docs/CHANGELOG.md.
