...
const BRAIN_BUILD = "v4327";   // v4327 -- *** AND A SAMPLER -- BUT WHAT FILTERS IS THE TEXTURE, NOT THE SHADER. *** v4326 closed by saying a filtered sample into a shell was unchecked because no fleet look declares a sampler; this round gives a shell one. render/physicsTsl.mjs spriteLookShell takes `sampler`, spriteSampledShell is viewProj with the atlas at binding 1 and the sampler at 2, and render/tslSource.mjs rewrites three's own `<tex>_sampler` to the shell's name, so a FILTERED sample crosses where only a texel fetch could. The claim, tslRace-selfcheck section 7, on WebGPU and on WebGL2 alike: the filtered sprite drawn by the GENERATED pipeline is the hand-written twin's picture on 36,864 of 36,864 pixels (worst 0), and the SAME generated pipeline draws a different picture -- 898 pixels of it -- when a point-sampled texture is bound to it instead, because gfx/device.js picks the sampler by the bound texture's own filter mode and not by anything in the shader. THE FINDING, and it is the same shape one layer up: three decides between textureSample and textureLoad by the TEXTURE's filters, not by the graph. The identical TSL line emits a filtered sample for a LinearFilter texture and a texel fetch for a Nearest one, and in the second case the shell's sampler is declared, bound and never used -- so makeSpriteSampledTsl refuses a Nearest texture by name rather than quietly becoming makeSpriteAtlasTsl. Said plainly because it limits the guard: the sampler refusal is WGSL-side ONLY, and correctly so -- GLSL's sampler2D carries its own sampler, so a sampled graph into a sampler-less shell simply works on the WebGL2 backend; sabotage M measured exactly that, the page's soft path drawing the fetched picture with only the shell's name to say so. The guard exists for WebGPU, where an undeclared sampler is a binding nothing feeds. orrery-gpu.html?tsl=1&soft=1 draws the Glyph race sampled instead of fetched, with a linear atlas bound to it; ?tsl=1 alone is unchanged and still paints three races from graphs. Sabotaged and measured: the sampler rewritten to a name the shell never declares (3 red -- on WebGPU the frame is wrong everywhere, 0 of 36,864 pixels agreeing, worst 255, 12 device errors, while WebGL2 passes unchanged for the reason above), and the shell built without its sampler (3 red -- the declaration, the transplant refusing by name on both of section 7's backends, and the page's soft path falling back). Not built and said: the INK layout (a line-list, whose topology no shell here carries and whose fragment has no uv to read); a MIPPED or anisotropic sample, which the device cannot serve at all -- it makes one sampler per filter mode, repeat addressing, no mip chain; the double texel fetch three emits around a Discard, measured at v4326 and still not fixed; three's camera and model matrices inside a graph, still refused because the shell owns the transform; that render/physicsTsl.mjs is holding shells and looks that are not physics and still wants a split of its own; and every number the rig has not signed. The tree stands at 1407 gates. Full changelog on docs/CHANGELOG.md.
