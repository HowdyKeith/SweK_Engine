...
const BRAIN_BUILD = "v4040";   // v4040 -- new tools/ship/buildEngineCatalog.mjs regenerates engine-catalog.json's builtinDemos from main.js's live DEMO_MODES source (parsed, not evaluated -- entries carry real functions closing over module-scope helpers). engine-catalog.json had never been regenerated by anything: it claimed 58 built-ins, three days stale, corrected to the real 42; launch-index.json regenerated to 407 pages / 49 demos / 34 built-ins, 0 genuine name collisions (the 3 previously asserted were the same staleness bug, not a real main.js labelling bug). Wired into shipRitual.mjs before the launch-index step. Full changelog on main.js's ENGINE_VERSION line.
