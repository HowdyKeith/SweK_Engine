...
const BRAIN_BUILD = "v4397";   // v4397 -- *** v4396 SAID, IN ITS OWN FOOTER, THAT IT COULD NOT TELL WHETHER ITS FINDINGS WERE ABOUT DISCRETE PHYSICS OR ABOUT BOX3D. A SECOND ENGINE CAN, AND THE ANSWER IS ONE OF EACH. *** The same experiments, re-run on Jolt with the damping matched, and physics/backendDivergence.mjs is the list of ways the two backends physics/backend.js calls interchangeable are not -- with numbers, derived from the measurement rather than typed beside it. *** SHARED: THERE IS NO TUNNELLING THRESHOLD IN EITHER ENGINE. *** Jolt is non-monotonic too -- 52 m/s passes through a wall while 100 m/s is stopped, the same inversion box3d showed at 13 and 90. So v4396's correction generalises: a bisection for a threshold returns a property of its own bracket on either engine, and pass-through is a question of PHASE. *** BOX3D'S ALONE: THE HOLE. *** With continuous collision enabled box3d still passes a body through at exactly 34 m/s while Jolt's LinearCast stops all 96 sampled speeds, so "necessary and not sufficient" is a statement about box3d and v4396 was right to ship a counterexample rather than a law. The hole reproduces in the SHIPPED wasm, not only the native build, so the two builds agree where both can be asked. *** SHARED AND DIVERGENT AT ONCE: BOTH ENGINES SILENTLY CAP LINEAR SPEED AND THE CAPS ARE 100 m/s APART. *** box3d 400, Jolt 500, both MEASURED and neither quotable -- box3d's default is assigned in its own .c and Jolt's PhysicsSettings in this build does not expose the field at all. physics/esBox3d.js hands a hull's top speed to whichever backend the router picked, so ev/tools/es-arena.mjs's Fighter at 430 flies at 400 or at 430 depending on a decision nothing surfaces. That is v2468's damping finding in a second place, found the same way and still unchosen by the engine. *** AND THE SENSOR APIS ARE SHAPED DIFFERENTLY IN THE DANGEROUS DIRECTION. *** box3d has a dedicated buffer that holds sensor overlaps and nothing else; Jolt routes them through the ORDINARY ContactListener, so a portable reader must call Body.IsSensor() or it reports every solid collision as a trigger firing -- measured on a sensor above a solid floor, two contact events, one of each. The asymmetry runs the other way for creation: Jolt's Body.SetIsSensor lets a body become a sensor afterwards, which is exactly the setter box3d lacks and the reason v4396's shim needed a second creation entry point. *** AND JOLT NEEDED NO REBUILD AT ALL. *** box3d's half took C, a native build and a PENDING_REBUILD entry still outstanding for the browser; Jolt is a vendored wasm whose sensors, CCD and cap are reachable from a page today. Five sabotages, 4/1/2/3/1 RED by name, both files md5-identical -- and B is v2468's mistake reproduced on demand: dropping the damping match reads 25/96 onset 45 against the matched 19/96 onset 52, an engine difference that is partly a difference between two damping settings. The first Jolt scan of this round really did read 25/96 before the match was made. *** AND THE ROUND NEARLY DESTROYED AN EXISTING MODULE TO FIND ITS BEST RESULT. *** This was first written to physics/backendDivergence.mjs, a file that has existed since v3845 doing the OTHER half of this comparison -- hash trajectories rather than capability limits. `cat >` overwrote it, render/perceptual-selfcheck went red inside one verify, and the module is now backendLimits.mjs. Reading the file it nearly replaced turned up the finding worth the most: physics/backend-qa-check.mjs, the tree's own cross-backend harness, imports the BROWSER loader in Node, calls createWorld without init(), and prints the resulting `call init() first` as `(box3d WASM absent -> Jolt baseline only)`. The wasm is not absent -- box3dNode.mjs loads the same artifact with 45 functions and every physics gate uses it -- so the envelope that harness exists to record has never been recorded, and its own header at line 87 warns about exactly that: `a control that cannot fail`. Reported, filed, and deliberately NOT fixed here, because the repair makes that gate start comparing two engines against a baseline recorded from one. The tree stands at 1433 gates. Full changelog on docs/CHANGELOG.md.
