...
const BRAIN_BUILD = "v4284";   // v4284 -- *** THE BLOOM CHAIN IS THREE FULL-SCREEN PASSES AND WebGL2 CANNOT MAKE IT FEWER. *** Keith asked whether more shaders can move to WebGPU; the census answers first -- 169 GLSL-only, 19 WGSL-only, FOUR in both, and the WGSL side is almost all COMPUTE that WebGL2 could never have run rather than a partial port. Of 44 GLSL files in render/, 27 are geometry. So the target is the passes that WANT compute. render/bloomPass.js binds three framebuffers and issues three draws -- bright, blur-H, blur-V -- each writing a texture the next reads straight back, because a fragment shader cannot hand a value to its neighbour and the only channel between passes is memory. render/bloomFused.mjs fuses all three into ONE dispatch with the intermediate in WORKGROUP SHARED MEMORY, the thing WebGL2 has no equivalent of: 3 round trips -> 1. *** THE CHAIN OF TRUST HAD TO CLOSE OR THE ROUND PROVED NOTHING: *** the fused shader was compared to a CPU oracle, and the oracle to the SHIPPING SHADERS on a real WebGL2 driver, because a CPU model written by whoever is porting shares the porter's misreadings. Measured: oracle vs BRIGHT_FS 0.500/255, vs BLUR_FS 0.494 and 0.487/255 -- half a byte is the FLOOR of an 8-bit readback, not a loose tolerance -- then fused vs oracle at 3.03e-7 relative, 2.5 float32 epsilons, on google/swiftshader. Bit-identical was never available and claiming it would have been a lie: a GPU may contract a multiply and add into one FMA where JavaScript does not. *** NO TIMING IS REPORTED AND THAT IS THE POINT *** -- the only device here is a SOFTWARE rasteriser, fusing passes is a memory-traffic win, and timing memory traffic on a CPU measures the CPU; a number that would look like evidence and be noise is worse than none. AND THE PORT ADDS NO DUAL-AUTHORED PAIR: the census still reports FOUR, because every constant in the WGSL is PARSED OUT OF bloomPass.js at call time rather than retyped. Two of my own claims went red: the table said the chain was THREE draws and the check counted FIVE (SSAO and god rays sit between the blur and the composite, guarded and easy to forget), and the kernel sums to 0.999999 -- reported, NOT corrected, since rounding it would alter every bloomed image for a difference no eye can see. TWO SILENT GAPS FOUND IN THE SHARED HARNESS: renderGlslToPixels draws with an EMPTY vao, so a vertex shader reading an attribute collapses to a degenerate triangle and returns a BLACK FRAME with ok:true (it now returns a distinct-colour count); and it supported only scalar uniforms while BLUR_FS takes two vec2s and a vec4, where an unset uniform reads ZERO and uEyeRect at zero clamps every tap onto texel 0 -- a harness gap that looks exactly like a shader bug. Four sabotages: the apron one row short 2 red with tile-shaped seams; the workgroup barrier deleted 3 red with the peak collapsing 1.7480 -> 0.0495; *** the luma vector rounded in BOTH the oracle and the port goes 2 red while section 3 PASSES *** -- wrong together, seen only by the check against the shipping shader, which is the chain of trust demonstrated. *** AND ONE WENT 0 RED TWICE, BOTH TIMES THE GATE'S FAULT: *** two layers of clamping meant removing either changed nothing, and with one layer gone it was still 0 red because both bright spots sat in the interior so the whole border extracted to BLACK -- an edge check on an edge with no light in it is an assertion that cannot fail, the sixth of that family this session. A third source now straddles the border, 180 of 2880 border samples lit with a control saying so, and the sabotage then reddens the edge check by name. Full changelog on docs/CHANGELOG.md.
