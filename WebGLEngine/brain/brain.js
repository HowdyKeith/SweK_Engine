...
const BRAIN_BUILD = "v4360";   // v4360 -- v4350 added tools/ship/versionPreflight.mjs to give rule 3 a mechanism, and THE VERY NEXT VERIFY IN THE SAME TREE WAS REFUSED BY IT: origin/main carried v4350 because this build had just pushed v4350 there. The check compared NUMBERS, and rule 3 is about "two builds with the same number but DIFFERENT BYTES" -- so a follow-up commit to a round already on main, which is the most ordinary thing that happens after a ship, read as a fleet-jamming collision. THAT IS THE EXACT FALSE-FAULT THIS FILE'S OWN GATE WARNS ABOUT, in the words already written in it: "a guard that fires on the next legitimate number teaches people to skip it, and then the rule is unenforced again with extra steps." Committed inside the guard that says it, and found by using the guard rather than by reading it. The same-number case now compares the BUILDS: identical bytes under one number is what shipping means and passes with a note saying so, one changed byte still refuses, and a pair that cannot be read refuses rather than assuming a match, because "cannot compare" is not "they match". AND FIXING IT TURNED FIVE OF THE GATE'S OWN CHECKS GREEN AT ONCE, WHICH WAS THE SECOND FINDING. The historical collisions are driven through a stub that overrode main's VERSION and let its SOURCE fall through to the real file -- so once bytes mattered, every one of them compared this tree with itself, found them identical, and passed. The checks would have been measuring the working tree instead of the rule, silently, in the direction that looks like success. The stub now supplies two DIFFERENT sources, because a collision is two builds and a fixture that models it with one is not modelling it. Two sabotages: same number always allowed, and unreadable builds treated as matching -- both caught. This round adds no gate; it repairs one. The tree stands at 1413 gates. *** AND THE NUMBER TAKES REAL HEADROOM THIS TIME, BECAUSE v4350 GAVE ITS AWAY. *** v4350 jumped for headroom and then this fix shipped as v4351 -- a plus-one, straight back into the contested zone, and the other line took v4351 within the hour. The guard refused it correctly, which is the sixth collision of the session and the second it has caught. A headroom jump followed by a plus-one is not a headroom jump; it is a delay. v4360 leaves nine numbers of margin, which is the distance the last hour has shown to be the real cost of a verify. Full changelog on docs/CHANGELOG.md.
