...
const BRAIN_BUILD = "v4128";   // v4128 -- two blobulator bugs reachable only from a non-default control. A temporal-dead-zone ReferenceError (wLY read above its const) crashed every frame the moment any skin but the default "native" was chosen -- it hid behind the "!== native" short-circuit, so page-load checks saw the one state that could not fail; the new gate cycles every skin instead. The HUD hamburger was not a drag handle and not inert but a SYNTAX ERROR: \' inside an HTML attribute never parses, copied across three pages. The h1 toggles now too, skipping <a> so the WebGPU link still navigates. The gate's own first version would have "fixed" three working pages that use \' correctly inside JS strings -- it strips script blocks and keeps those three as a negative control. Embers round via a canvas-generated sprite. Full changelog on docs/CHANGELOG.md.
