...
const BRAIN_BUILD = "v4470";   // v4470 -- *** THE ZERO-RANGE SWEEP HAS HAD NO POSITIVE CONTROL SINCE v3313, AND THIS ROUND DROVE IT ACROSS 78 DEVICES AND CAME BACK WITH SIXTEEN CLAIMS. *** tools/roundhouse/zeroRangeSweep.mjs carries a real pre-registration: A, a POSITIVE CONTROL -- the sweep must find optics.airy.airyRingErrFrac = 0, because "if the sweep misses a zero known to be present, it is not measuring what it claims and nothing else it reports counts" -- and B, the bet that it finds at least one FURTHER unregistered exact zero elsewhere. *** A DOES NOT HOLD, AND THE ZERO WAS CURED RATHER THAN LOST. *** v2931 replaced the raw grid estimator with firstMinimumRefined and moved grading from the rounded 1.22 to the exact j(1,1)/pi, destroying the coincidence on purpose; v3314 read that out of the source after v3313 had guessed at the cause and written the guess into a changelog. The gate already asserts the control's ABSENCE and states the cost in its own words -- "the sweep now has NO DEMONSTRATED DETECTION POWER for exact zeros ... UNPROVEN until a replacement control is planted". *** NO REPLACEMENT WAS EVER PLANTED, AND THAT SENTENCE HAS STOOD FOR ELEVEN HUNDRED VERSIONS. *** THE LOSS IS CONFIRMED WIDER THAN THE GATE CHECKS: the gate looks at optics.airy and optics.slit; all FIVE optics modes -- airy, slit, edge, converge, radiusconfusion, 175 builds and 140 error-field readings in 533 s -- produce not one exact zero anywhere. *** B HOLDS SIXTEEN TIMES OVER, WHICH IS THE PROBLEM AND NOT THE PAYOFF. *** acoustics.propagate, chaos.feigenbaum, fdtd.dispersion, freeze.asymmetry, invariants.gzk, invariants.naive, three fields of mpmstep.freefall/noplastic, multigridgpu.window, nuclear.chain, refscan.decision, refscan.furnace, sdfmarch.sphere, splat.detnotsqrt and strokeMorph.morph. Sixteen positive results from a detector with no demonstrated ability to find a positive is worth more than any of the sixteen. *** AND THE OBVIOUS REPLACEMENT CONTROL IS DISQUALIFIED BY MEASUREMENT. *** The natural candidate is the hit B already rested on, splat.integral.isoRollDeviation, which the gate reports as exactly 0 "at dyadic sigma" from five dyadic probes against three non-dyadic. Tested as RULES over twenty values rather than described: "dyadic" is true on all eight points the gate chose and FALSE over twenty; "sigma >= 1" fits neither; "sigma >= 1 OR dyadic" fits all twenty. 1.05, 1.1, 1.2, 1.3 and 1.5 are not dyadic and are exactly zero. THE RECORDED MECHANISM IS NOT WRONG, IT IS INCOMPLETE -- one of two disjuncts, and the second operates only at or above 1 where all three of the gate's non-dyadic probes never went. A SAMPLE DRAWN ENTIRELY ON ONE SIDE OF A BOUNDARY CANNOT SHOW THE BOUNDARY IS THERE. So no replacement control is planted here: a control whose mechanism is not understood is not a control, it is a second unexplained zero standing where the explanation should be. *** THE SCOPE THAT MADE THIS POSSIBLE WAS A COST MEASURED ONCE. *** The gate sweeps FOUR of the lab's 484 device/modes because v2912 wrote "the full sweep is seven values per knob across 642 knobs and did not finish in twenty minutes" and nobody re-measured it -- the same shape v4425 spent a round on one level down, an observation taken once over a budget and therefore never taken again. Measured now: 199 minutes across 78 devices, TWO OF WHICH HOLD HALF THE TOTAL, eight hold 91%, and the MEDIAN DEVICE TAKES 1.5 SECONDS. *** AND THIS ROUND GOT THAT WRONG ONCE, ON ITS OWN DATA, AND COMMITTED IT. *** At 20 devices the top was bell at 89% of the total; at 78 it is blobbodies at 40% and bell is THIRD. The claim was not false so much as measured on a sample too small to carry it, which is corroborationCensus.mjs's complaint quoted approvingly in this same session one round earlier -- "a rate measured on a sample I selected is not a rate". The share is REPORTED now and what is GATED are the properties that do not move with the sample. *** THREE STATES, NOT TWO, EACH WITH A DIFFERENT REMEDY: *** nine devices BUILD NOTHING and their clean result is vacuous; the expensive ones finish; and diffusion RAN 4.3 HOURS WITHOUT TERMINATING and produced no verdict -- slow can be answered with a bigger budget and that cannot. *** PARALLEL HERE, SERIAL IN THE LAST TWO ROUNDS, AND THE REASON IS WHAT IS BEING MEASURED. *** redCensus.mjs's warning -- an 8-way sweep turned seven green gates red by starving them -- is about DURATIONS, and a starved process misses a clock deadline without computing a different float. This sweep asks whether a value is exactly zero. Getting it backwards costs either way: serial here would be four times the wall clock for nothing, parallel in v4425 would have manufactured red gates. Also recorded: the per-device costs are quantised to 100 ms because they were parsed from a progress log, so a 0 meaning "built nothing" is kept distinct from a 0 meaning "faster than the log resolves" (13 devices); and THE FIRST DRAFT OF THAT PARSE WAS WRONG BY FIFTY TIMES -- a regex swallowed the integer part of each duration and fourteen devices reported six seconds of work. IT WAS CAUGHT BY READING THE NUMBER, NOT BY A CHECK, and catching it needs the raw duration rather than the log's, which is named rather than faked with a floor. tools/roundhouse/zeroRangeFull.mjs and tools/roundhouse/zeroRangeFull-selfcheck.mjs, four sections and 24 checks, pure; ten sabotages by name, THREE GAPS FOUND AND CLOSED (the optics control set was EMPTY so an optics-only fixture passed vacuously, vacuousDevices was checked on the wrong field, and the boundary sample needed a minimum span) and ONE EARNED 0: deleting one of five redundant non-dyadic points above the boundary does not change the conclusion and should not, and tightening the threshold to notice it would be pinning a sample size for its own sake. STILL UNMEASURED AND NAMED: 50 of the 128 devices were never reached, and diffusion has no verdict at any budget. The tree stands at 1448 gates. Full changelog on docs/CHANGELOG.md
// v4450 -- *** SEVEN CONTROLS ORDERED BY THEIR COMMIT DATES, AND NOTHING ON SCREEN SAID WHICH ONES OVERLAP. *** Keith: "I can't tell which functions overlap, and which I need to get our release all merged and on main for fleet runs." He is right and the tab was built to make that hard: the Releases buttons arrived one per round -- v1129 the auto-tag publish, v3941 the source clone, v3964 the chain, v4014 Launch, v4133 the branch select -- each appended under the last, SO THE READING ORDER WAS THE WRITING ORDER, which put the two-press safe route last and the unverified shortcut first. Two pairs nearly duplicate each other and no pixel said so: "Get newer source" is the first half of "Clone -> verify", and "Release current engine" is "Publish the verified clone" with the verify taken out. The tab is a numbered route now, steps 1..6, with both shortcuts moved BELOW it under a heading that names which step each one skips -- kept rather than deleted, because a control removed for being dangerous is a control somebody re-adds without the warning. The step numbers are GENERATED BY A COUNTER rather than typed, and the gate reads them off the rendered page and asserts 1..N with no gaps: gateSweep-selfcheck learned that a hand-picked ordinal collides silently when two branches both reach for the next one, and a panel is the same shape one level down. *** THE FINDING NO BUTTON IN THAT PANEL COULD HAVE FIXED, AND IT IS WHY STEP 1 IS NOT A BUTTON. *** createRelease passes `target_commitish: target || undefined` and publishVersion never passes a target, so GITHUB PUTS THE TAG ON THE DEFAULT BRANCH'S HEAD while the zip is packed from a LOCAL folder. Those are the same commit only if the tree was pushed first. Publish from an unpushed tree and the releases page carries a tag naming a commit that does not contain the code in the asset beside it, and nothing in the panel would have said a word. Step 1 says it, in the panel and in the ritual. *** AND v4449'S LEDGER WAS TERMINAL-ONLY, WHICH IS THE DEFECT v4449 FIXED WEARING ONE LEVEL'S CLOTHES. *** That round built releases.json, refreshReleases.mjs and the ratchet, then left all three on the command line -- so step 7 of the ship ritual asked somebody to leave the panel, open a terminal and remember two commands, which is exactly the "somebody will remember" that produced 1.1% over 261 rounds. They are steps 5 and 6 now, with a dry run beside the write. THE REFRESH GOES THROUGH githubBridge RATHER THAN SPAWNING THE SCRIPT, MEASURED: refreshReleases.mjs reads GITHUB_TOKEN from the environment and the rig's token lives in ~/.voxelbridge/github.json, so spawning it would have sent an ANONYMOUS request at 60/hour; the parse and the merge are IMPORTED from that file so there is one definition of what a ledger update is, and only the fetch is the bridge's, because the credential is. It also REFUSES to write when a release the ledger already recorded has vanished from the page -- v4449's sabotage 2 found that case going 0 red, and a refresh that faithfully recorded "they are gone now" would lower the very numbers the ratchet holds. *** THE PANEL IS 640 WIDE INSTEAD OF 440 AND v4102'S RULE IS UNTOUCHED: *** width:100% is still what does the work, the cap only says how far the panel goes when a host offers the room, and swekOverlay takes the width as a parameter with the old literal as its default so no other overlay on the page moved. Measured on the rendered page: card scrollWidth - clientWidth = 0. *** SECOND ITEM: flex:1 1 auto IS NOT AXIS-NEUTRAL, AND #dialsRow IS MOVED BETWEEN PARENTS OF DIFFERENT flex-direction AT RUNTIME. *** Keith: "on server.html the svg grows huge and scrolls down screen." At home the row's parent is a row-direction flex, so v4413's flex:1 1 auto means GROW WIDE -- which is what that round was asked for. panelStage moves the same node into #stageInfo, which is flex-direction:column, and the identical declaration then means GROW TALL, up to the height of the staged panel. MEASURED on one live page moments apart: 720 px tall before the pin, 184 after, 536 PX OF EMPTY COLUMN RECLAIMED. *** SAID PLAINLY: THAT IS A GAP AND NOT A GIANT ROBOT, AND IT IS NOT YET PROOF OF WHAT KEITH SAW. *** The avatar is HIDDEN while a panel is staged (onPresent raises the info tab, which is what hides it), so the 720 px held nothing; and driven headlessly the avatar's own SVG measures 621x147 at home and does not move with the window height at 700/900/1200/1600/2000, so a scene that is itself huge was NOT reproduced. What IS reproducible and unfixed by this round: the surface grows without bound sideways -- 341 px at a 900 window, 621 at 1600, 1005 at 2560 -- around a figure whose viewBox is 60x88 and which letterboxes to about a sixth of that width. THE FIRST FIX FOR THE AXIS WAS WRONG AND THE NEW GATE CAUGHT IT: onRestore wrote rail.style.flex = "", the idiom the handler beside it uses for justifyContent, but this flex lives in the element's INLINE style attribute, so clearing DELETES the declaration instead of falling back to a sheet -- computed "0 1 auto" against a markup that says "1 1 auto", v4413's row silently un-filling itself after the first staged panel. The home value is read off the node at stage setup and put back verbatim. *** THIRD ITEM, A QUESTION ANSWERED BY MEASUREMENT RATHER THAN A CHANGE: *** the debug console showing only long rows and no chips is the chip rule working, not a regression. blkIsChip takes a message under 30 characters, under three words, with no sentence punctuation. Run over the two screenshots: 16 of 16 chips in the box logging serverTick / gaugeAttention / rlTrain, 0 of 5 in the box logging nothing but autoPull sentences of 24 to 101 characters. Nothing changed; the two boxes are logging different things. Recorded here rather than repaired, along with what a repair would cost: Merge x N collapses only ADJACENT identical keys, so a POLLING CYCLE -- A, B, C, A, B, C, which is exactly what a poll produces -- can never merge, and a non-adjacent merge would break the strict newest-first DOM order the block decay walk depends on and says it depends on. NEW tools/ship/releasePanelRoute-selfcheck.mjs, three sections; avatarDock-selfcheck widened from eight checks to eleven. Four sabotages on the new gate, MEASURED 1/1/1/0-then-1 by name, and two on the widened one, 2/1. *** THE ZERO FOUND A DEFECT IN THE GATE RATHER THAN CONFIRMING THE CODE: *** section 2 asked whether "up.gone.length" appeared BEFORE "fs.writeFileSync" in the source text, and `if (false && up.gone.length)` satisfies that with the guard switched off -- an assertion about where text sits is satisfied by a branch that is present and dead. ledgerRefresh takes an injectable fetchRaw and writeFile now and the section CALLS it three times, asking what it did. I also predicted that sabotage would go 2 red and wrote the number down first; it went 1, because STEP() is still called so the medallion still renders. Both corrections are in the gates' headers rather than quietly fixed. WHAT IS NOT CLAIMED: that any of these buttons publishes correctly, since every /github/* and /source-chain/* call is stubbed and must never reach github.com; that the wording is good, only that the overlap is stated; and that v4449 or v4450 is published, because publishing needs the rig and this round can only make it easier to ask for. *** AND THE RATCHET REFUSED THIS ROUND, WHICH IS THE RULE WORKING AND NOT A BUG IN IT. *** v4449 built the rule, reached main and was never published, so releaseLedger-selfcheck went red on v4450 with "OWED 1: v4449" -- the first real outing of a rule written one round earlier, refusing the very next ship. It was not routed around. Asked directly, Keith chose to WRITE v4449 OFF rather than cut a build for it, on the ground that v4450 contains everything v4449 does and one release covers both; releases.json carries raisedAt/raisedWhy saying so, and what is lost is named rather than glossed -- no box will ever run exactly v4449. THAT EDIT THEN CAUGHT A PINNED CONSTANT IN THE GATE ITSELF: the baseline check asserted `=== 4448`, a second copy of a number that lives in releases.json, so a legitimate raise goes red until somebody edits the gate too -- AND EDITING THE GATE'S LITERAL IS INDISTINGUISHABLE FROM THE DRIFT IT EXISTS TO CATCH. The pin is gone; a raise must now carry the round that made it and a reason over 200 characters, the write-off is REPORTED on every run so it can never be quiet, and a second line refuses a baseline at or above ENGINE_VERSION, which is the one edit that would make the ratchet vacuous for every future ship. Three more sabotages, 1/1/1 red by name. The tree stands at 1477 gates. Full changelog on docs/CHANGELOG.md.
// v4447 -- *** v4436 MEASURED ITS OWN ROUGH DIELECTRIC CREATING TWENTY-EIGHT PER CENT MORE LIGHT THAN ARRIVED AND SAID PLAINLY THAT IT COULD NOT SAY WHY. *** Its honest scope: "WHY the single-scatter BTDF gains where the BRDF loses is NOT DERIVED here, only measured and localised." It cleared the reflection lobe against an independently graded module and left the transmission lobe ACCUSED WITHOUT A TRIAL. v4446 built the conductor walk and named the gap in its own scope: conductor-only, so the rough dielectric still had no ground truth. This is the trial. NEW physics/render/dielectricWalk.mjs extends the v4446 microsurface walk to refraction -- Heitz et al. 2016 again, the same paper, nothing vendored -- with a stochastic choice of reflection or refraction at each microfacet by its Fresnel reflectance, and the ray tracked across the interface by FLIPPING THE FRAME rather than by duplicating the walk, because a second copy of that arithmetic is a second place to get the sign of Lambda wrong and v4446 spent a run finding that sign the first time. *** VALIDATED TWICE BEFORE ANY VERDICT IS DRAWN, WHICH IS v4446'S RULE AND IT STANDS. *** At alpha 0.002 the walk gives R = 0.050833 and T = 0.949167 against the exact Fresnel equations' 0.050917 and 0.949083 -- a bounce simulation arriving at a closed form this tree graded rounds ago in physics/render/fresnel.mjs, sharing no code with it beyond the Fresnel call at each facet. And R + T is EXACTLY 1.000000 at every roughness with ZERO STUCK PATHS, which is what makes the split a measurement rather than a normalisation: a total of one with paths still bouncing would prove nothing at all. *** THE VERDICT IS SHARPER THAN THE ACCUSATION. THE BTDF DOES NOT MERELY LACK MULTIPLE SCATTERING -- IT OVER-COUNTS ITS OWN SINGLE-SCATTER LOBE, BY A FACTOR OF FOUR. *** At alpha 1, cosO 0.25, Walter's BTDF reports T = 1.244351. The walk's SINGLE-BOUNCE transmission is 0.306750, and the walk's FULL multiple-scattering total is 0.953675. THE INNOCENT EXPLANATION IS RULED OUT RATHER THAN ARGUED AWAY: had the excess been an absent term, the walk's single bounce would have matched Walter and its complete total would have EXCEEDED him. It is the other way round, and A MISSING TERM CANNOT MAKE THE COMPLETE ANSWER SMALLER. The pattern holds across the sweep -- 1.068211 against 0.738650 and 0.978912 at cosO 0.7, and 1.081830 against 0.747337 and 0.907563 at alpha 0.4 -- and collapses at alpha 0.05, where Walter, the walk's one bounce, and the walk's total all sit within 0.003 of each other. TWO INDEPENDENT METHODS LANDING TOGETHER WHERE THE PHYSICS IS EASY IS WHAT LICENSES READING THEM APART WHERE IT IS NOT; without that row the disagreement at alpha 1 would be two unrelated calculations rather than a finding. *** A SECOND FINDING ARRIVED ALONG THE WAY AND IT IS THE OPPOSITE OF THE CONDUCTOR'S STORY. *** A rougher dielectric TRANSMITS MORE AND REFLECTS LESS: T rises from 0.9475 to 0.9791 and R falls from 0.0509 to 0.0199 as alpha goes from 0.002 to 1. Light trapped in the microfacet valleys eventually gets through, where a rough conductor's trapped light is progressively absorbed. That is asserted as a row rather than remarked in passing, because it is the mechanism that makes the dielectric's multiple scattering INCREASE the total where the conductor's decreases it. FOUR SABOTAGES, MEASURED 6/4/3/0-THEN-1 BY NAME. *** THE ZERO IS v4435'S UNFALSIFIABLE PATH CHECK ARRIVING IN A NEW FILE. *** Counting stuck paths as transmitted cost NOTHING, because on every configuration in the gate the stuck count is zero and the branch never fires -- so the handling could have said anything at all and no row would have known. The repair is v4435's: make it reachable. A bounce cap of one makes every path stick, and then the branch has to be right -- a stuck path is neither reflected nor transmitted, so the total must fall BELOW one, and it does. UNREACHABLE CODE IS NOT CONSERVATIVE CODE; IT IS CODE NOBODY HAS RUN. UNCHECKED AND SAID PLAINLY: NO FIX IS OFFERED FOR THE BTDF. The lobe is convicted and left convicted, which is a smaller and more honest thing than a repair nobody has validated -- and the right repair is not obvious, because the over-count is in the single-scatter term rather than in a missing one, so it is a question about Walter's Jacobian or the height-correlated G2 rather than about adding a compensation lobe. THE BTDF IS ALSO NOT UNUSABLE, and saying only the bad half would be its own kind of dishonesty: it agrees with the walk to 0.003 at alpha 0.05, which is most of what a renderer actually ships, and the failure is at HIGH ROUGHNESS AND GRAZING INCIDENCE SPECIFICALLY rather than everywhere. This is the same UNIFORM-HEIGHT Smith microsurface as v4446, so what it measures is the model this tree's own D and G2 already assume rather than reality; a Beckmann height distribution or a measured surface would give different numbers. The walk is monochromatic and non-dispersive -- one index, no wavelength dependence -- and it models a single interface rather than a slab, so nothing here says anything about absorption inside the medium, which is where a real piece of glass gets its colour. The tree stands at 1475 gates. Full changelog on docs/CHANGELOG.md.
// v4446 -- *** v4445 WIRED THE MULTI-SCATTER TERM IN AND COULD ONLY BOUND THE COLOURED CASE, AND SAID SO IN ITS OWN HONEST SCOPE: *** "a coloured metal is NOT claimed correct, only bounded -- pinning the value would need a random-walk ground truth on a GGX microsurface, which is NOT here and is the honest next step." This is that walk, and the bound becomes a number. NEW physics/render/microsurfaceWalk.mjs implements Heitz, Hanika, d'Eon and Dachsbacher 2016, the uniform-height Smith microsurface -- a paper rather than a repository, nothing vendored, thirty lines of walk. IT IS INDEPENDENT BY CONSTRUCTION: it simulates bounces on a microsurface and never consults a table, an albedo fit, or any part of the machinery it is being used to check. *** A GROUND TRUTH NOBODY CHECKED IS WORSE THAN NO GROUND TRUTH, SO IT IS VALIDATED TWICE BEFORE ANYTHING IS CLAIMED FROM IT. *** FIRST, CONSERVATION, WHICH IS A LAW AND NOT A FIT: with Fresnel identically one -- a lossless white conductor -- the walk returns EXACTLY 1.000000 at alpha 0.16, 0.49 and 1.0, because energy that goes in has nowhere else to go, and no parameter was tuned to make that hold. The mean path length rises 1.05, 1.39, 2.06 with roughness, which is the mechanism doing the conserving rather than a coincidence beside it. SECOND, ITS SINGLE-SCATTER COMPONENT REPRODUCES A NUMBER THIS TREE GRADED ROUNDS AGO BY AN UNRELATED QUADRATURE: 0.950920, 0.795565, 0.582680 and 0.378855 against microfacet.directionalAlbedo's 0.950620, 0.794751, 0.582111 and 0.378889 -- agreement to 3.4e-5 at roughness 1. A bounce simulation and a hemisphere quadrature agreeing to a thousandth is what makes the walk a ground truth rather than an opinion. *** AND THE SECOND VALIDATION FAILED FIRST, FOR A REASON WORTH KEEPING. *** Restricting the walk to one bounce with a maxBounces cap read 0.587860 against the table's 0.378889 -- FIFTY PER CENT HIGH. A CAP TRUNCATES THE WALK BEFORE THE ESCAPE TEST, so a ray that would have been shadowed on its way out is counted as having left. SINGLE SCATTER IS NOT "ONE BOUNCE", IT IS "ONE BOUNCE AND THEN ESCAPES", and the gap between those two numbers IS the shadowing term. The walk was right and the way of asking it was wrong, which is the third round in a row where the instrument was innocent and the question was not. *** THE BOUND BECOMES A NUMBER, AND THE ERROR CHANGES SIGN -- WHICH THE ROUND DID NOT PREDICT. *** Kulla-Conty against ground truth at roughness 1, cosO 0.7: exact at F0 = 1, then -3.7% at 0.9, -7.3% at 0.5, -3.5% at 0.2, -0.6% at 0.1, and +4.0% at 0.04. IT UNDER-COMPENSATES IN THE MIDDLE AND OVER-COMPENSATES AT THE DARK END. So it cannot be repaired by a single scale factor -- there is no constant to multiply by that fixes both ends -- and a claim that Kulla-Conty is "within a few per cent" owes the reader a WHERE. The fair reading is asserted alongside it, because a finding that only accuses is not a measurement: BEING SEVEN PER CENT OFF IS NOT THE SAME AS BEING SIXTY-TWO PER CENT OFF, and the uncompensated model was the second one. Every compensated value is closer to truth than its uncompensated counterpart, at every F0, and that is a row rather than a remark. *** AND THE WALK RETURNED ZERO AT EVERY ROUGHNESS ON ITS FIRST RUN. *** Smith's Lambda is SIGNED -- below the horizon it is -1 - Lambda(-w) -- and physics/render/microfacet.mjs's takes a cosine and squares it, which is CORRECT THERE and wrong here. That module only ever asks about directions above the horizon; a walk goes below it on its FIRST STEP. Using the unsigned value meant no ray ever satisfied the escape test, every path ran to the bounce cap, and the albedo came out 0.000000 at every roughness -- a total failure rather than a subtle one, which is the good kind. The signed version lives in the walk and the two are asserted to agree above the horizon, so they are one function rather than two. FOUR SABOTAGES, MEASURED 4/10/5/1 BY NAME, NONE ZERO. The 5 is worth reading: sampling the ordinary NDF instead of the VISIBLE normal distribution breaks CONSERVATION, so section 1 catches it -- which is why a law is worth more than any number the walk produces. UNCHECKED AND SAID PLAINLY: THIS IS THE UNIFORM-HEIGHT SMITH MICROSURFACE, which is one model of what a rough conductor IS. A Beckmann height distribution or a real measured surface would give different numbers, so what this measures is THE MODEL THIS TREE'S OWN D AND G2 ALREADY ASSUME rather than reality, and it would be wrong to call it "the truth" without that qualifier. IT IS CONDUCTOR-ONLY: there is no transmission in the walk, so v4436's rough dielectric STILL HAS NO GROUND TRUTH and its multi-scatter loss is still uncorrected and still unmeasured -- that is now the only named next step left from this line of work. The walk is achromatic per run, so three channels means three walks, and nothing here makes the achromatic table chromatic. And no timing is reported: the walk is used as a reference, not as a renderer, and 120,000 paths per data point is a measurement budget rather than a performance claim. The tree stands at 1474 gates. Full changelog on docs/CHANGELOG.md.
// v4445 -- *** v4432 AND v4436 BOTH SHIPPED SAYING THE SPECULAR LOBE WAS A CEILING RATHER THAN AN ANSWER, AND THIS IS THE ROUND THAT MAKES IT AN ANSWER. *** v4432's honest scope said it in as many words: "The specular lobe is SINGLE-SCATTER GGX, so a white metal at roughness 1 returns 0.379 of what it receives, and this tree's own energyCompensation.mjs -- a multi-scatter table already graded -- IS NOT WIRED IN. That makes every furnace number above a CEILING rather than an answer." v4436 repeated it. Wired: 0.378889 becomes 0.999817, and the white metal conserves energy at EVERY roughness, worst |1 - albedo| 1.8e-4 across 0.2 to 1.0. *** AND THE TWO MODULES AGREED ABOUT THE LOBE BEFORE THEY WERE CONNECTED, WHICH IS WHAT MAKES THIS A WIRING RATHER THAN A FIT. *** physics/render/principled.mjs's specular-only directional albedo reads 0.378889 where physics/render/energyCompensation.mjs's table reads E(mu) = 0.378981 -- two independent descriptions of single-scatter GGX, written in different rounds for different purposes, matching to five decimals. Had they disagreed, connecting them would have been tuning one to the other. THE TERM IS OPT-IN AND OMITTING IT IS BIT-IDENTICAL to the pre-v4445 model across twenty-seven configurations, asserted as an exact zero rather than assumed -- so every furnace number the earlier rounds reported can still be reproduced exactly, and v4432's 0.379 is still there on demand as the control. *** THE COLOUR SCALING IS KULLA-CONTY'S GEOMETRIC SERIES AND NOT F0, AND THE REASON WAS WRITTEN BACKWARDS BEFORE IT WAS MEASURED. *** The first version of that comment called the cheap F0 scaling "short" of F_avg -- 4.4 per cent at F0 = 0.5, 51.7 per cent at F0 = 0.04 -- as though reaching F_avg were the goal. IT IS NOT. F_avg IS WHAT A MATERIAL WOULD RETURN IF EVERY BOUNCE WERE FREE, AND EVERY BOUNCE IS NOT: light that scatters twice on a rough conductor is attenuated TWICE, so the recovered energy is a geometric series F_avg^2 Eavg / (1 - F_avg(1 - Eavg)) that lands WELL BELOW the ceiling -- 0.2912 at F0 = 0.5, against an F_avg of 0.5238. So the cheap scaling is too HIGH for a dark metal rather than too low, which is the OPPOSITE SIGN to what the round wrote an hour earlier. A framing error costs nothing until somebody builds a check on it, and this one nearly had a check built on it. F_avg itself has a CLOSED FORM for Schlick -- F0 + (1 - F0)/21, because the integral of (1 - mu)^5 2mu is exactly 1/21 -- and it is verified against quadrature to eight decimals rather than quoted. *** TWO EXACT BOUNDS ARE ASSERTED AND THE VALUE BETWEEN THEM IS MEASURED RATHER THAN CLAIMED. *** Compensation can never REMOVE energy, so the compensated value is never below the uncompensated one; and it can never exceed F_avg, which is a hard ceiling because every bounce attenuates by at most F_avg. Both hold across seven materials from a white mirror to a black conductor. What sits between those bounds is reported and not defended: PINNING IT WOULD NEED A RANDOM-WALK GROUND TRUTH ON A GGX MICROSURFACE, which is not here and is named as the next step rather than implied to be unnecessary. *** RECIPROCITY IS DECOMPOSED HONESTLY RATHER THAN ASSERTED AT THE STRENGTH THAT SOUNDED BEST. *** The first version claimed the composed model was reciprocal BIT FOR BIT and went red at 5.6e-17. Measured, the BASE model without any multi-scatter term is ALREADY 2.8e-17 asymmetric at the same inputs, because the specular lobe divides by 4 cosO cosI and a division whose operands arrive in a different order rounds differently. So the claim splits: THE ADDED LOBE IS BIT-EXACT, which is a property of its construction -- (1 - E(muO))(1 - E(muI)) is symmetric by inspection -- and THE COMPOSITION IS SYMMETRIC TO ONE ULP, which is a property of IEEE 754. Asserting the stronger thing about the weaker one would have been blaming this round for arithmetic it inherited. FOUR SABOTAGES, MEASURED 0-THEN-3/1/1/5 BY NAME. *** THE ZERO IS v4443'S DEFECT ARRIVING AGAIN, TWO ROUNDS LATER, IN THE SAME SHAPE. *** Section 4 compared the two candidate scalings ARITHMETICALLY -- it computed F0, computed F_ms, and compared them to each other -- which is true whatever the module actually does, so swapping the module back to F0 scaling cost NOTHING. A check that re-derives both candidates and compares them grades the copy, exactly as v4443's normalisation integrated a hand-substituted integrand and never called the profile. Repaired to test BEHAVIOUR instead: F0 scaling pushes a coloured metal up to roughly F_avg and the series leaves it well below, and those two are far enough apart that the module's choice is visible in its output. UNCHECKED AND SAID PLAINLY: A COLOURED METAL IS NOT CLAIMED CORRECT, only bounded. The table is ACHROMATIC and is scaled by a single scalar F_ms, so a metal with a strongly coloured F0 -- gold, copper -- receives one compensation for all three channels where it needs three, and the error that introduces is not measured here. Nothing touches the DIFFUSE lobe, whose own energy question roughDiffuse.mjs answers separately, and the gate asserts the multi-scatter term never reaches it. The table must be built per roughness by the caller, because building one per evaluation would be far too slow, so this is a change to an API as much as to a model. And the compensation is derived for REFLECTION: v4436's transmission lobe gets none of it, and a rough dielectric's multi-scatter loss is still uncorrected and still unmeasured. The tree stands at 1473 gates. Full changelog on docs/CHANGELOG.md.
// v4444 -- *** v4443 CLOSED THE SUBSURFACE GAP AS A MODEL AND SAID IN ITS OWN HONEST SCOPE THAT IT WAS LEFT OPEN AS AN INTEGRATION. THIS IS THAT INTEGRATION. *** A BSSRDF needs a SURFACE -- light enters at one point and leaves at another -- and physics/render/principled.mjs is a BRDF evaluated at a single point with no surface anywhere in its signature. The surface used here is the one the tree already renders on: physics/render/pathTracer.mjs's furnace sphere, which has exact analytic geometry. Keith asked which of three demonstrations was worth building -- the dipole, importance sampling, or hierarchical clustering -- and the answer was importance sampling, for a reason worth recording: it is the only one of the three whose BENEFIT IS A NUMBER and whose CORRECTNESS HAS A SEPARATE CHECK. Clustering was declined as the wrong order and as item 10's BVH in different clothes: a speed structure whose correctness key is easy and whose value key is hard, optimising something that does not exist yet, on a tree with no scene big enough to make the measurement mean anything. *** THE ESTIMAND IS WRITTEN DOWN BEFORE EITHER ESTIMATOR. *** With constant irradiance and a profile that depends only on distance, the surface integral collapses to one dimension in the GEODESIC radius, whose area element on a sphere of radius a is 2 pi a sin(r/a) dr. AND ITS FLAT LIMIT IS A NUMBER PROVED IN ANOTHER ROUND BY A COMPLETELY DIFFERENT ARGUMENT: I(a) runs 0.529916, 0.834307, 0.977815, 0.999767, 0.999997 at a = 1, 3, 10, 100, 1000 -- ARRIVING AT THE PLANE NORMALISATION THAT v4443 PROVED ANALYTICALLY, from a spherical integral that knows nothing about that proof. Two rounds, two methods, one number. *** THE PAIR OF CHECKS IS THE WHOLE DESIGN. *** The two estimators must AGREE IN THE MEAN, because both are unbiased -- that is where a wrong Jacobian shows, and it is the check that catches WRONGNESS. They must DIFFER IN VARIANCE -- that is the benefit, and it is the check that MEASURES it. NEITHER CAN BE FAKED BY THE OTHER, and a round that checked only the second would pass with a biased sampler that happened to be quiet. Unbiasedness is judged against each estimator's OWN measured standard error rather than a chosen tolerance, which is v4437's lesson applied rather than restated: that round's hand-picked "half a per cent" went red on 1.35 standard deviations of ordinary noise. Every case here lands inside 2.1 standard errors of a quadrature ground truth computed by a third method. *** AND THE IMPORTANCE WEIGHT COLLAPSES TO THE JACOBIAN AND NOTHING ELSE. *** Sampling the radius from the profile's own CDF gives a pdf of R(r) 2 pi r, and every factor of R cancels against the integrand, leaving a * sin(r/a) / r EXACTLY -- which is the ratio of the sphere's area element to the plane's, and is what importance sampling MEANS when the sampler matches the integrand. So as the sphere flattens the weight goes to one and THE ESTIMATOR BECOMES EXACT: every sample returns the same number and the variance goes to zero. Measured standard deviations at a = 1, 3, 10, 100 are 4.07e-1, 2.56e-1, 5.19e-2 and 6.03e-4, against the uniform estimator's 1.54e0, 4.28e0, 1.21e1 and 5.49e1 -- one collapsing while the other GROWS, because R is singular where the sphere's area density vanishes and the uniform weight has a heavy tail. THE VARIANCE RATIO RUNS FROM 14x TO 8.29e9 ACROSS THE SWEEP, which is the demonstration and is a number rather than a picture. *** AND THE COMPANION MEASURES WHY v4443 CHOSE BURLEY OVER THE DIPOLE, RATHER THAN ASSERTING IT. *** The classical Jensen dipole's radial integral, taken on the same quadrature shape and the same limits, is 0.080453, 0.226020 and 0.475773 at reduced albedos 0.5, 0.8 and 0.95 -- NEITHER ONE NOR THE ALBEDO IT WAS HANDED. So ALBEDO IS NOT A FREE MULTIPLIER with the dipole: hitting a target diffuse reflectance means inverting a function of three parameters numerically, and the inversion at least exists because the integral is monotone in the albedo, which is checked. Burley's profile integrates to exactly one on that same quadrature. That is the contrast, and it is an INCONVENIENCE RATHER THAN AN ERROR -- the dipole is not wrong, it simply does not hand you the normalisation, and no row here says otherwise. FOUR SABOTAGES, MEASURED 2/4/1/2 BY NAME. THE 1 IS THE INTERESTING ONE: sampling the uniform estimator from a linear density instead of the sphere's area density costs exactly one row, AND IT IS THE UNBIASEDNESS ROW -- the variance comparison still looks entirely reasonable, and only the mean knows. A round that measured only the benefit would have shipped it. *** AND THE SABOTAGE PROTOCOL ITSELF FAILED ONCE, WHICH IS v4439'S LESSON ARRIVING A SECOND TIME. *** A `cd` into a directory already occupied returned non-zero, the backup variable stayed empty, the backup never happened, and the first restore had nothing to restore from -- leaving the module sabotaged with the gate reporting green on a file nobody had checked. Caught by the restore printing an error rather than by anything clever, reversed by hand, and redone with the backup MADE AND ITS MD5 PRINTED BEFORE the first sabotage ran. v4439 said a restore that reaches for the last COMMIT rather than the last KNOWN GOOD is a different operation wearing the same word; this is the same rule one step earlier, at the point where the known good is recorded at all. UNCHECKED AND SAID PLAINLY: THIS IS A ONE-DIMENSIONAL COLLAPSE OF A SURFACE INTEGRAL, and it is valid only because the irradiance is CONSTANT and the profile depends only on distance. A textured surface, a shadow, or any spatially varying illumination breaks that collapse and needs the real two-dimensional disc sampling and its projection, WHICH IS NOT HERE. Geodesic distance is used throughout, where a real implementation uses the chord or a ray-traced probe; on a sphere those differ by a known amount and NOTHING HERE MEASURES IT. Nothing renders, nothing is wired into principled.mjs, and the composed BSDF still has no subsurface term -- what exists now is a profile, a sampler, and a proof that the sampler is worth having. The tree stands at 1472 gates. Full changelog on docs/CHANGELOG.md.
// v4443 -- *** THE FIFTH AND LAST OF THE FIVE GAPS v4432 NAMED IN ITS OWN HONEST SCOPE. *** That round shipped the principled BSDF listing sheen, clearcoat, anisotropy, TRANSMISSION and SUBSURFACE as the five parameters that make Disney's model what it is and were absent here. v4436 closed the fourth; this closes the fifth. NEW physics/render/subsurface.mjs carries Christensen and Burley's NORMALISED DIFFUSION -- the profile Disney, Cycles and Arnold actually ship in place of the classical dipole -- and it was chosen over the dipole for one reason: IT IS NORMALISED BY CONSTRUCTION AND THE DIPOLE IS NOT, so more can be ASSERTED here than in any of the other four gaps. The proof is one line: INT 2 pi r R(r) dr = (1/4d) INT (e^(-r/d) + e^(-r/3d)) dr = (1/4d)(d + 3d) = 1, exactly, for every d. Nothing is vendored; this is a paper and a two-term exponential. MEASURED ABSENT FIRST, with tools/ship/absenceScope.mjs, before a line was written: subsurface, burley, diffusionProfile, translucency, meanFreePath and albedoInversion are all ZERO in this tree's code, and the four `subsurface` hits are DENIALS -- v4432's and v4436's own notes saying it is missing. TWO FALSE FRIENDS ARE NAMED RATHER THAN COUNTED, so this file does not claim more absence than it measured: physics/em/currentLoop.mjs has a DIPOLE, which is magnetic and shares nothing with the SSS dipole but the word; and render/atmosphere.mjs SCATTERS light, which is Rayleigh and Mie through a participating medium rather than diffusion under a surface. Neighbouring physics, different model. *** FOUR THINGS ARE EXACT AND ARE ASSERTED AS EXACT. *** The normalisation integral is one and is INDEPENDENT OF d. The CDF is closed form -- 1 - (1/4)e^(-r/d) - (3/4)e^(-r/3d) -- with CDF(0) exactly zero and CDF(inf) one to machine precision. The mean radius is exactly 2.5d. And the profile is SELF-SIMILAR in r/d, which makes the searchlight limit not a separate claim but the same one read along the other axis: "as d goes to zero the BSSRDF becomes a BRDF" and "as the radius grows the fraction goes to one" are one statement, and the gate asserts the COLLAPSE rather than two coincidences -- the fraction within k*d is bit-identical at d = 1e-3, 1 and 1e3. *** THE SELF-SIMILARITY POWER WAS WRONG IN THE FIRST DRAFT AND THE MEASUREMENT CAUGHT IT. *** The header said d R(r, d) depends only on r/d. Across d = 0.01, 1 and 100 the mantissa of d R was IDENTICAL TO TWELVE DIGITS while the exponent stepped by two per decade -- which is the signature of a MISSED POWER rather than a wrong formula. R is a density PER UNIT AREA and must carry two inverse lengths; r R(r), the thing that integrates, carries one. A DIMENSIONAL SLIP THAT LEAVES EVERY DIGIT RIGHT IS INVISIBLE TO ANYTHING EXCEPT THE EXPONENT, and a row checking only that the digits match would have passed it. The gate now asserts BOTH directions: d^2 R collapses to a relative spread of zero, and d^1 R does NOT, so the exponent is part of what is claimed. *** AND THE ROUND'S BEST FINDING WAS A ZERO-RED AGAINST THIS GATE'S OWN HEADLINE ROW. *** The first normalisation() integrated a HAND-SUBSTITUTED copy of the integrand, with u = r/d cancelling d analytically -- which made the answer BIT-IDENTICAL across seven decades of d and read as the strongest assertion in the file. IT NEVER CALLED profile(). Replacing 8 pi with 4 pi inside the profile cost ZERO RED, because the check could not see the thing it was supposed to be grading. A NORMALISATION THAT RE-DERIVES ITS OWN INTEGRAND IS TWO DECLARATIONS OF ONE FORMULA AND IT GRADES THE COPY -- and the bit-identity, which looked like rigour, was the tell: it was a property of the algebra rather than of the model. It integrates the real profile now, with the singular head added in closed form since INT[0,a] 2 pi r R dr is just CDF(a). That COSTS the bit-identity and BUYS a check that can fail: d-independence is now MEASURED at 4.4e-16 across seven decades rather than assumed, and the sabotage bites. A SECOND NEAR-MISS OF THE SAME FAMILY: the E[r] quadrature used 1/2 where the measure needs 1/4, and it was hand-checked AT d = 2 -- where a factor-of-two error returns 5.000, BECAUSE 2.5 TIMES 2 AND 2 TIMES 2.5 ARE THE SAME NUMBER. A constant that scales with the parameter you happened to test at is invisible at that parameter, and the repair is to check the DIMENSIONLESS ratio E[r]/d, which has one right answer at every d. FOUR SABOTAGES, MEASURED 0-THEN-1/2/2/1 BY NAME. UNCHECKED AND SAID PLAINLY: THE FIFTH GAP IS CLOSED AS A MODEL AND LEFT OPEN AS AN INTEGRATION. A BSSRDF needs a SURFACE to integrate over -- light enters at one point and leaves at another -- and physics/render/principled.mjs is a BRDF evaluated at a single point with no surface anywhere in its signature, so NOTHING IS WIRED IN and the composed model still has no subsurface term. That is the honest state: the profile is here and graded, and the renderer cannot yet ask it anything. Burley's albedo-to-distance relation is a CURVE FIT to Monte Carlo ground truth rather than a derivation; it is carried because a renderer needs it, is labelled `fitted` in the export, and NO ROW ASSERTS ANYTHING EXACT ABOUT IT -- the gate holds it only to being finite, positive and monotone where a renderer would ask. There is one channel, one scattering distance and no colour, where real subsurface scattering is chromatic and is most of the effect. And nothing here is sampled in two dimensions or projected onto a surface, so the sampler is graded as an inverse CDF and not as a disc sampler. The tree stands at 1471 gates. Full changelog on docs/CHANGELOG.md.
// v4442 -- *** THE ROUND'S REAL DECISION WAS WHAT NOT TO TAKE. *** DaveFace/UnrealRetroShaders (MIT) ships four PlayStation techniques, and NOTHING IN IT IS PORTABLE AT THE FILE LEVEL: it is UE4.27 Blueprint materials in binary .uasset, and its author states plainly that UE5 is unsupported because the rendering systems it relies on changed, so it is a dead-ended asset pack for one engine version rather than a library. The techniques are portable and are not the author's to license anyway -- they are 1994 console constraints. So the pack was split by whether each technique HAS A RIGHT ANSWER, and only half of it was taken. Bayer dithering is ALREADY HERE, in fx/dither.js with tools/ship/dither-selfcheck.mjs, and rebuilding it would be a second declaration of one thing. YUV colour space and posterise are genuinely absent -- zero files, code or prose -- AND THEY ARE AESTHETIC ONLY: there is no wrong answer for a check to catch, so they are DELIBERATELY NOT TAKEN. That is a decision rather than an oversight, and the reason is this session's own spine: shipping an aesthetic behind a gate that cannot fail is exactly the defect v4435 found as an unfalsifiable path check, v4439 found as a check satisfiable by the wrong cause, and v4441 found as a bisection returning its own floor. Building one on purpose would be worse than all three. *** WHAT WAS TAKEN IS THE HALF THAT IS ALMOST ENTIRELY EXACT. *** AFFINE TEXTURE WARPING IS EXACTLY "interpolate UV without dividing by w" -- the correct interpolation is sum(b a / w) / sum(b / w) and the PlayStation, having no divider in its rasteriser, used sum(b a). That gives TWO MACHINE-PRECISION AGREEMENTS, both asserted as exact zeros rather than tolerances. FIRST: at equal w the affine answer IS the correct one, over 612 (w, barycentric) samples, because the division cancels when the barycentrics sum to one -- a triangle parallel to the screen has nothing for perspective to do. SECOND: the error is EXACTLY ZERO AT ALL THREE VERTICES for wildly unequal w, over twelve (w, vertex) pairs, and THAT IS WHY THE ARTEFACT SWIMS RATHER THAN SHIFTS -- it is pinned at the corners and wrong in between, which is the thing anybody who played a PlayStation remembers without knowing why. Between those two limits the warp is large and orderly: it grows monotonically with the depth ratio to 0.848 of a full texture width at 16:1, and ITS MAXIMUM ALWAYS LIES ON THE EDGE SPANNING THE DEPTH RANGE rather than at a corner, which is the mechanism rather than a coincidence -- a maximum sitting on a vertex would mean the interpolation was broken rather than merely affine. VERTEX WOBBLE IS EXACTLY A LATTICE QUANTISER, and a quantiser owes two things which are both asserted exactly: IDEMPOTENCE, over twenty thousand samples across five precisions, and that NOTHING EVER MOVES FURTHER THAN HALF A LATTICE STEP, which is the definition rather than a measurement. Its site count has a CLOSED FORM -- n * 2^bits + 1 -- so a wobble subtler than it should be is COUNTABLE where an eyeball sees "about right". snapVertex is also asserted to be a function rather than a filter: it leaves its input alone, because a quantiser that mutated its argument would silently make every caller's source data lossy. *** WHAT WAS ALREADY HERE AND IS NOT THIS, NAMED SO THE ROUND DOES NOT CLAIM MORE ABSENCE THAN IT MEASURED: *** physics/mesh/meshCSG.mjs has snapVertices, which welds coincident vertices at a 1e-9 tolerance for boolean robustness. That is vertex snapping the way a spelling checker is a dictionary -- same word, different job. `perspectiveCorrect` and `noperspective` really are zero in this tree. FOUR SABOTAGES, MEASURED 3/3/1/1 BY NAME. The floor-instead-of-round sabotage costs exactly ONE row and it is the right one: floor is still idempotent and still gives the same site count, so ONLY the half-step bound can see it -- a quantiser that is off by up to a full step looks identical to a correct one in every property except the one that defines it. *** AND THE FIRST ATTEMPT AT A SABOTAGE READ ZERO RED BECAUSE THE SABOTAGE WAS INVALID, NOT BECAUSE THE GATE WAS BLIND. *** It replaced affine()'s body with perspectiveCorrect at equal w -- which is ALGEBRAICALLY THE SAME FUNCTION, as section 1 asserts three lines below it. A SUBSTITUTION THAT PRESERVES THE MATHEMATICS CANNOT BREAK ANYTHING, and reporting its zero as evidence of a weak gate would have been the OPPOSITE mistake to the ones this session keeps making: not a check that cannot fail, but a change that cannot break. That is a THIRD KIND OF ZERO and it is named as one, alongside v4435's check nothing can fail and v4436's check nothing reaches, because a rule that treats every zero as a finding will eventually manufacture one. UNCHECKED AND SAID PLAINLY: NOTHING HERE RENDERS ANYTHING, so no claim is made that any of it LOOKS right, and "reads as a PlayStation" is not a claim this tree can hold. The artefacts are not complete -- the real hardware also had no depth buffer, sorted per polygon, and clipped in fixed point, none of which is here -- so this is two artefacts rather than a console. The quantiser snaps whatever it is handed, where the hardware quantised after projection into screen space; the caller decides which space, and that is a real gap rather than a design. And no timing, no shader and no integration into any page: these are two functions and their laws, which is what the item asked for and no more. *** AND THE ROUND'S VERIFY WENT RED ON A TIMER RATHER THAN ON THIS ROUND'S WORK, WHICH IS THE STALENESS RULE FIRING EXACTLY AS DESIGNED. *** tools/ship/registerDrift-selfcheck.mjs requires that the register audit -- the record of what every standing red ACTUALLY says today -- be no more than twelve rounds old. It was frozen at v4429 and read at v4442, THIRTEEN. That is not a defect in this round and it is not a flake: it is a gate whose whole point is that A CANONICAL RECORD GOES STALE TOO, and whose own text says the difference is that re-taking it is a command while retyping nine numbers is a chore nobody does. Re-taken with tools/ship/freezeRegisterAudit.mjs: 26 rows, 0 now green, 0 printing FAIL to stderr -- so the register is unchanged in substance and only its timestamp moved, which is the outcome that says the rule is cheap rather than noisy. Recorded rather than run silently, because a red that clears on a command is exactly the shape that teaches a ritual to ignore reds. The tree stands at 1470 gates. Full changelog on docs/CHANGELOG.md.
// v4441 -- *** THE PLAN ITEM NAMED A MODULE THAT NO LONGER EXISTS. *** It said "check the tree's XPBD against warp.sim". warp.sim was DEPRECATED IN WARP 1.8 in July 2025 and REMOVED IN WARP 1.10; its successor is newton-physics/newton, a Linux Foundation project under Apache 2.0. That is the fifth round running in which checking a premise before building on it changed the round, and the second in which the stale premise was mine. IT MATTERS LESS THAN IT LOOKS, because the reference was never the source: warp, newton and Omniverse all implement two papers, and a paper's claim can be tested against this tree's own solver directly. Macklin, Muller and Chentanez 2016 says compliance makes stiffness ITERATION-INDEPENDENT, which physics/xpbd/compliance.mjs already grades. Macklin et al. 2019, "Small Steps in Physics Simulation", says that for a FIXED BUDGET the work should go into SUBSTEPS rather than iterations -- and that is the defining design choice behind all three implementations, and is exactly what physics/xpbd/xpbd.js's `iterations ?? 1` default already is. NOTHING WAS VENDORED AND NOTHING WAS READ: the GitHub source was not reachable from this session, which is said plainly rather than implied away, so what follows grades a PAPER'S CLAIM against this solver and is a smaller thing than a line-by-line differential. *** TWO RIGS COULD NOT SEE THE CLAIM, AND THAT IS WORTH MORE THAN THE THIRD ONE WORKING. *** RIG ONE is the tree's own hangingLink from compliance.mjs, which has ONE constraint. At a fixed budget of 1600 its error falls MONOTONICALLY as work moves toward iterations -- 4.0e-4 at 1600x1 down to 2.3e-14 at 50x32 -- which is the exact opposite of Small Steps and IS NOT A REFUTATION OF IT: with one constraint there is NO NETWORK for information to propagate through, so iterating merely converges a single scalar solve. A rig that cannot exhibit a mechanism cannot test a claim about it. RIG TWO is a chain of 32, which is a network, but measured at its TAIL MEAN -- a quasi-static equilibrium. Still no clear winner: 1.38e-2 against 8.17e-3, under 2x, and non-monotone at N = 8. Also not a refutation, because THE STEADY STRETCH IS PRECISELY WHAT XPBD'S COMPLIANCE MAKES ITERATION-INDEPENDENT, so measuring it asks the 2016 claim and not the 2019 one. *** RIG THREE SWEEPS THE STIFFNESS AND THE CLAIM APPEARS AT ONCE, WITH A SIGN CHANGE. *** At compliance 1e-3 iterations win, 1.38e-2 against 8.17e-3. At compliance 1e-5 SUBSTEPS WIN BY THIRTY-ONE TIMES, 9.80e-4 against 3.08e-2, at identical total work and monotone in the iteration share; and the infinitely stiff limit at compliance 0, which is ordinary PBD, behaves like the stiff case rather than the soft one. So Small Steps reproduces exactly in the regime its paper is about and inverts outside it, which is a more useful result than either slogan. *** THE CROSSOVER IS AT COMPLIANCE 3.487e-4 AND THIS TREE USES COMPLIANCES ON BOTH SIDES OF IT. *** Below the line, where substeps win: 0, 1e-6, 5e-6, 1e-4. Above it, where iterations do: 5e-4, 8e-4, 1e-3, 2e-2. physics/xpbd/xpbd.js defaults to one iteration, which is what warp and newton do and is RIGHT for the stiff half of that census; modules that pass 2, 4, 5 or 8 are right only above the line, and this round does NOT call any of them wrong -- whether a given module's constraints are stiff is a question about that module, and the round states which side each compliance falls on and stops there. *** AND THE CROSSOVER IS NOT A CONSTANT OF XPBD, WHICH IS ASSERTED RATHER THAN HOPED. *** It moves 38x across the budget -- 9.50e-3, 1.81e-3, 3.49e-4, 2.47e-4 at budgets 400, 800, 1600, 3200, monotonically -- and 620x across chain length. A round that quoted 3.487e-4 as a property of the method would be quoting its own test setup. *** A BISECTION THAT RETURNED ITS OWN FLOOR WAS CAUGHT, AND IT IS THE SESSION'S DEFECT IN A NEW COSTUME. *** At two links substeps never win anywhere in the searched range, so every step of the search pushed the interval downward until it collapsed onto the lower bound, and the function returned 1.000e-6 -- reported as a number it looks like a crossover a thousand times stiffer than its neighbours, and it is THE ABSENCE OF A CROSSOVER WEARING A NUMBER. A bisection is only meaningful across a sign change, so the search now tests both ends first and REPORTS SATURATION instead of returning a bracket. FOUR SABOTAGES, MEASURED 3/6/4/8 BY NAME, NONE ZERO. The rig's analytic answer is asserted STRUCTURALLY rather than by eyeball: link k carries the weight of the (N - k) particles below it, so the exact length must scale LINEARLY in compliance and as N(N+1)/2 in link count -- a per-link constant load would give a ratio of 2 and would pass a looser check. UNCHECKED AND SAID PLAINLY: warp and newton were not read, so nobody should treat this as a comparison against their code; anyone wanting a line-by-line differential still has to read the source. This is one chain, one gravity, one rest length and one budget shape, hanging under its own weight -- there are no collisions, no bending constraints, no self-contact and no rigid coupling, and those are where a real cloth solver spends its time and where the crossover would most plausibly move again. The tail mean is an equilibrium measure, so nothing here says anything about transient accuracy, which is the other half of what Small Steps argues. And no timing was taken at all: "fixed budget" here means substeps times iterations held constant, which is a proxy for work rather than a measurement of it, and on real hardware a substep costs more than an iteration because it re-integrates velocities and re-colours nothing. The tree stands at 1469 gates. Full changelog on docs/CHANGELOG.md.
// v4440 -- *** THE ROUND SET OUT TO ADD A SMOKE SOLVER AND FOUND THE TREE HAS TWO FLUID SOLVERS ALREADY -- THE FOURTH ABSENCE CLAIM OF MINE IN FOUR ROUNDS TO BE WRONG, AND THE FIRST CAUGHT BEFORE A LINE OF CODE WAS WRITTEN. *** I told Keith in as many words that this tree had no grid smoke solver and no vortex particles. Measured with tools/ship/absenceScope.mjs before building anything: fluid/flip2d.mjs and fluid/flip3d.mjs are FLIP/PIC solvers on a staggered MAC grid with pressure projection and RK2 advection, unit-tested to the point of "divergence collapses after the pressure solve"; and fx/vorton/vorton.js is a VORTEX-PARTICLE method, which is the exact technique I said was absent. Only `vorticity` and `confinement` really were zero in code. The habit that made this round useful instead of embarrassing is the one the last three rounds paid for: MEASURE THE PREMISE BEFORE BUILDING ON IT. *** AND THE TREE ALREADY CONTAINED THE CLAIM THIS TECHNIQUE EXISTS TO ADDRESS, SITTING UNMEASURED WITH BOTH OF ITS HALVES PRESENT. *** fx/vorton/vorton.js's own header says its method is "divergence-free by construction, which is why it keeps the beautiful filamentary wisps A GRID SOLVER SMEARS AWAY" -- a comparative claim about a grid solver, written in a file that is not one, while fluid/flip2d.mjs sits three directories away being exactly the thing described. Nobody had ever put the two in the same room. So the round is not "add smoke": it is MEASURE HOW MUCH VORTICITY A GRID ACTUALLY DESTROYS, implement the standard remedy (Fedkiw, Stam and Jensen 2001, the technique read from mmacklin/sandbox's smoke demo -- MIT, C++/CUDA, NOT VENDORED for the toolchain reason v4432 and v4436 both gave), and measure what it gives back and what it costs. NEW fluid/vorticity.mjs carries the discrete curl, the confinement force, and the semi-Lagrangian advection that was the genuinely missing piece -- genuinely missing because FLIP is ALREADY a partial answer to this same problem, carrying velocity on particles precisely to avoid the smearing a pure grid suffers, so the tree had two answers to one question and had compared neither. *** TWO THINGS ARE EXACT, AND THEY ARE THE PART WORTH HAVING. *** The discrete curl converges at SECOND ORDER against an analytic Taylor-Green field whose curl is known in closed form: errors 5.35e-1, 1.39e-1, 3.52e-2, 8.82e-3, 2.21e-3 at n = 16 to 256, ratios 3.84, 3.96, 3.99, 4.00. THAT IS A RATE RATHER THAN A VALUE, so no lucky constant satisfies it. And the confinement force is PERPENDICULAR TO ITS OWN GRADIENT to 1.388e-17 across 3844 cells -- f = eps h (N x omega) so f . N vanishes identically, an exact zero rather than a tolerance, and a wrong sign or index order would not have it. *** THE HEADLINE IS THAT CONFINEMENT IS NOT A RESTORATION, AND THE NUMBER SAYS SO WITHOUT ARGUMENT. *** Semi-Lagrangian advection retains 45.4 per cent of its enstrophy over sixty steps, monotonically -- that is "a grid solver smears away", as a number, for the first time in this tree. Confinement puts it back: 45.7, 46.9, 51.3, 67.6 per cent at eps 0.5, 2, 5, 10. BUT THE ENERGY COMES WITH IT. Bisected, the eps that returns enstrophy to EXACTLY 100 per cent is 15.546, at which KINETIC ENERGY IS 109.6 PER CENT OF INITIAL -- a dissipative scheme left holding more energy than it started with. And there is no ceiling to run into: eps 20 gives 130 per cent enstrophy, eps 40 gives 286, eps 160 gives 6837. So there is no value of eps that restores what advection destroyed, and the parameter that makes the vorticity number right IS CHOSEN BY THE VORTICITY NUMBER BEING RIGHT, which is circular -- there is no independent criterion anywhere in the method. That is what a hack with a free parameter looks like when somebody finally measures it, and it is worth saying that this is not a criticism of Fedkiw: the paper says it is a forcing term. It is a criticism of every renderer that ships it as though it were a correction. *** AND vorton's OWN CLAIM IS CHECKED FOR THE FIRST TIME. *** Its induced field is divergence-free to 1.33e-6 relative to the field's own magnitude, over two hundred points -- and the residual is shown to be THE PROBE rather than the field by COARSENING the finite difference and watching the number get worse, which is v4432's rule arriving in a fourth round and saying "instrument" again. FOUR SABOTAGES, MEASURED 7/2/4/3 BY NAME, NONE ZERO. The 2 is the interesting one: replacing the central difference with a one-sided one costs only two rows, AND THEY ARE BOTH THE CONVERGENCE RATE -- a one-sided difference is still roughly right, so a value check would not have noticed, and only asking for an ORDER catches it. *** AND THE ANALYTIC REFERENCE HAD THE WRONG SIGN WHILE THE CODE DID NOT. *** The first curl test failed at every resolution with the error sitting FLAT at 25.1, and what identified that as a sign rather than a discretisation error was the SIZE: 25.1 is exactly twice 4pi, twice the curl's own amplitude. AN ERROR THAT DOES NOT FALL WITH h IS NOT TRUNCATION, AND ONE EQUAL TO TWICE THE SIGNAL IS THE SIGNAL NEGATED. The direction nobody checks first is that the reference is wrong and the code is right. UNCHECKED AND SAID PLAINLY: NO WINNER IS DECLARED between the grid and the vortons. They are not the same discretisation of the same problem -- one is a fixed Eulerian grid with a pressure solve, the other a Lagrangian set of singularities with an analytic kernel -- so a head-to-head number would be comparing two different experiments, and section 5 checks only vorton's own claim on its own terms. Nothing here is three-dimensional, nothing has a free surface, and the whole measurement is one analytic field in one 2D box with a one-cell solid border, so the decay rate is not a universal constant and no claim is made that it transfers. The confinement force is applied to the faces by a transpose of the averaging the curl does, which is defensible rather than derived, and no order of accuracy is claimed for that step. And flip2d's OWN dissipation is not measured here -- the comparison is against pure semi-Lagrangian advection, which is what vorton's header means by a grid solver, not against the hybrid the tree actually ships. The tree stands at 1468 gates. Full changelog on docs/CHANGELOG.md.
// v4439 -- *** THE AUDIT v4438 DEFERRED, AND TWO OF THREE CAME BACK CLEAN, WHICH IS THE RESULT THAT MATTERED. *** v4438 said in its own honest scope that it narrowed the grid-integrator problem rather than closing it, and named three things it had not reached. physics/render/roughDiffuse.mjs builds a table on the SAME N=96, M=48 grid v4437 convicted, and it is CLEAN: worst error 0.036 per cent over sixteen (sigma, cosO) cells, three orders of magnitude better than the GGX case. microfacet.ndfIntegral at its N=4000 default is CLEAN: worst |value - 1| of 3.21e-5. *** THAT IS THE POINT RATHER THAN A FOOTNOTE. *** The defect is about NARROW LOBES falling between grid lines, and Oren-Nayar has no narrow lobe -- so the same grid is the RIGHT instrument there, and finding a defect nearby is not evidence for one here. The symmetric error to assuming absence, which this session has made three times, is assuming PRESENCE, and it is avoided by measuring rather than by reasoning about which files look similar. The third came back dirty: furnaceIntegral at ITS OWN N=500 default reads 0.418656 where the truth is 0.892115 at alpha 0.02, cosO 0.0208 -- FIFTY PER CENT -- and 6.9 per cent at alpha 0.05. v4438 repaired one CALLER; the integrator itself is still wrong. *** AND NOTHING IN THE TREE IS BREAKING ONLY BECAUSE NO CALLER GOES BELOW cosO 0.2. *** Every gate's shallowest angle is 0.2, where N=500 is sound. THAT IS LUCK RATHER THAN A DESIGN, so the round makes it explicit: the caller set is enumerated and FROZEN, and a new caller forces the list to be reviewed. What that check can and cannot do is stated in it -- it cannot know a caller's runtime alpha and cosO, only make a new caller visible, which is the whole difference between accidental safety and checked safety. *** THE ROUND'S FIRST DESIGN WAS WRONG AND AN EXISTING GATE IS WHAT SAID SO. *** The obvious fix is to put the routing INSIDE microfacet.directionalAlbedo so every caller gets the right number without asking. It was built, and physics/render/renderBsdf-selfcheck.mjs went red -- a gate from v3490 whose headline row reads "PATH SAMPLING AND MIDPOINT QUADRATURE AGREE ON THE DIRECTIONAL ALBEDO TO BETTER THAN 0.3 PER CENT ... THE TWO ROUTES SHARE ONLY THE DEFINITIONS OF D AND G2". THE GUARD WOULD HAVE MADE THE QUADRATURE SIDE INTO THE SAMPLER, so that gate would have been comparing the sampler against itself, AND A VERSION THAT PASSED WOULD HAVE BEEN WORTHLESS. That is the defect this session keeps finding -- a check satisfiable by the wrong cause -- very nearly built into the fix for it, and caught only because somebody three hundred rounds ago wrote a gate whose value was independence. The routing stays in albedoEstimator.mjs where a caller CHOOSES it, directionalAlbedo stays a deterministic marched grid, and the sabotage that puts the guard back is kept in the gate as the ARGUMENT for where it lives. *** THREE FINDINGS AGAINST THE ROUND'S OWN WORK. *** ONE: `export { x } from "y"` GIVES NO LOCAL BINDING. Re-exporting sampledFurnace with that form left trustworthy() throwing ReferenceError on every call, and renderBsdf-selfcheck REPORTED ZERO FAIL LINES WHILE EXITING 1 -- a crash, not a failed assertion. The tree has a written rule for exactly this shape ("a count of failures is not a verdict unless the process finished") and this is its seventh sighting; the trap itself last bit at v4419. Import, then export. TWO: THE CALLER SCAN MATCHED THE NAME AND NOT THE THING. Searching for `directionalAlbedo(` under physics/ returned TEN files including roughDiffuse.mjs and principled.mjs, both of which DEFINE A FUNCTION OF THAT NAME and call their own -- the detector matching the shape its author pictured, inside the file written to make a safety explicit. Keying on the IMPORT from microfacet.mjs is a mechanism rather than a spelling, and it gives five. THREE: RESTORING A SABOTAGE WITH `git checkout` INSTEAD OF A BACKUP COPY reverted the gate to its committed version and silently discarded three sections of this round's work. A restore that reaches for the last COMMIT rather than the last KNOWN GOOD is a different operation wearing the same word, and the sabotage protocol's own rule -- verify the restore by md5 before reporting -- exists because of exactly this, and was applied to the two module files and not to the gate. SEVEN SABOTAGES ACROSS THE TWO ROUNDS THIS GATE COVERS, the three new ones MEASURED 11/5/1 by name. AND THE SAMPLER'S NOISE IS NOW MEASURED ON THIS INTEGRAND rather than carried over: eight seeds at n=60k give a relative sd of 8.4e-3 at alpha 0.05, mu 0.0208, against the grid's 24 per cent BIAS at the same cell, and the spread falls with the sample count, which is what says it is noise rather than a second bias. v4438 refused to reuse v4437's figure because it was measured on a different estimator and a different integrand; this is the measurement it owed. UNCHECKED AND SAID PLAINLY: nothing outside physics/render/ has been looked at, and the audit reached three integrators rather than all of them. The frozen caller list is a tripwire and not a proof -- it cannot see a caller that computes its angles at runtime, and it covers physics/ alone. The guard is still OPT-IN, so a caller who wants the grid at a grazing angle can still have a silently wrong number; making it opt-out is what renderBsdf just demonstrated is not free. And no claim is made that NARROW_ALPHA and OBLIQUE_COS are optimal -- they are generous, and the round measured that they flag cells which are in fact fine. The tree stands at 1467 gates. Full changelog on docs/CHANGELOG.md.
// v4438 -- *** v4437 ENDED WITH A SENTENCE SAYING EVERY FURNACE NUMBER AT LOW ROUGHNESS AND GRAZING ANGLES SHOULD BE RE-CHECKED, AND THEN DID NOT CHECK THEM. *** Writing that down and shipping is leaving the tree standing on an instrument already known to be broken, so this round is that check rather than another note about it -- and it is WORSE than the round predicting it guessed. physics/render/energyCompensation.mjs BAKES A TABLE that renderBsdf, msDirect and directStrategy all consume, and its first mu row is (0 + 0.5)/K = 0.0208, THE MOST OBLIQUE ANGLE THERE IS -- so the table starts in exactly the regime a marched grid cannot integrate. At alpha 0.05, which is ALPHAS[0] in energyCompensation's OWN gate, the grid reads 0.705248 where the truth is 0.926606. TWENTY-FOUR PER CENT WRONG AND SHIPPED, on green gates, for as long as the table has existed; msDirect-selfcheck builds at N=120 where the same cell is FORTY per cent out; and at alpha 0.02 the error reaches seventy-three. *** AND THE FIX IS NOT A BIGGER GRID, WHICH THE TIMING SETTLES RATHER THAN TASTE. *** Converging that worst cell needs N greater than 4800 and takes 3456 MILLISECONDS FOR ONE POINT, against a sweep budget of three seconds for an entire gate; a 24-row table at six alphas would be minutes. "Just raise N" was never available and it is better to say so than to discover it later. *** THE TREE ALREADY OWNED THE RIGHT ESTIMATOR AND WAS USING IT ONLY TO CHECK THE WRONG ONE. *** microfacet.sampleHalfVector draws from the GGX distribution and microfacet.bounceWeight is the throughput with the pdf cancelled ANALYTICALLY -- both shipped at v3495 -- and together they estimate the same integral by drawing from the lobe instead of marching past it. Measured at alpha 0.02, mu 0.0208: sampled 0.892536 at 60k draws, 0.891673 at 400k, 0.892115 at 2M, FLAT, in 18 ms; grid 0.685697 at N=1200, 0.845081 at N=2400, 0.888784 at N=4800, STILL CLIMBING, in 3456 ms. A HUNDRED TIMES CHEAPER AND RIGHT, and the reason is structural rather than lucky: A SAMPLER CANNOT MISS THE LOBE IT IS DRAWING FROM, while a narrow lobe at a grazing view falls between grid lines. The grid was not a good instrument used carelessly. It was the wrong instrument for this integrand. NEW physics/render/albedoEstimator.mjs carries the sampler, the error measurement, and the routing RULE -- a narrow lobe AND an oblique view, both halves asserted separately so neither can be dropped -- and buildTable now routes per cell by that rule rather than by whatever N somebody typed at a call site. *** THE REPAIR IS BOUNDED AND THE BOUND IS ASSERTED: *** twenty of twenty-four rows move by less than half a per cent, an alpha 0.8 table is BIT-IDENTICAL, and the grid path stays reachable and is still exercised -- because energyCompensation-selfcheck's CONVERGENCE-ORDER STUDY genuinely needs a deterministic integrand underneath it, since Monte Carlo noise would put a floor under a refinement whose whole point is watching one error fall while everything below it holds still. That is the same failure the study's own text already diagnoses for the fixed quadrature error, so the study now says estimator: "grid" OUT LOUD instead of inheriting a default. THE DEFAULT AND THE STUDY WANT DIFFERENT THINGS AND BOTH ARE RIGHT, which is worth saying because the temptation was to call one of them wrong. *** TWO FINDINGS AGAINST MY OWN WORK, AND THE SECOND IS THE SHARPEST ZERO THIS SESSION HAS PRODUCED. *** First, the record conflated two different lists: the alphas THE RULE FLAGS are {0.05, 0.2} and the ones MATERIALLY wrong are {0.05} alone, since 0.2's grid error is 0.84 per cent against 0.05's twenty-four. A conservative rule flagging more than it must is THE RULE WORKING, and the honest record carries both numbers rather than tuning the threshold down to match the measurement and calling that agreement -- being wrong toward the sampler costs Monte Carlo noise and being wrong toward the grid costs seventy-three per cent. Second, SABOTAGE D READ ZERO RED. The row asserted that a planted table DIFFERS from the grid-only table; with plants routed through the sampler it STILL DIFFERED, because the ESTIMATOR had changed rather than the plant being applied. THE CHECK PASSED FOR A DIFFERENT REASON THAN THE ONE IT NAMES -- v4420's ratchet, a test satisfiable by the wrong cause -- and it would have left EVERY PLANTING GATE IN THE TREE passing for free. Repaired by asserting the MECHANISM rather than the symptom: a planted table must BE the grid with the plant applied, row for row. That is the third zero-red finding in four rounds, after v4435's unfalsifiable path check and v4436's unreachable half-vector flip, and the three of them are the same shape from three angles: a check that cannot fail, a check nothing reaches, and a check anything satisfies. FOUR SABOTAGES, MEASURED 4/4/1/0-THEN-1 BY NAME. UNCHECKED AND SAID PLAINLY: only directionalAlbedo's consumers were audited. physics/render/roughDiffuse.mjs builds a table of its own shaped the same way and IS NOT REACHED HERE, ndfIntegral and furnaceIntegral have their own defaults nobody has swept, and nothing outside physics/render/ was looked at at all -- so this round narrows a known problem rather than closing it. The thresholds are generous rather than tuned, and no claim is made that they are optimal. The sampler is not deterministic, which is why the grid path had to stay; a gate that needs reproducibility to twelve digits still cannot use it. And the seed-to-seed noise of the sampled estimator is NOT measured in this round -- v4437 measured it for a different estimator on a different integrand, and carrying that number over would be assuming what this round should check. The tree stands at 1467 gates. Full changelog on docs/CHANGELOG.md.
// v4437 -- *** ITEM 11 WAS WRONG, AND IT IS THE THIRD ABSENCE CLAIM OF MINE IN THREE ROUNDS TO BE WRONG. *** It said the tracer had never rendered an image and asked for a WGSL raygen pass. physics/render/pathTracerWgsl.mjs has generated WGSL and graded it AGAINST A REAL WEBGPU DEVICE since v4290 -- the LCG, the camera and the primary ray, 2304 coverage pixels agreeing -- and physics/render/pathTracerGpu.mjs ported the TRANSPORT at v4415, agreeing with the CPU BIT FOR BIT on 576 furnace pixels after finding three real bugs doing it. What made the false premise feel true is that v4432 and v4436 both shipped honest notes saying the SAMPLER was unchecked, and I read "the sampler is unchecked" as "there is no GPU path". *** AND v4415 HAD ALREADY WRITTEN DOWN WHY A GPU PATH COULD NOT HAVE HELPED. *** Its gate carries a row reading "the furnace CERTIFIES a broken cosine sampler, bit-exactly", with a note saying the pass is the point and is not good news. GPU-VERSUS-CPU IS NOT TWO INDEPENDENT PATHS WHEN BOTH RUN THE SAME SAMPLER: a shared sampler bug agrees perfectly and is perfectly wrong. What was missing was never a device. It was an estimator that shares no code with the one it checks -- quadrature marches a grid, Monte Carlo draws from the pdf, and the only thing they have in common is the BSDF they are both integrating. NEW physics/render/samplerCheck.mjs is that estimator, and it convicted four things on its first outing. *** ONE: principled.sample() RETURNED NaN ON EVERY SPECULAR DRAW, FROM v4432 UNTIL NOW. *** It read `h.cosTheta !== undefined ? h.cosTheta : Math.cos(h.theta)` from microfacet.sampleHalfVector, which returns a THREE-VECTOR WITH Y UP -- so cosTheta is undefined, the ternary falls through, h.theta is undefined too, and Math.cos of undefined is NaN. Both the SHAPE and the AXIS were wrong. THE TERNARY IS THE TELL: it guards between two GUESSED shapes and neither is the real one, and a guess with a fallback is still a guess. Five rounds of "ungraded" were carrying "broken", which is what an unexercised path is always free to do. *** TWO: THE PDF WAS THE CHOSEN LOBE'S RATHER THAN THE MIXTURE'S, AND A METAL HIDES IT COMPLETELY. *** evaluate() returns the WHOLE BSDF, so the estimator must divide by pSpec*pdfSpec + (1-pSpec)*pdfDiff evaluated at the sampled direction. Returning only the chosen lobe's term halves the denominator and the estimate came back at 1.9917x the quadrature for a dielectric -- while a metal reads correctly, because pSpec is 1 there and the mixture IS the one lobe. The obvious material on which to test a specular sampler is the one that hides the bug. *** THREE: THE TREE'S OWN QUADRATURE IS WRONG BY HALF AT ITS DEFAULT GRID. *** At metallic 1, roughness 0.2, cosO 0.35 the ladder reads 0.334246 at N=96 M=48 -- directionalAlbedo's DEFAULT -- then 0.876913, 0.987754, 0.991338, 0.991341. The Monte Carlo read 0.9921 at fifty thousand samples and 0.9916 at three million. THE ESTIMATOR THAT NEEDED NO REFINEMENT WAS THE ONE THAT LOOKED LIKE THE APPROXIMATION. The rule is a PRODUCT and both halves are asserted so neither can be dropped: roughness 0.2 at cosO 0.95 reads correctly, roughness 0.9 at cosO 0.35 reads correctly, and it is a narrow lobe AT a grazing view that falls between grid lines. GRID_FAILS_AT_V4437 records where not to believe the instrument. *** AND v4432'S HEADLINE SURVIVES, CHECKED RATHER THAN ASSUMED SAFE. *** The 1.0796 holds 1.079597, 1.079527, 1.079518, 1.079516, 1.079516 from N=96 to N=2048, because roughness 1 is a broad lobe -- the claim was not taken where the instrument fails -- and the second estimator confirms it independently at 1.0825. *** FOUR, AND IT IS THE SMALLEST AND THE MOST THIS SESSION: THE NEW RECORD'S OWN NUMBER WAS HAND-COPIED FROM THE WRONG GRID. *** GRID_FAILS_AT_V4437 first said the default grid reads 0.493436, which is the N=128 rung of an exploratory ladder and not the default at all; the default is worse, 0.334246. The row that RE-DERIVES it from the tree caught that within the hour, which is what those records are for. TWO MORE THINGS WERE MEASURED RATHER THAN CHOSEN. The Monte Carlo bound: a hand-picked "within half a per cent" went red at 1.35 standard deviations of ordinary noise, so eight seeds now give a relative sd of 5.04e-3 and the bound is three of those -- a Monte Carlo check with a tolerance somebody liked passes or fails by taste. And the first probe of this round REPORTED 0.00000 FOR A METAL rather than NaN, because its guard skipped bad samples and then divided by n: a guard that drops what it cannot use turns "every sample was NaN" into "the answer is zero", which is v4402's rule inside the instrument built to find v4402's rule. monteCarloAlbedo reports its nan and zeroPdf counts for exactly that reason. FOUR SABOTAGES, MEASURED 6/3/3/4 BY NAME, NONE ZERO -- the first two rounds this session each had a zero and this one does not, which is only worth saying because the zeros were the findings. UNCHECKED AND SAID PLAINLY: VARIANCE IS NOT MEASURED, only bias -- an unbiased estimator can need a hundred times the samples and nothing here would notice. The quadrature is CHARACTERISED and not fixed; every furnace number this tree has reported at low roughness and grazing angles should be re-checked against GRID_FAILS_AT_V4437 before it is believed. And physics/render/pathTracerGpu.mjs HAS ITS OWN SAMPLER which this round did not touch -- which is precisely the independence that made the CPU bug invisible to it, and is what item 11 should have asked for in the first place. The tree stands at 1466 gates. Full changelog on docs/CHANGELOG.md.
// v4436 -- *** A DIELECTRIC HAS LAWS WHERE A BSDF HAS LIMITS, AND FOUR OF THEM HOLD TO MACHINE PRECISION. *** docs/EXPLAIN-ITSELF.md item 9's next round, closing the fourth of the five gaps v4432 named in its own honest-scope sentence. NEW physics/render/transmission.mjs composes rough specular transmission onto the principled BSDF; the design is read from mmacklin/tinsel (Zlib), which extends Disney's BRDF exactly this way, and NO CODE IS TAKEN -- the maths is Walter et al. 2007, a paper rather than a repository, which needs no licence at all. v4432 could MEASURE its model and assert almost nothing, and its one attempted exact limit turned out to be a failed integral. This round is the opposite: Snell holds to 1e-12 relative, R + T = 1 to 1e-14 across 164 (angle, index-pair) samples, a smooth interface transmits EXACTLY ZERO past the critical angle by BRANCH rather than by tolerance, and the non-reciprocity is itself an exact law -- f(i->o)/etaO^2 == f(o->i)/etaI^2 to 4.7e-16. *** THAT LAST ONE IS THE TRAP THIS FILE EXISTS TO NAME: *** PLAIN reciprocity fails by exactly 2.25x ON A CORRECT LOBE, so v4432's reciprocity row, copied unchanged to the transmission side, would have gone RED ON THE RIGHT ANSWER and a model built to pass it would be wrong. *** THE FIRST DRAFT PARAMETERISED BY ROLE AND THAT WAS THE BUG. *** n1 and n2 name the incident and transmitted media, which is how every textbook writes Fresnel and is exactly wrong for a BTDF: A ROLE FLIPS WITH THE DIRECTION OF TRANSPORT AND A SIDE DOES NOT, so the same interface needs its arguments swapped depending on which way light goes and nothing in the call says so. The energy integral came back a converged 0.477 where Fresnel says 0.95 -- a deficit of almost exactly 1.5^2, THE ETA-SQUARED FACTOR FROM THIS FILE'S OWN HEADER ARRIVING AS A BUG IN THE FILE THAT DESCRIBES IT. nAbove/nBelow cannot be swapped by accident, because the geometry decides which is which. *** AND WHAT SORTED IT WAS REFINING THE INSTRUMENT, WHICH IS v4432'S RULE PAYING OFF BY SAYING THE OPPOSITE THING. *** The total held at 0.47700 from N=128 to N=1024 and moved 0.008 across the whole roughness range. A wrong number that does not move when you refine the grid is the MODEL; one that moves is the GRID. Last round that rule said the mirror limit was the instrument; here it said this was not, and a rule that only ever exonerates is not a rule. *** TWO PREDICTIONS WRITTEN INTO THE HEADER CAME BACK BACKWARDS, AND BOTH ARE CORRECTED IN PLACE RATHER THAN QUIETLY DELETED. *** It predicted the rough dielectric would LOSE energy the way single-scatter GGX does. IT GAINS: worst R + T is 1.28276 at alpha 1, cos 0.25 -- twenty-eight per cent more light than arrived, more than TRIPLE v4432's 1.0796 on the opaque model. And the reflection half is CLEARED rather than argued: this file's own brdf() with the Fresnel term forced to one agrees with physics/render/microfacet.mjs's ALREADY-GRADED directionalAlbedo to 1.6e-6 across the roughness range, including the 0.37889 that v4432 reported as its white metal. The reflection lobe loses energy exactly as it is known to; the transmission lobe gains more than that back; the two mechanisms do not cancel and the excess is held to a RANGE so a move in either direction is a red. The header also predicted the rough lobe would transmit zero past the critical angle. IT DOES NOT AND SHOULD NOT: a facet tilted away from the macro normal can present a LOCAL incidence inside the critical angle, so a rough dielectric has no sharp critical angle at all. Measured going glass to air at cos 0.5, well inside TIR: 3.1e-6 of the light leaks at alpha 0.001 and 0.435 at alpha 0.8, monotone the whole way -- so the falsifier is the MONOTONICITY and the vanishing, not a zero, and a gate asserting the zero would have been asserting something false about a correct model. *** AND THE SWEEP FOUND A NaN IN A MODULE THAT HAS BEEN GATED FOR HUNDREDS OF ROUNDS. *** physics/render/fresnel.mjs returns T = NaN at EXACTLY grazing incidence: the transmission factor is (n2 cosT)/(n1 cosI), so cosI = 0 is a division by zero multiplied by a |t|^2 of zero. Its own gate tests grazing at cos 1e-3, 1e-5 and 1e-7 -- APPROACHING A BOUNDARY IS NOT EVALUATING IT, which is this session's recurring defect wearing its tenth costume -- and a NaN in T propagates silently through every R + T that consumes it. Repaired as a branch sitting beside the total-internal-reflection branch it belongs next to; fresnel-selfcheck.mjs stays green. FOUR SABOTAGES, MEASURED 5/4/10/0-THEN-1 BY NAME. The 10 is Schlick standing in for exact Fresnel, and the index-matched limit is what makes it so loud: Schlick reflects 1.1e-1 off an interface that IS NOT THERE while exact Fresnel reflects 2.6e-31, so where v4432 could only see this as a 2.1e-2 disagreement to be traced, here it is wrong against an exact zero. AND THE ZERO IS THE SECOND ROUND RUNNING THAT A ZERO-RED FOUND UNFALSIFIABLE CODE: the half-vector flip cost nothing because every consumer takes Math.abs of its z and squares the denominator, so a real postcondition was unreachable -- v4435's path check one round later in a different file, and the repair is the same, assert the postcondition itself. UNCHECKED AND SAID PLAINLY: WHY the single-scatter BTDF gains where the BRDF loses is NOT DERIVED here, only measured and localised. Every number on the page is quadrature, and nothing checks the sampler -- which is exactly what item 11, added to the plan this round, exists to fix. One interface, one index, grey: no dispersion, no nested media, no coloured absorption, and no subsurface, which is the fifth gap and still open. Items 11 and 12 are added to the plan from reading camcimahir/webgpu-ray-tracer and DaveFace/UnrealRetroShaders, and NEITHER IS TAKEN THIS ROUND. The first is the only repository in this whole sequence that passes the toolchain test -- WGSL and JavaScript in a browser, where GLSL-PathTracer, tinsel, sandbox and warp all failed on C++/CUDA -- and it is the first blocked on the LICENCE instead, because it declares none and the default is all rights reserved. The tree stands at 1465 gates. Full changelog on docs/CHANGELOG.md.
// v4435 -- *** ITEM 10 WAS WRONG WHEN I SHIPPED IT TWO ROUNDS AGO, AND THE TREE HOLDS TWELVE OF THE THING IT SAID THERE WERE NONE OF. *** docs/EXPLAIN-ITSELF.md item 10, written at v4432, said the renderer has NO BVH AT ALL, citing `grep -li bvh` over physics/, render/ and world/ which "finds mesh CSG and a spatial-agreement gate". NEW tools/ship/absenceScope.mjs grades it: TWELVE FILES OF REAL BVH CODE, AND THE CLAIM NAMED TWO. *** IT FAILED THREE SEPARATE WAYS AND ONLY ONE OF THEM IS THE ONE ANYBODY EXPECTS. *** OUT OF SCOPE: mesh/meshBVH.mjs is a BINNED-SAH RAY-TRIANGLE BVH taken from gkjohnson/three-mesh-bvh, shipped at v4221 with a green gate, sitting in TOP-LEVEL mesh/ -- and the three directories searched were physics/, render/ and world/. The grep was correct; THE SCOPE WAS THE CLAIM, and nothing inside a claim ever says how wide it was. IN SCOPE AND SUMMARISED AWAY: physics/sph/bvhNeighbours.mjs, a Morton BVH, WAS in the searched directories and the prose summary of the grep's output dropped it. AND A DENIAL COUNTED AS A PRESENCE: physics/render/rtPipeline.mjs matched only because its comment says "Linear over the geometries. NO BVH" -- a file that matched BECAUSE IT ASSERTS THE ABSENCE is evidence FOR the claim, and so are main.js and brain/brain.js, which carry item 10's own text. That is item 5's defect, a record ABOUT a thing counted as the thing, arriving in the one place nobody thought to look for it. *** THE NARROW CLAIM SURVIVES AND THE SENTENCE SUPPORTING IT DOES NOT: *** the tracer really has no BVH and rtPipeline.mjs says so in its own words. *** AND THE DETECTOR WRITTEN TO CATCH THIS COMMITTED IT ON THE FIRST TRY, AGAINST THE SINGLE FILE IT EXISTED TO FIND. *** The first draft matched `\bbvh\b`, which is what anybody writes, and it MISSED mesh/meshBVH.mjs -- whose code carries the term in exactly ONE identifier, MeshBVH, with NO word boundary between Mesh and BVH because both sides are word characters. A REGEX WORD BOUNDARY IS A RULE ABOUT PUNCTUATION AND A PROGRAMMER'S WORD INCLUDES THE CAMEL HUMP, and those are not the same rule. tokenMatch grades humps instead: meshBVH, bvhNode and BVHNode all match, abvhc and subvh do not. That is the ninth sighting this session of a detector matching the shape its author pictured, and the first where the detector and the defect were the same round. *** THE ITEM SHRINKS, AND ITS STATED HARD PART TURNS OUT TO BE ALREADY SOLVED. *** It is not "build a BVH" but "two-level the SAH BVH the tree already ships and point rtPipeline's linear-over-geometries loop at it". v4432 said the value key was the hard part and the tree had no way to measure it; physics/sph/neighbourBakeoff-selfcheck.mjs measured exactly that once -- BVH against spatialGrid on identical particle sets, IDENTICAL NEIGHBOUR LISTS ASSERTED BEFORE ANY TIMING WAS BELIEVED, the rebuild counted, machine-independent check counts rather than milliseconds -- and concluded the GRID WINS for per-step SPH. The instrument exists and it has already answered NO once. *** FOUR SABOTAGES, MEASURED 10/2/0-THEN-1/6 BY NAME, AND THE ZERO IS THE ONE WORTH READING. *** Dropping the path check from scan() cost NOTHING: once tokenMatch understood humps, every file in this tree named for the term also spells it in code, so the path check rescued 0 of 14. NOT WRONG, UNFALSIFIABLE -- which in this tree is the same problem -- so it is now graded against a fixture tree holding a file named sceneBvh.mjs whose code never says the word, and the sabotage costs one row. A row in section 5 also went red ON ITS OWN TEST FIXTURE: a check asking whether a file BUILDS a BVH scanned raw text and matched the string "class MeshBVH {" in section 1, which is a fixture and not a class. What a file DOES is codeOnly's question and codeOnly blanks string bodies; that is v4422's two-stripper rule arriving uninvited. UNCHECKED AND SAID PLAINLY: ONE absence claim is wired in, item 10's. The other nine items in EXPLAIN-ITSELF.md make absence claims and none is graded, because each needs a TERM chosen by a person and a wrong term produces a confident wrong answer -- building a term-guesser would be committing this round's defect a third time inside this round. The denial patterns are eight English shapes and English has more; an unmatched denial scores `mention`, which OVERSTATES what the tree holds and so makes an absence claim look worse rather than better, an asymmetry that is deliberate. And `exclude` is a real hole: no mechanism here can tell a register of BVHs from a BVH, so the two exclusions -- this module and its own gate, both of which the gate went red on before they were named -- are asserted BY NAME in section 5 rather than trusted. *** AND THE ROUND'S VERIFY TURNED UP A GATE FROM ANOTHER ROUND THAT HAS NEVER BEEN GREEN. *** tools/ship/orreryUniverse-selfcheck.mjs, shipped at v4433, requires vendor/box3d to hold MORE THAN TEN .c/.h files and its own message asserts fifteen. The tree holds EIGHT -- and `git ls-tree` says it held eight at the commit that shipped the gate and twenty commits before that, so nothing went missing and the number was never measured. A THRESHOLD AND A MESSAGE THAT BOTH DISAGREE WITH THE TREE ARE TWO HAND-TYPED RENDERINGS OF SOMETHING THE GATE CAN COUNT, which is item 1's defect in a third place this session. Repaired here rather than registered, because registering a one-round-old red hides it: the count is derived, and the assertion now states what the row actually needs -- jolt vendors NONE of its upstream language and box3d vendors SOME. *** AND A SECOND GATE FROM ANOTHER ROUND CRASHED RATHER THAN SKIPPED, WHICH IS A CONVENTION THIS TREE WROTE DOWN AND THEN BROKE. *** tools/ship/pageFxOverlay-selfcheck.mjs, shipped at v4434, imports jsdom STATICALLY. jsdom is not vendored -- it is `npm i jsdom --no-save` -- so on any box without it the gate exits 1 with a stack trace and ZERO FAIL LINES, which is the exact shape v3605 and v4392 keep finding: A COUNT OF FAILURES IS NOT A VERDICT UNLESS THE PROCESS FINISHED, and this process never started. tools/ship/placementRender-selfcheck.mjs states the rule in its own header -- 'A MISSING DEPENDENCY MUST SKIP RATHER THAN THROW' -- so the repair is the tree's own pattern rather than a new one: a dynamic import in a try, and a skip that NAMES WHAT IS MISSING and says in as many words that the listener-leak check is UNANSWERED here rather than green, because v4402's rule is that an absence read as a skip is an absence read as a pass. TWO GATES REPAIRED THIS ROUND THAT THIS ROUND DID NOT WRITE, and both were shipped by rounds whose own verify should have caught them. The tree stands at 1464 gates. Full changelog on docs/CHANGELOG.md.
...
// v4425 -- *** CROSSING THE SHIP-TIME BUDGET IS A ONE-WAY DOOR, AND TEN REGRESSIONS WERE HIDING BEHIND IT. *** quickSweep decides what a round runs with two rules: selectGates skips a gate whose LAST RECORDED time exceeds the 3 s budget, and runQuickSweep rewrites timings ONLY for the gates it ran. A skipped gate's number is therefore never rewritten, and the only thing that could rewrite it is the sweep that just refused to run it. ONE SLOW OBSERVATION EXILES A GATE FROM EVERY FUTURE SWEEP, PERMANENTLY, and no amount of the gate getting faster brings it back. Demonstrated end to end through the real runQuickSweep and IN BOTH DIRECTIONS, because a fixture that can only come out one way is not a demonstration: seed a 42 ms gate's row with 999999 ms and it is not run and the lie survives the write; seed it with 1 ms and it runs and the row comes back measured at 45 ms. *** ALL 378 EXILES THAT EVER FINISHED, RE-TIMED ONE AT A TIME: 361 GREEN, 17 RED. *** 228 of the 378 clear the 3 s budget alone -- they were exiled by eight-way contention, not by being slow. The number that exiled them is inflated by a median of 2.2x and up to 182x: exitBanner-selfcheck is recorded at 12,940 ms and takes 71. It is wrong in the other direction too, which is why this is a measurement and not a story -- shippedLadder has gone from 4,906 ms to 20,479, and the exile hides the GROWTH just as completely. *** SEVENTEEN RED: SEVEN ARE ON redCensus.RED_AT_V4279 AND TEN ARE ON NO REGISTER AT ALL -- AND EVERY ONE OF THE TEN CARRIES A RECORDED EXIT CODE OF 0. *** The file says they passed. They went red AFTER being exiled, which is the whole mechanism: the door shuts on a slow observation and the regression happens on the far side of it. *** FOUR OF THE TEN ARE THIS SESSION'S OWN AND ARE REPAIRED HERE. *** runnerBudget: v4424 shipped tools/ship/slowCensus.mjs spawning gates under a flat cap while neither reading gateBudget.mjs nor declaring its own -- it declares one now, and the reason is real, because every gate it runs is one with NO measured budget and a per-gate table has nothing to say about it. crossBackend: NINE WGSL producers from v4408-v4416 were neither in the corpus nor excluded -- seven are enrolled compile-only (each already RUNS on a device in its own gate; what the corpus adds is the SECOND BACKEND'S COMPILER, a different question from whether the numbers are right), and lobeWgsl and glslFnToWgsl are excluded by name as a function with no entry point and a translator. Corpus 43 -> 50. staleness and caseStudy were the ritual's own stale gate count. THE OTHER SIX ARE NAMED AND LEFT RED, NOT REGISTERED: gateSweep.SWEEP_V4297's rule is that a regression is a thing to repair and that listing it would make its red acceptable again. physicsReach is owed and attributed -- SEVEN of its 36 doorless modules are this session's shader sources, and moving the baseline to 36 would be the "record of having given up" this arc refused for graveyard at v4424, while building a device page for seven WGSL strings is a round rather than a patch. Three are left UNATTRIBUTED rather than guessed. *** AND SO THIS ROUND SHIPS ALL GREEN OVER TEN REDS IT MEASURED ITSELF, WHICH IS THE FINDING AND NOT AN OVERSIGHT. *** verify.mjs swept 937 gates and reported 18 known reds; every one of the ten is still exiled over the budget, so the sweep still cannot see them, and the budget is deliberately unchanged. A round that quietly raised it to make its own ALL GREEN mean more would have been choosing the answer. *** ONE REPAIR TO THE RECORD, AND IT IS NOT A POLICY DECISION. *** sweep-timings.json's note said its contents were "OBSERVED at the last quickSweep run" -- true of the rows the run rewrote, false of every other row, and the other rows are exactly the exiles. `captured` dates the RUN and was being read as dating the ROWS. Each row now carries its own `observed` stamp, null for rows older than the field. The budget, the cap and the sweep's cost are UNCHANGED: what the sweep should cost is a ship-ritual decision and, following v4424, not one to make inside the round that measured it. *** AND THE WIDER RUN BROKE THIS ARC'S OWN INSTRUMENT, WHICH IS THE CORRECTION WORTH KEEPING. *** tools/ship/wgslSpec-selfcheck prints its verdict on STDERR and nothing at all on stdout, so a check counter reading one stream saw zero checks and called a RED a CRASH. v4424 argued that its counter's undercount "cannot change a verdict, since an undercount can only turn a RED into a CRASH and nothing here was classified CRASH" -- THAT WAS SOUND FOR ITS FORTY-THREE GATES AND IS NOT A GENERAL LAW, and 378 gates is what found the case. slowCensus.runGateSerial counts both streams now and records which stream a gate spoke on. Six stale exit-1 codes v4424 found are all GREEN when run, and four of the six would clear the budget today (orrery 13931 -> 2369, fetchCap 5976 -> 2157, typecheck 5292 -> 2391, splatSort 3615 -> 1230) on a number that has been wrong for as long as it has been unread; one got SLOWER. tools/ship/budgetExile.mjs and tools/ship/budgetExile-selfcheck.mjs, five sections and 23 checks, running one cheap real gate as a fixture; ten sabotages by name and NO 0 REDS. STILL UNCHECKED AND NAMED: the 132 exiles recorded at the sweep's 20 s cap were not re-timed here, so what they hold is still unknown, and the six owed regressions are owed. The tree stands at 1447 gates. Full changelog on docs/CHANGELOG.md
// v4424 -- *** SIXTY-THREE GATES WERE EXEMPT FROM THE SHIP GATE BECAUSE NOBODY HAD MEASURED THEM, AND THREE OF THEM WERE RED. *** tools/ship/verify.mjs closes on "no gate outside the red register is red". redRegister() builds that register and SIXTY-THREE OF ITS NINETY-ONE GATES were there under the reason "redCensus.UNCONFIRMED_SLOW" -- which redCensus.mjs is scrupulous to define as NOT red and NOT green but UNMEASURED. All sixty-three were run one at a time at a 180 s cap: 39 GREEN, 21 still unfinished, THREE RED, no crashes. doorKinds, graveyard and orphanDisposition exit 1 in 75 to 151 seconds alone on an idle box -- not one is a timing failure, and every one has been red and waved through for a hundred and forty-five rounds. *** THE FIRST FORTY-THREE MEASURED WERE ALL GREEN, AND THAT INTERIM CLAIM WAS WRITTEN DOWN BEFORE THE LAST TWENTY REFUTED IT. *** "Zero red -- the bucket has been hiding successes" was the honest summary of the evidence in hand at 43 gates, it was committed, and finishing the measurement destroyed it. An unmeasured gate is not a green one however many of its neighbours turn out green; that is exactly what referenceKind demonstrated at v4279 and what the last twenty gates demonstrated again. *** ONE OF THE THREE IS A RATCHET THIS SESSION HAS BEEN BREAKING WHILE SHIPPING ALL GREEN OVER IT. *** graveyard-selfcheck counts modules that export functions and are imported only by their own gate: baseline 93 set at v4153, count now 145. Twenty-seven of the 145 were first committed in September and SIXTEEN on the day that shipped v4408 through v4424 -- fresnelWgsl (v4416), paintGenerators (v4420), polyBrush (v4421), paintTransforms (v4422) and paintFields (v4423) among them. EVERY ONE OF THOSE ROUNDS REPORTED ALL GREEN, TRUTHFULLY, because verify.mjs runs a different and much smaller set and this gate was in the bucket. THE EXEMPTION IS WHAT MADE THE ALL GREEN TRUE AND USELESS AT THE SAME TIME. The baseline is NOT moved to 145; that would turn a ratchet into a record of having given up. *** THE EXEMPTION HAD ALSO SURVIVED ITS OWN RESOLUTION. *** redCensus.SLOW_PARTIAL, IN THE SAME FILE as the list that grants the exemption, already held nineteen GREEN verdicts for gates on that list, and redRegister() reads one object and not the other. Thirty-nine gates are now exempt-as-unmeasured with a green verdict on record. *** AND THEY WERE EXEMPT TWICE OVER, THE FIRST LAYER RECORDING A CAP AS A TIME. *** selectGates() never runs a gate whose last observed time exceeds the 3 s ship budget, and all sixty-three sit at 20 s in sweep-timings.json with exit code 124 -- the sweep's CAP and "gave up", written into the two fields a reader takes for how long the gate takes and what it returned. Measured, the forty-two that finish take 1.6x to 7.6x that, median 3.4x. AND THE DECISION IT FEEDS IS STILL RIGHT, WHICH IS EXACTLY WHY NOBODY NOTICED: a lower bound of 20 s is over a 3 s budget however far above the cap the truth is, and the file's own note scopes itself to that one use. *** THEY WERE NEVER TOO SLOW FOR THE CAP THAT BUCKETED THEM: *** 38 of the 42 finish under the v4279 sweep's own 120 s, alone. The bucket was about the sweep being EIGHT-WAY, not about the gates being slow -- the same contention redCensus.mjs recorded when seven of its forty-six "reds" turned green on an idle box. THE FIRST ATTEMPT AT THIS ROUND WAS THAT EXACT MISTAKE and is in PROTOCOL.abandoned rather than deleted: four workers at a 30 s cap, eight timeouts in the first eight gates, killed. *** THE RUNTIMES REPRODUCE IN THE ORDERING AND NOT IN THE MAGNITUDE, AND ONLY ONE OF THOSE IS ABOUT THE GATES. *** All sixteen comparable greens came back FASTER than v4279 recorded, median ratio 0.880 -- a uniform shift is what a quieter box looks like as much as what better code looks like, so it is reported and NOT gated. What is gated is the ordering, Spearman rho 0.891, and the residual: one global 0.880 scale still leaves rig/cinematicShot 36% off. Zero contradictions against the v4279 record in sixteen comparisons. *** AND THE BUCKET IS THE NAMED PART OF A MUCH LARGER UNNAMED ONE, WHICH HAS A FOURTH RED IN IT THAT IS MINE. *** The ship gate runs 936 of the tree's 1446 gates; 510 are skipped over the 3 s budget and FOUR HUNDRED AND THIRTY-SEVEN of those are on no register at all -- not red, not green, not even filed as unmeasured. The 63 are the part that at least has a list. One gate outside the bucket got run in this round, for the unrelated reason that it is this round's own dependency: tools/ship/redCensus-selfcheck.mjs, ten minutes long and never executed by any ship sweep -- AND IT IS RED. Its failing check is the one written to catch exactly this: 'each recheck's own count must equal the length of the list it names'. RECHECK_V4313.nowGreenGates filtered FIXED_SINCE_V4279 with !/^v43(1[4-9]|[2-9])/ -- a regex meaning 'at or before v4313' that knows only rounds beginning v43 -- so when V4414, ONE OF THIS SESSION'S OWN ROUNDS, pruned shaderCensus into that list, the entry walked straight into a record of v4313 and stayed there for ten rounds. The comment above that field describes this exact defect from the previous time it happened. FIXED, not filed: the filter reads the version as a NUMBER now, and the cheap half of the check is asserted in this round's own gate so a reader need not wait ten minutes to learn it. THIS IS AN ANECDOTE ABOUT THE 437 AND NOT A RATE -- nothing sampled them. *** WHAT IS REPAIRED AND WHAT IS NOT. *** The three reds in the bucket are FILED -- redCensus.RED_AT_V4424, each with the check that fails and why -- and redRegister() now credits them to their failure instead of to the absence of a measurement, so the exemption is 60 rather than 63. Nothing else is fixed on purpose: the bucket is not emptied (18 gates still have no verdict from any run and stay in the third state), redRegister() is not taught to read the green verdicts, and no baseline is moved. Section 2's headline number is REPORTED AND NEVER ASSERTED, because it RISES when somebody measures more of the bucket and falls when somebody repairs the register -- a bound in either direction punishes one of the two things a later round could do. The two monotone quantities are the ones ratcheted, and they move in opposite directions under the same work. tools/ship/slowCensus.mjs and tools/ship/slowCensus-selfcheck.mjs, ten sections and 58 checks, pure, running no gate at gate time; twenty-three sabotages by name. TWO EARNED ZEROS: ranking BOTH arrays descending inside spearman is the same statistic -- the rank difference is negated and then squared -- so no fixture can see it and a one-sided reversal is used instead; and REMOVING the exemption from redRegister() passes, which is correct, because a ratchet that fails on its own repair is a broken ratchet. Also found by sabotage and closed: stillUnmeasured counted having a RECORD as having a VERDICT; capRecordedAsTime would have divided this run's cap by the v4279 cap and called it a ratio; and budgetSkip reporting 510 where 437 was meant still cleared the floor the gate had set, so the difference is checked against the register directly. And the runner's check counter knew one of the tree's two house styles, undercounting " ok " gates -- it changed no verdict and cannot, since an undercount can only turn a RED into a CRASH and nothing here was classified CRASH. The tree stands at 1446 gates. Full changelog on docs/CHANGELOG.md
// v4423 -- *** FOUR MORE GENERATORS, THE FIRST TARGET THAT IS NOT A PICTURE, AND A UNIFICATION THAT DOES NOT HOLD. *** A sweep of the tree for target potential found five candidates and four of them run headless: physics/render/pathTracer.mjs (a CPU path tracer), physics/tomography's filtered back-projection, fx/nebula's procedural fBm, and world/procPlanet's globe. *** THE FIFTH IS DELIBERATELY ABSENT AND SAID SO: *** ev/pict.js decodes classic-Mac PICT images and would be the only HAND-DRAWN distribution in the tree -- art rather than a formula, which is exactly what v4420's finding most wants -- but the tree contains ZERO .pict files and nothing calls the decoder. "We have a PICT decoder" and "we have PICT images" are two facts and only the first is true. *** ONE: THE PATH TRACER RETURNS A SCALAR, SO THE COLOUR HAS TO BE PUT ON, AND THAT IS A CHOICE NOTHING HAS HAD TO MAKE BEFORE. *** Every target in this arc has been an RGB raster; render() returns a Float64Array of RADIANCE, one number per pixel, and the tomographic reconstruction the same. The same field can be replicated to three identical channels or mapped through fx/voxelize/fireRamp.js's blackbody ramp -- A SHIPPED RAMP, because inventing one to make the point would have been choosing the answer. THE INFORMATION IS IDENTICAL AND THE FITTER'S PROBLEM IS NOT: primitiveFit minimises L2 in RGB, and L2 in RGB after a non-linear ramp is not L2 in the field. Measured over twelve seeds: on the traced source, shapes chosen against one colouring are 12% worse on the other; on the tomographic source the two are interchangeable to within 3%, a factor of four apart. *** AND THE OBVIOUS EXPLANATION IS MEASURED AND FAILS: *** a heavier tail would compress a field's bulk into a small part of its range and make a ramp bite harder, and the traced field's middle 80% occupies 0.81 of its range against tomo's 0.64 -- THE WRONG SIGN. The effect is real; what causes it is not established here, and naming a candidate that fails is worth more than leaving a reader to supply one that would. Also exact: optimalColour on a grey target returns EXACTLY grey, 0 of 20 by === on the channels, because the solver averages per channel and three identical inputs give three identical outputs -- so two thirds of its work is a copy on a scalar source. And in 7 of 24 cases a search run against the OTHER colouring found geometry that fits THIS one better than its own search did, which is only possible because the greedy fitter is far from optimal (v4417 measured that gap at a factor of two, v4421 at "cannot close it at all") -- reported as a symptom, not as a technique. *** TWO: ELEVEN COLUMNS, AND A NEW POSITIVE. *** A policy trained on flat regions scores +4.7 sd on the PATH-TRACED column -- above its own training distribution's +4.4 -- and +3.2 on the same field through the ramp, while tomo gives -15.2, tomoRamp -14.5 and planet -12.7. v4420 concluded "worse than random on every generator it did not train on" FROM FOUR OFF-DIAGONAL COLUMNS, AND ELEVEN SAYS THAT WAS TOO STRONG. The correction is this round's, not a reader's: two spheres and a ground plane are large smooth regions, which is what flat quadrants are too. *** THREE: AND THE TEMPTING EXTENSION OF v4422 DOES NOT SURVIVE. *** That round measured the policy's 4x4 grid contrast HALVING under a CRT and predicted, FOR TRANSFORMS OF FIXED CONTENT, that energy above the grid's Nyquist blinds it. It is tempting to read that as "the statistic ranks targets". IT DOES NOT: across these eleven columns the Pearson correlation between grid spread and the policy's sd is 0.34, and the positive columns' spreads (0.24 to 0.50) sit STRICTLY INSIDE the negatives' (0.18 to 0.51). v4422's claim is about transforms and is untouched; the generalisation to different CONTENT is what this round tried and could not support. A MECHANISM THAT EXPLAINS ONE EXPERIMENT IS NOT A VARIABLE THAT ORDERS A DIFFERENT ONE. Also earned: the nebula fixture is NOT PORTABLE ACROSS PRECISIONS. Its hash is sin(x*127.1 + y*311.7)*43758.5453, and at f64 against an f32 simulation the mean difference is 0.32 on a value living in [0, 1), worst 0.82 -- essentially UNRELATED numbers, not slightly different ones. tools/ship/webgpuHarness.mjs has recorded the same construction returning 0.921690 on a CPU and 0.240234 on a GPU since v4270 and called it a caution; this is that caution as a measurement, and every other target in this arc would survive being generated on a device while this one would not. fx/paintFields.mjs and tools/ship/paintFields-selfcheck.mjs, seven sections and 15 checks in 13.2 s, one training, no device and no model; eight sabotages 2/3/1/2/1/1/1/1 by name and no 0 reds. ONE IS INSTRUCTIVE ANYWAY: freezing the tracer's SCENE leaves its sampling seed varying, so the pictures still differ by Monte Carlo noise and seedSpread correctly does NOT fire -- "does the picture vary" and "does the scene vary" are two questions and this gate only ever claimed the first. *** AND THE NEBULA GENERATOR HAD TO BE BUILT TWICE. *** nebulaColorAt offsets its noise by cam.x * 0.00035 against a domain scaled by 2.4 -- a flight-view parallax, deliberately slow, for a camera that travels thousands of units -- so the first draft's camera in +/-20 moved the sampling point by 0.007 and produced twenty-four nearly identical pictures: mean pairwise distance 0.0088 against the other three sources' 0.18 to 0.27. THE MODULE WAS BEHAVING EXACTLY AS DESIGNED AND THE CALLER HAD THE WRONG SCALE. v4420's seedSpread caught it -- the third generator in three rounds it has caught, after a Krbn mesh chosen by seed % 4 and a frozen liquefy field. STILL UNCHECKED AND NAMED: one training row, on one generator, so eleven columns settle that four were not enough to say "everything" and settle nothing else. The tree stands at 1445 gates. Full changelog on docs/CHANGELOG.md
// v4422 -- *** THE SAME PICTURES, ONE POST-PROCESS, AND THE POLICY GOES FROM +4.4 SD ABOVE RANDOM TO TWELVE BELOW. *** v4417 varied representability, v4418 varied whether a pixel is on the subject, v4419 varied time, v4420 varied the generator -- all four change what the picture is OF. A TRANSFORM CHANGES NONE OF THAT: it takes a finished picture and re-renders it, so the content is identical BY CONSTRUCTION -- the same makeTarget seeds, the same four quadrants and disc -- and only the presentation moves. Measured, in sd above each variant's OWN random floor: identity +4.4, crtOff +4.4, liquefy +4.6, crtTrinitron -11.6, crtArcade -12.1, crtPipboy -12.7. *** v4420 NEEDED A DIFFERENT GENERATOR TO INVERT THIS POLICY; A SCANLINE MASK OVER ITS OWN TRAINING PICTURES DOES IT HARDER, and the policy then improves LESS than a uniform painter does. *** AND A DISPLACEMENT FIELD MOVING CONTENT UP TO THIRTEEN PIXELS DOES NOT DO IT AT ALL, which is what turns the result from "CRT is hard" into a statement about why. *** THE MECHANISM IS MEASURED RATHER THAN GUESSED. *** The policy's entire view is a 4x4 grid of MEAN residual per cell -- on a 32-pixel image, an 8x8 box filter, 20 floats in all. The CRT's shadow mask repeats every 3 pixels and its scanline term runs 224 to 480 cycles over 32 rows, BOTH FAR ABOVE THAT GRID'S NYQUIST, so averaging over a cell turns them into a constant offset added to every cell alike: THE GRID'S RELATIVE SPREAD HALVES, 0.251 to 0.132, and the ratio of its brightest cell to its dimmest falls from 2.75 to 1.60. The policy is looking at a picture where everywhere is equally wrong. Liquefy's peak displacement is 6 to 13 pixels AGAINST AN 8-PIXEL CELL, so it moves structure BETWEEN cells and the grid still reads it: spread 0.244, ratio 2.57, both within 3% of untransformed. SO THE FAILURE IS NOT ABOUT CONTENT AND NOT ABOUT DIFFICULTY -- IT IS ABOUT WHETHER THE TRANSFORM SURVIVES THE POLICY'S OWN DOWNSAMPLING, and the sign of the outcome follows the FREQUENCY rather than the severity. Two transforms above the Nyquist and two below, and that is stated as a PREDICTION so it can be wrong: any transform whose energy sits above an 8-pixel box filter should blind this policy and any below it should not, and nothing here tests a fifth. *** AND IT IS NOT ONLY THE POLICY, WHICH IS A SEPARATE FACT. *** The greedy fitter -- which has no observation grid at all -- goes from 0.030 to 0.205 under arcade, 6.8x worse. So "the CRT makes pictures hard" and "the CRT blinds the policy" are two claims, and the second is the one that needs the mechanism: DIFFICULTY ALONE CANNOT PUT SOMETHING BELOW RANDOM. Liquefy costs the fitter 17% and the null transform costs it nothing at all, by ===. *** THE NULL TRANSFORM IS A CONTROL THIS ROUND DID NOT HAVE TO BUILD. *** crtImage with PRESETS.off is the identity BIT FOR BIT -- 4096 of 4096 bytes at every picture -- through the same function, the same registry entry and the same call as crtArcade, differing only in that its scanDepth and maskDepth are zero. The policy and the random floor come back byte-identical through it, by === on the float, which is what lets every other number be read as the transform rather than as the harness. A MUST-NOT-MATTER THAT SHARES THE CODE PATH IS WORTH MORE THAN ONE THAT SKIPS IT. And the two real transforms move opposite things ON PURPOSE: the CRT leaves 1028 of 4096 bytes untouched and raises the flat-average start distance by a quarter to a fifth again, adding structure that was not in the picture; liquefy leaves 3757 untouched and returns the start distance to within 2%, because a resample that only REORDERS pixels cannot move the mean colour. Geometry without colour against colour without geometry. fx/paintTransforms.mjs and tools/ship/paintTransforms-selfcheck.mjs, six sections and 13 checks in 12.2 s, one training, no device and no model; seven sabotages 3/1/5/5/6/1/1 by name. ONE WENT 0 RED AGAINST A COMMENT THAT NAMED IT: freezing liquefy's displacement field makes every episode meet the same warp, which the module's own header calls "v4420's defect one level down" and which nothing enforced -- and spreading the transformed GENERATOR would not have caught it either, because the underlying pictures already differ by seed. The cure is ONE FIXED PICTURE through six transform seeds, which isolates the field from the content. The widest sabotage was the bug this round actually hit while writing it: liquefyModel's warp destructures `width` and `height`, and handed `w` and `h` it loops zero times and returns a blank image -- a two-character mistake in a bridge between two modules that each name their dimensions correctly for themselves. STILL UNCHECKED AND NAMED: the obvious repair is a finer observation grid, and it is not tried -- GRID is 4 in brain/rl/paintEnv.js and nothing measures what an 8x8 or a multi-scale observation would do to any row of this table. The tree stands at 1444 gates. Full changelog on docs/CHANGELOG.md
// v4421 -- *** THE FIFTH PRIMITIVE, AND THE FILL RULE THAT WAS NEVER A CHOICE. *** A sweep of the tree for brush potential found the answer already built: physics/mesh/strokeMorph.mjs carries ten digit outlines as SVG paths with cubic Beziers plus a MORPH between any pair, and ui/svgPath.mjs flattens an arbitrary path to closed polylines. So `polygon` joins KINDS, and it is the first added kind that costs fx/primitiveFit.mjs nothing structurally: A POLYGON IS A FILLED REGION WITH ONE FREE COLOUR, so optimalColour applies unchanged, and its coverage is spans, so differenceChange does too -- where doom fire breaks both (a fixed 37-colour palette and a stateful order-dependent update) and procBrush emits segments at alpha 0.12 that ACCUMULATE, a different compositing model rather than a different shape. *** WHAT HAD TO CHANGE: spansOf's POLYGON PATH TOOK THE MIN AND MAX CROSSING ON EACH SCANLINE AND FILLED BETWEEN THEM, WHICH IS A CONVEX RULE NOTHING SAID WAS ONE. *** For a triangle or a rectangle it is exactly right -- a convex shape meets a horizontal line exactly twice -- and for a concave outline it FILLS THE NOTCHES: a five-pointed star came out 18.2% too big, silently. Pairing every crossing instead is SVG's fill-rule evenodd, AND THE CHANGE IS SAFE PRECISELY BECAUSE THE THREE EXISTING POLYGON KINDS ARE CONVEX -- the two rules are BIT-IDENTICAL over 3000 random triangles, rects and rotatedRects, and bit-identical to the function as it ships, measured rather than argued. SVG's other rule, nonzero, is deliberately NOT implemented and its gap is measured instead: it differs from evenodd by 66 pixels on the tree's own digits 6 and 9, which carry exactly the self-crossing loop that separates the two. *** AND ADDING THE KIND SILENTLY CHANGED THE SEARCH DISTRIBUTION FOR EVERY EXISTING CALLER, WHICH AN UNRELATED GATE CAUGHT. *** randomShape() cannot invent a polygon and returns an ELLIPSE for a kind it does not know, so the moment "polygon" joined a single KINDS list, fitStep's proposer began drawing ellipses two times in five instead of one in four -- shifting the rng stream and moving numbers in krbnPaint-selfcheck, which is about screen-space versus surface. NOTHING THREW. ONE FROZEN LIST WAS ANSWERING TWO QUESTIONS -- "can this be rasterised" and "can the search invent one" -- and they were the same set for exactly as long as there were four kinds. They are now KINDS and SEARCH_KINDS. fitStep also takes its PROPOSER and its MUTATOR as arguments now, both defaulting to what it did before, because mutateShape jitters every numeric field and would turn a `points` array into NaN: a new primitive had to be a caller rather than a case in a switch. *** AND THE BRUSH LOSES ON ITS OWN HOME GROUND, WHICH IS THE ROUND. *** A target is built OUT OF four glyphs on a background. FIVE SHAPES -- a background rectangle and those four glyphs with their own colours -- REPRODUCE IT EXACTLY, 0.00000000 at every seed. The fitter given FORTY glyph proposals reaches 0.082 to 0.104, and ROTATED RECTANGLES BEAT IT ON THE SAME PICTURE, at every seed. BOTH OBVIOUS EXCUSES ARE RULED OUT BY MEASUREMENT: the two tools are the same size (6.8% of the canvas per proposal against 6.5%, with none of 2000 glyph proposals landing empty, and the digit outlines filling 31% to 81% of their own box rather than being thin loops), and quadrupling the search budget helps BOTH and does not change the order. *** A PRIMITIVE THAT RESEMBLES THE SUBJECT IS NOT THE SAME AS ONE THAT FITS IT: *** a rectangle adapts its outline to whatever the residual is, a glyph can only be moved, scaled and turned, and under greedy least-squares flexibility beats resemblance. *** AND ON THOSE EXACT FIVE SHAPES THE EXACT LEAST-SQUARES COLOUR GIVES 0.10 WHERE THE TRUE COLOURS GIVE 0. *** v4417 found the solver 33% worse than the true colours on an occluded shape; here it is the difference between exact and not at all, because at alpha 1 an under-shape loses its pixels ENTIRELY and its least-squares colour is averaged over pixels the finished picture never shows. The colour is optimal for each shape at the moment it is drawn, and the per-shape optimum is not on the path to the picture. Also earned: the morph is a shape parameter no other kind has, and it walks through a TOPOLOGICAL event -- from digit 0 to digit 8 the shoelace area falls monotonically 2191 to 184 while the painted region dips from 2188 pixels to 1330 and returns to 1822, because the shoelace cancels when the outline crosses itself and the paint does not. fx/polyBrush.mjs and tools/ship/polyBrush-selfcheck.mjs, seven sections and 17 checks in 4.0 s, no device and no model; eight sabotages 5/2/2/1/1/1/3/1 by name and TWO WENT 0 RED FIRST. Only 4 vertices of 651 across all ten glyphs and the star land exactly on a scanline, so no random fixture tested the half-open crossing rule at all -- a designed U with every vertex on one, including a horizontal edge lying along the scanline, is what now does. And a mutator returning garbage went 0 RED because fitStep SCORES every mutant: a nonsense polygon covers nothing, is rejected, and the fit falls back to its best random draw, silently worse -- every check was an inequality against rectangles that glyphs already lose, and nothing asserted that mutation HELPS. It does, by 9%. *** THE SABOTAGE HARNESS ALSO REPORTED "0 RED" TWICE FOR A GATE THAT HAD CRASHED ON A SYNTAX ERROR AND RUN NO CHECKS AT ALL; it now reports a non-zero exit with zero checks as a crash. A SUITE THAT CANNOT TELL "NOTHING FAILED" FROM "NOTHING RAN" MEASURES NEITHER. *** STILL UNCHECKED AND NAMED: nothing here puts the morph parameter in front of a LEARNER -- paintEnv emits five numbers for a rotatedRect and a polygon brush wants seven, two of which choose a glyph and walk between them. The tree stands at 1443 gates. Full changelog on docs/CHANGELOG.md
// v4420 -- *** "HELD-OUT PICTURES" MEANT HELD-OUT SEEDS, AND ONE GENERATOR OVER THE SIGN FLIPS. *** v4220 trained a policy to PLACE shapes and evaluated it on twenty-four seeds its gate calls "held-out pictures", finding it beats a measured random floor by more than three standard deviations. THAT CLAIM IS TRUE, IT STILL PASSES, AND IT IS REPRODUCED HERE TWICE. What nobody had said is that every one of those twenty-four is makeTarget -- four flat quadrants and a disc, five flat regions at every seed -- so varying the seed varies the four quadrant colours, the disc colour, the split and the disc's place, AND NOT WHAT KIND OF PICTURE IT IS. Held-out seeds and a held-out distribution are different claims and only the first was ever measured. *** THE TRANSFER MATRIX, IN SD ABOVE EACH GENERATOR'S OWN RANDOM FLOOR: trained on flat -- flat +4.4, ramp +4.0, marched -6.0, krbn -1.2, fire -2.0; trained on fire -- flat -12.0, ramp -1.5, marched -5.4, krbn -3.1, fire +19.2. *** The diagonal is strongly positive and SEVEN OF EIGHT OFF-DIAGONAL CELLS ARE BELOW THE RANDOM FLOOR. A policy trained on flat regions is SIX SD WORSE THAN UNIFORMLY RANDOM PLACEMENT on a ray-marched frame; one trained on fire is TWELVE SD WORSE on flat regions. IT DOES NOT MERELY FAIL TO TRANSFER, IT TRANSFERS NEGATIVELY -- worse than having learned nothing, because a uniform painter would have done better. The single positive off-diagonal is flat -> ramp at +4.0, and it has a reason -- both are large low-frequency fields where one big rectangle in roughly the right place is most of the answer -- so it is NAMED rather than left out. *** AND THE TWO ROWS RULE OUT "THE SECOND TRAINING SIMPLY DID NOT WORK": *** on fire the fire-trained policy scores +19.2 against the flat-trained one's -2.0, and on flat the positions reverse, +4.4 against -12.0. Each policy is excellent exactly where it trained and worse than random almost everywhere else, which is a statement about the EVALUATION and not about the learner. *** THE FAILURE IS UNIFORM, NOT A MEAN DRAGGED BY OUTLIERS: *** on its own generator each policy loses to the random painter on 1 to 8 of the 24 episodes; off it, on 13 to 22 of 24. *** AND EACH GENERATOR HAS ITS OWN RANDOM FLOOR, WHICH IS NOT A DETAIL. *** They span 4.5x -- 0.028 on a Krbn frame against 0.124 on a ramp -- so "beats random by N sd" is a statement about a DISTRIBUTION, and carrying one floor to another generator would make a policy look transformative on an easy target and broken on a hard one with nothing about the policy having changed. *** WHAT THE TRAINING DID NOT CHANGE IS THE SIZE OF THE SHAPE, AND THAT KILLS THE OBVIOUS EXPLANATION. *** The trained policy covers 24% of the canvas per proposal on every generator; so does an UNTRAINED one, 23.7% to 24.8%, indistinguishable, while random covers 15.8% by construction. A fresh tanh network outputs near zero and near zero maps to the MIDDLE of the env's size range -- so "it learned that big shapes win, and big shapes lose on a fire" is WRONG, and the measurement is what says so. What training moved is WHERE the shape goes, and where is precisely the part that is a fact about one distribution. The env change is one parameter with makeTarget as its default -- brain/rl/paintEnv.js takes opts.targetOf -- so v4220's gate passes untouched, which is the test of whether a change like this is a widening or a rewrite. fx/paintGenerators.mjs holds five generators behind makeTarget's own signature (flat, ramp, marched, krbn, fire) plus the shared evaluation protocol: THE FLOOR AND THE POLICY GO THROUGH THE SAME FUNCTION, because a floor measured by one loop and a policy by a slightly different one is the comparison that drifts in silence, and this whole round is a subtraction between the two. tools/ship/paintTransfer-selfcheck.mjs, seven sections and 10 checks in 17.4 s, no device and no model; nine sabotages 5/3/1/1/1/1/1/3/5 by name. ONE WENT 1 RED FOR THE WRONG REASON: a cache keyed on nothing rather than on the seed handed all twenty-four episodes ONE PICTURE, and the check written to catch exactly that was measuring the RAW generators while every episode is fed the MEMOISED wrappers -- the check and the thing under test were different objects. It now measures the wrappers. THE KRBN GENERATOR ALSO HAD TO BE BUILT TWICE: its first draft picked a mesh with seed % 4 and varied nothing else, so two of six seeds produced BYTE-IDENTICAL pictures, and seedSpread caught it before any policy number was read off it. *** AND THE SABOTAGE HARNESS ITSELF HAD TO BE FIXED: *** a run that timed out mid-suite left the tree sabotaged AND a clean backup beside it, and the next run copied the SABOTAGED file over that backup, destroying the original. It now refuses to start when a stale backup exists. A SABOTAGE SUITE THAT CANNOT RESTORE IS WORSE THAN NO SUITE -- it edits the thing it is grading. STILL UNCHECKED AND NAMED: nothing here trains on more than one generator, and a policy shown all five might well hold up on all five. The tree stands at 1442 gates. Full changelog on docs/CHANGELOG.md
// v4419 -- *** A TARGET THAT MOVES, UNDER A RULE THAT IS EXACTLY KNOWN -- AND THE STILL CONTROL THAT MAKES THE MEASUREMENT A MEASUREMENT. *** Every target the painter has ever been shown stands still: makeTarget's five flat regions, v4417's ramp and ray-marched frame, v4418's four Krbn projections. render/doomFire.mjs is a SEEDED cellular automaton, and v4178 took its RNG as a constructor argument from the start precisely so "same seed, same field, frame for frame" would be true by construction -- "an unseeded automaton cannot be checked against anything; you are left asserting that fire looks like fire". This round is what that decision was for. *** THE FIRE MOVES BY MORE THAN A WHOLE RESIDUAL PER FRAME. *** One step changes 23% of the pixels and moves the image by 0.0702, against the 0.0662 left after SIXTY shapes of painting -- a ratio of 1.06, so the painter is behind after a single frame and cannot catch up. And the motion SATURATES at once: +1 frame is 0.0702, +24 is 0.0952, so this is a stationary process wandering around a fixed point rather than a drift a tracker could fall progressively behind. *** THE QUESTION NOTHING HAD ASKED: DOES THE FITTER TRACK, OR RE-SOLVE? *** primitiveFit is greedy and stateless -- no notion of a previous frame -- while a renderer would obviously keep the canvas. Answering it honestly needs THREE runs: warm (one canvas, 10 shapes added per frame), cold at the same per-frame cost (10 shapes from flat, every frame), and cold at the same TOTAL shapes (from flat, as many as warm has accumulated). Warm is 32.7% BETTER than the second and 15.1% WORSE than the third. *** AND THE STILL-TARGET CONTROL IS WHAT MAKES THAT THIRD NUMBER MEAN ANYTHING: on a target that does not move, "add ten shapes to the canvas" and "fit ten more from flat" are THE SAME COMPUTATION, because fit() is itself incremental -- and over 25 frames the two curves are equal BY === ON THE FLOAT at every single frame. *** So on a moving target the entire gap is the motion, isolated rather than argued. *** AND THE STALENESS COMPOUNDS: 1.6%, 4.1%, 9.9%, 15.1% at 4, 8, 16 and 24 frames, monotone. *** The accumulated shapes carry information about a picture that is gone and the painter has no way to retire them -- fit() discards a candidate that cannot improve the canvas and has NO OPERATION AT ALL for removing one it already drew. Warm settles into a 9.4%-wide band after three frames while cold at the same shape counts spans 42% and keeps marching down; warm takes 44% of the improvement from the same extra shapes and goes BACKWARDS on 12 of 23 frames against cold's 6. *** THE FIRST DRAFT SAID WARM "HITS A FLOOR AND STAYS THERE" AND THE MEASUREMENT SAID OTHERWISE -- it does keep improving, by 17.1% -- so the claim is now about the RATE, which is what is true. *** AND A PALETTE THE EXACT COLOUR SOLVER CANNOT REACH. *** Doom fire is 37 fixed colours; optimalColour returns a continuous least-squares mean that is almost never one of them -- 12 of 60, median 8.12 RGB units away, because the mean of a mixture is not a member of it. Snapping every colour to the nearest entry costs 5.7%. SO primitiveFit's FIRST HEADLINE IDEA IS WORTH ABOUT AN EIGHTEENTH OF ONE FRAME OF THE TARGET MOVING: it is exact, it is correct, and on this target it is not where the error is. v4417 found the same solver actively WRONG for a shape that gets occluded; this is the other half of its boundary. *** AND A KIND THAT IS NOT IN KINDS USED TO BE AN ELLIPSE, BIT FOR BIT. *** spansOf({ kind: "doomFire", x, y, rx, ry }) returned 16 rows and 208 pixels -- THE SAME SPANS as an ellipse with the same fields, not merely plausible ones -- because pointsOf() returns null for anything it does not recognise and spansOf read that as "so it must be the ellipse". A fifth primitive added to KINDS by name alone did not look like it worked, IT WAS ROUND. It now covers nothing, which is the refusal that costs nothing: fitStep already treats an empty coverage as a rejected candidate, so no caller changes and KINDS becomes load-bearing rather than decorative -- this is the first thing in that module to consult it at run time. *** AND ONE CORRECTION TO v4418, ONE ROUND LATE. *** Fire's convergence exponent is p = 0.235 against 0.572 flat / 0.713 ramp / 0.433 marched / 0.378 Krbn -- 1.61x below the lowest of the four. v4418's section 5 said "a fourth independent generator lands in the same band, which is more evidence for that round's conclusion". THE FIFTH DOES NOT. v4417's conclusion is untouched -- representability is still not the governing variable, and fire is no more representable than the ramp -- but "the exponents sit in a narrow band" was a claim about four points and it is not a law. Spatial frequency is the obvious candidate for what fire has that the others do not, AND NAMING A CANDIDATE IS NOT MEASURING ONE. fx/firePaint.mjs and tools/ship/firePaint-selfcheck.mjs, seven sections and 14 checks in 6.7 s, no device and no model; nine sabotages 2/0/5/5/3/1/1/1/1 by name -- ONE 0 IS EARNED and one bought a check. The earned one removes the discard rule and is INERT: measured, the branch never fires in 250 fitStep calls because an improving shape is always among sixty candidates, and it is already gated in primitiveFit-selfcheck. The other read only the red channel of a pixel comparison and went 0 RED because every check that consumes it reads a ratio or a zero -- and the fire palette repeats its red component across six pairs of adjacent intensities, so red alone genuinely misses changes. STILL UNCHECKED AND NAMED: fit() has no operation for RETIRING a shape, every finding in section 3 points at one, and nothing here builds it. The tree stands at 1441 gates. Full changelog on docs/CHANGELOG.md
// v4418 -- *** THE PAINTER IS 2D, KRBN IS THE GATED EXACT BRIDGE TO A 3D SURFACE, AND NOTHING HAD EVER CONNECTED THEM. *** Keith asked what the painter can reach, and the survey said: nothing. fx/primitiveFit.mjs's KINDS is a frozen list of four flat convex shapes, its universe is RGBA rasters and scanline spans, and no shader, brush, morph or renderer in the tree feeds it. But the other half was already built and already graded: v2721 settled that Krbn's flattening is one-way FOR THE PICTURE and reversible GIVEN THE GEOMETRY, v2722 lifted whole strokes onto the surface as OBJ polylines, v4047 made the lift return { tri, bary } so a lifted mark follows an animation EXACTLY rather than approximately, and v4221 put a cached BVH behind it. *** AND THE PAINTER'S OUTPUT IS ALREADY IN liftStrokes's INPUT FORMAT: *** pointsOf() returns a shape's corner polygon, an ellipse samples, and a screen-space polyline is what the lift takes. So fx/krbnPaint.mjs is a CONNECTOR, not a subsystem -- it owns no rasteriser, no ray-caster, no projection and no BVH, and the gate asserts that on the import graph. *** WHAT THE CONNECTION MEASURES IS A NUMBER THE PAINTER REPORTS AS ONE THING. *** The fitter scores in SCREEN SPACE, where a pixel beside a silhouette costs exactly what a pixel in the middle of a face costs. ON THE SURFACE THEY ARE NOT THE SAME PIXEL. Measured over four Krbn meshes of 22 to 1320 triangles at a budget of 80 shapes: only 1 to 19 of the 80 lie WHOLLY on the surface while 40 to 49 straddle a silhouette; 22 to 34 of them COVER NO MESH PIXEL AT ALL, a third of the budget spent on the empty space around the object; the whole painting lifts to just 33 to 58 polylines, so under half of it becomes 3D geometry; and 34% to 51% OF THE IMPROVEMENT THE PAINTER REPORTS WAS BOUGHT OFF THE OBJECT. A reader taking primitiveFit's distance as "how well the object was captured" is off by that much, and nothing in the image says so -- the split needs the mesh and the camera, which is exactly what Krbn supplies and what the painter has never had. *** AND THE FITTER'S OWN RANKING OF WHAT MATTERED AGREES WITH THE SURFACE'S ON THE FIRST SHAPE AT EVERY MESH AND THEN DIVERGES AT ONCE: *** top-1 shared at all four, top-5 shared only 1, 3, 2 and 2 of five. The first shape is the one choice where "cover the most error" and "cover the most OBJECT" coincide, because the object IS most of the error at that point; after that it wanders into the background and its ordering stops being a statement about the subject. *** THE MUST-MATTER, AND IT COSTS NOTHING. *** A fitter told which pixels are the object -- same shipped search, same 80 shapes, same seed, ONE CHANGED TARGET -- gets 16% to 34% more of the object's error out and drops background shapes from 27/24/22/34 to 15/12/12/29. v4417 measured that REPRESENTABILITY is not what governs this fitter and that the SEARCH costs a factor of two at the budget where the answer is known exactly; this is a third variable and the cheapest of the three: THE SCORE. And what the construction is gets said rather than glossed -- off-mesh target pixels are set to the colour the canvas starts at, so covering one COSTS. That is a PENALTY, NOT A MASK, and calling it masking would describe an experiment nobody ran. *** THE BRIDGE ITSELF IS EXACT FOR A WHOLE SHAPE BOUNDARY, NOT ONLY FOR A POINT. *** krbnCompare-selfcheck holds the round-trip for ONE point at err < 1e-6; here it is 1295 points, the boundaries of 320 painted shapes across four meshes, worst |project(backProject(s)) - s| = 1.1e-13 px. That also makes this a SECOND INDEPENDENT CALLER holding project() and rayThroughScreen() together, which krbnCompare's own v4045 note says is the thing that stops "a silent, plausible-looking wrongness rather than a crash" -- and the sabotage reverting one of them to the two-focal-length form drives BOTH gates red, which is what that claim has to mean if it means anything. A single gate holding a two-sided invariant is one edit from holding nothing, and this round did not set out to add a second; it needed the inverse and got the check for free. Also earned: a Krbn frame is a FOURTH target from a fourth generator for v4417's series, exponent p = 0.378 against 0.572 flat / 0.713 ramp / 0.433 marched -- the same narrow band, which is stronger evidence for that round's conclusion than another point on one of its own curves. fx/krbnPaint.mjs and tools/ship/krbnPaint-selfcheck.mjs, seven sections and 14 checks in 1.7 s, no device and no model; nine sabotages 4/4/2/1/1/1/1/1/1 by name, one going 0 RED first -- boundaryOf's promise of a CLOSED polyline was in its comment and in nothing else, so dropping the repeated first point left a rectangle's fourth side never sampled, never lifted and never exported while every count above barely moved. Its first draft also summed its per-shape replay to 1.0004 of the painting's real improvement, by compositing in raw floats where drawShape writes through a Uint8ClampedArray -- a 0.04% drift that is a different arithmetic, now the exactness control that section 5 rests on. STILL UNCHECKED AND NAMED: the lift returns { tri, bary }, so every polyline this round produces is RIGGABLE by v4047's identity -- linear blend skinning is linear in the vertex position -- and NOT ONE OF THEM IS POSED HERE. The tree stands at 1440 gates. Full changelog on docs/CHANGELOG.md
// v4417 -- *** THE AI SCENE COMPOSER MAKES A SCENE AND THE PAINTER PAINTS IT -- AND THE ANSWER IS A NULL WITH A CONTROL. *** The question was Keith's: let tools/ship/composePropose.mjs propose a diorama and let v4220's learned painter paint it. TWO THINGS STOP THAT BEING A MEASUREMENT AND THE FIRST IS ALREADY WRITTEN DOWN. composeValidate.mjs's header names it: "A MODEL WHOSE OUTPUT IS ACCEPTED BECAUSE IT PARSED IS THE VOYAGER FAILURE THIS TREE ALREADY CRITICISED -- the model judging its own work." Scoring a generated composition by how well a learned painter reproduces it is that failure with an extra hop. The second is that the producer needs ollama, and a gate that depends on a local model is a SKIP. So the VALIDATOR runs and the PRODUCER does not, the composition's declared PROP COUNT is the independent variable, and the picture is rendered by physics/render/sdfMarch.mjs sphere-tracing a Wyvill field out of physics/mesh/marchingCubes.js -- two shipped, gated modules that the fixture file only supplies rays to. *** AND THE FIRST THING WORTH SAYING IS WHAT THE PAINTER HAS EVER BEEN SHOWN. *** brain/rl/paintEnv.js's makeTarget draws four flat quadrants and one flat disc -- FIVE FLAT REGIONS at every seed, and exactly the five colours its generator drew, no others -- while fx/primitiveFit.mjs's model class is unions of flat-coloured convex shapes. EVERY NUMBER v4220 MEASURED WAS MEASURED ON A TARGET IT COULD IN PRINCIPLE FINISH EXACTLY, and neither that module nor its gate says so anywhere. *** THE ROUND EXPECTED A REPRESENTABILITY FLOOR AND THE MEASUREMENT SAYS REPRESENTABILITY IS NOT THE VARIABLE. *** Given exactly FIVE shapes -- the number the picture is made of -- the greedy fitter reaches 0.1018 where the five true shapes with the true colours reach 0.0514: A FACTOR OF TWO, spent on search, at the one budget where the answer is known. And the convergence exponent over N = 5 to 320 is 0.572 on the exactly-representable target, 0.713 on a ramp that needs sixty-four strips and 0.433 on a ray-marched frame -- THE ONE TARGET THE MODEL CLASS CAN FINISH SITS IN THE MIDDLE. If representability governed this fitter the flat target would fall off a cliff at five shapes; it decays smoothly for three hundred shapes past the point where it was already finishable. The render is the hardest of the three by about 2x, which is LESS than what the search costs on its own. *** TWO THINGS FELL OUT OF LOOKING. ONE: THE EXACTLY-REPRESENTABLE TARGET IS NOT EXACTLY REPRESENTABLE. *** The five true shapes with the generator's own colours still miss 78 to 163 pixels of 4096 -- a one-pixel band where paintEnv's per-pixel hypot test and primitiveFit's analytic scanline rasteriser disagree about an edge. Two percent of the picture and a QUARTER of the RMS distance, because RMS is bought by a few maximally wrong pixels: a distance that averages over an image cannot tell "slightly wrong everywhere" from "completely wrong on a thin line". "Representable" is always "representable up to whose rasteriser draws the edge". *** TWO: THE SOLVED COLOUR IS SOLVED FOR A PICTURE THAT WILL NOT EXIST. *** primitiveFit's first headline idea is that the colour is the exact least-squares optimum rather than a search, and it is -- FOR A SHAPE IN ISOLATION, which is what v4220's gate grid-searches 256 channel values to confirm. On the SAME five shapes in the same order the least-squares colours land 0.0685 against the true colours' 0.0514, worse at every seed by 23% to 102%, because a rectangle the disc will be drawn over is solved including the pixels it is about to lose. The greedy fitter never revisits it, and no ordering removes it -- only a second pass, which the model does not have. *** AND THE COMPOSER AXIS DOES NOT SURVIVE THE TRIP, WHICH IS THE ANSWER TO THE QUESTION. *** Shapes needed to reach a quarter of the starting distance, by declared prop count 1 to 6: 29, 41, 32, 38, 29, 20. SIX PROPS NEED FEWER SHAPES THAN ONE, and the sequence is monotone in neither direction -- more props means more of the frame is ball rather than backdrop, and the backdrop's gradient is the expensive part. The control settles it: six TRANSLATIONS of one unchanged three-prop scene span 13 shapes against the prop sweep's 21, 62% as wide, while distinct colours (368 to 570) confirm the render sees the props perfectly well. A round measuring only the first row would have read a signal with noise in it; THE CONTROL SAYS THE WHOLE OF IT IS THE NOISE. So the painter is not a complexity meter for the composer -- the tempting build is the Voyager failure AND it would not have carried the signal either. Also earned: the best N-piece approximation of a linear ramp has the closed form range/(N sqrt 12), verified against a per-pixel sum to within 1.8%, with the fitter 1.26x to 2.72x above it -- MEASURED and explicitly NOT asserted as a bound, because N shapes are not N pieces and an overlap of k shapes at alpha below 1 can make more than k levels. fx/paintTargets.mjs and tools/ship/paintFloor-selfcheck.mjs, seven sections and 18 checks in 3.1 s; eight sabotages 6/1/5/1/4/1/2/2 by name, AND TWO WENT 0 RED FIRST. The disc test flipped from < to <= left five seeds of bit-identity untouched and would at any number of seeds, because no integer pixel lands exactly on a radius that generator produces -- true at every seed and constraining nothing, which is why the boundary is now pinned on a geometry that hits it. And the exponent check compared three numbers to each other and never to zero, so flipping every sign -- a fitter that gets WORSE with more shapes -- preserved the ordering and passed. STILL UNCHECKED AND NAMED AS THE NEXT ROUND: v4220 holds its policy out on twenty-four SEEDS of makeTarget and its gate calls them "held-out pictures", but every one of them is five flat regions -- HELD-OUT SEEDS ARE NOT A HELD-OUT DISTRIBUTION. The tree stands at 1439 gates. Full changelog on docs/CHANGELOG.md
// v4416 -- *** FRESNEL ON A DEVICE, AND THE TWO DEFICITS MADE EQUAL -- THE F = 1 THAT SIX ROUNDS CLOSED THEIR UNCHECKED LIST WITH. *** v4408 through v4413 put the GGX lobe on a real GPU -- the NDF integral, the visible-normal sampler, the compensation table, anisotropy, MIS -- and every one of them ended by saying "FRESNEL, F = 1 THROUGHOUT". physics/render/fresnel.mjs has held the exact equations since v3491 and is gated hard at f64; it had never been near a GPU, and the lobe had never carried it. *** THE HEADLINE IS A SENTENCE THAT WAS AN ASSERTION UNTIL SOMEBODY MADE THE TWO NUMBERS EQUAL. *** That file's header says the white furnace test sets F = 1 because "with a real Fresnel term energy LEGITIMATELY LEAVES the reflection lobe -- it is transmitted, not lost -- and those two deficits look identical from the number alone, and one is a model failure while the other is physics". At each roughness there is an index -- BISECTED FOR, NEVER TYPED -- at which a CORRECT lobe on a metal and a BROKEN lobe (the separable G2) on a white surface return the same directional albedo BIT FOR BIT at f64 and to 1.3e-8 on a device: F0 = 0.9934 at alpha 0.5 and 0.9527 at alpha 1, ordinary metals rather than a parameter tortured until it collides. So a check on the furnace number certifies both, and there is no tolerance, however tight, that separates them. *** AND THE FIRST DRAFT GOT THE REASON WRONG, WHICH A 0-RED SABOTAGE NOW HOLDS IN PLACE. *** It said the transmitted share separates them because "T is never defined as 1 - R" -- fresnel-selfcheck.mjs's words, borrowed for a place they do not apply. At one INTERFACE T is computed from the transmission amplitudes and R + T = 1 is a real closure. In the LOBE, sum (1-F)w and sum w - sum Fw are the same quantity by linearity, 2.1e-15 apart over 65,536 terms, so the split buys ATTRIBUTION and not independence -- and the sabotage that computes it the second way goes 0 RED, as it must. What actually tells the two surfaces apart is RUNNING THEM AGAIN WITH F = 1, where the albedos differ by 4.5e-3. That is to say: the instrument that separates physics from a model failure is the white furnace test with Fresnel taken out. NOT A WORKAROUND FOR v3490's CHOICE -- IT IS v3490's CHOICE, and this is the first thing in the tree that has needed it. *** THE DEVICE THEN SAYS THREE THINGS f64 CANNOT. *** ONE: the f32 floor for the exact equations is 7.4e-8 at air/glass and 8.3e-8 at air/diamond, AND 5.1e-6 in a band 0.02 wide around total internal reflection -- twenty times worse -- because cos_t = sqrt(1 - sin_t^2) cancels as sin_t -> 1. THE FIFTH ROUND RUNNING IN WHICH (1 - x^2) IS THE ANSWER, after v4408's NDF integral, v4409's cdf, v4411's 1/(1-E) and v4413's 2/t. The damage is LOCALISED, so a worst-case number over the whole sweep would have reported one bad floor and hidden that the function is fine everywhere else. TWO: Brewster survives f32 to 6e-8 by the SIGN route and equally by hunting the minimum of r_p^2 -- I PREDICTED THE MINIMUM ROUTE WOULD LOSE HALF THE MACHINE'S DIGITS AND IT DOES NOT, because r_p is computed and then SQUARED, and a square is a product that keeps full relative precision however small it gets. The flatness is in the value, not in the error. THREE: pow(m, 5.0) and m*m*m*m*m differ by 4.2e-7, five times the curve's own floor -- a spelling that is not one, and a gate holding a ported Schlick to 1e-7 would fail on it and send the author looking at the physics. *** AND A CORRECTION TO v3491's PROSE, MEASURED RATHER THAN ARGUED. *** fresnel-selfcheck.mjs said a renderer in unpolarised light "sees a MINIMUM at Brewster, never a zero". IT SEES NOTHING AT ALL: (R_s + R_p)/2 is strictly monotone in cos for every index pair and its least value over the range is F0, at NORMAL INCIDENCE. The device runs that search and it walks to the end of its bracket and reports F0 -- not imprecise, LOOKING FOR SOMETHING THAT IS NOT THERE. Also measured: the critical angle recovered from a BOOLEAN on f32 to 2.9e-8, twenty times finer than the reflectance error surrounding it, with R exactly 1 and T exactly 0 past it; energy closing to 3.0e-7 on f32 with the dropped solid-angle ratio six orders above that floor; the polarisation plant bit-identical to the clean run at cos = 1 on the device too AND NOWHERE ELSE, its gap halving with cos exactly as v3491 measured at f64; Schlick 16.9% short of the exact equations at a near-mirror and 11.8% at alpha 0.5, so ROUGHNESS AVERAGES ITS ERROR AWAY RATHER THAN AMPLIFYING IT; and Fresnel evaluated at cos_o instead of dot(wo, wh) costing 2.1e-5 at alpha 0.00625 against 1.9e-3 at alpha 0.2 -- a bias that a polished-metal fixture certifies and a rough one catches, which is the wrong way round from how materials get authored. physics/render/fresnelWgsl.mjs and its gate, 7 sections and 25 checks in 3.3 s; directionalAlbedoSplit added to microfacet.mjs; nine *** AND THE PARITY CENSUS'S OWN RECORD HAD DRIFTED, WHICH ADDING TO IT IS WHAT FOUND. *** render/backendParity.mjs's baseline carries a written note for every file that has ever moved it, and that run of notes stops at v4409 while the numbers ran on to v4413: three rounds took wgslBearing and wgslOnly up by one each and left the reason in a commit message. Recovered from the history and written down beside this round's own entry, because a record that LOOKS complete and is not is worse than one that admits a gap. sabotages, 0/2/2/7/2/1/3/2/1 by name, and the 0 is the one worth reading. The tree stands at 1438 gates. Full changelog on docs/CHANGELOG.md
// v4415 -- *** THE TRIAGE'S LAST TWO CANDIDATE KEYS. ONE WAS REAL AND IS NOW WRITTEN; THE OTHER WAS ALREADY WRITTEN, AND THE KEY THE TRIAGE IMAGINED CANNOT EXIST IN THAT DESIGN. *** tools/ship/coverageTriage.mjs has carried both since v3853. *** FIRST: physics/voxel/fracture.js, WHICH THE TRIAGE SAID HAD A KEY NOBODY HAD WRITTEN -- AND THE MODULE'S OWN HEADER NAMED A SHARPER ONE. *** The entry said "total mass before and after a fracture is conserved exactly". True, and BIT-IDENTICAL rather than within a tolerance, because every term is the same float (density * cell^3) added the same number of times -- asserted with === . But the header carried the better key: "a voxel summation can be made to EARN [the analytic box tensor], exactly, at every resolution -- but only if each voxel's OWN inertia about its own centre is included alongside the parallel-axis term". It does: 1.1e-14 at every resolution from a single voxel to 4096, AND a fragment that has broken off earns it about a centre of mass that did not exist before the fracture, which is the number that actually reaches the solver. *** AND THE COST OF THE OMISSION IS A CLOSED FORM, NOT "ABOUT cell^2". *** The dropped term is the same for every voxel, so it sums to M cell^2/6 against an analytic M(b^2+c^2)/12 and the relative error is 2 cell^2 / (b^2 + c^2) -- predicted to three figures at four shapes, from 8.0% down to 0.05%. The header is right about the SCALING and silent about the CONSTANT, and the constant is what separates a missing term from a coarse grid. It is always LOW, never high, which spins a fragment slightly too fast and reads as a damping constant somebody should tune -- the header's own "indistinguishable from a tuning problem forever". *** THE FIXTURE HAD TO BE BUILT TWICE AND NEARLY SHIPPED VACUOUS. *** The first draft carved SPHERES out of a solid block, exactly as the module's own carveSphere invites, and every carve left the block in ONE piece: a radius-4 sphere through a 14x10x12 block removes 153 voxels and detaches nothing, because the block is simply connected around the hole. Mass conservation over a partition of one part is arithmetic with nothing to check, and a vacuous key reads exactly like a satisfied one. A slab carve detaches; the piece count is now ASSERTED so it cannot go vacuous again without going red, and the sphere result is kept as a check rather than deleted. *** SECOND: physics/fluidBox3d.js, WHERE THE TRIAGE WAS WRONG TWICE. *** Its entry read "Momentum exchange across the coupling COULD be an exact key (what one side loses the other gains), which would be a real gate rather than a smoke test". It is not a smoke test: physics/fluidBox3d-selfcheck.mjs has held the velocity nudge to dv = forceScale * scale * dt * F / mass at THREE scales, pinned the torque's scale^2 exponent where the only consumer runs at scale 1 and cannot see it, and asserted the ORDER of the exchange, which no final value reveals. And the momentum key cannot exist: THE COUPLING IS DELIBERATELY ONE-WAY -- that module's header says box3d owns the motion and the fluid supplies external forces -- so the fluid is never told to give up what the body receives and there is no ledger with two sides to balance. A new section 8b measures both halves rather than arguing: the body gains EXACTLY the stamped impulse (that is the transfer being exact, not conservation), and the fluid's own force is unchanged by the transfer, with externalBodies true so the fluid has no body momentum to lose. A CANDIDATE KEY THAT CANNOT BE WRITTEN IS WORTH RECORDING AS SUCH, or it stays on a list forever -- which is what happened here since v3853. Five sabotages bite 2/2/1/3/3 and THE 1 WENT 0 RED FIRST: flipping the sign of every product of inertia moved nothing, because every fixture was a BOX and a box about its own centre is diagonal by symmetry, so the sign of zero is not observable. Section 3b adds an L whose Ixy is 2.386 and whose reference is TWO SOLID BOXES composed by the parallel-axis theorem -- an independent route to the same tensor, and the one physics/voxelMassProps.js takes from a box cover; the two agree to 4e-16 with the masses bit-identical. The convention is the inertia TENSOR's, Ixy = -sum m dx dy, which is the opposite sign to the bare product of inertia and a real thing to get backwards. UNCHECKED: THE SOLVER, which is where these numbers go -- physics/mechanics/fragmentRotation-selfcheck.mjs is that half; NON-CUBOID fragments beyond the L, since a closed form needs boxes; the FLOOD FILL's 6-neighbour connectivity, a modelling choice this gate takes as given; and DENSITY VARIATION, since massProperties takes one density per label and a real structure is made of more than one material. The tree stands at 1437 gates. Full changelog on docs/CHANGELOG.md
// v4414 -- *** THE IR DECISION v3274 DEFERRED AND v4380 REFUSED TO MAKE -- AND IT WAS ALREADY MADE BY BUILDING. *** shaderCensus-selfcheck.mjs has held since v3274 that a hand-written shader pair beats an intermediate representation "while few files carry both languages -- if this count climbs toward twenty the arithmetic inverts, and THAT is when to re-open llvm-tutorial-book's three-stage shape: parse, lower to an IR, emit per target". It said 3 when filed and 14 now; main's v4380 found the red register recording 4, noted that this session's TSL rounds put some of them there, and deferred outright: "whether the arithmetic has inverted is a decision about the engine's shape and belongs to a round of its own". IT IS NOT A JUDGEMENT ROUND. THREE THINGS ARE MEASURABLE AND THEY SETTLE IT. *** ONE: THE TRIGGER COUNTS CO-OCCURRENCE AND THE DECISION NEEDS DUPLICATION. *** `both` means "this text contains markers of both languages", which is not "this file writes one computation twice". Of the 14: FIVE duplicate a computation, two share only an entry-point name, and SEVEN share nothing at all. render/bloomFused.mjs exists precisely BECAUSE WebGPU can fuse what WebGL2 cannot, so its two halves are different algorithms and no IR could emit both from one source. The instrument is new -- shared DECLARED FUNCTION NAMES rather than shared numeric constants, and it needs no split at all, where shaderCensus.pairShape cuts at "the first WGSL entry point" and calls itself crude for it: WGSL declares `fn name(` and GLSL `<type> name(...) {`, and neither syntax appears in the other language. *** TWO: THE POPULATION INCLUDES THE MACHINERY THAT WOULD BE THE IR. *** render/tslSource.mjs holds eleven GLSL markers BECAUSE IT EMITS GLSL, and render/fleetTsl.mjs the same. An emitter necessarily contains text in every language it emits, SO BUILDING THE IR RAISES THE COUNT THE TRIGGER READS. An instrument that fires harder the more the problem is solved is not a threshold anybody should act on, and that is provable rather than arguable. Three more of the fourteen are TOOLS whose subject is shader text -- a conformance checker, a corpus, a variant table -- and all three classify DISJOINT without being told. So 5 of the 14 are the answer or tools about the question. *** THREE: THE THREE-STAGE SHAPE WAS ALREADY RE-OPENED, AT v4319-v4320, AND HAS SHIPPED TEN ROUNDS SINCE. *** three's node builders parse and lower; tslSource's transplantFragment(fragment, language) emits per target. That IS parse, lower, emit, with five shader modules authoring through it today. "When to re-open it" has an answer in the changelog rather than in a threshold, and nobody noticed because the trigger was watching a number the work made worse. *** THE VERDICT: THE ARITHMETIC HAS NOT INVERTED AT FIVE PAIRS -- and the reason is no longer the count, because the count was never measuring the claim. *** Two of the five (nebula, wormhole) are ALREADY authored once in JavaScript and mirrored by hand, which their own headers say; what is missing there is a machine to do the mirroring, a narrower thing than a compiler. SO A ~200-ROUND STANDING RED IS REPAIRED BY REPLACING THE INSTRUMENT, NOT BY WIDENING THE THRESHOLD: the co-occurrence count is still REPORTED, and what is GATED is the duplication count against a measured baseline of 5. shaderCensus leaves the red register, and the full repair chain main's v4380 built was followed -- remove the entry, re-freeze the audit, confirm registerDrift. ALSO THIS ROUND, FOUND WHILE DOING THAT: the freeze is a claim about the past and registerDrift reads it, so a gate REPAIRED after a freeze was invisible -- section 1 catches an entry ADDED without re-freezing and nothing caught the opposite. A deterministic cheapest-first sample of the register is now re-run LIVE under a stated 1200 ms budget, 10 of 28, with the coverage reported rather than implied -- AND IT WOULD NOT HAVE CAUGHT THIS ROUND'S OWN REPAIR, since shaderCensus cost 279 ms and the ten cheapest stop at 153 ms; raising the budget until it reached that one case would be fitting the instrument to the answer, which is the error this round is ABOUT. The freeze also stamped a HARDCODED "v4380" however long ago it was taken -- a staleness marker that could not go stale, in the file whose job is making staleness visible -- and now reads the version from the tree. Five sabotages bite 1/1/1/1/1 and TWO WENT 0 RED FIRST, both defects in this round's own gate: a check comparing a list against ITS OWN LENGTH (an arrangement, not a property), and a fixture whose planted leak collided with a name already present, so the deduplicated count could not move. UNCHECKED: whether the five should MOVE onto the TSL path that exists -- a port per file with a key per file, not a decision, and v4400 and v4402 are what one costs; whether the five are drifting right now, since this counts pairs and does not compare their halves; and whether an IR would be good for anything else, since v3274 asked only about drift between two hand-written halves. The tree stands at 1436 gates. Full changelog on docs/CHANGELOG.md
// v4413 -- *** MULTIPLE IMPORTANCE SAMPLING WITH THE TWO STRATEGIES ACTUALLY COMBINED -- WHICH v4409 COMPUTED THE WEIGHTS FOR AND NEVER USED. *** That round measured the balance heuristic's defining property and found it is not exact -- 12.6% of pairs miss 1 by one ULP on a device and 10.8% at f64, the SAME rate, so arithmetic rather than precision -- then named what it had not done: "the MIS weights are computed and never USED; a real next-event estimator pairs this with a light sample, and combining two estimators is a round of its own". *** WHAT IS NEW CODE RATHER THAN NEWLY EXERCISED: THE BSDF's PDF AT A DIRECTION SOMEBODY ELSE CHOSE. *** A sampler never needs it -- it knows the pdf of what it drew. An MIS estimator must evaluate the OTHER strategy's pdf at its own sample, which means RECONSTRUCTING the half-vector from wo and wi, and nothing in v4408 through v4412 needed that. *** AND THE RECONSTRUCTION IS EXACT TO ONE ULP, AMPLIFIED BY 2/t -- PREDICTED TO THREE FIGURES RATHER THAN FITTED. *** The rebuilt cos_h differs from the sampler's by 3.33e-16, the smallest step a double has there, and D turns that into 5.26e-13 because D goes as 1/t^2 with t = (1-c^2) + a^2 c^2, which is a^2 as cos_h -> 1. Predicted 2 dt/t = 5.255e-13. THE SAME (1 - c^2) THAT HAS ANSWERED IN EVERY ROUND OF THIS ARC -- v4408 wrecking the NDF integral, v4409 in the sampler's cdf, v4411 amplifying the compensation by 1/(1-E) -- here amplifying a single ULP by 1600x. *** THE ARGUMENT FOR MIS BECOMES A MEASUREMENT. *** The light is a cone of half-angle tmax, so all three Veach regimes are ONE PARAMETER apart rather than three fixtures. At alpha 0.01 with a 0.05-rad light BSDF sampling returns EXACTLY ZERO from 65,536 samples -- not noisy, SILENT, and a renderer would render black -- while light sampling returns 9.6e-6. At alpha 0.01 under a hemisphere light the positions reverse: BSDF sigma/mean 3.77e-2 against the light's 4.92e+1, a factor of 1300. *** SO THE CLAIM IS NOT THAT MIS IS BEST -- IT IS NOT, AT 10 OF 20 CONFIGURATIONS THE OTHER TWO WIN -- BUT THAT IT BOUNDS THE DAMAGE: *** its worst showing against the better strategy is 3.6x, while each single strategy is up to 1300x worse than the other somewhere in the same sweep. Veach's argument as a number, on one code path. *** AND v4409's ULP IS FINALLY PRICED. *** That round said "one ULP is beneath the sampling noise of any renderer" with no estimator to test it on. Forcing the weights to sum to exactly 1 changes the answer by 1e-7 at f32 and by nothing at all at f64 -- four orders under the sampling noise of even the BEST configuration here, so the sentence was right and is now measured rather than asserted. *** A SABOTAGE WENT 0 RED AND IT WAS A THEOREM RATHER THAN A GAP. *** Breaking the kernel's pdf reconstruction -- dividing by |wo.wh| instead of cos_o, the half-vector pdf mistaken for the direction pdf, a real and common bug -- moved NOTHING. Correctly: MIS is unbiased for ANY pair of weights summing to 1, and a wrong pdf still produces a partition of unity, so it cannot move the MEAN at all. It moves the VARIANCE. Every mean-based check in the gate is blind to it BY CONSTRUCTION, and the cure is to compare the two routes to the pdf directly -- which section 1 already did on the CPU and which the device now does too, because the WGSL is a separate transcription. Re-run at 1 red, three orders above its f32 floor. A wrong pdf leaves the answer exactly right and the variance quietly worse, which is the most expensive kind of bug to find in a renderer. Four sabotages bite 3/1/2/4. UNCHECKED: MORE THAN TWO STRATEGIES, since the balance heuristic is defined over any number and this measures a PAIR, so the sum-to-one property is checked in its easiest case; a TEXTURED OR SHAPED emitter, which changes which strategy wins and this sweep would not see; NO OCCLUSION, so the light sample is never shadowed and the estimator never learns what a zero-visibility sample costs -- which is most of what next-event estimation is FOR; ANISOTROPY, since v4412's lobe and this kernel are separate; and COLOUR, still F = 1. The tree stands at 1435 gates. Full changelog on docs/CHANGELOG.md
// v4412 -- *** ANISOTROPIC GGX -- THE PARAMETER EVERY KEY IN THIS ARC HAS BEEN AVERAGING OVER, AND THE FRAME v4409 PROVED IT COULD NOT TEST. *** v4408 put the lobe on a device, v4409 the sampling half, v4410 the visible-normal sampler, v4411 the compensation; all four graded an ISOTROPIC model, and that was never a choice. microfacet.mjs's D(cosM, alpha) takes a COSINE, and a lobe that knows only the angle from the normal cannot know which way the surface is brushed. D(m) = 1/(pi ax ay ((m.x/ax)^2 + (m.z/ay)^2 + m.y^2)^2), y up, x tangent, z bitangent. *** THIS CLOSES v4409's SECTION 7. *** That round measured that its fixture could not see a tangent-frame error AT ALL -- wo lay in the plane z = 0, so wh.z never reached dot(wo, wh) and the departure was exactly zero -- and it called handedness UNTESTED rather than harmless. Here the frame is a PHYSICAL parameter: the view azimuth moves the directional albedo by 17.6% at 16:1 anisotropy and by 3.95e-14 when ax = ay. A must-matter and a must-not-matter on the same code path, twelve orders apart, which is the control this subject was missing. *** AND A KEY WITH NO ISOTROPIC COUNTERPART: THE SWAP IDENTITY. *** Rotate the tangent frame a quarter turn about the normal and exchange ax with ay -- the same surface from a turned frame, so nothing may move. It is BIT-EXACT at f64 AND at f32, over every sampled direction, which almost nothing in this arc can be asserted as. *** AND IT IS EXACT ONLY BECAUSE OF A PARENTHESISATION THAT THIS GATE GOING RED IS WHAT BOUGHT. *** The first draft claimed exactness from a narrow probe and failed on a wider sweep: 170 of 216. Addition IS commutative so the lobe's shape term was never at risk; the rounding was in the NORMALISING CONSTANT, where (PI ax) ay and (PI ay) ax are different numbers because multiplication does not ASSOCIATE. Written PI * (ax * ay) * (t * t) it is 216 of 216. v3494 re-associated to avoid a cancellation; this re-associates to preserve an identity. THREE MORE FIRST-DRAFT OVERCLAIMS WERE CORRECTED THE SAME WAY: Lambda's isotropic limit is bit-identical on 54 of 64 directions and 5.4e-15 on the rest, not flat; the isotropic azimuth spread is 3.95e-14 rather than zero, because the phi grid is fixed while wo rotates; and on the device it is 6.5e-5, nine orders of azimuth dependence invented out of nothing BY THE DEVICE'S OWN sin AND cos -- v4408's finding answering for the fourth consecutive round -- still 2700x under the signal, so the discriminator survives. THE GENERAL FORM CONTAINS THE SHIPPED ONE to one part in 1e14, and the isotropic exports are KEPT rather than replaced, because they are what render/microfacetShader.js ships in GLSL and what v4408 graded on a device. THE NDF NORMALISES at every roughness PAIR including 16:1 -- a two-parameter key where v4408 had one, and the phi integral is real work now rather than a 2 pi factor. THE WEAK FURNACE holds at every (ax, ay) AND every view azimuth: three parameters. HEITZ'S SAMPLER WAS ALWAYS ANISOTROPIC -- v4410 supplied ax = ay, the two roughnesses enter at exactly the 3.2 stretch and the 3.4 unstretch, so sampleVisibleNormal gained ONE OPTION and every v4410 caller is untouched; G2/G1(wo) is still the weight, holding to 1e-14 at 16:1, and it still proposes a backfacing facet exactly never. Six sabotages bite 1/1/1/1/3/1, and FOUR GOING EXACTLY ONE RED IS A PARTITION RATHER THAN A THIN GATE, which the 3 demonstrates: swapping ax and ay inside D produces a perfectly VALID lobe for a surface brushed the other way -- it still normalises, is still self-consistent under a quarter turn, and has the same azimuth spread rotated -- so ONLY the weak furnace can object, because only it ties D to G1. The same is true of the isotropic Lambda used with an anisotropic D, which is the anisotropic form of the Beckmann mistake. UNCHECKED: THE TANGENT FIELD ITSELF, which is where anisotropy actually fails in a renderer -- this round supplies a frame and a real surface derives one from UVs or a hair direction, where the failure is a discontinuous or degenerate tangent rather than a wrong lobe; THE SHIPPED GLSL, still isotropic, so v4408's chain from shipped text to device does not extend here and this kernel is hand-written like v4409's; ENERGY COMPENSATION at ax != ay, since v4411's table is indexed by ONE roughness and an anisotropic E depends on the azimuth too -- a 2D table and a round of its own; and COLOUR, still F = 1. The tree stands at 1434 gates. Full changelog on docs/CHANGELOG.md
// v4411 -- *** THE MULTIPLE-SCATTERING COMPENSATION ON A DEVICE -- AND THE CLOSURE THAT GRADES IT CANNOT SEE ITS OWN TABLE. *** physics/render/energyCompensation.mjs is the rare case where the answer key existed before the code: v3490 measured what single-scattering GGX throws away and this term returns it, under a requirement written a round earlier -- E + INT f_ms cos dw = 1, at every roughness and every view angle, green since v3492. *** IT IS ALGEBRA IN WHATEVER E IT IS HANDED. *** Integrate f_ms against cos and the denominator cancels: INT f_ms cos dw = 1 - E(mu_o) EXACTLY, for ANY table, so E cancels out of the very check meant to grade it. Hand it a flat 0.25 that is not the albedo of anything and the closure reads 1.0000019. That module's header said "AN EXACT CLOSURE IS PROOF OF CONSISTENCY, NEVER OF CORRECTNESS" at v3492 as a caution; this is that sentence with a number, in the one place it was pointed at itself. *** AND THE SHIPPED TABLE BUILDER WAS THAT WRONG. *** buildTable asked directionalAlbedo -- a QUADRATURE -- at N = M = 220, and path-tracer.html asked for 160. v4409 measured what a grid does to a lobe narrower than its step: E reads 0.145 against 0.99993 at roughness 0.001 (85.5% out) and 0.039 against 0.982 at 0.0005 (96.0%), the compensation then manufactures that fraction of the surface's energy, and EVERY CHECK IN THAT MODULE'S OWN GATE PASSES ON IT -- closure 1.00023 while injecting 85.6%. Threshold measured: fine at 0.01 and above, total below. NOT REACHED BY A SHIPPED CALLER TODAY (path-tracer.html sweeps alpha from 0.05 up, where 160x160 is good to 1e-6), so it was LATENT -- and it is FIXED rather than merely named, the opposite call from v4409's cdf denominator, because there the measurement said the hazard did not bite and here it says it bites totally. THE REPAIR IS v4410's VISIBLE-NORMAL SAMPLER, which has no grid: 3.4e-5 at every roughness from 0.0005 to 1, on 4096 evaluations against the quadrature's 48,400 -- more accurate AND cheaper, so nothing is traded. *** AND IT CHANGED WHAT THE EXISTING GATE HAD BEEN MEASURING SINCE v3492. *** energyCompensation-selfcheck.mjs held that the closure residual is SECOND order in the table size K, "so it is the TABLE's and not the algebra's". The conclusion was right and the exponent was the instrument's: it is THIRD. Proved by removing the instrument gradually rather than asserted -- the fitted order climbs 2.11, 2.09, 2.09, 2.23, 2.57, 2.84 as the quadrature is refined from N=160 to 3000, and is exactly 3.00 grid-free at 1024, 4096 and 65536 samples, FLAT IN SAMPLE COUNT, which is what says the 3 is the interpolation and not the estimator's noise. Three because linear interpolation error on a MIDPOINT grid alternates in sign cell to cell and integrating it against a smooth weight cancels one order; the quadrature's E carries a wobble that does not alternate, so it dominates and reads 2. THE EXPONENT IDENTIFIES WHICH INSTRUMENT BUILT YOUR TABLE, which is sharper than the residual's size and is what that section now asserts, on both instruments. *** ON A DEVICE: the closure survives f32, and its ENTIRE departure from the CPU is the transcendentals, for the third time in this arc. *** The device tracks the CPU with a CONSTANT offset -- flat to three figures across a 32x range of the hemisphere grid, which rules out summation -- and handing the kernel the same grid's sin and cos from the host, changing nothing else, drops the worst offset from 2.23e-5 to 3.85e-8. v4408 found this in the NDF integral at 16%, v4410 found it was why the device could not show a sampling gain, and here it is the whole f32 residual. The lobe is reciprocal on the device BIT FOR BIT over 256 pairs. *** THE AMPLIFICATION NOBODY HAD COMPUTED: the compensation adds exactly (1 - E), so an error in E is amplified by 1/(1 - E) -- 6064x at roughness 0.01, falling monotonically to 1.6x at 1. *** This arc's three instruments agree on E to 9.6e-6 there and therefore disagree by 5.84% about how much to add; by roughness 0.25 that is 0.0008%. The absolute stakes are tiny -- 1e-5 of the surface's energy -- and saying so is what makes it a measurement rather than an alarm. Five sabotages bite 6/4/1/6/2, and ONE OF THEM BUILT A CHECK THAT DID NOT EXIST: replacing the hemisphere integral with the closed form (1 - E) -- the shortcut energyCompensation.mjs forbids BY NAME -- went 1 red on its first run, and that one was incidental. The check that catches it now runs the closure at five grids and requires the residual to MOVE, because a real integral does and a closed form does not; its own first draft read backwards, asserting the finer grid lands closer to 1 when the residual crosses zero between nTheta 50 and 100. And reverting the repair goes only 1 red, in section 1 -- every device check stays green on an 85%-wrong table, which is the round's whole thesis. UNCHECKED: whether the compensation is RIGHT, which no closure can say and this round least of all; the lobe restores the ENERGY and not the DISTRIBUTION, so any two BRDFs with the same albedo get the identical compensation; THE BAKE itself, which a renderer does once and ships as a texture and which is a different round; and COLOUR, since F = 1 throughout and the compensation is where a metal's saturation shifts show up first. The tree stands at 1433 gates. Full changelog on docs/CHANGELOG.md
// v4410 -- *** THE VISIBLE-NORMAL SAMPLER ON A DEVICE -- THE ONE A MODERN TRACER ACTUALLY USES, AND THE ONE microfacet.mjs DID NOT CARRY. *** v4409 named it in its own UNCHECKED list. sampleHalfVector draws from D: it proposes microfacets by how MANY there are, not by how many the viewer can SEE, so at grazing angles it keeps proposing facets that face away from wo and every one is a sample worth zero -- 22,943 of 65,536 at roughness 1, cos_o 0.3. Heitz 2018 draws from D_visible = G1(wo) max(0, wo.wh) D(wh) / cos_o. *** THE WEIGHT COLLAPSES FURTHER THAN v4409's DID: f cos_i / pdf = G2 / G1(wo). THE ENTIRE LOBE CANCELS. *** v4409's NDF weight still carried |wo.wh| / (cos_o cos_h); this carries nothing but the masking-shadowing ratio, and both sides are computed and subtracted pointwise -- 4.5e-16 at f64 over 24,568 directions, 1e-6 on the device. *** BOTH SAMPLERS LIVE IN ONE SHADER TEXT, so "the answer must not depend on which distribution drew the samples" is a claim about one kernel rather than a comparison between two that could differ for a second reason. *** FOUR ROUTES, ONE NUMBER: device VNDF, device NDF, the CPU sampler, and v4408's quadrature, agreeing to 5e-4 over twelve configurations. *** AND A FINDING ABOUT v4409's OWN METHOD. *** That round chose STRATIFIED MIDPOINTS to keep the RNG out of the comparison, which was right for a sampler whose u1 maps monotonically to cos_h. It is WRONG for this one: Heitz maps (u1, u2) onto a DISK, where a square grid becomes wildly anisotropic near the centre, and the estimator oscillates instead of converging -- 0.99977, 0.99598, 0.99763, 0.99691, 0.99717 against a true 0.9971374. The fix keeps the RNG out: a HAMMERSLEY set from a van der Corput radical inverse over SIXTEEN BITS, which is under 2^24 so f32() of it is exact and 65536 is a power of two so the division is -- identical on both machines BY CONSTRUCTION, the property v4290 proved an RNG cannot have, and verified bit-for-bit against the host over 1024 indices. It reaches 3.1e-6 at 65,536 samples where stratified is 1.2e-3 with the same count. *** BUT THE DEVICE CANNOT SHOW THE SIZE OF THAT GAIN, and this round says so: summing 1024 f32 terms per lane costs 1.54e-4 on its own, above the CPU's Hammersley residual, so the two patterns separate by 376x on the CPU and only 7.6x on the device. A CPU FINDING CONFIRMED IN DIRECTION AND NOT IN MAGNITUDE -- the opposite of v4408, where only the device could see the thing. *** WHAT THE SAMPLER BUYS, STATED PRECISELY BECAUSE IT IS USUALLY STATED LOOSELY: IT GUARANTEES THE FACET IS VISIBLE, NOT THAT THE REFLECTION CLEARS THE HORIZON. *** Counted separately: backfacing facets are 0 of 196,608 at f64 -- a count, not a tolerance -- while below-horizon reflections remain and at roughness 1 are still 31,923 of 65,536. At f32 even the first is only nearly exact, 19 of 786,432, because dot(wo, wh) can be a small positive that rounds through zero; a check asserting a flat 0 there would be asserting f64 of an f32 machine, and the first draft did. The variance gain is a function of the VIEW ANGLE and barely of the roughness: 1.03x at cos_o 0.95, 3.15x at cos_o 0.3, which is exactly where the wasted proposals are. THE TWO TRAPS IN THE LISTING, both reached: dropping the lensq == 0 special case returns NaN at cos_o = 1 -- the centre of every flat surface facing the camera -- and dropping the section-4.2 projected-area warp is wrong by 35% WHILE THE CHEAP STRUCTURAL CHECK STILL PASSES, same facet count as the correct sampler, so "it never proposes a backfacing normal" is no evidence a visible-normal sampler is right. Five sabotages bite 8/5/1/4/1 and none goes 0 red. The 8 is the frame swap half-applied -- the error v4409's section 7 said this arc could not see, and this sampler builds its basis in-shader so it is load-bearing here. The 1 is the pdf's missing G1, caught by the IDENTITY ALONE because the weight never calls the pdf. The 4 WENT 2 RED FIRST AND WIDENED THE GATE: the shipped CPU sampler was resting on one section, because the identity is blind to a wrong direction and the normalisation never calls a sampler at all. UNCHECKED: ANISOTROPY, since Heitz's listing takes alpha_x and alpha_y and microfacet.mjs's D is isotropic; the OTHER STRATEGY, still, since the MIS weights are computed and never used; Fresnel, F = 1 throughout; energy compensation, which is what the shortfall these four routes agree on is FOR; and whether a real card agrees, which now leans on inverseSqrt and normalize as well. The tree stands at 1432 gates. Full changelog on docs/CHANGELOG.md
// v4409 -- *** THE SAMPLING HALF OF THE BSDF ON A DEVICE -- AND IT IS A BETTER INSTRUMENT THAN v4408's QUADRATURE AT THE ROUGHNESS WHERE THE QUADRATURE IS 49% WRONG. *** v4408 put the LOBE on a device and closed by naming what it had not carried: "the SAMPLING half (sampleHalfVector, sampleDirPdf, bounceWeight, misWeight), which is not ported here". This is that half. *** IT IS A DIFFERENT KIND OF PORT AND THE ROUND SAYS SO FIRST. *** v4408's WGSL was TRANSLATED from render/microfacetShader.js's shipped GLSL, so agreement was a statement about the shipped shader; there is no shipped GLSL for a sampler, because a fragment shader evaluates a lobe and does not choose a direction. So the lobe is still v4408's, COMPOSED rather than copied (lobeWgsl, spliced whole and asserted character for character), and everything below it is a NEW transcription that cannot be graded by textual identity. Hence three analytic keys. *** KEY 1: AN ALGEBRAIC IDENTITY WITH NO FREE PARAMETER AND NO STATISTICS. *** microfacet.mjs asserts in a comment that f cos_i / pdf collapses to F G2 |wo.wh| / (cos_o cos_h) -- D DISAPPEARS, which is the whole reason importance-sampling the NDF is worth doing -- and ships only the right-hand side. Both sides are computable, so this computes BOTH and subtracts, per direction, no averaging: 6.4e-16 over 41,158 directions at f64, and 3.0e-7 on the device, which is two roundings the short route never pays. *** KEY 2: THE MIRROR LIMIT, WHERE THE TWO INSTRUMENTS PART COMPANY. *** As alpha -> 0 every sample returns wh = n and the weight collapses to G2(cos_o, cos_o) = 1/(1 + 2 Lambda), so the estimator has ZERO VARIANCE and its answer is a closed form: the device lands on it to 5.4e-8 at three view angles, while the f64 estimator on the identical strata is 7.2e-10 -- so the device's number is arithmetic and not the estimator. *** AND v4408's QUADRATURE READS 0.512 AT THE SAME POINT, 49% LOW. *** A 500x500 grid cannot resolve a lobe of width 0.001 and needs 1500x1500 to reach 0.982. v4408 shipped that curve; this is the round that can say where it stops being trustworthy -- and where the quadrature IS converged the two agree to 1.7e-4, two methods sharing D, G2 and nothing else. *** KEY 3: WHAT THE ESTIMATOR IS BLIND TO, WHICH COMPLETES THE PARTITION. *** Scaling D by pi leaves it BIT-IDENTICAL at all six roughnesses, and leaves the LONG route unmoved too because D appears in the pdf as well -- so NEITHER route can grade D. The NDF integral sees D and is blind to G; the weak furnace sees both together; this sees G2 and is blind to D (wrongPdf 29%, beckmann 17%, separable -0.66%). Two rounds that SPLIT the model rather than each half-covering it. *** ONE THING FOUND AND DELIBERATELY NOT FIXED: microfacet.mjs's cdf inverse carries THE EXACT CANCELLATION v3494 REMOVED FROM D. *** u1*(a2-1)+1 against (1-u1)+u1*a2, the same shape in the same file, missed because that round was looking at D: 3.28e-3 in cos_h at alpha 0.001 against 1.4e-8, five orders. AND IT DOES NOT BITE -- through the estimator the rewrite moves the answer by 1.2e-8 at worst and by exactly 0 at alpha >= 0.25, because the corrupted samples live where u1 -> 1 and are a vanishing fraction. A latent hazard named with a number beats a fix nobody could justify from a measurement, and the rewrite stays one bit away for a consumer that clusters u1 near 1. ALSO: the balance heuristic is NOT exactly one -- 12.6% of 4096 pairs miss by 1.0 ULP on the device and 10.8% at f64, the SAME rate, so it is not a precision question and "one BY CONSTRUCTION" is a statement about the reals; no weight leaves [0,1], which is the part that would break an estimator. Five sabotages bite 6/5/2/3 and the sharpest is the 2: a wrong pdf Jacobian is caught by the IDENTITY ALONE, because bounceWeight never calls the pdf -- a round shipping only the estimator would have shipped a wrong pdf. A SIXTH WENT 0 RED and earned section 7: negating the azimuth moves nothing because this fixture puts wo in the plane z = 0, so wh.z never reaches dot(wo, wh) -- exact and arithmetic, worst departure 0 over 3456 samples; tilt wo out of that plane and 3226 of them move, so handedness is UNTESTED here rather than harmless, which is the opposite conclusion from v4407's furnace. UNCHECKED: whether a real card agrees, though this round's keys are far less exposed than v4408's -- nothing here subtracts two numbers near 1 except the cdf denominator that does not bite, and the built-in sin and cos enter only through the azimuth; the OTHER strategy, since the MIS weights are computed and never USED and combining two estimators is a round of its own; Fresnel, F = 1 throughout; energy compensation, which is what the shortfall is FOR; and the VISIBLE-NORMAL sampler, which a modern tracer uses and microfacet.mjs does not carry either. The tree stands at 1431 gates. Full changelog on docs/CHANGELOG.md
// v4408 -- *** THE MICROFACET LOBE ON A DEVICE, AND THE MODEL'S ARITHMETIC PREDICTION IS RIGHT TO FOUR FIGURES WHILE ITS TRANSCENDENTAL ASSUMPTION IS WRONG BY SIXTEEN PERCENT. *** render/microfacetShader.js has carried the GGX lobe in GLSL since v3494 and had never run on a GPU; its header said so -- "THE SANDBOX MODELS binary32; THE GPU IS THE AUTHORITY, and the page reads the numbers back to ask it". physics/render/microfacetWgsl.mjs asks, and does it by TRANSLATING THE SHIPPED GLSL TEXT rather than retyping it: glslFnToWgsl lifts each `float name(args){...}` out of FRAG_SRC_GGX and rewrites it, the only real translation being `c ? t : e` -> `select(e, t, c)` whose arms SWAP, and the gate proves the rewriter on a fixture with arms 7 and 9 before it goes near D. *** THE PREDICTION IS SCORED AND IT HOLDS. *** At roughness 0.001, cos 1 the device returns 2.604e-2 for the textbook denominator and 1.333e-7 for the rewrite -- v3494's two Math.fround numbers, to four significant figures, off real f32; over a 20-cell (roughness, cos) grid the device and the model agree on both forms to a last-bit difference. *** AND THE ONE THING A fround MODEL CANNOT MODEL IS A TRANSCENDENTAL. *** WGSL bounds sin and cos by an ABSOLUTE error of 2^-11 inside [-PI, PI]; Math.cos is near-correctly-rounded, so the model silently assumed accuracy no device promises. The bill arrives at the identity a fragment shader cannot take at all: INT D(m)(n.m)dm = 1 reads 0.837 at roughness 0.02 on this device, 1.63e-1 out, falling monotonically to 4.10e-5 at roughness 1 -- four orders across the knob, and the 4.10e-5 end would pass any band anybody would write, which is microfacet.mjs's own rule holding in a new subject: TOLD APART BY THE TREND, NOT BY WHETHER THE NUMBER IS SMALL. THE CAUSE IS PINNED RATHER THAN GUESSED AT: handing the SAME kernel the same grid's sine and cosine from the host, changing nothing else, drops the residual to 2.50e-5 and lands the partial sums on the f32 mirror -- 36 of 64 lanes bit-identical, 1.9e-7 worst on the rest. So the deficit is the transcendental entirely and the arithmetic port is exact. AND THE DEVICE IS CONFORMANT: worst |cos_device - cos_true| is 5.97e-5 against a permitted 4.88e-4, and ggxD's (1.0 - c2) -- a difference of two numbers within 2^-11 of 1 -- then reads 2.6e7 times too large near the pole. *** SO THE ROUND'S SUBJECT IS ONE EXPRESSION, AND v3494 REMOVED THE OTHER HALF OF IT. *** A SECOND FINDING ABOUT THAT ROUND'S OWN CLAIM: "the rewrite holds at 1e-7 THROUGHOUT" is a roughness sweep at cos = 1, the single point where (1 - c2) has nothing to cancel; every pixel of that shader is a different (roughness, cos) pair, and along the axis it did not sweep the rewrite is 9.9e-5 out at cos 0.9999 -- confirmed on the device to the same figures. True where it was measured. Also on a device for the first time: the weak white furnace at 15 (roughness, view) pairs, worst 1.11e-3, ON THE SAME BUILT-IN sin AND cos THAT COST THE NDF SIXTEEN PERCENT -- the difference is not the hardware, it is whether the expression SUBTRACTS; the Beckmann Lambda used with GGX's D reading 1.098 while the NDF cannot see it at all, which is the mistake the weak test was invented to catch; noJacobian reading exactly 4.000721 and noPi reading exactly PI; and the strong test's shortfall curve, 0.05% at roughness 0.02 to 62.1% at 1. Four sabotages bite 19/4/3/6 and TWO WENT 0 RED, which earned a section: the shipped max(c2, 1e-16) guard and the 1e-9 half-vector cutoff are unreachable on a midpoint grid by five and six orders (3.93e-3 and 5.09e-3 measured), so they are named as dead code here rather than counted as coverage. UNCHECKED: whether a real card's sin and cos are better -- 2^-11 is a ceiling and most hardware is far under it, but what is established is that the SPECIFICATION permits 0.837; whether the tree's shaders should stop computing (1 - c2) from c, which they should and which is a round of its own because every consumer moves with it; the SAMPLING half of microfacet.mjs, not ported here; and speed, which nothing times. The tree stands at 1430 gates. Full changelog on docs/CHANGELOG.md
// v4407 -- *** THE FIRST PIECE OF THE PATH TRACER TO CROSS TO A DEVICE, AND IT IS GRADED AGAINST SIX CONSTANTS RATHER THAN AGAINST THE CPU. *** physics/render/pathTracerWgsl.mjs at v4290 ported the two DECIDABLE pieces of the tracer -- the generator and the ray-sphere intersection -- and closed by naming what it would not attempt: "`trace` is roughly three hundred lines of multiple importance sampling, microfacet lobes, Fresnel, energy compensation and Russian roulette... Porting it is a real round; porting it before anyone has established what f32 does to the primary ray would be building on an unmeasured floor." That floor is established. This is not that port either: it is the piece whose answer is a CLOSED FORM, which is the one piece that can cross without anybody inventing a tolerance first. *** THE FOURTH CLAIM SHAPE IN THIS ARC, AND THE FIRST THAT NEEDS NO COMPARISON AT ALL. *** v4370 held smooth f32 arithmetic to bit-identity; v4400 held a decision ending in floor() to exact set equality against an f32 mirror; v4402 held integer-end-to-end code to zero tolerance because the kernel's own contract says so; v4405 earned a measured f32 floor before the first failure. physics/render/furnace.mjs's key has no free parameters -- a white furnace returns the albedo, and each named fault returns the albedo times a constant: EXPECTED = { clean: 1, noPdf: 1/(2 PI), noCosine: 2, badSampler: 4/PI } and EXPECTED_COSINE = { clean: 1, wrongPdf: 4/3 } -- so f32 against f64 never enters the grading. Measured at 262,144 samples, the device lands on all six: 3.86e-4, 3.86e-4, 5.79e-7, 2.56e-4, 5.79e-7, 2.59e-4 relative. A different seed lands on the same six. *** AND THAT MATTERS BECAUSE v4290 PROVED THE DEVICE CANNOT DRAW THE SAME RANDOM NUMBERS. *** That round measured it: the generator's STATE is portable and its OUTPUT is not portable even between conformant devices -- 98.02% of the first 65536 draws differ, because f32(u32) rounds and WGSL leaves which of the two neighbours you get implementation-defined. A pixel comparison would have been measuring the RNG. 4/PI is not, and the port therefore does not even try to reproduce the CPU's stream: it splits the work across lanes, each with its own seed, which would be a defect if the key were the CPU's answer and is not one here. *** THE TOLERANCE IS EARNED BEFORE THE DEVICE IS ASKED ANYTHING. *** Section 1 runs the CPU estimator over 48 seeds and measures its own spread: the four noisy cases have a worst relative error of 2.49e-3, 2.49e-3, 2.23e-3 and 1.56e-3, so the band is set at 5e-3 -- about twice the worst of 48 draws. That is magmapGpu.mjs's rule followed rather than cited. *** AND TWO OF THE SIX ESTIMATORS CARRY NO SAMPLING NOISE AT ALL, WHICH IS THE ROUND'S OTHER HALF. *** Over those 48 seeds noCosine and cosine-clean return the IDENTICAL value every time, sigma EXACTLY 0 -- noCosine integrates the constant 2 PI and cosine-clean's integrand is constant once the pdf cancels. So in those two the entire error is arithmetic, and the device's number is a statement about f32 with nothing to hide behind. It is not flat: it GROWS with the sample count, 1.04e-7 then 5.79e-7 then 1.62e-6 at 65,536, 262,144 and 1,048,576 samples, which is the per-lane f32 sum accumulating, while the CPU sits at 1.48e-12 throughout. More samples is WORSE there, which is the opposite of what a noisy estimator does and is the signature that identifies it. The other four cannot say anything of the kind and the gate does not let them. *** A SABOTAGE WENT 0 RED AND IT CORRECTED THIS ROUND'S OWN HEADER. *** Swapping the shader's tangent from +Z to +X moves nothing. That is not luck: it is a different, left-handed, still-orthonormal frame about the same N, and the furnace integrand is azimuthally symmetric, so the estimator reads only cos(theta) = dot(dir, N). The module had BORROWED pathTracerWgsl's argument that building the basis in-shader puts a handedness error inside the comparison -- true of a camera, FALSE of this key -- and the sabotage is what found it. The claim is corrected in the module and the blindness is now pinned in section 1 as arithmetic: cos comes out as sv.y for every orthonormal frame about the same N, worst departure exactly 0. What the key CAN see is N itself: left non-unit at 0.9 it moves the four cosine-bearing cases to 1.90e-1 together, while the zero-variance pair stays green, because noCosine drops the cosine and cosine-clean cancels it against the pdf -- so even there, two of the six cannot see it. Three sabotages that bite, 2/3/2, and the two that do the most work are furnace.mjs's own named traps arriving on a device: the cosine read from r1 rather than from the direction sends badSampler to 2.15e-1 and cosine-wrongPdf to 2.50e-1 while clean stays at 3.86e-4 -- exactly half the faults, which is what that module's comment predicts; and the cosine pdf multiplied by PI instead of divided leaves its band by four orders. UNCHECKED: THE TRACER, which is most of it. `trace` is still ~300 lines of MIS, microfacet lobes, Fresnel, energy compensation and roulette, and none of it runs on a device; what crosses here is the estimator those pieces are integrated BY, plus the sampling and the basis it needs. The microfacet lobes, which furnace.mjs's own white-furnace test can grade and this port does not carry yet -- radiance() is the constant 1 here, the furnace's simplest form. A real card's f32 against SwiftShader's, since section 3's growth curve is an accumulation fact that should hold anywhere but whose constant is this adapter's. And SPEED, which nothing here times, on a software rasteriser where a millisecond would be a fact about a CPU pretending to be a GPU. The tree stands at 1428 gates. ALSO THIS ROUND: physics/render/furnaceWgsl.mjs joins render/backendParity.mjs's census -- wgslBearing 56 -> 57, wgslOnly 43 -> 44 -- and the baseline MOVES rather than the shader hiding, which is the opposite call from the carve and roles fixtures that went into JSON at v4372 and v4404. Those are gates carrying a fixture; this is a shader-bearing module, and the census is right to count it.
...
// v4380 -- *** THE RED REGISTER, READ RATHER THAN KEPT -- AND ONE OF THE COUNTERS IT WAS HOLDING IS ONE THIS SESSION HAS BEEN PUSHING. *** v4371 found vendoredLicences red for 52 rounds because nobody ran a 15-second gate; v4379 found rigJobs red for 250 rounds on a line about a page whose panel had been deliberately removed. Two standing reds in one session turned out to be unopened mail, so this round asked the obvious question of the other twenty-nine: does each entry still say what it says it says? ALL 29 WERE RUN. Nothing is secretly green -- every one still fails, so the register is not carrying a repaired gate as an excuse -- and 25 of them fail with exactly the line recorded. FOUR CAME BACK DIFFERENT, AND TWO OF THOSE WERE THIS GATE'S OWN FAULT: the first draft compared the register's line against the gate's FIRST failing line, and a gate with several reds is ordinary -- boundaryLint and pagePlacements each print another line above the one recorded, so both read as drifted when the recorded line was still there. The audit now freezes EVERY failing line and asks whether the recorded one is among them. Four became two, and the two that remain are real. *** FIRST: A GATE WHOSE REASON COULD NOT BE READ. *** engine/frameDirtyCensus-selfcheck.mjs printed its FAIL line to STDERR, alone among the twenty-nine. It still exited 1, so the sweep always saw the red -- but every consumer in this tree scrapes stdout for "  FAIL  ", so the REASON was invisible, and the register's entry for it could only have been written by someone reading it by hand once. A red whose reason cannot be read is most of the way to a red nobody opens. One word changed; the verdict is untouched. *** SECOND: AN ENTRY THAT NEVER RECORDED A FAILING LINE AT ALL. *** referenceKind's `fails` was the annotation "confirmed red serially at 73.7s -- mis-bucketed as a timeout by the parallel sweep", which is a fact about the SWEEP and not about the gate. What the gate actually says is that a prose-rescued population may only SHRINK and stands at 223 against a ceiling of 181 -- and its own text reads "A RISE MEANS A NEW ORPHAN IS BEING HIDDEN BY A SENTENCE". It rose from 221 to 223 between two runs inside this round. A live, worsening signal filed under a note about bucketing; both are recorded now, the line and the note. *** THIRD, AND IT IMPLICATES THIS BRANCH DIRECTLY: shaderCensus SAID 4 AND NOW SAYS 14. *** That gate has held since v3274 that a hand-written shader pair is cheaper than an IR "while few files carry both languages -- if this count climbs toward twenty the arithmetic inverts, and THAT is when to re-open the three-stage shape: parse, lower, emit per target". docs/TSL-ROADMAP.md records 12 at v4319. It is 14 now, and the TSL rounds of this session put some of them there: every module that authors both languages counts. The register was quietly holding a counter that this branch was driving toward the tree's own stated trigger, under a line that said 4. That is not repaired here and is not pretended to be: whether the arithmetic has inverted is a decision about the engine's shape and belongs to a round of its own. THE MECHANISM, so this stops being something somebody stumbles on: tools/ship/freezeRegisterAudit.mjs runs every gate in the census and freezes what each one actually says -- exit code, every failing line, and which stream it printed on -- into tools/ship/register-audit.mjs, and tools/ship/registerDrift-selfcheck.mjs holds the register to it in four sections and stays inside the quick sweep by reading the freeze rather than re-running twenty-nine gates. Adding a register entry without re-freezing fails there rather than going unnoticed. Two sabotages: the shaderCensus entry put back to 4 (1 red, printing both lines -- what the register says and what the gate says), and the FAIL line moved back to stderr with the audit RE-FROZEN so the change is observed rather than assumed (1 red; re-freezing is part of the sabotage, because a check reading source instead of a run would have caught it for the wrong reason). UNCHECKED and said plainly: whether a red SHOULD be accepted, which is a judgement and not a comparison; the three findings themselves, none of which are repaired by a drift check; and shaderRefs, which times out at the audit's two-minute cap and is recorded as a bound rather than a verdict. The tree stands at 1420 gates. Full changelog on docs/CHANGELOG.md.
