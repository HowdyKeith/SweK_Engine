...
const BRAIN_BUILD = "v4288";   // v4288 -- *** THREE WAYS A UNIFORM CAN FAIL TO BIND, AND THE THIRD HAS NO SYMPTOM AT ALL. *** v4286 found the first -- a name that does not resolve. This round found the other two. Float ARRAYS (uHeatRadii and uHeatStrength are float[8]; the harness set only scalars and vectors) now bind via uniform1fv/2fv/3fv/4fv. And then: *** uHeatCount IS DECLARED `uniform int`, getUniformLocation SUCCEEDS FOR IT, AND uniform1f ON AN INT RAISES INVALID_OPERATION AND WRITES NOTHING. *** The name was never unresolved, the program linked and drew and produced a picture, uHeatCount stayed 0, the whole heat block never ran, and the footprint measured 0 with a clean bill of health -- a uniform that RESOLVES and still does not take the value, which defeats v4286's guard by construction. The harness now supports uniformInts and drains gl.getError() after every write. With all three closed heat is demonstrably LIVE: 4,792 pixels of a 256x256 frame change between off and on. *** AND THE PERTURBATION METHOD STILL CANNOT GIVE IT A RADIUS, WHICH IS A BETTER ANSWER THAN A NUMBER: *** it reads 0 at N=128, MINUS ONE at N=256, and 1 at N=512 for displacements of 0.45, 0.90 and 1.79 texels. Perturbation measures a GATHER and works only while every source texel is read by somebody; a DISPLACEMENT field stops that map being onto, so a poked texel can be read by NOBODY and "nothing changed" becomes indistinguishable from "never reads there". Heat is recorded as EXERCISED with its literal cap (0.0035 of the image, about 7 texels at 1080p) rather than given the radius it happens to yield at one resolution. *** AND THE HALF-FLOAT ASSUMPTION WAS RIGHT, WITH NOBODY HAVING LOOKED: *** render/bloomPass.js Round 136 already promoted sceneFBO to RGBA16F "so emissive surfaces can output values > 1.0", falling back to RGBA8 with a warning that "bloom will be weaker" -- and v4287 measured what weaker means, the peak clamping 1.7480 to 1.0000 with all 181 samples above 1.0 destroyed. THE FILE'S OWN OVERVIEW CONTRADICTED ITS OWN CODE: line 5 said "sceneFBO (RGBA8)" and had since Round 136 changed it. Corrected, and the gate now COMPARES header against texImage2D rather than asserting either alone. Two things found in the heat block, reported and NOT changed: its vertical bias puts the distortion BELOW the source (affected rows 0..149, source at v=0.5) while its comment says ABOVE, "rising heat, not below"; and the expression is smoothstep(0.5, -0.5, x), which GLSL ES specifies as UNDEFINED when edge0 >= edge1 -- it behaves as a reversed smoothstep wherever the naive formula is used, which is not the same as being defined. Sabotages: the stale RGBA8 header restored goes 2 red WITH its control, because header and code are compared; and heat filed with evidence "measured" and radius 1 goes 1 red -- *** THE RADIUS IS GENUINELY 1 AT N=512, SO THE NUMBER IS NOT EVEN WRONG, IT IS UNEARNED, *** and the check is that the entry DECLINES a radius. Full changelog on docs/CHANGELOG.md.
