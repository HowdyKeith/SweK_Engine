...
const BRAIN_BUILD = "v4273";   // v4273 -- *** THE ORRERY HAS A SHADER STAGE, AND FINDING IT A CONSUMER FOUND THREE HOLES THREE ROUNDS OF COUNTING MISSED. *** ui/orreryDraw.js is canvas 2D -- no shader stage, so not one of the tree's 134 GLSL or 39 WGSL modules could touch it. ui/orreryPost.mjs keeps the 2D drawing and makes the canvas the SOURCE TEXTURE of a gfx/device.js pipeline: the first non-demo consumer that abstraction has ever had. The gate attaches it in a real browser, feeds it the actual drawSystem output (1,851 lit texels), and reads 2,031 lit texels back off the target with the effect on and nothing with it off. *** AND IT SETTLES v4272's ORIENTATION QUESTION BY BEING A CONSUMER RATHER THAN AN ARGUMENT: a 2D canvas's row 0 IS its top, so framebuffer space is the source's own layout and needs no flip anywhere. *** THREE HOLES, ALL FOUND BY ATTACHING RATHER THAN COUNTING: (1) gfx/device.js's WebGPU pass.texture was `() => {}` -- MEASURED, the pipeline builds and every texture bind is silently dropped, so that backend could carry a texture-free render and no post effect at all, a post effect being a texture consumer by definition. It refuses by name now. (2) device.texture() took only raw bytes, so the only route from a canvas was a full getImageData readback per frame; `source` accepts a canvas directly. (3) requestDevice reads `backend`/`prefer` and I passed `backends` -- an option name I invented, which my own probe masked by asking for WebGPU, the default. *** AND A CORRECTION: v4271 AND v4272 BOTH CALLED badTvPass.js "what main.js actually draws with". IT HAS NO CALLERS. *** makeCrtPass has ten call sites, swiftShader six, aquarelle four, transition two; badTv, grassField and holoFoil none. Ninth self-counting scan too: a check forbidding a string its own explanation contains. Sabotage A 1 red, B 1, C 2 -- B being the only one whose damage is invisible to everything except counting pixels. Full changelog on docs/CHANGELOG.md.
