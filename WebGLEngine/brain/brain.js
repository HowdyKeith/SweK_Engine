...
const BRAIN_BUILD = "v4320";   // v4320 -- *** THE ROADMAP'S FOURTH STEP: A PAIR NOBODY TYPED. *** docs/TSL-ROADMAP.md said step 4 is the one that changes the engine's economics -- reading the WGSL and GLSL out of three's node builders and feeding them to gfx/device.js, so a hand-written pair could become a generated one. Built. render/tslSource.mjs: emitShaders() asks three (WebGPURenderer.debug.getShaderAsync) for the shaders of a mesh, in WGSL on the WebGPU backend and GLSL on the WebGL2 one; what three hands out is a whole MATERIAL shell -- its camera and object uniform groups, its varyings, its own bindings and entry name -- which the device does not speak, so transplantFragment() moves the fragment across: the helper functions three emitted and the body of its main(), with three's names rewritten to the device's (the varying to the device's uv, the object struct to the device's uniform struct at binding 0, a LABELLED texture to the device's texture binding, its sampler to the device's one sampler, output.color to a return), inside the device's own full-screen vertex stage; devicePipelineFromTsl() joins the two into a pipeline descriptor with the uniform list in three's order and typed. THE RULES ARE NARROW AND REFUSE BY NAME: a fragment-only effect (a camera or object matrix in the fragment refuses), every uniform and texture labelled (an unlabelled nodeUniformN has no stable name and refuses), a bare NodeMaterial with fragmentNode (a MeshBasicNodeMaterial adds an opacity uniform and a clamp the effect did not ask for -- badTvTsl and blackbodyTsl moved to it, their uniforms and textures labelled, and tsl-selfcheck still holds their pictures to the byte), one varying, types the device carries; two builders that disagree about the uniforms or the textures refuse. tslSource-selfcheck: on the CPU the rewrite is shown on a fixture in three r178's shape and every refusal fires; on BOTH backends badTv is emitted by three, transplanted, run by gfx/device.js, and draws the hand-written badTvDevicePass picture on 4,096 of 4,096 pixels with worst difference 0 -- three's WGSL builder's fragment against ours on WebGPU, three's GLSL builder's against ours on WebGL2, and with the device's own vertex stage no mirror is needed; the blackbody key transplants too and finds Wien's root (the brightest column at x_lambda within a column, the root in the blue byte, 158). One thing three does that the device pass does not: it emits only what the fragment READS, so the probe-only `rows` knob is dropped by both builders, and the gate says so. The emitted and transplanted pair is written to tools/ship/tsl-emitted.json on every run and the WGSL corpus compiles it as generated code (two entries, absent until the gate has run once and said so). One new gate. Sabotaged and measured: the object-struct rewrite dropped (3 red: the generated WGSL no longer compiles and WebGPU draws the clear, WebGL2 untouched and green), the uniform struct emitted backwards (3 red: gfx/device.js REFUSES the pipeline by name before any draw -- the v4278 host/shader agreement check catching a generated pair), and the varying's y turned over in the transplant (2 red: 0 of 4,096 pixels agree). Two census corrections rode along: the parity baseline (render/tslSource.mjs carries both languages; the gate's fixtures moved to JSON so a file that only grades a transplant does not count as a shader module) and the corpus (v4318's PICK_MASK_WGSL and COMPOSITE_WGSL, which crossBackend named as unregistered, and the transplant's vertex shell excluded as the source fragment it is). Not built and said: a vertex-stage transplant (a graph with three's camera in it, or more than one varying, is refused); linear-filtered textures through three's sampler on the device (badTv's is nearest, which three reads with textureLoad; the sampler path is exercised on the CPU fixture only); and the SPEED of the generated code, whose nodeVar chain is longer than the hand-written one and which nobody timed. The tree stands at 1397 gates.
