...
const BRAIN_BUILD = "v4279";   // v4279 -- *** NOBODY KNEW WHAT WAS RED, AND THREE DIFFERENT ANSWERS WERE ON RECORD, ALL WRONG. *** Backlog #134 said FIVE gates were red at HEAD. tools/ship/gate-timings.json's `failingAt` said NINETEEN, snapshotted around v3211. A full sweep of all 1,348 runnable gates at v4278 -- every one actually executed -- found THIRTY-NINE, and a fortieth was hiding in the timeout bucket. The old register was wrong in BOTH directions at once: TWELVE of its nineteen entries are GREEN now, fixed by rounds that never knew they were listed, so it had been accusing working code for a thousand versions; and THIRTY of the thirty-seven that really are red were absent from it entirely. Appended-to-only, a register becomes a list of grievances; never appended-to, a list of fiction. That one managed both, which is only possible if nothing ever re-ran it. *** THE REASON NOBODY LOOKED WAS ALSO NEVER TESTED. *** The suite runner buffers, and the full run was believed to take ninety minutes, so every attempt looked like an hour of silence. Running the gate files DIRECTLY, eight at a time, appending each verdict as it lands, finished in twenty-five minutes with progress visible throughout. The obstacle was the runner, not the work. *** BUT A PARALLEL SWEEP LIES, AND IT LIED ABOUT SEVEN. *** The 8-way sweep reported 46 red; re-running those ONE AT A TIME on an idle box turned seven green -- a battle sim asserting its clock passed 0.5s after 2.5s of wall time, a frame budget, a sort benchmark, three browser gates. Every one measures against the clock and every one was starved by the other workers. So the method is two phases and the second is not optional: sweep wide to find candidates, then CONFIRM EACH SERIALLY. A parallel red is a hypothesis; 15% were false. *** AND ATTRIBUTION BY CHECKING OUT AN OLD COMMIT HAS ITS OWN TRAP. *** Re-running the 39 in a worktree at v4266 said three were broken by this session. Two really were mine -- gateQuality (four prose-matching regexes in gates I wrote at v4270-71, one of them an ABSENCE check where a re-wrap would have gone quietly green with a false claim still in the file) and orreryEjecta (webgpuHarness.mjs really does import three, so 67 -> 68, established by diffing importer lists). The third was a FALSE ATTRIBUTION: duplicateFiles walks the filesystem and found two git-ignored agent worktrees that exist on this box and in no commit. A CLEAN CHECKOUT IS A DIFFERENT WORLD from a working tree. Fixed at its cause -- the walk skips .claude -- not by moving a baseline. tools/ship/redCensus.mjs records the 37 with what each fails and its measured runtime, so re-verifying ALL of them costs 142s and the census is never sampled. Its gate RE-RUNS every one and goes RED WHEN A LISTED GATE IS FIXED, so the list can only shrink deliberately -- the mechanism the old register lacked. *** AND THE FIRST DRAFT OF THE CENSUS SHIPPED THE EXACT BUG IT EXISTS TO CATCH: *** referenceKind-selfcheck runs in 73.7s and exits 1, was starved past a flat 120s cap, filed as "timed out", then read as exonerated because it was not in the confirmed-red list. Caught only by the control that re-runs a sample of the wrongly-accused instead of trusting a set difference. 64 gates remain UNMEASURED rather than red or green, named in the census; 20 confirmed so far, 18 green, 2 still unfinished at 400s. So the honest total is AT LEAST 37. Sabotages: a passing gate listed as red 3 red; the unmeasured bucket emptied 1; the mis-bucketing restored 3. Full changelog on docs/CHANGELOG.md.
