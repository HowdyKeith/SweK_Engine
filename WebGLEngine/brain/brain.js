...
const BRAIN_BUILD = "v4384";   // v4384 -- *** simulation/lbm/lbmShader.js SAID IT WAS "CORRECT-BY-CONSTRUCTION AND UNRUN". IT DID NOT COMPILE. *** And the sentence directly above that one, in the same header, is a v2977 correction catching this file making a claim it had not earned: an earlier version ended "the test below pulls the constants back OUT of the generated WGSL and checks them against the module" and THERE WAS NO SUCH TEST. That was fixed -- simulation/lbm/shaderConstants-selfcheck.mjs exists, reads the text, and passes. *** THE SAME SPECIES SURVIVED IN THE NEXT SENTENCE, four hundred rounds later, hidden behind the very gate that fixed the first. *** Nothing had ever handed the text to a COMPILER. `macro` is a RESERVED KEYWORD in WGSL, so the binding made the entire module a parse error -- "13:48 error: 'macro' is a reserved keyword" -- and the first run attempted here had all 48,124 of its dispatches rejected before any began. A constants gate that reads a string cannot tell you the string is a shader. The buffer is `moments` now, which is what it holds: density and momentum ARE the moments of the distribution. *** AND IT TOOK A gfx/device.js CHANGE, BECAUSE THIS IS THE FIRST MODULE IN THE TREE WITH TWO ENTRY POINTS. *** WebGPU's layout:"auto" builds a bind group layout from what the CHOSEN entry point USES, not from what the module DECLARES: collideStream touches all five bindings and stream touches three. Both halves of that mismatch were errors and there was no way to be right -- binding the extra two to stream is rejected by the device ("binding index 2 not present in the bind group layout") and not binding them was refused by device.js itself ("nothing was bound to it"). compute() takes `uses` now; omitting it requires every declared binding exactly as before, so nothing that already worked changed, and the four device* gates plus every TSL and carve gate were re-run to completion to say so. *** THE KEY IS THE ONE THE CPU IS ALREADY HELD TO. *** tools/ship/floors.mjs POISEUILLE is the tree's own statement of channel flow's exact analytic answer -- the relative L2 against (force / 2nu) * y * (H - y), with y = j - 0.5 and H = ny - 2 because halfway bounce-back puts the no-slip surface between the last fluid node and the wall -- against a tolerance of 0.005, on its nominal fixture unchanged (tau 0.8, 12x34, 12,000 steps, a step count that file sized by measurement after a first guess of 3,000 came out eleven times the tolerance). MEASURED: the device reproduces the parabola to 7.437e-4 and the CPU reads 7.038e-4, with a centre-line velocity of 1.2780 against a rest state of 0. The parabola is never given to the kernel. *** AND THE FORCING, WHICH floors.mjs PROVED VACUOUS, IS WHERE f32 SHOWS. *** That file declares `force` a vacuousKnob with a measurement behind it: "Stokes flow is linear in the forcing, so the measured profile AND the analytic target scale together and the RELATIVE residual is invariant. Measured: a 20% error in force moves the observable by EXACTLY 0.000000%." True in f64 and NOT true on the device: the CPU returns 7.038e-4 at force 1e-6 and at 1e-3 alike, and the device returns 1.505e-3 and 7.437e-4 -- a 2.0x spread against 0.0000%. At the nominal forcing the perturbation to a population of about 1/9 is roughly seventeen f32 epsilons. Both device readings are still inside the tolerance, so this measures what the precision costs rather than failing, and it is the cheapest possible statement of it: the knob was already PROVEN to carry no physics, so everything the device does with it is precision. Three sabotages, 2/3/1, and the first is a RE-ENACTMENT of the state the shader actually shipped in. The second is lbm2d.js's own prediction arriving on a device: drop the Guo half-force correction and the residual goes to 6.254e-1 with the centre line at 4.7909e-1, which is the comment's "wrong by exactly g/2 and the parabola sits low". *** AND A SECOND GATE, EARNED BY REPEATING A MISTAKE THIS SESSION HAD ALREADY WRITTEN DOWN TWICE. *** This branch merged main eight times in one session and twice committed a conflicted file with its markers intact, both times because `git add -A` STAGES a conflicted file, marks it resolved, and then `git diff --diff-filter=U` reports nothing -- the conflict list taken after the command that hid it. v4381's own commit message says "a conflict list taken after `git add -A` is not a conflict list", and v4384 did it again to docs/CHANGELOG.md three rounds later. The first time tools/check.mjs caught it, because brain/brain.js is JavaScript and markers are a SyntaxError; the second time NOTHING did, because a Markdown file with markers parses fine, ships fine and is wrong. The difference was not care, it was that one file type had a mechanism. tools/ship/conflictMarkers-selfcheck.mjs is the mechanism: 5,427 tracked text files, scanned for a PAIR of markers at line start, with the markers assembled at run time rather than spelled and the negative half -- a Markdown setext rule of seven equals signs is not a conflict -- as the load-bearing check, because a reader that fired on those would be switched off within a round. Its first sabotage went 0 RED and that was a finding about the sabotage: it spliced the block in mid-line, so the marker was not at the start of a line and the reader was right to ignore it. UNCHECKED: SPEED, which is the entire reason an LBM kernel exists -- the module's own header opens "LBM is the ideal GPU workload and this is why FluidX3D is so fast" and SwiftShader is a software rasteriser, so nothing here times anything. simulation/lbm/lbm3dShader.js, which has no gate either and was checked only for the same reserved word (it does not have one). Obstacles, inflow and thermal coupling, all of which lbm2d.js has and this shader does not. Where between the two forcings the device's residual leaves the tolerance. And whether a conflict was resolved CORRECTLY, which no scanner can tell -- the new gate only says somebody finished. The tree stands at 1425 gates.
