...
const BRAIN_BUILD = "v4255";   // v4255 -- *** THE TREE HAS HAD A JUDGE SINCE v3337 AND NOTHING TO JUDGE; THIS IS THE SCULPTOR, AND IT PROVES LESS THAN ITS SCORE SUGGESTS. *** No image-to-geometry path existed anywhere: spriteHullCore gets a bitmap to an outline, extrudePolygon gets an outline to a prism, and NOTHING LATHED -- the wrong gap, since the objects a photo is most often of are solids of revolution. New mesh/lathe.mjs revolves a radius-per-row profile and is EXACT: measured volume matches pi*r^2*h*(N/2pi)sin(2pi/N) to 3.71e-8 at every N from 3 to 1024, because an inscribed N-gon's area is itself a closed form rather than a tolerance. TWO BUGS THE MEASUREMENTS CAUGHT: the end caps were wound backwards, and since the divergence theorem sums SIGNED tetrahedra a reversed cap SUBTRACTS its cone -- a cylinder read exactly one third of pi*r^2*h while a SPHERE read 0.3% out, because a sphere's caps are degenerate at the poles and could never have shown it; and one gate check was written as ok(..., true, ...), an assertion that cannot fail. *** THE HONEST RESULT: A LATHE DOES NOT RECOVER A SHAPE FROM A PHOTOGRAPH, IT ASSUMES ONE. *** Front-view IoU is 0.99 for revolvable inputs and still 0.61-0.64 for an L-bracket and a crescent that are not solids of revolution in any sense, because the front view of a revolved profile IS the profile mirrored and most of that number was never at risk. Mirror asymmetry separates them completely -- exactly 0.0000 against 0.9063 and 0.9439 -- and is the only number in the loop that tests the assumption rather than the arithmetic. The score is taken from render/silhouette.mjs itself (0.988783), not a private copy. Full changelog on docs/CHANGELOG.md.
