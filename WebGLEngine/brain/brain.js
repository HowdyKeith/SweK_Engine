...
const BRAIN_BUILD = "v4287";   // v4287 -- *** v4284 PROVED THE FUSED BLOOM SHADER AND STOPPED SHORT OF A RENDER PASS IN TWO WAYS THAT BOTH MATTERED: *** it wrote f32 into a storage BUFFER, because that is what the compute harness bound, and it COMPUTED its own input from srcAt, because there was no way to hand it a texture. Neither is a thing a renderer can use -- a post pass reads the scene somebody else drew and writes a texture the next pass samples. Both are closed, each link measured on a real device: the shipping GLSL chain == a CPU oracle at 0.500/255 (v4284, WebGL2); that oracle == the fused dispatch into a buffer at 2.5 float32 epsilons (v4284); that buffer result == the same shader into a storage TEXTURE at 4.72e-4 relative; and the procedural input == a real SAMPLED input texture at 6.24e-4. Half-float carries a ten-bit mantissa so ~1e-3 relative is the FORMAT's resolution rather than slack. *** THE STORAGE FORMAT IS A CORRECTNESS DECISION AND THE CLIPPING IS MEASURED: *** rgba8unorm is the obvious default -- it validates, dispatches, produces a picture -- and clamps to [0,1] while bloom's entire job is carrying values above 1. The reference scene peaks at 1.7480 with 181 samples above 1.0; through an 8-bit target the peak reads exactly 1.0000 and all of them are destroyed, a 43% loss on the brightest. Nothing but a measurement separates that from the right answer, because the failure is invisible wherever the scene is dim. The harness gained runWgslComputeToTexture with an optional rgba16float SAMPLED input; nothing in this tree had ever used a storage texture, the only textureStore in the repository being inside vendored taichi. *** WHAT IS STILL NOT WIRED, ASSERTED IN THE GATE SO THE CLAIM CANNOT ROT: *** render/bloomPass.js still runs its three draws and does not import the fused module. What exists is a pass a WebGPU renderer COULD call, proven equivalent; swapping the live path needs a runtime device, a WebGL fallback, and a memory-traffic measurement this sandbox cannot make with a software rasteriser. Building the wiring before the measurement puts it one RUN away from anyone with hardware instead of one ROUND away; claiming the measurement happened would not. Three sabotages: the HDR default changed to rgba8unorm goes 6 red and even the not-empty CONTROL fails, because a clamped bloom looks exactly like a bloom that never happened; textureStore's coordinates transposed goes 4 red at 8.43e+1 relative while *** THE SHADER STILL COMPILES, DISPATCHES AND FILLS EVERY TEXEL *** -- a transposed image is a plausible picture and only a reference that knows where each pixel belongs catches it. *** AND ONE WENT 0 RED BECAUSE A BRANCH OF THE READER WAS UNREACHABLE AT THE SIZE THE GATE USED: *** copyTextureToBuffer pads rows to a 256-byte multiple, and at N=64 an rgba16float row is already 512, so the padding path never ran and deleting it changed nothing. N=40 pads 320 to 512 and is now checked with a control asserting N=64 does NOT pad. That is the third round running where a sabotage found machinery present, correct and UNTESTED rather than a defect -- v4285's overstated label, v4286's unreached guard, and now this. Full changelog on docs/CHANGELOG.md.
