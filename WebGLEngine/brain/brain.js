...
const BRAIN_BUILD = "v4377";   // v4377 -- *** THE SWAP: THE ORRERY AND THE UNIVERSE DRAW DISCS NOW, AND NEITHER CARRIES A TYPED LOD THRESHOLD. *** v4376 measured that the bodies were squares while orrery-gpu.html's own comment called them discs, built the ladder that comment describes, priced it, and left the choice with whoever owns the look. The choice is made: both pages draw render/gpuDriven.mjs discMesh, and the comment and the code agree for the first time since the line was written. A body is rounder and, being inscribed, 0.7743 of its old area; the cheap rung goes from 2 triangles to 5. Both numbers were on the table before the swap and neither moved. *** AND THE THRESHOLDS ARE DERIVED AT PAGE LOAD, FROM A FROZEN MEASUREMENT, BECAUSE A THRESHOLD CANNOT BE ONE CONSTANT. *** A rung's cost in PIXELS is an area on the screen, so it grows with the square of the frame width and a pixel budget is met at a different angular size on a different canvas. render/lodRecord.mjs freezes the disc ladder priced on a device -- four widths, ten distances, two rungs, every rung under one colour so only geometry can differ -- and render/lodBudget.mjs discLadderThresholds derives from it. MEASURED: the crossings for rung 1 are 0.0625, 0.0354, 0.0151 and 0.0075 at widths 128, 256, 512 and 1024, and metric x width is 8.00, 9.06, 7.72 and 7.75 -- constant to a fifth across an EIGHTFOLD change of resolution. So the law is metric = K / width, K is derived rather than typed, and a width outside the measured range is served but FLAGGED as an extrapolation instead of answered silently. The old typed numbers -- 0.012 in the orrery, [0.004, 0.012] in the universe -- are gone. *** K IS THE SMALLEST OF THE MEASURED WIDTHS AND NOT THE MEAN, AND THAT WENT UNCHECKED UNTIL A SABOTAGE FOUND IT. *** The policy is a CEILING, so the threshold that honours it at every width measured is the conservative one; swapping the minimum for the mean cost 0 RED, the fifth 0-red sabotage of this session and the fifth to name a check nobody had written. The gate now feeds every derived threshold BACK into the record at its own width and demands the implied cost stay inside the budget -- eight cases, worst 64.0 px against a 64-pixel ceiling -- and the mean then reads 80.9 px at width 128 and costs 1 red. *** AND THE RECORD HAD TO GROW TWICE, BOTH TIMES BECAUSE THE GATE ASKED IT SOMETHING IT COULD NOT ANSWER. *** The first freeze stopped at width 512 while the orrery's canvas is 640, which made the shipped page an EXTRAPOLATION of its own record; extended to 1024, the distance sweep did not reach far enough for the coarse rung to come inside 64 pixels at that width, so the derivation FAILED CLOSED and refused the whole ladder rather than returning a number -- the v4374 fail-closed path working on real data instead of a fixture. Extended again to ten distances, both widths derive and 640 interpolates. THE PAGES ARE LOADED, NOT JUST PARSED: both are served and driven headless, both throw nothing, and the threshold the orrery ACTUALLY RAN (0.01022 at width 640) is held to what the record derives for that canvas. tools/ship/freezeLod.mjs re-freezes the record when discMesh, the render pipeline or the ladder's segment counts change, so the thresholds two shipped pages use follow a measurement rather than a memory. Two sabotages beyond BC: the orrery deriving at a fixed 256 instead of its own canvas width (2 red -- a resolution-scaled law typed back into a constant is exactly what this round replaced). And this gate's section 1 WENT STALE FROM ITS OWN ROUND: it asserted the pages were built from quadMesh, which stopped being true the moment the swap landed. It now reads what they are, and section 2 keeps the quad ladder's pricing under its own name -- a measurement does not stop being true when the thing it measured stops shipping. UNCHANGED and named: gpu-rig-check.html's typed [0.025, 0.04], whose ladder is a rig probe rather than a look; the colour tell, which both pages keep, because watching a ladder work is what it is for; whether 64 pixels is the right budget, which is a judgement about eyes this tree cannot make and has put in one place; and every number the rig has not signed -- the record is SwiftShader's, and a rig's rasteriser will not agree with it exactly. The tree stands at 1419 gates. Full changelog on docs/CHANGELOG.md.
