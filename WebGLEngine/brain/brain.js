...
const BRAIN_BUILD = "v4280";   // v4280 -- *** A SONG IS ALREADY A HEIGHTFIELD, AND NOBODY HAD WRITTEN THE LINE THAT SAYS SO. *** physics/fft.js was built to be BIT-REPRODUCIBLE -- twiddles from the strict-trig core, then nothing but +,-,*,/ -- and its header promises a spectrum somebody else can recompute exactly. That was never exercised against the thing it makes possible, because THE FFT HAD NO GEOMETRIC CONSUMER: nine files reach it and every one either drives a meter or checks the transform against itself. A short-time Fourier transform is a 2D array indexed by TIME and FREQUENCY. That is a height grid -- not a metaphor for one, the same object read with a different name. world/songHeightfield.mjs writes the mapping down and returns EXACTLY what world/repoHeightfield.js returns: the same thirteen keys, same types, water as {areas,ways} polygons, so world/realTerrainStamp.js needs no song-specific branch and neither does the fly-in. The solver and the consumer were both already in the tree with nothing between them. *** WHAT MAKES IT CHECKABLE RATHER THAN PRETTY: *** a terrain generator can only be argued about on taste, but the transform of a pure tone has a closed form. A bin-centred sine lands in EXACTLY bin f*frameSize/sampleRate (250 Hz at N=1024, SR=8000 -> bin 32, measured 32) with magnitude EXACTLY A*frameSize/2 (measured 512, exact equality, no tolerance) and neighbours at 1.9e-12, fourteen orders down. An octave is exactly twice the bin -- an integer, needing no tolerance. Every ridge is compared against a number the gate computes independently, which is physics/rebar.mjs's rule applied to sound. *** AND THE HONEST LIMITS, MEASURED ON BOTH SIDES: *** a tone half a bin off centre is not one column any more -- 4.42% of its energy lands more than 4 bins away under a rectangular window, and it loses 3.856 dB of height (scalloping, against the -3.92 dB rect approaches as N grows). Hann takes that to 0.00%. rect is the DEFAULT because it makes the closed forms exact, which is what a gate can grade, and that is a stated trade rather than an oversight. Bit-reproducibility is checked ACROSS SEPARATE PROCESSES, since reading a value twice in one process proves nothing -- and the same run shows the ridge is NOT perfectly flat: 8.44e-15 relative across 30 frames, because Math.sin is evaluated at a larger i each frame and its last bits differ. The transform is exact; the generator wobbles. THREE DEFECTS THE ROUND FOUND IN ITSELF: the first draft POINT-SAMPLED the spectrum, so a 250 Hz tone -- one bin in 512 -- was stepped over entirely and became a lake covering all 256 cells; max-pooling puts the ridge exactly at floor(32*grid/512) at every resolution. The first draft returned `water` as a FLAT MASK, and realTerrainStamp reads data.water.areas -- undefined on an array, so the guard is falsy, no water is painted and NOTHING REPORTS A PROBLEM. And it was missing three of repoHeightfield's thirteen keys while calling the contract matched. WIRED, NOT SHELVED: window.songTerrain.load() flies over a rising sweep with no arguments, .load({url}) decodes any audio the browser can play, .load({hz}) one tone. Sabotages: reverting max-pooling 1 red; water back to a mask 2; binOfFrequency rounded 1; the wiring deleted 7. *** TWO OF THE FOUR FIRST WENT ZERO RED AND THE GATE WAS AT FAULT BOTH TIMES *** -- section 4 had asserted that a point sampler would miss the tone by writing one INSIDE THE GATE, grading an expression it had just authored rather than the module; and nothing tested that binOfFrequency is continuous, so Math.round was invisible. Both are fixed and both reds are real now. Full changelog on docs/CHANGELOG.md.
