...
const BRAIN_BUILD = "v4382";   // v4382 -- *** THE ARC'S FIRST PURE-INTEGER TRANSPLANT, AND THE ONLY ONE WHOSE EXACTNESS IS A CONTRACT RATHER THAN A RESULT. *** Three claim shapes now. v4370 put tools/roundhouse/hmcGpu.mjs's leapfrog through a TSL graph and got bit-identical results because that kernel is SMOOTH f32 arithmetic -- an ulp stays an ulp, and the round would still have stood at 1e-6. v4380 put the silhouette carve through and it ends in a floor(), which is DISCONTINUOUS, so an ulp can flip a voxel; the twin had to become an f32 mirror with the discontinuity measured beside it (66 pixel flips in 17.3 million voxel/view pairs, none of which propagated). *** tools/roundhouse/isingGpu.mjs IS NEITHER, BECAUSE IT HAS NO FLOATING POINT TO ARGUE ABOUT. *** Its v3283 header states the contract -- "INTEGER ARITHMETIC END TO END, so the CPU mirror and the device must agree BIT-EXACTLY on every spin, every sweep" -- and adjudicateSpins() carries tol: 0. There is no weaker claim available. render/isingTsl.mjs is that kernel as nodes and MEASURED: 40 sweeps at T = 2.0 on a 32x32 lattice, 80 dispatches, and the generated pass agrees with the shipped CPU mirror on all 1024 spins at tolerance zero; the SHIPPED kernel agrees with the same mirror; and the two device passes agree with each other on all 1024. Philox4x32-10 alone is bit-exact on all 1024 u32 words -- the 32x32 -> (hi, lo) multiply in 16-bit limbs WITH its carry chain, and ten rounds of bit mixing, emitted by three. And the lattice really moved: |M|/N goes 0.0410 to 0.8008, which is ordering below the 2D Ising Tc of 2.269 as it should, so the agreement is not two passes doing nothing. *** SABOTAGE A IS THE WHOLE ARGUMENT FOR ADJUDICATING THIS AT ZERO. *** Drop the mulhilo carry chain -- u32 addition wraps in WGSL and lh + hl reaches 2^33 -- and ALL 1024 PHILOX WORDS DIFFER, a completely different random stream, while only 135 OF 1024 SPINS MOVE. The lattice still orders, |M|/N still lands where a healthy run lands, THE PHYSICS LOOKS ENTIRELY FINE. An observable check passes that sabotage; the bit claim is the only thing in the loop that does not. The other two: the accept comparison inverted (3 red, 512 of 1024 spins, half the lattice, because inverting a threshold test flips acceptance about its own median) and the vertical neighbour wrap dropped (3 red, 54 of 1024 -- one row of a 32-wide lattice is 32 sites and the rest is what those wrong neighbours went on to influence over 40 sweeps). *** AND THE SABOTAGES ARE WHAT PROVED THE DEVICE PATH WAS LIVE AT ALL. *** This gate runs in 0.9 s, which read as impossible for a WebGPU gate and was not investigated by reading: node-webgpu, installed by the SessionStart hook at v4359, serves it in-process through Dawn instead of launching a browser. A suspiciously fast green is worth one check. WHAT IT COST THE SHELL, REFUSED BY NAME BEFORE THE PASS WAS BUILT: render/tslSource.mjs's uniform vocabulary was float-only in its VECTORS -- f32, vec2/3/4<f32>, mat4x4<f32>, with i32 and u32 present only as scalars -- and this pass carries sweep, parity, seed and key in a vec4<u32>. The transplant refused it: "uniform cfg has type vec4<u32>, which the device's uniform list does not carry". The guard was right and the vocabulary was short; ivec2/3/4 and uvec2/3/4 are in it now, which is the same shape of shell growth as the struct element at v4363 and the uniform arrays at v4364, and the gate checks BOTH directions -- the new type transplants, and a type that really is outside the vocabulary is still refused. Every gate that runs through that machinery -- tslPhysics, tslRace, tslRig, carve, carveGpu, carveJudged -- was re-run to completion beside this one rather than left to the quick sweep's 3 s cap. UNCHECKED: a REAL GPU. Philox is integer arithmetic, exact on any conforming device, so this is the one kernel in the arc whose bit claim should NOT be hardware-dependent -- but that is an argument and SwiftShader is the only device that has answered it. Also unchecked: the physics, since |M|/N ordering below Tc is a direction rather than a measurement of Tc and roundhouse's own ising gate is where the observable is graded; whether a larger lattice or a longer run stays exact, because a counter-based RNG has no sequence to drift but a wider dispatch has more lanes to index wrongly; and WebGL2, which has no compute stage and is refused by name. The tree stands at 1422 gates.
