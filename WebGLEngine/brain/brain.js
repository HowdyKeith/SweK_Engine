...
const BRAIN_BUILD = "v4289";   // v4289 -- *** CONTRIBUTORS AS TRADERS, BUILT FROM GIT HISTORY BECAUSE THE GITHUB API IS SHUT. *** Three distinct refusals were probed and recorded verbatim, each with its own remedy: an unattached repo says "use add_repo"; our OWN repo says "an org admin must connect"; and a contributor profile says "This GitHub API path is not available: sessions are bound to their configured repositories" -- *** THE WHOLE /users PATH CLASS IS REFUSED, NOT RATE-LIMITED, *** so the armed-ship axis (do they own anything else) is exactly the one that cannot be reached. The gate RE-PROBES all three every run: if one opens it goes red, and that red means "go use what you said you could not". What IS available is better -- git clone --filter=blob:none fetches full history in seconds and every commit carries its author, the same substrate GitHub derives its own contributor graphs from. All 35 licenceSweep repos cloned, 11,172 commits, ZERO failures, no API call. *** TWELVE OF THE THIRTY-FIVE CONTAIN NO COMMIT BY THEIR OWNER *** -- forks, cargo parked in someone else's dock, six of redcamel's nine and five of but0n's thirteen. And history beats the API's boolean fork flag: ownerShare says how much of what they own they WROTE, and but0n/THREE.js-PathTracing-Renderer counts as owner-authored on a handful of 1,992 commits. *** THERE IS NO CORRECT IDENTITY KEY AND THE MODULE USES TWO: *** by display name 473 identities and 13 crossings, by address hash 466 and 10. Email-keying is right about but0n -- "but0n", "Jeff Ma", "Jeffrey Ma" and "Jeffrey" are ONE address, one trader with four names -- and still WRONG about Jamie Portsmouth, who holds two addresses and stays two traders at 11 repos and 5. NO EMAIL IS STORED; identity is a truncated SHA-256. *** AND THAT PROMISE WAS FALSE WHEN WRITTEN: *** it hashed the address FIELD and trusted the name field, and one contributor set their git NAME to their address, which shipped in a topAuthor field. Address-shaped names are hashed now and the gate scans the whole file, prose included. TWO ENTRIES IN THE CROSSING LIST WERE NOT PEOPLE: ImgBotApp, a bot whose name ends in "App" and so missed the first predicate; and *** e3b0c44298fc, WHICH IS sha256("") *** -- two repos carry commits with an EMPTY author address, and hashing that missing field made a stable plausible identity the graph reported travelling between them. A coincidence of ABSENCE read as a connection. The rule is EXPORTED now rather than buried in the generator, and exercising it immediately found the false positive its own comment warned about: "bot@" matches talbot@example.com. Three sabotages, none 0 RED. What this deliberately is NOT: a survey of GitHub -- it covers the 35 repos this project already opened and says who moves between THEM. Full changelog on docs/CHANGELOG.md.
