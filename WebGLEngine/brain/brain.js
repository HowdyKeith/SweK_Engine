...
const BRAIN_BUILD = "v4131";   // v4131 -- es-box3d-fly3d.html gets a Reset Battle button and a ships-per-side picker, neither of which needed new simulation: newBattle() was already a full teardown-and-rebuild, and the only missing pieces were a way to ask for one and a count that was not the literal 6 written twice inside it. The two controls differ on purpose -- changing the fleet size clears the battle tally, a plain reset keeps it -- because a score mixing 2v2 with 20v20 adds up two different games. spawnEnemies reads opts.count || 3, so a zero count quietly spawns three rather than failing; both paths clamp and the gate proves the trap. The gate is source AND live: it reads the picker's own options out of the page, spawns real fleets at each, then boots a real browser and checks the SIM CLOCK GOES BACKWARDS on reset, because "restarted at 2 v 2" and "the click did nothing" look identical in the HUD. Round A / Round B are renamed to what the pages are (planar fight with a 3D view vs the fight itself in 3D) in everything a user reads, with module stamps left alone. Gate: ev/esFleetSize-selfcheck.mjs, 62 checks. Full changelog on docs/CHANGELOG.md.
