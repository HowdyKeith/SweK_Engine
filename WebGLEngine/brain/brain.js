...
const BRAIN_BUILD = "v4451";   // v4451 -- *** THE AVATAR HAD A FLOOR AND NOTHING ABOVE IT, AND THE ROOM IT CLAIMED WAS MEASURED DEAD. *** Keith: "cap the avatar width." v4413 passed mountAvatarSwitch a minSize and no ceiling, so the scene tracked its host upward without limit. MEASURED on a live server.html at four window widths: the SVG surface came out 341, 621, 749 and 1005 px at 900 / 1400 / 1920 / 2560 -- AND THE ROBOT DRAWN INSIDE IT WAS 100 px WIDE AT ALL FOUR. The figure's viewBox is 60x88 and preserveAspectRatio letterboxes it, so its drawn width is min(boxW, boxH * 60/88), and boxH is pinned at 147 by the row: a box that tripled held the same 100 px robot and 905 px of nothing. maxSize is the ceiling, symmetric with the floor, and it is NOT OPTIONAL for a measured box -- maxSize = 0 means "not stated" and ui/avatarSwitch-embed-selfcheck.mjs refuses it, on exactly the argument that file already makes about minSize ("a caller that says sizeFromHost:true and no minSize inherits minSize = 96, a second declaration of a floor"). A DEFAULT CEILING WOULD HAVE BEEN A NUMBER NOBODY RE-DERIVES, which is what this whole family of defects keeps being. After: 336 px at every width tested, figure unchanged at 100, which is the proof the capped room was empty rather than an argument that it was. 420 is a judgement stated as one -- about 2.3x the row's rendered height, leaving space for a scene twice the robot's width, which is the gauges v4413 was asked to make room for. *** AND v4413'S OWN CHECK HAD TO CHANGE SHAPE, BECAUSE THE OLD FORM WOULD NOW PASS ON A DEAD OBSERVER. *** It asserted surface(1400) > surface(900) + 20 to prove the ResizeObserver was alive. With a ceiling in place both those rows sit ABOVE it, so the two read equal -- and equal for a good reason is indistinguishable from equal because the observer died. The property is split in two, TRACKS below the ceiling and PINNED above it, each driven at a width where it can actually fail. The narrow probe is 360 and not 500 BECAUSE 500 WAS MEASURED AND FOUND STILL ABOVE THE CEILING: the page reflows to one column below a breakpoint, so the row is 474 px wide at a 500 px window and 334 at a 360 one. THE WIDTH OF THE WINDOW IS NOT THE WIDTH OF THE ROW, and a probe picked by eye would have tested nothing. *** SECOND ITEM: ONE PRESS FOR STEPS 3 THROUGH 6, AND IT KEEPS v3964'S GUARANTEE RATHER THAN ITS RITUAL. *** Keith: "can we have a button that does 3, 4, 5, and also then 6?" v3964 split clone-and-verify from publish on purpose and its reason deserves restating before a press is removed: "a clone that fails says so and an upload that fails says so, but A SUBTLY BROKEN TREE BOOTS FINE AND PACKAGES FINE... so Publish is a SECOND press, and it is disabled until the verdict is green." WHAT THAT PROTECTED IS "NEVER PUBLISH A RED TREE" AND IT IS KEPT EXACTLY: the runner reads the verify verdict, stops on red, publishes nothing and names the step it stopped at. WHAT IT GIVES UP IS A HUMAN READING THE LOG BEFORE THE UPLOAD, and that is said out loud rather than traded away quietly -- a person clicking a button that is merely ENABLED on green has not necessarily read anything either, so the press was never the safeguard the predicate is. The server's canPublish() still refuses independently, so a bug in this button cannot publish a tree the backend thinks is red. It calls the same endpoints the individual buttons call rather than synthesising clicks on them, because a click on a disabled button does nothing at all and would have made a skipped step look like a completed one. Steps 1 and 2 stay with the person: push first, pick the branch. Section 4 of the panel gate drives BOTH outcomes with a stub that varies exactly one thing -- the verify verdict -- so any difference in what the runner does is attributable to that and nothing else, and it asserts the ORDER of the four calls rather than merely their presence, because recording the ledger before the release exists would write the old answer and read as success. Sabotages this round: publish-on-red goes 2 red with the endpoint list showing all four calls where only one belonged; step 5 moved above step 4 goes 1 red by name; the ceiling removed from the one caller goes red in BOTH avatar gates. WHAT IS NOT CLAIMED: that any of these buttons publishes correctly, since every /github/ and /source-chain/ call in the gate is stubbed and must never reach github.com; that a scene which is itself oversized was ever reproduced, since v4450 measured the avatar at 621x147 at home and unmoved by window height at five sizes and said so; and that v4450 or v4451 is published, because publishing needs the rig. *** AND THE AUTO-UPDATER COULD HAVE RESTARTED THE SERVER IN THE MIDDLE OF ANY OF IT. *** Keith: "while the github repo commands are initiated, we need to make sure that the running swek does not start an auto update that kills the github tasks." server.js has deferred auto-updates during live work since v3075, and its own note says the guard belongs where the update is applied rather than in each runner -- correct, and it named THREE runners and stopped: render QA, the gate suite, rig jobs. The clone (several hundred MB), the verify inside the clone, the pack and the upload were none of those. *** AND THE PUBLISH WAS INVISIBLE EVEN TO ITS OWN BRIDGE: *** sourceChainBridge set R.phase all through start() and NEVER ONCE in publish(), so during packRelease and the upload status().running read FALSE and canPublish() still returned ok -- the window nothing could see was exactly the window holding the action this tree calls hardest to take back, and a second concurrent publish fitted inside it. publish() has a phase now, set before the work and restored in a FINALLY because it has eight return paths and a restore at each is seven chances to miss one -- the one missed leaves the phase stuck and updates deferred forever, a guard that fails closed and never reopens. githubBridge counts its two long operations the way updatePause-selfcheck already blesses for gatesBridge, floored at zero and decremented in a finally. A queued update is not lost: v1933's rule stands, it stays queued and applies once the work ends. *** REPAIRING THAT GATE THEN EXPOSED A GATE THAT HAD BEEN DEAD AT IMPORT FOR TWENTY-ONE ROUNDS. *** v4430 deleted RECORDED_BUT_GREEN from redCensus.mjs -- rightly, it was a frozen copy of a set its one reader derives four lines later -- and left that reader importing it. tools/ship/redCensus-selfcheck.mjs, the gate that RE-RUNS THE RED REGISTER, threw before its first check from v4430 to here, while this very session repaired entries on the register it guards. *** IT WENT UNSEEN BECAUSE IT IS BUDGETED AT 140,941 ms AND THE QUICK SWEEP RUNS EVERYTHING UNDER 3,000: the number that describes a gate is what hides it, and fifty-one gates sit above that cap. *** Reviving it took one edit and immediately produced three reds, all the same family as everything else this round: RECHECK_V4313's now-green list derived through the regex /^v43(1[4-9]|[2-9])/, which excludes v4314-v4319 and v432x-v439x AND NOTHING ELSE, so it silently stopped excluding anything at v44xx and a three-gate record was naming six -- and the comment directly above it says "A RECORD OF ONE MOMENT MAY NOT DERIVE FROM A LIST THAT SPANS SEVERAL", having been repaired once already with a filter instead of a list; an equality between shaderRefs' typed 379,838 ms and a getter whose audit caps it at 120,081, written when both numbers were typed by hand and never revisited when one became a measurement the instrument cannot take, which v4430 had already documented as "NOT a stale reading -- an absent one"; and a claim about what was red AT v4279 measured against RED_AT_V4279, which is not that set but the LIVE register, shrunk every time somebody repairs a gate and prunes the entry -- so the historical claim went false BECAUSE THE WORK THE CENSUS ASKS FOR WAS DONE, eight repairs in. All three fixed by deriving or freezing whichever the claim actually is. The revived gate also caught winPathGuard RED AGAIN after its v4423 repair: tools/ship/demosReach-selfcheck.mjs, added afterwards, carried `new URL(import.meta.url).pathname`, which yields "/C:/Users/..." on Windows -- the box this engine is developed on. The guard worked and nothing was reading it. And updatePause-selfcheck's own standing red, on the register since v4279, was A STALE REGEX AND NOT BROKEN CODE: it required `c._errored = true; tally();` adjacent on one line and somebody later inserted a statement between them. It is off the register with a receipt. NEW tools/ship/importHealth-selfcheck.mjs closes the hole that hid all of this: every relative import in every gate resolved, every named binding checked against what the target actually exports, 9,410 bindings across 1,478 gates in about 200 ms -- STATICALLY, which is what makes it cheap enough to ask of all of them rather than only the 51 the sweep skips. Five sabotages, MEASURED 1/1/1/0-then-1/2 by name. *** THE ZERO WAS THIS GATE'S OWN UNREACHED BRANCH: *** nothing in this tree imports from a module that re-exports, so the `export * from` guard never ran, and its own report line said "0 imports whose target re-exports" while I read past it. Closed with temp-directory fixtures rather than planted files, because a gate that leaves a gate behind grows the population it measures. *** AND THE GATE WAS WRONG TWICE BEFORE IT WAS RIGHT, BOTH TIMES ACCUSING CORRECT CODE: *** 36 false reds from taking one identifier out of `export const RLE_SX = 32, RLE_SY = 256, RLE_SZ = 32, RLE_N = ...` and none at all out of `module.exports = {...}`, which Node resolves statically; then about 110 more from anchoring a declaration on a semicolon AT END OF LINE in a tree that writes `export const MU0 = 1.25663706212e-6;   // H/m, CODATA`. Running one of the accused -- voxelRLE-selfcheck, exit 0 -- was the entire cost of learning each, and a check that goes red on correct code teaches nothing. The tree stands at 1478 gates. Full changelog on docs/CHANGELOG.md.
