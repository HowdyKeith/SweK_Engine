...
const BRAIN_BUILD = "v4385";   // v4385 -- *** THE JOINT LIMIT HAS BEEN WRITE-ONLY SINCE v2515, AND THE TREE'S OWN GATE SAYS SO IN ITS FOOTER. *** swk_joint_revolute has taken loDeg and hiDeg for four hundred rounds and swk_joint_spherical has taken coneDeg just as long. Seven swk_joint_* entry points existed before this round -- create three kinds, destroy, count, and toggle collide-connected -- and NOT ONE OF THEM COULD READ AN ANGLE BACK. So physics/ragdollFromSkeleton.mjs derives a knee bound of [-145, 0] degrees from a bone name, hands it to box3d, and it becomes unobservable; tools/ship/ragdollStep-selfcheck.mjs states the consequence about itself: "the limits are checked as VALUES by v4245 and never as BEHAVIOUR". A value written and never read is not a setting, it is a hope. swk_joint_state, swk_joint_limits, swk_joint_kind and their two published strides are the reading half; swk_joint_motor is the drive a limit implies -- a joint that can only be stopped is a ragdoll, one that can also be pushed is a rig. *** WHAT MAKES A MOTOR GRADEABLE RATHER THAN "THE RAGDOLL LOOKS BETTER" IS THAT HOLDING A LIMB LEVEL IS A STATICS PROBLEM WITH A CLOSED FORM. *** torque = m*g*d*cos(theta), and box3d lands on it: over five independent (mass, gravity, lever) triples predicted 800.0000 / 784.8000 / 200.0000 / 1800.0000 / 1600.0000 against measured 800.0000 / 784.8000 / 200.0000 / 1800.0001 / 1600.0000, worst relative error 6.78e-08, which is float32 storage and nothing else. AND THE SAME FORM IS THE THRESHOLD, which is what maxTorque means: the arm needs 800 N m: a cap of 700 sags 49.76 degrees, 750 sags 34.75, 850 holds within 0.007. The number separating a limb that resists from one that hangs is computed before the simulation runs. *** THE CHECK WAS WRITTEN AS A CLEAN BINARY AND THE MEASUREMENT SPLIT IT IN THREE. *** A cap of EXACTLY m*g*d holds, but MARGINALLY -- 0.4531 degrees of sag, SIXTY-TWO TIMES the 0.0073 of a cap six percent larger, because a motor given exactly what the statics demand has nothing spare for the solver's own slack. The threshold is a knee, not a cliff, and only measuring found that. The cosine half is checked on the weak-cap family and not the level case, because at level cos = 1 and a dropped cosine is invisible: with a cap of 700 the arm settles at 49.7583 degrees and the motor then reports 516.80, which is 800*cos(49.7583) = 516.81 and NOT the cap it slipped past. *** AND THE OBSERVABLE HAD TO BE THE EXTREME, NOT THE FINAL VALUE, WHICH NEARLY SHIPPED A COINCIDENCE. *** The first probe read the angle after five seconds: a free hinge gave -30.331 and the same hinge limited to [-30, 0] gave -30.010 -- two numbers a fifth of a degree apart, reading as "the limit did nothing". Both wrong: the free hinge was still SWINGING. Run to rest it reaches -176.560 while the limited one stops at -30.017. A limit does not say where a joint ends up, it says where it never goes, so every check reads a running extreme. The knee this tree actually derives is held 31.575 degrees short of where it would otherwise swing. *** AND THE EXCURSION PAST A SOFT STOP IS SIGNED, WHICH THE SECOND CLAIM GOT WRONG. *** Written as "a fixed overshoot whatever the bound", it went red: [-1,0], [-10,0] and [-30,0] overshoot by +0.0121, +0.0155, +0.0166, and [-145,0] UNDERSHOOTS by -0.0153. Not noise and not a second mechanism -- at -145 the arm is past vertical and arrives almost weightless, so it never leans on the stop hard enough to push through. A one-sided check would have passed the knee for the wrong reason. Controls: a cone of 0 reaches 176.560, the same extreme the free HINGE found from a different joint type; and a motor ENABLED WITH A ZERO CAP is the free joint to the digit (-176.560 / -47.602 both ways), which is what isolates the torque as the cause rather than the enable flag or the extra constraint row. Five sabotages, 3/5/1/1/3 red by name, both files md5-identical, the three shim ones REBUILT NATIVELY so what went red is compiled physics. Sabotage E also caught a DETAIL STRING asserting "float32 and nothing else" while printing 7.00e+0 beside its own FAIL, and that is fixed. *** AND box3dFilter-selfcheck WAS RED AT HEAD AND IS GREEN NOW. *** It counts the shim's swk_* against build-box3d-wasm.sh's hand-typed export list: 2 missing at HEAD, because v4382's raycast pair shipped without ever reaching it. All eight are listed now. A hand-maintained list is a ratchet only if something counts it. UNCHECKED: a ragdoll -- every joint here is one arm on one anchor. Also the TWIST limit, which the spherical joint has and swk_joint_spherical still does not pass. SENSORS and CCD are deliberately left: real box3d capabilities with NO consumer in this tree, and #133's rule is find the consumer first. The limit had one waiting since v4245. The tree stands at 1424 gates. Full changelog on docs/CHANGELOG.md.
