...
const BRAIN_BUILD = "v4258";   // v4258 -- *** #53 WAS BUILT BEFORE IT WAS RECORDED, AND THE CENSUS THAT FOUND THAT SAYS 54 SOURCES ARE IN THE SAME POSITION. *** Backlog #53 asked for jsfx as sound-effects-as-data and animatelo as DOM-animation-the-dirty-flag-can-see. BOTH WERE ALREADY BUILT: audio/sfxModel.mjs at v4190 (a parameter block in, samples out, so a gate can hash a sound -- with strictSin substituted for Math.sin because the latter is not bit-specified across engines, measured at 2.3x cost for a worst per-sample difference of 1.11e-16) and ui/domAnimation.mjs at v4191, whose header already read 'DOM ANIMATIONS AS DATA, AND AS SOMETHING THE DIRTY FLAG CAN SEE' -- the item's own wording, in the file, while the item sat open. Each cited its source by name and NEITHER was in world/reachedLicences.mjs. Censusing every owner/repo (LICENCE) citation in a module header found 54 distinct sources across 65 files and ZERO of them registered, while all 13 register entries appear in NO header: two records of the same fact grown from opposite ends, never overlapping, because the register is filled by ASSESSMENT rounds and the headers by BUILD rounds. jsfx and animatelo are now entered, leaving 52 as a ratchet the ledger owns. Four of the cited are (A)GPL and are named. *** AND A SABOTAGE WALKED THROUGH THE RATCHET: *** it asserted debt <= baseline, so raising the baseline to 999 stayed green; it now asserts EQUALITY, because a ratchet whose stop can be moved is not a ratchet. Full changelog on docs/CHANGELOG.md.
