...
const BRAIN_BUILD = "v4266";   // v4266 -- *** THE ORRERY DREW THREE PLANETS MADE ENTIRELY OF PAPERWORK, AND COULD NOT SEE TWO MADE OF CODE. *** Backlog #46 asked for "vendoring as impact ejecta" and nothing modelled it: world/orrery.mjs places a body by licence state, size and arrival date -- what was taken and whether it is papered -- and could not say HOW FAR THE MATERIAL SPREAD. world/orreryEjecta.mjs measures that as the engine files importing each body: three 70, box3d 31, krbn 9, htmx 5, taichi-js 5, jolt 4, gifenc 3, draco 2, fonts 2, heerich 1, wasm 1, and grass / keyhunt / slug at ZERO. *** AND THE THREE ZEROES ARE NOT UNUSED DEPENDENCIES: THOSE DIRECTORIES CONTAIN NO CODE. *** vendor/grass is one LICENSE, vendor/keyhunt one ATTRIBUTION.txt, vendor/slug a LICENSE and a PROVENANCE.txt -- licence RECORDS filed under vendor/ because that is where the orrery looks, drawn as captured code with their bytes counted as mass. 21% of the orrery is paperwork. massOf discounts it: grass goes from 1064 bytes to 0. That is the exact mirror of v4263, which found two bodies of real copied code the orrery cannot see because its scanner reads ONE hardcoded directory -- and there are TWO named vendor, ui/vendor holding a 2,237-line QR generator. There is no vendored CODE in this tree that nothing imports. REFUSED: the citation count. "wasm" is named in 216 files and imported from vendor/wasm by 1; "grass" scores 73 on grassField and grassModel. Re-inserted as sabotage B it takes three from 70 to 1397. *** AND THE GATE COUNTED ITSELF: *** its own control fixture names vendor/box3d/box3d.js, so box3d read 32 against a recorded 31 -- the fourth self-counting scan in three rounds, now excluded by path. Two sabotage results were worthless first: a "0 RED" that was a CRASH whose exit code I never read, which also let me claim a guard was proven inert by a sabotage that proved nothing. The guard IS inert, measured directly; my evidence was not. Full changelog on docs/CHANGELOG.md.
