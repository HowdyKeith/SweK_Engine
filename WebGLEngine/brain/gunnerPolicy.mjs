...
export const FEATURES = 9, HIDDEN = 8, OUTPUTS = 3;
export const FEATURE_NAMES = Object.freeze(["bias", "bearing", "pitch", "range", "closing", "lateral", "reloading", "reachable", "aligned"]);
export const WEIGHT_COUNT = FEATURES * HIDDEN + HIDDEN + HIDDEN * OUTPUTS + OUTPUTS;   // 72 + 8 + 24 + 3 = 107
export const REWARD = Object.freeze({ hit: 1, waste: 0.02 });                          // a shot costs 2 % of a hit, so spraying is a loss
...
/** The forward pass: features -> [yaw rate, pitch rate, fire] in [-1, 1]; fire is honoured when its output is positive. */
export function forward(w, x) {
    const [l1, l2] = layersOf(w), h = mlpLayerCpu(l1, Float32Array.from(x), 1), y = mlpLayerCpu(l2, h, 1);
    return [Math.tanh(y[0]), Math.tanh(y[1]), Math.tanh(y[2])];
}

const unit3 = (v) => { const L = Math.hypot(v[0], v[1], v[2]) || 1; return [v[0] / L, v[1] / L, v[2] / L]; };

/** The nine features of a gunner on `pose` with `turret`, against a target { pose } (null: nothing to shoot at). Each of order one. */
export function features(pose, turret, target) {
    if (!target) return [1, 0, 0, 0, 0, 0, turret.reload > 0 ? 1 : 0, 0, 0];
    ...
    return [1, e.bearing / Math.PI, e.pitch / (Math.PI / 2), Math.min(2, range / 40), -along / 20, lat / 20, turret.reload > 0 ? 1 : 0, e.reachable ? 1 : 0, e.aligned ? 1 : 0];
}

/** A gunner (pose, turret, target -> the turret contract) from weights. */
export function gunnerFor(w) {
    return (pose, turret, target) => { const [yaw, pitch, fire] = forward(w, features(pose, turret, target)); return { yaw, pitch, fire: fire > 0 ? 1 : 0 }; };
}

/** The hand gunner: turn at `turn` x the bearing error, lift at `lift` x the pitch error, fire when aligned. */
export function handWeights({ turn = 12, lift = 8, fireOn = 4 } = {}) {
    const w = zeroWeights(), W1 = (h, k, v) => { w[h * FEATURES + k] = v; }, W2 = (o, h, v) => { w[FEATURES * HIDDEN + HIDDEN + o * HIDDEN + h] = v; };
    const F = { bias: 0, bearing: 1, pitch: 2, aligned: 8 };
    W1(0, F.bearing, 1); W1(1, F.bearing, -1); W1(2, F.pitch, 1); W1(3, F.pitch, -1); W1(4, F.aligned, 1); W1(5, F.bias, 1);
 ...
 * fired and flown, hits turned into impulses on the target chassis. Returns the commands (for the log) and the hit events.
 */
export function turretTick(world, cars, turrets, shells, poses, cmds, t, spec) {
    ...
    return { cmds: fired.map((f) => ({ yaw: f.yaw, pitch: f.pitch, fire: f.fire })), events };
}

/**
 * THE DUEL: two hand-driven cars on the track of `seed`, the target ahead by `gap`, the candidate gunner in the rear car's turret
 * for `seconds`. Returns { hits, shots, score, fingerprint, ticks, seed }. A world is made and destroyed here.
 */
export function duel(worldFrom, w, { seed = 1, seconds = 20, gap = 10, shellSpeed = U.TURRET.shellSpeed, driver = null, targetDriver = null } = {}) {
    ...
    const drivers = [D.policyDriver(targetDriver || D.handWeights(), surface, cars[0]), D.policyDriver(driver || D.handWeights(), surface, cars[1])];
    const spec = Object.freeze({ ...U.TURRET, shellSpeed }), turrets = [U.createTurret(spec), U.createTurret(spec)], shells = [], gunner = gunnerFor(w);
    const ticks = Math.round(seconds / C.CAR.dt); let h = 0x811c9dc5;
    for (let t = 0; t < ticks; t++) {
        ...
        const cmds = [{ yaw: 0, pitch: 0, fire: 0 }, U.clampGun(gunner(poses[1], turrets[1], { pose: poses[0] }))];
        turretTick(world, cars, turrets, shells, poses, cmds, t, spec);
        C.stepCars(world, cars, surface, inputs, C.CAR.dt);
        h = C.foldHash(h, world.stateHash()); h = U.turretHash(h, turrets, shells, C.foldHash);
    }
    world.destroy();
    const hits = turrets[1].hits, shots = turrets[1].shots;
    return { hits, shots, score: REWARD.hit * hits - REWARD.waste * shots, fingerprint: (h >>> 0).toString(16).padStart(8, "0"), ticks, seed, shellSpeed };
}
...
export function raceWithGunners(worldFrom, drivers, gunners, { seed = 1, seconds = 60, fleet = "00000000", gap = 4, shellSpeed = U.TURRET.shellSpeed, inputsLog = null, onTick = null } = {}) {
    ...
    for (let t = 0; t < ticks; t++) {
        ...
        const cmds = rec ? rec.map((r) => U.clampGun(r)) : poses.map((p, i) => { const j = nearestOther(i, poses); return U.clampGun(gun[i](p, turrets[i], j === null ? null : { pose: poses[j] })); });
        const tt = turretTick(world, cars, turrets, shells, poses, cmds, t, spec);
        log.push(inputs.map((u, i) => ({ throttle: u.throttle, steer: u.steer, brake: u.brake, yaw: tt.cmds[i].yaw, pitch: tt.cmds[i].pitch, fire: tt.cmds[i].fire })));
        ...
        h = C.foldHash(h, world.stateHash()); h = U.turretHash(h, turrets, shells, C.foldHash);
        if (onTick) onTick(t, rs.map((r) => r.pose), turrets, shells, tt.events);
    }
    const poses = cars.map((c) => C.carPose(world, c)); world.destroy();
    const results = drivers.map((w, i) => ({ car: i, laps: trackers[i].laps, metres: metres[i], lapTime: lapTimes[i], hash: D.weightsHash(w), gunnerHash: D.weightsHash(gunners[i]), hits: turrets[i].hits, shots: turrets[i].shots, pose: poses[i] }));
...
/** THE ADJUDICATOR: the hand gunner in a duel at this shell speed on the held-out track must land HIT_BOUND hits in KEY_SECONDS. */
export function adjudicateShell(v, opts = {}) {
    ...
    const r = duel(worldFromMod, handWeights(), { seed: opts.seed || KEY_SEED, seconds: opts.keySeconds || KEY_SECONDS, shellSpeed: v });
    const pass = r.hits >= HIT_BOUND;
    return { pass, evidence: { shellSpeed: v, hits: r.hits, shots: r.shots, seed: r.seed, seconds: r.ticks * C.CAR.dt, bound: HIT_BOUND, fingerprint: r.fingerprint,
                               law: "the hand gunner, whose aim is the exact vacuum solution, must land the bound in a duel on a track the score never sees; a shell the target outruns has no solution and no hits",
                               reason: pass ? `${r.hits} hits of ${r.shots} shots in ${KEY_SECONDS} s` : `${r.hits} hit${r.hits === 1 ? "" : "s"} of ${r.shots} shots in ${KEY_SECONDS} s, under the bound of ${HIT_BOUND}` } };
}

/** The front door. */
export function reportLines() {
    return [
        "[gunnerPolicy] the gunner: a 9 -> 8 -> 3 relu MLP on the turret's aim errors, the hand gunner as weights, the duel, the ES, the race with turrets, the shell-speed knob",
        `  ${WEIGHT_COUNT} weights (${FEATURES} x ${HIDDEN} + ${HIDDEN} + ${HIDDEN} x ${OUTPUTS} + ${OUTPUTS}); features ${FEATURE_NAMES.join(", ")}`,
        `  reward: ${REWARD.hit} per hit, -${REWARD.waste} per shot; knob ${KNOB} over [${SHELL_CANDIDATES.join(", ")}] m/s, score 1 / speed, key ${HIT_BOUND} hits in ${KEY_SECONDS} s on seed ${KEY_SEED}`,
