...
console.log("\n6. *** THE CATALOG ADMITS SOMETHING IT CANNOT INSTALL ***");
{
    ...
    ok("!! install() refuses a source-build entry before reaching spawn()",
        /if \(!item\.install\) return \{ ok: false, installable: false/.test(src) &&
        src.indexOf("if (!item.install)") < src.indexOf("spawn(cmd, args"),
        "the refusal is ahead of the spawn in the function body, not a check after the fact");
