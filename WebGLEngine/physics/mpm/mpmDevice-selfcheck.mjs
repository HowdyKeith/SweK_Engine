...
console.log("\n1. THE PLASTIC BRANCH IS THE IDENTITY ON THIS FIXTURE (no device needed)");
{
    ...
    // v4526 MERGE -- the page no longer builds pipelines of its own: this branch's v4466 routed it through
    // physics/mpm/mpmDevice.mjs's makeMpmDevice on gfx/device.js, whose v4461 rule answers the same defect the
    // declared layout answered here (a binding the entry point does not use is left out of the bind group, by
    // reading the entry point's reachable code) and is graded by tools/ship/mpmDevice-selfcheck.mjs on the device.
    // Either route is the fix; the row holds that the page takes one of them and not the raw auto layout.
    const viaDevice = /makeMpmDevice\(/.test(src) && !/createComputePipeline\(/.test(src);
    ok("  and the check page creates its four pipelines with a DECLARED layout rather than an automatic one (or, since v4466, runs them through makeMpmDevice, whose device reads the entry point's bindings)",
        viaDevice || (/createBindGroupLayout/.test(src) && /createPipelineLayout/.test(src) && pipeCall && pipeCall[1] === "pipeLayout"),
...
if (skip) { console.log(`  SKIP  ${skip}`); fails++; }
    ...
    if (r.ok && r.result && !r.result.error) {
        ...
        // v4526 MERGE -- INVERTED, AND THE REASON IS THIS BRANCH'S v4466: "the interpreter's f64 had been hiding the MPM
        // return map". The four-orders gap this row asserted at v4406 was the CPU twin's, not the kernel's: the graded loop
        // now runs the return map the way the kernel does, and on the merged tree the plastic path reads 2.9e-8 .. 2.3e-7
        // against the same twin -- INSIDE the page's threshold at every step count. The row now holds the fact that
        // replaced the finding, and the ratio detail keeps the old reading's shape so a regression shows as a number.
        ok(`*** and the PLASTIC path reads ${rows.map((x) => x.on.toExponential(2)).join(", ")} -- INSIDE that threshold at EVERY step count since v4466 closed the twin's gap (v4406 read it four orders OVER) ***`,
        ...
        ok(`  the collapsing column, where the clamp DOES fire, reads ${col.toExponential(2)} -- the same order as free fall's plastic figure rather than worse (v4526 merge: inside the threshold too)`,
            col < PAGE_TOL,
