...
// measured), so there is no spacing to read and the candidate is refused as unmeasurable -- AND THE PAGE'S SLIDER STARTED
// AT 8, inside that region, until v4587 moved it to 10: its default range opened on a hologram whose separation could not
// be read back. The tolerance is the screen's pitch: a peak is located to within half a sample either side, so the spacing of two
...
export function adjudicate(sep) {
    ...
    if (m.peaks < MIN_PEAKS) return { pass: false, evidence: { ...m, law, reason: `${m.peaks} bright fringe${m.peaks === 1 ? "" : "s"} on a screen of +-${HW}: fewer than ${MIN_PEAKS}, so no spacing can be read (the page's slider started at 8, inside this region, until v4587)` } };
...
export const MEASURED_V4586 = {
    belowTenNothingCanBeRead: "peaks on the screen: 1 at sep = 4, 6 and 8; 3 at 10, 12, 16; 5 at 20; 15 at 60 -- the slider's minimum of 8 sat inside the unreadable region until v4587 moved it to 10",
