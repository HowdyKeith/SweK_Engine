 ...
 * depends on it.
 */
/** Area of one triangle in 3D, from a flat position array. */
function area3(P, T) {
    const [i, j, k] = T;
...
/** Area of one chart in UV and in 3D. */
export function chartArea(P, chart) {
    let uv = 0, xyz = 0;
    for (const T of chart.tris) {
        const a = chart.uv.get(T[0]), b = chart.uv.get(T[1]), c = chart.uv.get(T[2]);
        if (a && b && c) uv += Math.abs((b[0] - a[0]) * (c[1] - a[1]) - (b[1] - a[1]) * (c[0] - a[0])) / 2;
        xyz += area3(P, T);
    }
    return { uv, xyz, density: xyz > 1e-12 ? uv / xyz : 0 };
}

/**
 * *** EVERY CHART ARRIVES AT WHATEVER SCALE ITS OWN SOLVE LEFT IT AT, AND NOTHING HAD EVER ASKED. ***
 *
 * LSCM determines a chart's map up to a SIMILARITY -- rotation, translation AND SCALE -- so the scale that
 * comes out is a property of which two vertices the solver pinned, not of the surface. Packing charts at
 * those scales hands each one however much texture its pin distance happened to imply. Measured on
 * RobotExpressive's 448 charts, UV area per unit surface spans 2.55e+1 to 8.84e+2: A FACTOR OF 34.7 between
 * the best-served chart and the worst, with p90/p10 at 1.63. The small fixtures hide it -- their charts are
 * near-copies of each other and span only 1.2x to 1.4x -- which is why six rounds of gates on this file did
 * not see it.
 *
 * This scales every chart to ONE texel density, chosen so the total UV area is unchanged. A uniform scale of
 * a conformal map is still conformal and every per-triangle distortion number is invariant under it, so this
 * costs nothing that distortion measures; what it changes is WHO GETS THE TEXTURE.
 *
 *   RobotExpressive   densityP10 3.94e+1 -> 7.20e+1 (+83%), p10/median 0.631 -> 0.980,
 *                     stretchP90 1.178 -> 1.018, and coverage RISES 37.2% -> 41.1%
 *   sphere 24x16      densityP10 +12%, coverage +10%, stretchP90 1.117 -> 1.077
 *   torus 16x10       densityP10 unchanged, uniformity 0.718 -> 0.905, coverage 44.6% -> 37.2%
 *   sphere 12x8       densityP10 -9%, coverage -14%, uniformity 0.908 -> 0.949
 *
 * SO IT IS NOT FREE ON EVERY MESH AND THAT IS STATED RATHER THAN AVERAGED AWAY: on the two coarsest fixtures
 * it trades coverage for uniformity, because their chart scales barely differed to begin with and all the
 * change does is perturb the boxes the packer was already doing well with. On the real asset, where the
 * spread is 34.7x, every number improves at once.
 */
export function equaliseChartScale(P, charts) {
    let sumUv = 0, sumXyz = 0;
    const per = charts.map((c) => { const a = chartArea(P, c); sumUv += a.uv; sumXyz += a.xyz; return a; });
    const target = sumXyz > 1e-12 ? sumUv / sumXyz : 1;    // preserves the total UV area
    return charts.map((c, i) => {
        const k = Math.sqrt(target / (per[i].density || target));
        if (!isFinite(k) || k <= 0) return c;
        const uv = new Map();
        for (const [v, p] of c.uv) uv.set(v, [p[0] * k, p[1] * k]);
        return { ...c, uv };
    });
}

/** The axis-aligned box a chart's own UVs occupy. */
export function chartBox(chart) {
    let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
    for (const T of chart.tris) for (const v of T) {
        const p = chart.uv.get(v); if (!p) continue;
        if (p[0] < x0) x0 = p[0]; if (p[0] > x1) x1 = p[0];
        if (p[1] < y0) y0 = p[1]; if (p[1] > y1) y1 = p[1];
    }
    return { lo: [x0, y0], w: x1 - x0, h: y1 - y0 };
}

/** Cut a chart in two across its longer axis, by triangle centroid, keeping every UV exactly as it is. */
export function halveChart(chart) {
    const b = chartBox(chart);
    const axis = b.w >= b.h ? 0 : 1, lo = b.lo[axis], span = (axis ? b.h : b.w) || 1e-12;
    const bins = [[], []];
    for (const T of chart.tris) {
        let m = 0, n = 0;
        for (const v of T) { const p = chart.uv.get(v); if (p) { m += p[axis]; n++; } }
        bins[(n ? m / n : lo) - lo < span / 2 ? 0 : 1].push(T);
    }
    // each part carries ONLY its own vertices: a chart's box is read from its map, so a shared map would
    // give both halves the whole chart's box and the packer would be placing shapes that do not exist
    return bins.filter((t) => t.length).map((tris) => {
        const uv = new Map();
        for (const T of tris) for (const v of T) if (!uv.has(v)) uv.set(v, chart.uv.get(v));
        return { tris, uv };
    });
}

/**
 * *** A CHART IS CUT TO PACK IT ONLY WHILE ONE CHART IS SETTING THE ATLAS SIZE, AND ONLY IF THE MEASUREMENT
 * AGREES AFTERWARDS. ***
 *
 * The premise this was filed under -- "split a chart and the atlas gets better" -- is WRONG as a general
 * rule, and measuring it is what said so. Slicing every chart whose box is more than 1.5x from square, at
 * k = 2, 3 and 4, LOSES coverage on four of five meshes: the torus goes 44.6% -> 40.9%, RobotExpressive
 * 37.4% -> 36.0%, and only the cylinder gains. So the trigger is DERIVED rather than chosen: the widest
 * chart's span against the square root of the total box area is exactly the question "can any rearrangement
 * of these shapes reach a square, or is one chart already wider than the square would be?".
 *
 *   cylinder 16x6   widest span 0.9948, sqrt(total) 0.6493, ratio 1.53   <- one chart sets the atlas
 *   torus 16x10     0.9948 against 0.9543, ratio 1.04                    <- marginal
 *   sphere 12x8     0.3862 against 0.9378, ratio 0.41                    <- area sets the atlas
 *   sphere 24x16    0.3849 against 0.9224, ratio 0.42
 *   robot           0.5629 against 0.9105, ratio 0.62
 *
 * AND THE TRIGGER IS STILL ONLY A PROPOSAL: the cut is kept only if a real pack of the cut set covers more
 * of the square than a real pack of the uncut one, packing THE SHAPES THE ATLAS WILL PACK rather than
 * rectangles standing in for them. The cylinder gains 37% (coverage 42.2% -> 57.7%, densityP10 4.49e-2 ->
 * 6.14e-2) and is kept. The torus is the case that shows the accept test doing work rather than decorating
 * one: on its own the cut is REFUSED (trigger 1.04, 15 charts, nothing changes), and after
 * equaliseChartScale has moved every box it is ACCEPTED and pays -- 15 charts -> 16, densityP10
 * 2.517e-2 -> 2.661e-2. The same trigger, the same chart, opposite answers, because the question is about
 * the pack and not about the chart.
 */
export function nestShapes(charts) {
    return charts.map((c) => {
        const b = c.lo ? { lo: c.lo, w: c.w, h: c.h } : chartBox(c);
        const tri = c.tris.map((T) => [c.uv.get(T[0]), c.uv.get(T[1]), c.uv.get(T[2])])
                          .filter((t) => t[0] && t[1] && t[2]);
        const W2 = b.w || 1e-12, H2 = b.h || 1e-12;
        return { w: W2, h: H2, cells: (mw, mh, cell) => conservativeMask(tri, b.lo, W2, H2, mw, mh, cell) };
    });
}

export function cutWideCharts(charts, { cells = 384, padCells = 1, maxCuts = 8, pack = null } = {}) {
    // *** THE ACCEPT TEST PACKS THE SHAPES THE ATLAS WILL PACK, NOT A RECTANGLE STANDING IN FOR THEM. ***
    // The first version scored a candidate with an all-cells-filled mask, which is a different packer -- it
    // cannot nest, so it mis-ranked the torus, accepted a cut that the real pack then paid 12% of the atlas
    // for, and the comment above it claimed "a real pack" while the code did no such thing.
    const packer = pack || ((cs) => {
        const p = rasterPack(nestShapes(cs), { cells, padCells });
        const span = Math.max(p.width, p.height) || 1;
        return cs.reduce((s, c) => { const b = chartBox(c); return s + b.w * b.h; }, 0) / (span * span);
    });
    let cur = charts, score = null, cuts = 0;
    for (let i = 0; i < maxCuts; i++) {
        const boxes = cur.map(chartBox);
        const root = Math.sqrt(boxes.reduce((s, b) => s + b.w * b.h, 0));
        let wi = -1, widest = 0;
        boxes.forEach((b, j) => { const sp = Math.max(b.w, b.h); if (sp > widest) { widest = sp; wi = j; } });
        if (wi < 0 || widest <= root) break;                       // area sets the atlas: nothing to gain
        const parts = halveChart(cur[wi]);
        if (parts.length < 2) break;
        const next = cur.slice(0, wi).concat(parts, cur.slice(wi + 1));
        if (score === null) score = packer(cur);
        const after = packer(next);
        if (!(after > score)) break;                               // measured, not predicted
        cur = next; score = after; cuts++;
    }
    return { charts: cur, cuts };
}
...
export function unwrapCurved(positions, indices,
        { maxNormalDeg = 40, paddingTexels = 2, textureSize = 1024, relTol = 1e-6, from = null,
          merge = true, maxConformal = 1.05, orient = true, nest = true, nestCells = 384,
          equalScale = true, cutWide = true } = {}) {   // v4560: maxConformal was 2.0 -- see mergeCharts
    const bbox = (uv) => {
        let lo = [Infinity, Infinity], hi = [-Infinity, -Infinity];
    ...
    // *** THE TWO PASSES BETWEEN THE SOLVE AND THE PACK, BOTH ADDED AT v4561 AND BOTH MEASURED ABOVE. ***
    // equaliseChartScale is the one that matters on a real asset: LSCM fixes a chart up to a SIMILARITY, so
    // the scale is the pin distance's and not the surface's, and RobotExpressive's charts span 34.7x. It is
    // applied first because it changes every box the cut trigger then reads.
    let cuts = 0, scaleSpread = null;
    if (laid.length > 1) {
        // the spread the solve arrived with, measured BEFORE anything corrects it -- this is the finding
        // equaliseChartScale exists for, and a result that only reported the corrected state would leave
        // every gate asserting that a repair it cannot see is unnecessary
        const d = laid.map((c) => chartArea(w.positions, c).density).filter((x) => x > 0).sort((a, b) => a - b);
        if (d.length) scaleSpread = { min: d[0], max: d[d.length - 1], ratio: d[d.length - 1] / d[0],
                                      p10: d[Math.floor(d.length * 0.1)], p90: d[Math.floor(d.length * 0.9)] };
    }
    // *** REBUILT INTO A FRESH ARRAY, NEVER CLEARED IN PLACE. *** The first version did `laid.length = 0`
    // and then read the pass's result -- which is the SAME array the moment a pass returns its input
    // unchanged, so the charts were destroyed before they could be copied back and the unwrap silently
    // produced nothing. Found by a sabotage that made equaliseChartScale a no-op: the atlas came back empty
    // and a row asserting "every chart is at one density" PASSED ON THE EMPTY SET.
    const replace = (next) => { const out = next.map((c) => ({ tris: c.tris, uv: c.uv, ...bbox(c.uv) }));
        laid.length = 0; for (const c of out) laid.push(c); };
    if (equalScale && laid.length > 1) replace(equaliseChartScale(w.positions, laid));
    if (cutWide && laid.length) {
        const r2 = cutWideCharts(laid, { cells: nestCells, padCells: 1 });
        cuts = r2.cuts;
        if (cuts) replace(r2.charts);
    }
    if (nest) {
        // occupancy of a chart at a normalised (u,v) inside its own box: is any triangle over that point?
        const shapes = nestShapes(laid);
        ...
        if (verified === 0) {
            ...
            return { weld: w, charts: laid.map((c, i) => ({ tris: c.tris, uv: uvs2[i] })),
                     ...
                     worstResidual, grown, mergeStats, splitStats, packer: "nest", padCells: padUsed, cuts, scaleSpread };
        }
