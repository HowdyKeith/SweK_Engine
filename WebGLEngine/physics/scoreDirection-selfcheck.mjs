...
// the thing that must never happen is it being indistinguishable from "the loop only looked at one candidate".
{
    resetRegistry();
    registerAll();
    // *** THE PROPOSERS THAT DECLARE A ready() ARE READIED FIRST, AND UNTIL v4664 THEY WERE NOT. ***
    ...
    ok("!! *** every proposer that declares a ready() was READIED before it was adjudicated ***",
       ...
       notReady.length ? "NOT READY and therefore never searched: " + notReady.join(", ") +
                       ...
                       : rows.filter((x) => x.needsReady).length + " declare one (" +
                         ...
                         "Until v4664 this route awaited none of them and could not see that it should");
