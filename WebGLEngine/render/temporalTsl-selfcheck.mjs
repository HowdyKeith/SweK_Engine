...
import { rectifiedAccumulateCPU, disocclusionCPU, historyFactorCPU } from "./temporalReject.mjs";
...
console.log("\n1. WITHOUT A DEVICE: the refusals and the depth mapping");
{
    ...
    // v4729: clipGapThreshold against an independent route to the same clip z -- motionVectors.mjs's transform4
    {
        ...
        const got = TT.clipGapThreshold(Array.from(vp), [0, 0, 0.4], [0, 0, -1.5]);
        ok(`  v4729: clipGapThreshold is a quarter of the clip-z gap, ${got.toExponential(4)} -- and null where the two points are closer than the clip test can separate`,
           Math.abs(got - want) < 1e-12 && TT.clipGapThreshold(Array.from(vp), [0, 0, 0.4], [0, 0, 0.4 - 1e-6]) === null, `independently ${want.toExponential(4)}`);
    }
...
console.log("\n4. v4729 -- THE DEPTH CLIP: dilation, disocclusion and the history factor, on a box sliding past a wall");
if (!skip) {
    const N4 = 6, D4 = 64;
    const r = await runInEngineOrigin({ engineRoot: ENG, timeoutMs: 240000, args: { N4, D4 }, script: `async (a) => {
        ...
        for (const mode of ["webgpu", "webgl2"]) {
            try {
                ...
                const threshold = TT.clipGapThreshold(vp, [0, 0, 0.4], [0, 0, -1.5]);
                o.threshold = threshold; o.vp = vp;
                const dMot = tgt(a.D4), rec = [tgt(a.D4), tgt(a.D4)], mask = tgt(a.D4), fac = tgt(a.D4);
                const dil = TT.dilateNodes(T, stage.depth.texture, stage.motion.texture, { w: a.D4, h: a.D4 });
                ...
                const dis = [0, 1].map((k) => TT.disocclusionNode(T, dMot.texture, rec[1 - k].texture, { w: a.D4, h: a.D4, threshold }));
                const scX = dis.map((d) => quad(d.node));
                const scF = quad(TT.historyFactorNode(T, { disocclusion: mask.texture }).node);
                ...
                await draw(quad(TT.disocclusionNode(T, mSyn.texture, rec[1 - last].texture, { w: a.D4, h: a.D4, threshold }).node), mask2);
