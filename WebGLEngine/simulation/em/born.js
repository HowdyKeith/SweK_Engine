...
// *** v3716 -- THE HALF IS NAMED BECAUSE DROPPING IT IS THE CLASSIC SLIP. The Born phase is k*delta*d/2 and the
// half comes from sqrt(1+delta) - 1 ~ delta/2, the first-order expansion of the refractive index. Writing
// k*delta*d instead is dimensionally correct, is what you get if you set n = 1 + delta rather than
// n = sqrt(1 + delta), and DOUBLES the approximated phase. `contrastHalf` exists so the plant is a MODE OF THIS
// FUNCTION rather than a second copy of it that could drift; see bornBind. ***
const BORN_CONTRAST_HALF = 0.5;

function bornSlab(delta, k, d, contrastHalf = BORN_CONTRAST_HALF) {
    return { re: 1, im: k * delta * d * contrastHalf };
}

// How wrong Born is here, as a fraction of the field's own size.
function bornError(delta, k, d, contrastHalf = BORN_CONTRAST_HALF) {
    const e = exactSlab(delta, k, d), b = bornSlab(delta, k, d, contrastHalf);
...
export { phaseShift, exactSlab, bornSlab, bornError, maxPhaseFor, BORN_CONTRAST_HALF };
