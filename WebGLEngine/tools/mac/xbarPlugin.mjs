...
export function buildPlugin({
    ...
    // v4649 -- POSIX-joined, not path.join. THE ARTIFACT IS A macOS PLUGIN whatever box generates it, and
    // path.join on Windows produced `file:///C:\\...\\ui\\runnerGauge.mjs` -- backslashes inside a file
    // URL, which is not a path any host resolves. The gate that checks this shape read the same box's
    // separator and so agreed with the bug on both platforms at once.
    const gauge = String(engineRoot).replace(/\\/g, "/").replace(/\/+$/, "") + "/ui/runnerGauge.mjs";
    return `#!${nodePath}
