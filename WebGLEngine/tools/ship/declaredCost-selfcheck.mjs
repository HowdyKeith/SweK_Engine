// WebGLEngine/tools/ship/declaredCost-selfcheck.mjs -- v4653
//
// Run: node tools/ship/declaredCost-selfcheck.mjs   (~0.6s MEASURED v4653)
//
...
console.log("\n3. *** costOf: WHICH NUMBER A CALLER GETS, AND WHERE IT CAME FROM ***");
{
    ...
    ok("  ...and a caller can always tell a reading from a claim, which is the whole point of the field",
        ...
        "v4652 found three gates reporting the shape of a checkout as fact for want of exactly this " +
...
console.log("\n5. *** THE LIVE CENSUS, RATCHETED ***");
{
    ...
    const ROTTED_AT_V4653 = 138;
    ok("*** no NEW header has rotted: the count ratchets down, never up ***",
        c.rotted.length <= ROTTED_AT_V4653,
        `${c.rotted.length} against a frozen ${ROTTED_AT_V4653}. Each one is a gate whose header claims a ` +
