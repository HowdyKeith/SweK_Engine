...
// So this REPORTS the split and RATCHETS ONLY THE FIRST CLASS, cluster by cluster as they are closed.
{
    ...
    const unexercised = cov.ungated.filter((entry) => {
    ...
    // cluster is one where every definition its own gate does not name is nonetheless driven SOMEWHERE. This is
    // a RATCHET DOWNWARD ON A SET, not a number that can be lifted to meet the tree: a new ungraded export in a
    // closed cluster fails here on the round that lands it.
    const CLOSED = {
    ...
    for (const e of unexercised) {
        const d = e.slice(0, e.lastIndexOf("/")).split("/").slice(0, 2).join("/");
        openDirs.set(d, (openDirs.get(d) || 0) + 1);
    }
    const worst = [...openDirs].sort((a, b) => b[1] - a[1]).slice(0, 6);
    console.log("  ----  what is still dark, by cluster   " + worst.map(([d, n]) => d + " " + n).join(", ") +
        `  (${openDirs.size} clusters, ${unexercised.length} definitions). THE NEXT ROUND IS THE TOP OF THAT LIST, ` +
