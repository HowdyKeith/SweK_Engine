...
const SEG = [32, 10, 5], DIST = [3, 4, 6, 9, 14, 22, 36, 60, 95, 150], WS = [128, 256, 512, 1024], BACKENDS = ["webgpu", "webgl2"];
const r = await runInEngineOrigin({ engineRoot: ENG, args: { WS, DIST, SEG, BACKENDS }, script: `async (a) => {
  ...
  // BOTH BACKENDS. A rung's cost is what a RASTERISER draws, and gfx/device.js has two of them; v4377 froze one
  // and shipped its number to pages that run on either, which is the fault this re-freeze exists to fix.
  for (const backend of a.BACKENDS) { out[backend] = {};
  for (const W of a.WS) {
 ...
 */
export const LOD_RECORD = Object.freeze(${JSON.stringify({ at: "v4378", segments: SEG, distances: DIST, widths: WS, backends: BACKENDS, radius: 0.5, byBackend: r.result }, null, 1)});
`;
fs.writeFileSync(path.join(ENG, "render/lodRecord.mjs"), body);
console.log("frozen:", body.length, "bytes;", BACKENDS.length, "backends x", WS.length, "widths x", DIST.length, "distances x", SEG.length - 1, "rungs");
