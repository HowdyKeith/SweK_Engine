...
export const NEXT_ROUNDS = [
    ...
    { id: "terrain-controller", state: "OPEN", what: "physics/character/terrainWalk.mjs shipped at v4544 and does the GROUND; vertical velocity and capsule-vs-triangle are not built.", note: "DONE at v4544 for the part that was missing. The tree's only character controller, physics/character/kinematic.js, is an AABB against axis-aligned unit voxels, and a voxel world has no slopes -- only staircases of unit lips. MEASURED BEFORE ANYTHING WAS WRITTEN, which is what said a new file was owed rather than a patch: on voxelised ramps it either cannot climb at all (0.31x the requested speed at 14 degrees, 0.11x at 26.6, 0.01x and stuck at 45) or, with a step allowance, climbs while moving along the surface at EXACTLY sec(theta) -- 1.020x, 1.118x, 1.414x at those angles. That is the horizontal speed convention arrived at by accident rather than chosen, and it means you sprint up hills faster than you run on the flat. *** THE CENTRAL CLAIM IS THAT A SLOPE LIMIT IS A PROPERTY OF THE GROUND AND NOT OF THE FRAME RATE, AND THE CONTROL IS IN THE GATE RATHER THAN THE ARGUMENT: *** the tempting per-step height-difference test climbs a 76-degree wall at 60 and 240 fps and refuses the SAME WALL at 15 and 30, and climbs 26 units up a 63-degree face at every timestep against a 45-degree limit. A normal-based limit refuses both at all four. Two ground oracles are cross-checked rather than each trusted -- a bilinear heightfield gradient and a meshBVH raycast plus cross product agree to 4.8e-14 in height and exactly in normal over 200 samples. STILL OPEN: (1) NO VERTICAL VELOCITY. Jumping, falling and ceilings are not owned by this module; leaving the ground ends the step with airborne=true and the caller takes over. That contract is deliberate and it is also a hole: nothing in the tree currently plays the other half, so a body that walks off a ledge stops there. (2) CAPSULE AGAINST TRIANGLES. *** CLOSED AT v4541, AND IT IS THE PIECE THAT SEPARATES THE BRIDGE FROM THE PILLAR. *** physics/character/capsuleMove.mjs: exact closest-point kernels over meshBVH's trianglesInBox, deepest-first depenetration, and a swept move. One rule with NO ground oracle walks under the bridge to x = 20.0000 and is stopped by the pillar at x = 7.6000 -- x0 minus the RADIUS at 0.1, 0.25, 0.4 and 0.9, so the stop is the body's size rather than a tuned number -- on the columns v4539 proved byte-identical from above. FOUR MEASUREMENTS BEFORE ANY CODE, THREE OF WHICH CHANGED THE DESIGN: the closest-point direction is the ZERO VECTOR for every depth past the axis crossing and the depth saturates at the radius, so the formulation has a domain rather than a tolerance and a degenerate contact is reported rather than dropped; that domain's boundary is the radius EXACTLY, bisected to twelve decimals, with 0 dropped degenerate contacts at a safe advance and 2 at a tunnelling one -- THE DROPPED CONTACT IS THE TUNNEL -- so the substep cap is a fraction of the radius and 20 metres in one call becomes 100 substeps of 0.2; summing every contact drifts a body 0.634583 m sideways and costs 2.256872 m of progress on a PERFECTLY FLAT tessellated floor while deepest-first drifts exact zero, and an inside corner needs no summing because iterating re-queries; and every floor triangle in this tree's own fixtures is wound INWARD (n.y = -1), so nothing reads a cross product and reversing both fixtures' winding gives bit-identical walks. AND THE GATE CONVICTED THE MODULE'S OWN JUSTIFICATION: the header claimed five boundary sub-problems were complete because a segment crossing the triangle reports zero in all five. It does not -- a body with a thin ledge through its chest reads 0.400000, exactly the radius, so depth 0, so NO CONTACT AT ALL. The sixth is meshBVH's own rayTriangle. STILL NOT BUILT AND NAMED RATHER THAN IMPLIED: moving platforms, which need a BVH that changes between frames; stepping up onto a ledge, which is a POLICY over contacts rather than a contact; and NOTHING IS WIRED -- stepTerrain still asks an oracle over (x, z), so piece (3) now has its instrument and is still open. On terrain the capsule was never needed -- a wall IS a very steep slope and the slope limit is what stops you at it -- which is exactly why the GROUND controller's scope stopped where it did, and why this is a separate module rather than a patch to it. (3) THE GROUND IS A FUNCTION OF (x, z). An overhang has two surfaces over one point and the oracle takes the topmost. *** CLOSED AT v4543 IN THE ADAPTER THIS CLAUSE NAMES, AND THE SHIPPED ORACLE'S DEFECT WAS WIDER THAN THE CLAUSE. *** physics/character/capsuleGround.mjs casts from the body's reach for a candidate surface and asks capsuleMove whether a body of that size could stand on it, descending until one works or returning null -- which stepTerrain reads as blocked. MEASURED ON SIX WORLDS: meshGround answers x = 8.000 at a bridge whose roof is at 5, at a solid pillar, at a roof of 1.8 that a 1.8 body clears exactly, at a roof of 1.7 that it does not, and at a solid wall -- ONE NUMBER, FIVE DIFFERENT REASONS -- while the body-aware oracle gives 20.000, 7.583, 20.000, 7.667 and 7.583; and on a sixth, a doorway whose jamb the body clips, the shipped one walks the body THROUGH to 20.000, because a downward ray never meets the jamb the body's radius overlaps. Six formulations were built and five died to a fixture the one before it did not have, each named in the module header: a cast from the body walks through the pillar because the ray starts INSIDE it; refusing on any contact refuses every ramp; refusing on sideways push or on depth both fail because the ranges OVERLAP (a 45-degree ramp displaces 0.0828 and penetrates 0.1172 against a wall's 0.1-0.3 and a too-low roof's 0.1000); falling back to the sky walks the body through the doorframe; and classifying every contact makes a ramp refuse ITSELF at its own tessellation seam, which is v4541's internal-edge artefact arriving from the other side and takes v4541's own answer, deepest-first. With no body it is meshGround byte for byte, 660 of 660 probes. NOT CLOSED BY IT: nothing uses it. No shipping caller builds a MESH oracle at all -- BotManager's world exposes a height FUNCTION and goes through world/surfaceProbe.mjs, which v4542 made body-aware by the voxel route -- so this answers the question the clause asks and adds no consumer. Also not built, and named in both modules: step-up for a body with a radius, which neither this nor capsuleMove does (both stop a 0.4 body before a 0.4 riser, at 7.583 and 7.600, while a 0.2 riser IS mounted), and containment, since `contacts` is a surface query and a body fits inside a wall thicker than its own diameter -- 0.79 blocked, 0.80 clear, and 0.80 is 2r. *** MEASURED AT v4539 AND IT CANNOT BE CLOSED WHILE (2) IS OPEN, WHICH IS A PROOF RATHER THAN A JUDGEMENT. *** The defect is real: on a covered walkway the body stops dead at x=8.0000 with blocked=true, so an overhang READS AS A WALL and you cannot walk under a bridge, into a doorway or through a tunnel. But the obvious fix -- cast from the body's own height plus its step allowance -- opens the walkway to x=20.0000 AND WALKS THROUGH A SOLID PILLAR to x=20.0000, where the shipped oracle stops at 8.0000 correctly. It moves the error from a refusal that is annoying to a permission that puts a character inside the world. AND NO DOWNWARD RULE CAN DO BETTER: build the pillar to the roof's height and every surface under a vertical ray is [5, 0] at the bridge and [5, 0] at the pillar, byte-identical, because a ray never touches a side face and the side faces are the whole difference between a doorway and a wall. A horizontal ray at knee height separates them instantly. The missing information is in the SWEPT VOLUME, which is piece (2). Two further corrections to this entry's own wording: it is the STEP test that refuses, not the slope test -- the roof is horizontal and the slope test passes -- and there IS a teleport, in the zero-wish early return, which moves a body standing still under the bridge five metres onto the roof. *** AND THIS ENTRY POINTS AT AN ADAPTER NOTHING CALLS. *** meshGround has no consumer outside its own gate and the shipping caller uses a height FUNCTION, which cannot hold two surfaces -- so as filed this is latent. The LIVE instance is world/surfaceProbe.mjs's standHeightAt, read by the bot manager and the pathfinder pool: on a column standable at y=1 and y=21 a body on the floor was told the ground was at 21, twenty voxels up on the deck. 'The first thing wrong on a real mesh' is future tense about a path nothing walks, while the present tense was already shipping. *** CLOSED AT v4542, AND THE POPULATION WAS LARGER THAN THE FILING. *** Booting index.html and asking for EVERY standable surface rather than one per column: 863 of 1,681 columns are multi-surface (51.3%, and 57.7 / 51.9 / 53.5 in three other boots -- not reproducible, because the fluid and erosion systems edit voxels all through the run), up to 47 voxels apart, so standHeightAt was right in 1,681 of the 2,644 places a body could actually be standing. 63.58%, AND THAT NUMBER IS EXACTLY THE COLUMN COUNT: a function returning one y per column is right once per column however good it is. It takes the body's feet and the caller's own step allowance now, and scans DOWN from the body's reach -- 2,644 of 2,644, at 4.0 voxel reads per call against the old path's 7.4 and a full scan's 42.4, so the correct answer is also the cheap one. *** AND v4539's DIAGNOSIS WAS HALF WRONG IN A WAY WORTH KEEPING: *** it aimed at the topSolidAt fallback, because its fixture returned NaN from _heightAt and forced that path. On the real world the MODEL names the upper surface -- (-42,-60) holds surfaces at 2 and 19 and _heightAt says 19 -- so fits(h) passes and the fallback never runs. The defect was in the signature, which had nowhere to put the body. What it got RIGHT is that the repair needed something: choosing which surface a body is ON needs no swept volume, because a voxel column has no side faces within itself, but whether the body may MOVE to the next column's surface does, and that is unchanged. physics/character/groundProbe.mjs and its gate hold the fixtures and the proof so the next attempt meets the pillar before the merge does. (4) NOTHING IS WIRED, the same state nav/funnel.mjs has been in since v4254 and nav/navmesh.mjs since v4543. *** (4) IS CLOSED AT v4545: *** simulation/BotManager.js follows the ground through this module instead of writing bot.y = _heightAt(x, z) + 1, so a bot moves at 5*cos(theta) horizontally rather than sec(theta) along the ground, stops at a wall it used to walk straight up, and slides along one approached at an angle rather than sticking. functionGround() was added for it, because a live world exposes a height FUNCTION and neither existing adapter fits that. (1), (2) and (3) are still open." },
    ...
    { id: "navmesh-recast", state: "OPEN", what: "nav/navmesh.mjs shipped at v4543 and closes funnel-selfcheck's standing question; three named pieces of it are not built.", note: "DONE at v4543 for the part that was blocking. tools/ship/funnel-selfcheck.mjs ended with 'unchecked here: a NAVMESH' and its section 4 said why: on a wall with one gap, string-pulling a GRID corridor gives 302.20 m and walks through walls at 18 of 616 samples, and insetting it to the staircase's safety costs 319.59 m against that staircase's 318.39 -- LONGER than the thing it improved, because the whole saving was a margin the grid held by accident. Eroding by the agent radius BEFORE the polygons exist puts the clearance in the mesh instead: 297.42 m on the same wall at the same radius, 0 of 605 in a wall, measured clearance 2.687, and 0.27% off an optimum derived from the fixture's own inequalities. *** THE ROUND'S REAL DEFECT WAS FOUND BY CHANGING THE INSTRUMENT RATHER THAN THE FIXTURE: *** Recast's own chamfer 2/3 distance field passed 'in a wall: 0 of 599' while delivering 0.708 clearance where 1 was asked and 2.829 where 3 was, both exactly the diagonal a chamfer prices at 3 against 2.828. Not entering a wall is a much weaker property than standing clear of one, and only the second is what a navmesh is for. STILL OPEN, and each is a round rather than a gap: (1) WATERSHED PARTITION AND CONTOUR SIMPLIFICATION. *** v4536 MEASURED THIS PIECE BEFORE BUILDING IT AND THREE OF THE FOUR CLAIMS THIS ENTRY MADE ABOUT IT DID NOT SURVIVE. THE ENTRY IS CORRECTED IN PLACE RATHER THAN QUIETLY REPHRASED. *** It read '737 thin rectangles where a contour mesh would give a handful', as though the sweep were being wasteful. IT IS EMITTING THE PROVEN MINIMUM. Lipski/Ohtsuki gives the smallest rectangle partition of a rectilinear region as N - L + 1 - H, and on that mask it is 738 reflex corners - 0 independent chords + 1 - 2 holes = 737, exactly what the sweep produces -- so a rectangle-merge pass over its output cannot buy one polygon, and since a union of axis-aligned unit cells is convex only when it is a rectangle, nothing that stays on the cell lattice can either. The handful has to be bought by LEAVING the lattice, which is the same move that puts exactly-shared portal edges at risk. It also read 'this buys memory and A* nodes': planPath holds between 1 and 2 ms over meshes from 737 polygons to 11,242, so A* nodes are not what polygon count costs. What it costs is the ADJACENCY BUILD, quadratically -- 14.5 ms of a 119 ms build at 737, 1,064 ms of 1,152 ms at 11,242 -- and that is a different repair. And 'nothing yet is short of either' is right for a reason the entry did not give: the one shipping caller decomposes its snapshot into THREE rectangles, so the 737 lives on one gate fixture and nowhere else in the tree. *** WHAT SHIPPED INSTEAD WAS THE INSTRUMENT, BECAUSE NOTHING COULD HAVE GRADED THE PARTITION THIS ASKS FOR. *** Two holes, both driven rather than argued. Deleting the row sweep's VERTICAL mutual-step test left navmesh, navWiring, navWiringLive and funnel ALL GREEN -- while turning a z-oriented ledge into one polygon and returning a 110 m path UP a cliff, the exact defect the module's longest comment says was fixed; its horizontal twin costs two reds, so one axis of a two-axis correctness property was graded and the other was not. And crossesAllPortals, the tree's only membership instrument, cannot see the failure contour tracing introduces: a portal one cell longer than its true shared span -- a T-junction, which a contour trace produces routinely and a row sweep cannot -- puts 91 of 427 path samples OUTSIDE the mesh while the portal test reports 0 missed, because it asks whether the path crosses the portal and never whether it stays inside the polygons. nav/partitionScore.mjs and its gate now carry the optimality floor, a both-axes mutual-step counter and pathInMesh. ALSO REFUSED WITH CAUSE: the WATERSHED half specifically. It partitions on the distance field, and on the ledge fixture that field takes ONE distinct value across all 16,384 cells -- blind to the cliff, not approximately but completely, because a cliff is a fact about the connectivity graph. A faithful watershed would restore the 110 m path. WHAT REMAINS GENUINELY OPEN is the contour trace and convex decomposition, and it is owed a fixture where a shipping caller actually produces an oblique boundary; on today's snapshot shape the count is 3 either way (2) CLOSED AT v4538, AND THE ITEM ASKED THE WRONG QUESTION. It asked for a fixture separating g + distance from a flat g + 1, and named a trap map -- a gap pointing at the goal opening into a dead chamber -- as having failed to. *** THAT FIXTURE COULD NEVER HAVE SEPARATED THEM: IT PUNISHES GREEDINESS, AND BOTH MODELS ARE GREEDY ONCE THE HEURISTIC DOMINATES, *** so both fall for it identically and agreeing proves nothing about either. What separates them is the UNIT -- metres against polygons -- so the fixture had to rank two routes OPPOSITELY by the two, and building that found the cost term was not merely untested but WRONG. On a map offering a 45-degree diagonal (shortest on the ground, ~100 thin rectangles) against an L round two edges (longer, five polygons), the planner returned 193.13 m while the mesh held 141.42 -- AND THE SAME PLANNER FOUND IT, answering 70.71 m from the start to the diagonal's midpoint and 70.71 m onward. Splitting the query improved the answer by 36.6%, which needs no oracle: both legs are routes this planner returned on this mesh. THE CAUSE IS ONE LINE. `g` chained entry point to portal MIDPOINT, and a midpoint is an arbitrary point on the portal, so a long portal let g cut a corner the polygon does not permit and HOW MUCH IT CUT DEPENDED ON POLYGON SIZE: on the L, g accumulated 100.10 for a route the funnel walks in 193.13, under-pricing by 48.2%, while on the hundred-hop diagonal it read 140.71 against 141.42, accurate to half a per cent. The search was discounting routes of few large polygons in proportion to how large they were, which is why BOTH models chose the L and why no fixture ever caught it. Entering each portal at the point nearest to where the path came from returns 141.42 -- the witness exactly. It moves NOTHING already measured (obstacle 386.76 m / 3 corners / 7 polygons and 45-degree 261.10 m / 5 corners / 105 polygons, identical before and after, every nav gate green), which is the other half of the finding: no gate the tree had would catch the repair being undone. nav/pathCost.mjs carries the split-query witness and the fixture, corridor() takes an `entry` option so the old model stays reachable for the comparison, and tools/ship/pathCost-selfcheck.mjs grades it at 362-376 ms. NOT CLAIMED: that the nearest-point entry is optimal. It is a tighter under-estimate that no longer varies with polygon size; a cost matching the funnel exactly would have to know the whole corridor before choosing it. The heuristic is still Euclidean against a g that under-estimates, so the pair is not a consistent A*, and no fixture here offers three competing routes. (3) NOTHING IS WIRED. simulation/BotPathfinderPool.js still receives the grid staircase and no shipping module calls nav/navmesh.mjs -- the same state funnel.mjs has been in since v4254. Wiring it is not free: the mesh is built per connected component from a seed, so a caller needs one mesh per reachable region and a policy for rebuilding when the heightmap changes, and neither exists. ALSO NOT BUILT: multi-storey worlds, because a heightmap has one surface per column and real spans are what Recast carries; and off-mesh links, jumps and doors. *** (3) IS CLOSED AT v4545: *** worker/botPathfinder.worker.js tries this module first and falls back to its own grid A*, simulation/BotPathfinderPool.js exposes route and agentRadius, and tools/ship/navWiring-selfcheck.mjs grades it end to end on the snapshot shape the pool actually builds -- shorter (96.61 against 99.88 at a separation of 80), further from the walls (clearance 2.000 against 0.850) and 4 waypoints against 21, at 1.4-3.0 ms per query against 0.25-0.90. *** AND THE WIRING FOUND TWO DEFECTS THIS MODULE'S OWN GATE COULD NOT: *** portals oriented from polygon CENTRES returned 147.35 m against a taut 96.61 on the first realistic snapshot it was ever handed, because every fixture here walked its corridor monotonically; and a stride of 0 made hm.length/stride Infinity and HUNG the worker forever, which is worse than throwing because the fallback recovers from a throw. (1) and (2) are still open." },
