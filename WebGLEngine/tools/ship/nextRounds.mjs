...
export const NEXT_ROUNDS = [
    ...
    },
    {
        ...
        why: "v4480 measured the divergence: the two tables share only lens and optics and disagree about both -- lens on the knob name, optics on the mode scoping. The census's copy predates the discipline that makes a knob state its argument. It is NAMED at v4480 rather than migrated, because a re-freeze of corroboration-reach-baseline.json is a round of its own. MEASURED AT v4482 AND NOT SPENT, so the round that does this inherits the numbers rather than taking them again: the blast radius is EXACTLY 10 of 484 device/modes (gained blackhole.escape, kepler.conserve, quantum.well, chaos.feigenbaum, splat.integral, lens.deflect; lost optics.slit, ct.parallel, ct.fan, lens.map). ct.nAngles CONVERGES -- fbpCorr 0.96659 -> 0.98514 -> 0.99277 -> 0.99460, successive differences 1.86e-2, 7.63e-3, 1.83e-3 -- and CLAMPS AT 240, so the census's declared 256 was a coerced duplicate. lens.map is the opposite and is the trap refinementKnobs' own header names: mapN moves ONLY mapN and mapCells, while mapPeak, mapPeakExact, mapPeakErrFrac, mapSpan and sourceRho are bit-constant from 9 to 31 -- A DECLARED REFINEMENT KNOB THAT REFINES NOTHING. optics.slit IS genuinely refinable (slitFirstMinErrFrac 5.04e-5 -> 1.25e-5 -> 4.00e-6) and is lost only because the canonical table is ONE MODE PER DEVICE and optics already holds airy, which is a structural limit rather than a judgement. AND v4480's NOTE OVERSTATED THE RISK: corroboration-reach-baseline.json is about reach(), which already uses the canonical table, so the real consumers are the census's own summary and one ratio assertion at corroborationCensus-selfcheck.mjs:176. A ct promotion was written and reverted at v4482 for one reason worth carrying: deviceReport-selfcheck.mjs:94 asserts Object.keys(REFINEMENT_KNOBS).length === 7, so the eighth knob reddens it -- THE THIRD PINNED COUNT THIS SESSION HAS MET, and it belongs to this round rather than to a drive-by.",
    },
    {
        id: "zoom-blur-wiring",
        blocker: "OPEN",
