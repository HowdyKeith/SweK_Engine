...
export const NEXT_ROUNDS = [
    ...
    },
    {
        ...
        what: "THE TWIN HALF IS DONE AND THE SHADER-ONLY HALF IS NOT -- AND THE COUNT IN THIS ENTRY WAS WRONG "
            + "IN BOTH DIRECTIONS. fract(sin(dot(p, K)) * 43758.5453) amplifies the last bits of its input by "
            + "four orders of magnitude, so a float32 GPU and a float64 CPU draw DIFFERENT random numbers "
            + "rather than rounding the same one differently. This entry filed the remainder as TEN FILES WITH "
            + "NO GATE. Re-counted at v4578 with the line rule the ratchet already uses plus a requirement "
            + "that the file hold SHADER SOURCE rather than prose about one, it is TWELVE -- and a THIRTEENTH "
            + "was not ungated at all. *** render/holoFoilShader.js HAD A CPU MODEL ALL ALONG AND THE TWO "
            + "COMPUTED DIFFERENT FUNCTIONS. *** render/holoFoil.mjs's hash2 is an integer avalanche; the "
            + "shader's hf_hash2 was the sin-hash, beneath the comment \"Integer lattice, matching the model's "
            + "hash2\". Both feed `if (cell > coverage) return 0`, which decides whether a metallic flake "
            + "EXISTS. Measured over the 1,600 cells of the 40x40 lattice with the GLSL emulated in float32: "
            + "81.8% of cells differed by more than 0.1, worst 0.9604, the draw decision flipped on 21.3%, and "
        ...
        why: "*** THE TREE ALREADY KNEW, IN THREE PLACES, AND NEVER JOINED THEM UP. *** "
            ...
            + "never be verified. *** AND THE RATCHET THAT WAS SUPPOSED TO STOP THIS ASKED A PROXY QUESTION. "
            + "*** It flags a file that computes the idiom in float64 AND emits it to a shader -- ONE FILE "
            + "holding both halves -- so a twin whose model and shader are two files was invisible to it, "
            + "which is exactly how holoFoil survived fifteen versions with a gate over it. The gate checked "
            + "five optics terms of that shader against the model and never the hash, and its one row about "
            + "the flake's position read `hash2(A) === hash2(A)`, which cannot fail. "
            + "*** THE FOUR FIXED TWINS SHOW WHAT THE NOTES WERE MISSING: THE GAS SURVIVES THE DIVERGENCE AND "
            + "THE THRESHOLD DOES NOT. *** Measured: grass 65.4% of drawn decisions FLIPPED; wormhole h2 70.0% "
            + "of 14,400 lattice points; nebula 3,006 CPU stars against 2,509 GPU with 378 in the same place; "
            + "holofoil 184 flakes against 214 with 29 in the same place. All four now read 0.0% and 100%.",
        upstream: "Nothing blocks the twelve, and the shape of the work is now known rather than guessed. A "
            + "shader-only site has NO CPU twin to disagree with today, so none of them is wrong the way "
            + "holoFoil was wrong -- what each carries is that sin at these magnitudes is "
            + "IMPLEMENTATION-DEFINED, so two devices, or one page's WebGL2 and WebGPU paths, need not agree. "
            + "The judgement each site needs is whether anything DEPENDS on the current pattern. Two worked "
            + "examples of getting that judgement wrong: fx/paintFields.mjs and physics/kernelVerdict-selfcheck"
            + ".mjs compute the idiom at both precisions to MEASURE the gap, and a mechanical pass over the "
            + "population would delete the measurement; and shaders/biome.frag.glsl says "
            + "\"BIOME DETECTION (matches JS logic conceptually)\" -- the same claim holoFoilShader.js's "
            + "\"matching the model's hash2\" turned out to be false about, hedged by one adverb. Recorded as "
            + "unexamined rather than investigated.",
    },
    {
