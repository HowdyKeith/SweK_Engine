...
// ---------------------------------------------------------------------------------------------------------
{
    ...
    ok(RED_AT_V4407.every((e) => reg.get(e.gate) === "redCensus.RED_AT_V4407"),
       "every RED_AT_V4407 entry is known, credited to that record", `${RED_AT_V4407.length} entries -- gates the over-budget population hid until the rotation ran them`);
    ok(reg.size === new Set([...RED_AT_V4279.map((e) => e.gate), ...RED_AT_V4407.map((e) => e.gate), ...UNCONFIRMED_SLOW, ...SWEEP_V4297.fromSlowBucket, ...SWEEP_V4297.unmeasured]).size,
