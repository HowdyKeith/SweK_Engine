...
// ---- 3. THE LANGUAGE AXIS AGAINST THE ANSWER THE TREE ALREADY HAD --------------------------------------------
{
    ...
    const tally = { agree: 0, built: 0, transpiled: 0, paperwork: 0, assets: 0, unmeasured: 0, unexplained: 0 };
    const verdicts = [];
    const rows = [];
    for (const b of AUTH.bodies) {
        ...
        const assetFiles = files.filter((f) => !U.PAPERWORK.has(path.basename(f)) && U.ASSET_EXT.has((f.split(".").pop() || "").toLowerCase())).length;
        ...
        const c = U.classifyLanguage({ dominantExt, treeBiome: biomeOf("x." + dominantExt),
                                       ...
                                       upstreamLanguageFiles, assetFiles });
        tally[c.verdict]++;
    ...
    say(`  agree ${tally.agree}  built ${tally.built}  transpiled ${tally.transpiled}  ` +
        `paperwork ${tally.paperwork}  assets ${tally.assets}  unmeasured ${tally.unmeasured}  unexplained ${tally.unexplained}`);
    ...
    // it were. Without it both bodies read "unexplained" -- not a disagreement about a language but the absence of any code to have one.
    ok("!! the asset kits are ASSETS, not an unexplained language: every non-paperwork file a model, texture or sound",
        tally.assets === U.MEASURED_AT_V4432.languageAssets,
        `${tally.assets} bodies: ${verdicts.filter((v) => v.verdict === "assets").map((v) => v.name).join(", ") || "none"}`);
