...
{
{
    // ---- EXILED_PASS_V4662: the whole pool, and the number nobody had ------------------------------------
    const R = SC.EXILED_PASS_V4662;
    say(`EXILED_PASS_V4662: ${R.pool} gates outside the sweep, ${R.green} green, ${R.redCount} red ` +
    ...
    const gone = [...R.red.map((x) => x.gate), ...R.stillKilled, ...R.repairedAtV4662]
    ...
    const claimed = R.repairedAtV4662;
