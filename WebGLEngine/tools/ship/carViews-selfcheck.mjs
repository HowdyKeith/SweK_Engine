...
sec("2. THE BRAIN VIEW IS THE POLICIES' ACTIVATIONS THROUGH THE KERNEL'S TWIN");
{
    ...
    ok("!! the driver's activations end in exactly forward()'s outputs, with 46 hidden values between (brain/epgTopology.mjs's connectome core)", a.hidden.length === 46 && a.output.length === 2 && near(a.output[0], f[0]) && near(a.output[1], f[1]), `${Array.from(a.output).map((v) => v.toFixed(4))} vs ${f.map((v) => v.toFixed(4))}`);
    ...
    ok("six rows: features, hidden, outputs for each policy, with their names (the driver's 46-unit and the gunner's 34-unit connectome-masked hidden layers)", rows.length === 6 && rows.map((r) => r.values.length).join() === "9,46,2,11,34,5" && rows[0].names === D.FEATURE_NAMES && rows[5].names.join() === "yaw,pitch,fire,drop,ignite" && rows[3].names === GP.FEATURE_NAMES);
    const calls = []; const ctx = { fillStyle: "", font: "", fillRect: (...q) => calls.push(q), fillText: () => {} };
    const bars = CV.drawBrainPanel(ctx, W, H, rows, { title: "t" });
    ok("!! the panel draws one bar per value (107: 9+46+2 driver, 11+34+5 gunner) plus the background, inside the window", bars === 107 && calls.length === 108 && calls.slice(1).every((q) => q[0] >= 0 && q[0] + q[2] <= W && q[1] >= 0 && q[1] + q[3] <= H + 1));
