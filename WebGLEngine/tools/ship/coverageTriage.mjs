...
export const NON_SCENE = {
    ...
    "physics/fluidBox3d.js": { verdict: "GATED", why: "a co-simulation coupler between box3d and the fluid. CARRIED AS A CANDIDATE UNTIL v4415 ON A HYPOTHESIS THAT WAS WRONG TWICE. It read: \"Momentum exchange across the coupling COULD be an exact key (what one side loses the other gains), which would be a real gate rather than a smoke test.\" It is not a smoke test -- physics/fluidBox3d-selfcheck.mjs has nine sections holding the velocity nudge to dv = forceScale * scale * dt * F / mass at THREE scales, pinning the torque's scale^2 exponent where the only consumer runs at scale 1 and cannot see it, and asserting the ORDER of the exchange, which no final value reveals. And the momentum key cannot exist: the coupling is deliberately ONE-WAY -- the module's header says box3d owns the motion and the fluid supplies external forces -- so the fluid is never told to give up what the body receives and there is no ledger with two sides. Section 8b measures both halves: the body gains EXACTLY the stamped impulse, and the fluid's force is unchanged by the transfer. A candidate key that cannot be written is worth recording as such, or it stays on the list forever." },
    "physics/voxel/fracture.js": { verdict: "GATED", why: "turns a broken voxel structure into rigid bodies. GATED at v4415 in physics/voxel/fracture-selfcheck.mjs, on a SHARPER key than this entry named. Mass conservation is there and is BIT-IDENTICAL rather than within a tolerance -- every term is the same float added the same number of times -- but the module's own header carried the better one: a voxel summation EARNS the analytic box tensor I = m/12 diag(b^2+c^2, ...) exactly, at every resolution, and a fragment that has broken off earns it about a centre of mass that did not exist before the fracture. The cost of dropping each voxel's own inertia turns out to be a CLOSED FORM, 2 cell^2 / (b^2 + c^2), where the header said only \"about cell^2\" -- right about the scaling and silent about the constant, and the constant is what separates a missing term from a coarse grid. The gate's first fixture carved spheres, as the module's own carveSphere invites, and every carve left the block in ONE piece; a partition of one part is arithmetic with nothing to check, so the piece count is now asserted." },
    "simulation/euler/eulerShader.js": { verdict: "GATEABLE", why: "compressible Euler as a WGSL compute kernel. Classified HARDWARE until v4403 -- \"needs a real GPU, like the bench pages, so it cannot be gated in a sandbox\" -- and that was TWO claims wearing one sentence. It cannot be TIMED in a sandbox, which is still true and is why tools/ship/eulerGpu-selfcheck.mjs makes no timing claim. It cannot be CHECKED there was true of a sandbox with no device and stopped being true at v4359, when node-webgpu arrived: the kernel now compiles, runs 4,490 dispatches and is adjudicated against its own CPU twin in seconds. GATED at v4403, and this entry is left in place rather than deleted because a verdict that was overturned is worth more here than one that was merely right." },
