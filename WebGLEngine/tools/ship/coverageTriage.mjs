...
export const NON_SCENE = {
    ...
    "simulation/euler/eulerShader.js": { verdict: "GATEABLE", why: "compressible Euler as a WGSL compute kernel. Classified HARDWARE until v4383 -- \"needs a real GPU, like the bench pages, so it cannot be gated in a sandbox\" -- and that was TWO claims wearing one sentence. It cannot be TIMED in a sandbox, which is still true and is why tools/ship/eulerGpu-selfcheck.mjs makes no timing claim. It cannot be CHECKED there was true of a sandbox with no device and stopped being true at v4359, when node-webgpu arrived: the kernel now compiles, runs 4,490 dispatches and is adjudicated against its own CPU twin in seconds. GATED at v4383, and this entry is left in place rather than deleted because a verdict that was overturned is worth more here than one that was merely right." },
