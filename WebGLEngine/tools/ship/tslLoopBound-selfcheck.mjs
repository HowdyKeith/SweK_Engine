...
console.log("\n2. EMITTED ONCE PER VARIANT, RUN AT FIVE STEP COUNTS WITH THE BUFFER ALONE CHANGED, HELD TO THE f32 TWIN BIT FOR BIT");
{
    ...
    if (skip) { console.log(`  SKIP  ${skip}`); report("*** NOT A PASS. ***"); fails++; }
        ...
        if (r.ok && r.result.variants) {
            ...
            if (!U.error && !Sv.error) {
                fs.writeFileSync(EMITTED, JSON.stringify(stampThreeRevision({ generatedFrom: "tools/ship/tslLoopBound-selfcheck.mjs", at: "v4471", note: "the logistic map stepped `bound` times, the bound a vec4 uniform in one variant and a storage buffer's element in the other -- the first generated pass whose trip count is not baked into its text; render/physicsTsl.mjs makeLogisticStepperTsl, transplanted by render/tslSource.mjs",
