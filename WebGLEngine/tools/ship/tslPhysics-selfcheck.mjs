...
const k = keyCpu(PARAMS.first);
{
    ...
    ok("  the uniforms AND the storage buffers are labelled (render/tslSource.mjs binds by the label): the two keys' ten, the compute sweep's span, the cull decision's two, and the cull PASS's four uniforms and five named buffers, and the HMC leapfrog's two here, and (v4471) the buffer-bound stepper's five -- out, r, x0, steps, bound -- across its two variants; the fleet looks' eight in render/fleetTsl.mjs", (src.match(/\.label\("/g) || []).length === 31 && (fleetSrc.match(/\.label\(/g) || []).length === 8, `${(src.match(/\.label\("/g) || []).length} physics (ten keys, the sweep's span, the decision's eye and thresholds, the pass's eye/thresholds/info/clock and inst/extras/cmds/records/fleetOf with planes labelled on BOTH of its two bindings, plus the leapfrog's sinv and mu), ${(fleetSrc.match(/\.label\(/g) || []).length} fleet`);
}
...
if (skip) { console.log(`  SKIP  ${skip}`); fails++; }
    ...
    if (r.ok && r.result.three.webgpu && !r.result.transplantError) {
        ...
        const rec = stampThreeRevision({ generatedFrom: "tools/ship/tslPhysics-selfcheck.mjs", at: "v4321", note: "emitted by three's node builders from render/physicsTsl.mjs and transplanted by render/tslSource.mjs; rewritten by tools/ship/tslPhysics-selfcheck.mjs on every run", ...R.emitted });
