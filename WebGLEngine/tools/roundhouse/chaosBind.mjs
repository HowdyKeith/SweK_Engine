...
export const chaosDevice = {
    // *** v3851 -- READER, AND THIS IS ONE OF THE TWO THE DEFAULT WOULD HAVE GOT WRONG. *** Read
    // logistic.js's lyapunov(): the orbit is advanced by `x = logistic(r, x)` and the plant does not touch it.
    // slopeOfMap enters ONLY the sum `sum += log|slopeOfMap(r, x)|`. *** THE DYNAMICS RUN IDENTICALLY AND ONLY
    // THE MEASUREMENT IS CORRUPTED *** -- the same shape as v3400's two readers (diffusion accumulating MSD
    // from wrapped coordinates, pipe3d substituting a plug profile). This file's own header calls it "THE
    // PLANTED INSTRUMENT" four times, which is the tell: an instrument is a reader, not a law.
    plantKind: "reader",
