...
export const chaosDevice = {
    ...
    // v3851 -- *** READER PLANT, AND THIS FILE'S OWN NOTE BELOW IS THE PROOF. *** The orbit is generated by
    // logistic(r, x), which the plant does not touch -- fixed / doubling / feigenbaum are BIT-IDENTICAL under
    // it, as the note says. slopeOfMap is consulted only inside lyapunov()'s sum of log|f'(x)|, which is the
    // MEASUREMENT of the exponent. The dynamics run identically and only the reading is corrupted, which is
    // v3393's reader kind exactly. Assuming `knob` from the property name would have mislabelled it.
    plantKind: "reader",
