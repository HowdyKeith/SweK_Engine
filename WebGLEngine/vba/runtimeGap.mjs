...
export const MEASURED_AT_V4462 = Object.freeze({
    ...
    // v4588 -- EIGHT ROWS AT ONCE, which is the most this census has moved in the arc, and every one of them is
    // the same two files: fx/fsr/fsrGPU.js and its gate. WebGPU 48 -> 49 and WebGL 142 -> 143 are the informative
    // pair -- the round's whole subject is that kernels proven on a device had no caller, so the row counting
    // device users is exactly the row that should move for it, and it did. performance.now 220 -> 221 is the
    // speed measurement the kernel gate's closing asked for and did not take.
    // v4589 -- RE-TAKEN: tools/ship/kernelReach.mjs and its gate. Four rows, and the WebGPU row did NOT move --
    // correctly: this round counts kernels rather than dispatching any, which is the distinction the whole round
    // is about. A census of device code is not device code.
    // v4590 -- RE-TAKEN: render/temporalGPU.mjs and its gate, the temporal arc's first caller. Six rows moved.
    // v4592 -- RE-TAKEN: render/motionVectorsGPU.mjs and its gate, MOTION_WGSL's first caller. WebGL 144 -> 145
    // and WebGPU unmoved at 49, which is right: the runner reaches the device through gfx/device.js rather than
    // naming an adapter itself, and the row that counts adapter mentions should not move for that.
    esModules: 3802, closures: 3670, asyncAwait: 1449, typedArrays: 839, promises: 346,
    fetchXhr: 243, performanceNow: 222, raf: 116, webgl: 145, webgpu: 49, threads: 22, wasm: 23,
